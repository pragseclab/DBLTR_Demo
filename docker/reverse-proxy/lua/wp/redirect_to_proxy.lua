...
-- Get the name of WP auth cookie
if ngx.req.get_headers()["cookie"] ~= nil then
    for k, v in pairs(common.split(ngx.req.get_headers()["cookie"], ";")) do
        if string.match(v:lower(), "wordpress_logged_in") then
            ngx.var.auth_cookie_name = common.split(v, "=")[1]
            ngx.var.auth_cookie = common.split(v, "=")[2]:lower()
        end
    end
end

-- read session cookie
if ngx.var.auth_cookie ~= nil then
    ...
    ngx.log(ngx.ERR, "Auth cookie empty? " .. tostring(ngx.var.auth_cookie_name))
end
