...
class Table
{
    ...
    protected function getUiPrefsFromDb()
    {
        ...
        $sql_query = ' SELECT `prefs` FROM ' . $pma_table . " WHERE `username` = '" . $this->dbi->escapeString($GLOBALS['cfg']['Server']['user']) . "'" . " AND `db_name` = '" . $this->dbi->escapeString($this->dbName) . "'" . " AND `table_name` = '" . $this->dbi->escapeString($this->name) . "'";
    ...
    protected function saveUiPrefsToDb()
    {
        ...
        $sql_query = ' REPLACE INTO ' . $pma_table . " (username, db_name, table_name, prefs) VALUES ('" . $this->dbi->escapeString($username) . "', '" . $secureDbName . "', '" . $this->dbi->escapeString($this->name) . "', '" . $this->dbi->escapeString(json_encode($this->uiprefs)) . "')";
        ...
        if (!$success) {
            ...
            $message->addMessage(Message::rawError((string) $this->dbi->getError(DatabaseInterface::CONNECT_CONTROL)), '<br><br>');
            return $message;
        }
        ...
        if ($rows_count > $max_rows) {
            ...
            $success = $this->dbi->tryQuery($sql_query, DatabaseInterface::CONNECT_CONTROL);
            if (!$success) {
                $message = Message::error(sprintf(__('Failed to cleanup table UI preferences (see ' . '$cfg[\'Servers\'][$i][\'MaxTableUiprefs\'] %s)'), MySQLDocumentation::showDocumentation('config', 'cfg_Servers_MaxTableUiprefs')));
                $message->addMessage(Message::rawError((string) $this->dbi->getError(DatabaseInterface::CONNECT_CONTROL)), '<br><br>');
                return $message;
            }
        }
    ...
    public function updateDisplayField($display_field, array $cfgRelation)
    {
        $upd_query = false;
        if ($display_field == '') {
            $upd_query = 'DELETE FROM ' . Util::backquote($GLOBALS['cfgRelation']['db']) . '.' . Util::backquote($cfgRelation['table_info']) . ' WHERE db_name  = \'' . $this->dbi->escapeString($this->dbName) . '\'' . ' AND table_name = \'' . $this->dbi->escapeString($this->name) . '\'';
        } else {
            $upd_query = 'REPLACE INTO ' . Util::backquote($GLOBALS['cfgRelation']['db']) . '.' . Util::backquote($cfgRelation['table_info']) . '(db_name, table_name, display_field) VALUES(' . '\'' . $this->dbi->escapeString($this->dbName) . '\',' . '\'' . $this->dbi->escapeString($this->name) . '\',' . '\'' . $this->dbi->escapeString($display_field) . '\')';
        }
        if ($upd_query) {
            $this->dbi->query($upd_query, DatabaseInterface::CONNECT_CONTROL, 0, false);
            return true;
        }
        return false;
    }
        ...
        foreach ($destination_db as $master_field_md5 => $foreign_db) {
            ...
            if (!empty($foreign_db) && !empty($foreign_table) && !empty($foreign_field)) {
                if (!isset($existrel[$master_field])) {
                    ...
                    $upd_query = 'UPDATE ' . Util::backquote($GLOBALS['cfgRelation']['db']) . '.' . Util::backquote($cfgRelation['relation']) . ' SET foreign_db       = \'' . $this->dbi->escapeString($foreign_db) . '\', ' . ' foreign_table    = \'' . $this->dbi->escapeString($foreign_table) . '\', ' . ' foreign_field    = \'' . $this->dbi->escapeString($foreign_field) . '\' ' . ' WHERE master_db  = \'' . $this->dbi->escapeString($this->dbName) . '\'' . ' AND master_table = \'' . $this->dbi->escapeString($this->name) . '\'' . ' AND master_field = \'' . $this->dbi->escapeString($master_field) . '\'';
                }
            } elseif (isset($existrel[$master_field])) {
                $upd_query = 'DELETE FROM ' . Util::backquote($GLOBALS['cfgRelation']['db']) . '.' . Util::backquote($cfgRelation['relation']) . ' WHERE master_db  = \'' . $this->dbi->escapeString($this->dbName) . '\'' . ' AND master_table = \'' . $this->dbi->escapeString($this->name) . '\'' . ' AND master_field = \'' . $this->dbi->escapeString($master_field) . '\'';
            }
            if (!isset($upd_query)) {
                continue;
            }
    ...
    public function getColumnGenerationExpression($column = null)
    {
        ...
        if ($serverType === 'MySQL' && $this->dbi->getVersion() > 50705 && !$GLOBALS['cfg']['Server']['DisableIS']) {
    ...
    public function getRealRowCountTable() : ?int
    {
        ...
        $result = $this->dbi->fetchSingleRow(sprintf('SELECT COUNT(*) AS %s FROM %s.%s', Util::backquote('row_count'), Util::backquote($this->dbName), Util::backquote($this->name)));
        if (!is_array($result)) {
            return null;
        }
