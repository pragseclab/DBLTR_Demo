...
class Key extends TwoFactorPlugin
{
    ...
    public function check()
    {
        ...
        try {
            ...
            $auth = U2FServer::authenticate($_SESSION['authenticationRequest'], $this->getRegistrations(), $response);
    ...
    {
        $request = U2FServer::makeAuthentication($this->getRegistrations(), $this->getAppId(true));
        $_SESSION['authenticationRequest'] = $request;
        $this->loadScripts();
        return $this->template->render('login/twofactor/key', ['request' => json_encode($request), 'is_https' => $GLOBALS['PMA_Config']->isHttps()]);
    }
    /**
    ...
    {
        $registrationData = U2FServer::makeRegistration($this->getAppId(true), $this->getRegistrations());
        $_SESSION['registrationRequest'] = $registrationData['request'];
        $this->loadScripts();
        return $this->template->render('login/twofactor/key_configure', ['request' => json_encode($registrationData['request']), 'signatures' => json_encode($registrationData['signatures']), 'is_https' => $GLOBALS['PMA_Config']->isHttps()]);
    }
    /**
    ...
    public function configure()
    {
        ...
        try {
            ...
            $this->twofactor->config['settings']['registrations'][] = ['keyHandle' => $registration->getKeyHandle(), 'publicKey' => $registration->getPublicKey(), 'certificate' => $registration->getCertificate(), 'counter' => $registration->getCounter()];
            return true;
        } catch (U2FException $e) {
            $this->message = $e->getMessage();
            return false;
