...
class ImportOds extends ImportPlugin
{
    ...
    public function doImport(?File $importHandle = null, array &$sql_data = [])
    {
        ...
        if ($xml === false) {
            $sheets = [];
            $GLOBALS['message'] = Message::error(__('The XML file specified was either malformed or incomplete.' . ' Please correct the issue and try again.'));
            $GLOBALS['error'] = true;
        } else {
            ...
            if (empty($root)) {
                $sheets = [];
                $GLOBALS['message'] = Message::error(__('Could not parse OpenDocument Spreadsheet!'));
                $GLOBALS['error'] = true;
            } else {
    ...
    {
        if ($_REQUEST['ods_recognize_percentages'] && !strcmp('percentage', (string) $cell_attrs['value-type'])) {
            return (float) $cell_attrs['value'];
        }
        if ($_REQUEST['ods_recognize_currency'] && !strcmp('currency', (string) $cell_attrs['value-type'])) {
            return (float) $cell_attrs['value'];
        }
    ...
    private function iterateOverRows(SimpleXMLElement $sheet, bool $col_names_in_first_row, array $tempRow, array $col_names, int $col_count, int $max_cols, array $tempRows) : array
    {
        ...
        foreach ($sheet as $row) {
            ...
            [$tempRow, $col_names, $col_count] = $this->iterateOverColumns($row, $col_names_in_first_row, $tempRow, $col_names, $col_count);
            /* Find the widest row */
    ...
    private function iterateOverTables($sheets) : array
    {
        ...
        foreach ($sheets as $sheet) {
            ...
            [$tempRow, $col_names, $max_cols, $tempRows] = $this->iterateOverRows($sheet, $col_names_in_first_row, $tempRow, $col_names, $col_count, $max_cols, $tempRows);
