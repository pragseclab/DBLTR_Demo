    ...
    public function doImport(?File $importHandle = null, array &$sql_data = [])
    {
        ...
        if ($compression === 'application/zip' && $this->zipExtension->getNumberOfFiles($import_file) > 1) {
            if ($importHandle->openZip('/^.*\\.shp$/i') === false) {
                $message = Message::error(__('There was an error importing the ESRI shape file: "%s".'));
        ...
        switch ($shp->shapeType) {
            ...
            default:
                $error = true;
                $message = Message::error(__('MySQL Spatial Extension does not support ESRI type "%s".'));
        ...
        if (count($rows) === 0) {
            $error = true;
            $message = Message::error(__('The imported file does not contain any data!'));
            return;
        }
