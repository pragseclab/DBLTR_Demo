...
class ImportXml extends ImportPlugin
{
    ...
    public function doImport(?File $importHandle = null, array &$sql_data = [])
    {
         ...
         */
        if ($xml === false) {
            echo Message::error(__('The XML file specified was either malformed or incomplete.' . ' Please correct the issue and try again.'))->getDisplay();
            unset($xml);
         ...
         */
        $db_attr = $xml->children($namespaces['pma'] ?? null)->{'structure_schemas'}->{'database'};
        ...
        if (isset($namespaces['pma'])) {
            ...
            foreach ($struct as $val1) {
                ...
                foreach ($val1 as $val2) {
                    ...
                    $create[] = 'USE ' . Util::backquote($attrs['name']);
