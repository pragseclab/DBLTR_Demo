...
class AuthenticationSignon extends AuthenticationPlugin
{
    ...
    public function setCookieParams(?array $sessionCookieParams = null) : void
    {
        ...
        if (version_compare(PHP_VERSION, '7.3.0', '>=')) {
            ...
            session_set_cookie_params($sessionCookieParams['lifetime'], $sessionCookieParams['path'], $sessionCookieParams['domain'], $sessionCookieParams['secure'], $sessionCookieParams['httponly']);
        }
    }
    /**
    ...
    public function readCredentials()
    {
        ...
        if (!empty($script_name)) {
            if (!@file_exists($script_name)) {
                Core::fatalError(__('Can not find signon authentication script:') . ' ' . $script_name);
            }
            include $script_name;
