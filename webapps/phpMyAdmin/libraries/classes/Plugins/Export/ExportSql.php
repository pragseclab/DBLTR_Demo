...
class ExportSql extends ExportPlugin
{
    ...
    {
        global $crlf, $dbi;
        $text = $this->exportComment() . $this->exportComment($name) . $this->exportComment();
        $used_alias = false;
        $proc_query = '';
        foreach ($routines as $routine) {
            ...
            $create_query = $this->replaceWithAliases($dbi->getDefinition($db, $type, $routine), $aliases, $db, '', $flag);
        ...
        if ($used_alias) {
            $text .= $this->exportComment(__('It appears your database uses routines;')) . $this->exportComment(__('alias export may not work reliably in all cases.')) . $this->exportComment();
        }
        $text .= $proc_query;
        return $text;
    }
    ...
    private function exportUseStatement($db, $compat)
    {
        global $crlf;
        if (isset($GLOBALS['sql_compatibility']) && $GLOBALS['sql_compatibility'] === 'NONE') {
            $result = $this->export->outputHandler('USE ' . Util::backquoteCompat($db, $compat, isset($GLOBALS['sql_backquotes'])) . ';' . $crlf);
        } else {
    ...
    public function exportMetadata($db, $tables, array $metadataTypes)
    {
        ...
        $comment = $this->possibleCRLF() . $this->possibleCRLF() . $this->exportComment() . $this->exportComment(__('Metadata')) . $this->exportComment();
    ...
    private function exportConfigurationMetadata($db, $table, array $metadataTypes)
    {
        ...
        if (isset($table)) {
            ...
            $types = ['bookmark' => 'dbase', 'relation' => 'master_db', 'pdf_pages' => 'db_name', 'savedsearches' => 'db_name', 'central_columns' => 'db_name'];
        }
        $aliases = [];
        $comment = $this->possibleCRLF() . $this->exportComment();
        if (isset($table)) {
            $comment .= $this->exportComment(sprintf(__('Metadata for table %s'), $table));
        } else {
            $comment .= $this->exportComment(sprintf(__('Metadata for database %s'), $db));
        }
        ...
        foreach ($types as $type => $dbNameColumn) {
            ...
            if ($type === 'pdf_pages') {
                $sql_query = 'SELECT `page_nr`, `page_descr` FROM ' . Util::backquote($cfgRelation['db']) . '.' . Util::backquote($cfgRelation[$type]) . ' WHERE ' . Util::backquote($dbNameColumn) . " = '" . $dbi->escapeString($db) . "'";
                $result = $dbi->fetchResult($sql_query, 'page_nr', 'page_descr');
                foreach ($result as $page => $name) {
                    // insert row for pdf_page
                    $sql_query_row = 'SELECT `db_name`, `page_descr` FROM ' . Util::backquote($cfgRelation['db']) . '.' . Util::backquote($cfgRelation[$type]) . ' WHERE ' . Util::backquote($dbNameColumn) . " = '" . $dbi->escapeString($db) . "'" . " AND `page_nr` = '" . intval($page) . "'";
            ...
            $sql_query .= Util::backquote($cfgRelation['db']) . '.' . Util::backquote($cfgRelation[$type]) . ' WHERE ' . Util::backquote($dbNameColumn) . " = '" . $dbi->escapeString($db) . "'";
            if (isset($table)) {
                $sql_query .= " AND `table_name` = '" . $dbi->escapeString($table) . "'";
            }
    ...
    private function getTableDefForView($db, $view, $crlf, $add_semicolon = true, array $aliases = [])
    {
        ...
        foreach ($columns as $column) {
            ...
            if (isset($column['Default'])) {
                $create_query .= " DEFAULT '" . $dbi->escapeString($column['Default']) . "'";
            } else {
                if ($column['Null'] === 'YES') {
            ...
            if (!empty($column['Comment'])) {
                $create_query .= " COMMENT '" . $dbi->escapeString($column['Comment']) . "'";
            }
            $firstCol = false;
        }
    ...
    private function makeCreateTableMSSQLCompatible($create_query)
    {
        ...
        $create_query = (string) preg_replace('/^CREATE TABLE IF NOT EXISTS/', 'CREATE TABLE', (string) $create_query);
        ...
        $create_query = (string) preg_replace("/\" date DEFAULT NULL(,)?\n/", '" datetime DEFAULT NULL$1' . "\n", $create_query);
        $create_query = (string) preg_replace("/\" date NOT NULL(,)?\n/", '" datetime NOT NULL$1' . "\n", $create_query);
        $create_query = (string) preg_replace('/" date NOT NULL DEFAULT \'([^\'])/', '" datetime NOT NULL DEFAULT \'$1', $create_query);
        // next we need to replace all lines ended with ') UNSIGNED ...,'
        // last preg_replace preserve us from situation with unsigned text
        // inside DEFAULT field value
        $create_query = (string) preg_replace("/\\) unsigned NOT NULL(,)?\n/", ') NOT NULL$1' . "\n", $create_query);
        $create_query = (string) preg_replace("/\\) unsigned DEFAULT NULL(,)?\n/", ') DEFAULT NULL$1' . "\n", $create_query);
        $create_query = (string) preg_replace('/\\) unsigned NOT NULL DEFAULT \'([^\'])/', ') NOT NULL DEFAULT \'$1', $create_query);
        ...
        // value
        $create_query = (string) preg_replace('/" (int|tinyint|smallint|bigint)\\([0-9]+\\) DEFAULT NULL(,)?\\n/', '" $1 DEFAULT NULL$2' . "\n", $create_query);
        $create_query = (string) preg_replace('/" (int|tinyint|smallint|bigint)\\([0-9]+\\) NOT NULL(,)?\\n/', '" $1 NOT NULL$2' . "\n", $create_query);
        $create_query = (string) preg_replace('/" (int|tinyint|smallint|bigint)\\([0-9]+\\) NOT NULL DEFAULT \'([^\'])/', '" $1 NOT NULL DEFAULT \'$2', $create_query);
        // we need to replace all lines ended with
        // '" FLOAT|DOUBLE([0-9,]{1,}) ...,'
        // last preg_replace preserve us from situation with
        // float([0-9,]{1,}) text inside DEFAULT field value
        $create_query = (string) preg_replace('/" (float|double)(\\([0-9]+,[0-9,]+\\))? DEFAULT NULL(,)?\\n/', '" float DEFAULT NULL$3' . "\n", $create_query);
        $create_query = (string) preg_replace('/" (float|double)(\\([0-9,]+,[0-9,]+\\))? NOT NULL(,)?\\n/', '" float NOT NULL$3' . "\n", $create_query);
        return (string) preg_replace('/" (float|double)(\\([0-9,]+,[0-9,]+\\))? NOT NULL DEFAULT \'([^\'])/', '" float NOT NULL DEFAULT \'$3', $create_query);
