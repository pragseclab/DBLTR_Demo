...
class ExportJson extends ExportPlugin
{
    ...
    {
        global $crlf;
        $meta = ['type' => 'header', 'version' => PMA_VERSION, 'comment' => 'Export to JSON plugin for PHPMyAdmin'];
        return $this->export->outputHandler('[' . $crlf . $this->encode($meta) . ',' . $crlf);
    }
    /**
    ...
    public function exportDBHeader($db, $db_alias = '')
    {
        ...
        $meta = ['type' => 'database', 'name' => $db_alias];
        return $this->export->outputHandler($this->encode($meta) . ',' . $crlf);
    }
    /**
    ...
    public function exportData($db, $table, $crlf, $error_url, $sql_query, array $aliases = [])
    {
        ...
        $buffer = $this->encode(['type' => 'table', 'name' => $table_alias, 'database' => $db_alias, 'data' => '@@DATA@@']);
