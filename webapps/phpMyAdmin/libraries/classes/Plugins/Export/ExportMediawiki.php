...
class ExportMediawiki extends ExportPlugin
{
    ...
    public function exportStructure($db, $table, $crlf, $error_url, $export_mode, $export_type, $do_relation = false, $do_comments = false, $do_mime = false, $dates = false, array $aliases = array())
    {
        ...
        switch ($export_mode) {
            case 'create_table':
                ...
                $output = $this->exportComment('Table structure for ' . Util::backquote($table_alias));
                // Begin the table construction
                $output .= '{| class="wikitable" style="text-align:center;"' . $this->exportCRLF();
                // Add the table name
    ...
    public function exportData($db, $table, $crlf, $error_url, $sql_query, array $aliases = array())
    {
        ...
        $this->initAlias($aliases, $db_alias, $table_alias);
        // Print data comment
        $output = $this->exportComment($table_alias != '' ? 'Table data for ' . Util::backquote($table_alias) : 'Query results');
        // Begin the table construction
        // Use the "wikitable" class for style
        // Use the "sortable"  class for allowing tables to be sorted by column
        $output .= '{| class="wikitable sortable" style="text-align:center;"' . $this->exportCRLF();
        // Add the table name
        if (isset($GLOBALS['mediawiki_caption'])) {
            $output .= "|+'''" . $table_alias . "'''" . $this->exportCRLF();
        }
