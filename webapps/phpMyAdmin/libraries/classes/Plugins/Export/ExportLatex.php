...
class ExportLatex extends ExportPlugin
{
    ...
    {
        global $crlf, $cfg, $dbi;
        $head = '% phpMyAdmin LaTeX Dump' . $crlf . '% version ' . PMA_VERSION . $crlf . '% https://www.phpmyadmin.net/' . $crlf . '%' . $crlf . '% ' . __('Host:') . ' ' . $cfg['Server']['host'];
        if (!empty($cfg['Server']['port'])) {
            $head .= ':' . $cfg['Server']['port'];
        }
        $head .= $crlf . '% ' . __('Generation Time:') . ' ' . Util::localisedDate() . $crlf . '% ' . __('Server version:') . ' ' . $dbi->getVersionString() . $crlf . '% ' . __('PHP Version:') . ' ' . PHP_VERSION . $crlf;
    ...
    public function exportDBHeader($db, $db_alias = '')
    {
        ...
        global $crlf;
        $head = '% ' . $crlf . '% ' . __('Database:') . ' \'' . $db_alias . '\'' . $crlf . '% ' . $crlf;
        return $this->export->outputHandler($head);
    }
    /**
    ...
    public function exportData($db, $table, $crlf, $error_url, $sql_query, array $aliases = [])
    {
        ...
        $buffer = $crlf . '%' . $crlf . '% ' . __('Data:') . ' ' . $table_alias . $crlf . '%' . $crlf . ' \\begin{longtable}{|';
        ...
        if (isset($GLOBALS['latex_caption'])) {
            $buffer .= ' \\caption{' . Util::expandUserString($GLOBALS['latex_data_caption'], ['texEscape', static::class], ['table' => $table_alias, 'database' => $db_alias]) . '} \\label{' . Util::expandUserString($GLOBALS['latex_data_label'], null, ['table' => $table_alias, 'database' => $db_alias]) . '} \\\\';
        }
        ...
        if (isset($GLOBALS['latex_columns'])) {
            $buffer = '\\hline ';
            for ($i = 0; $i < $columns_cnt; $i++) {
                $buffer .= '\\multicolumn{1}{|c|}{\\textbf{' . self::texEscape(stripslashes($columns_alias[$i])) . '}} & ';
            }
            ...
            if (isset($GLOBALS['latex_caption'])) {
                if (!$this->export->outputHandler('\\caption{' . Util::expandUserString($GLOBALS['latex_data_continued_caption'], ['texEscape', static::class], ['table' => $table_alias, 'database' => $db_alias]) . '} \\\\ ')) {
                    return false;
                }
            }
        ...
        while ($record = $dbi->fetchAssoc($result)) {
            ...
            for ($i = 0; $i < $columns_cnt; $i++) {
                if ($record[$columns[$i]] !== null && isset($record[$columns[$i]])) {
                    $column_value = self::texEscape(stripslashes($record[$columns[$i]]));
                } else {
    ...
    public function exportStructure($db, $table, $crlf, $error_url, $export_mode, $export_type, $do_relation = false, $do_comments = false, $do_mime = false, $dates = false, array $aliases = [])
    {
        ...
        if (isset($GLOBALS['latex_caption'])) {
            $buffer .= ' \\caption{' . Util::expandUserString($GLOBALS['latex_structure_caption'], ['texEscape', static::class], ['table' => $table_alias, 'database' => $db_alias]) . '} \\label{' . Util::expandUserString($GLOBALS['latex_structure_label'], null, ['table' => $table_alias, 'database' => $db_alias]) . '} \\\\' . $crlf;
        }
        $buffer .= $header . ' \\\\ \\hline \\hline' . $crlf . '\\endfirsthead' . $crlf;
        // Table caption on next pages
        if (isset($GLOBALS['latex_caption'])) {
            $buffer .= ' \\caption{' . Util::expandUserString($GLOBALS['latex_structure_continued_caption'], ['texEscape', static::class], ['table' => $table_alias, 'database' => $db_alias]) . '} \\\\ ' . $crlf;
        }
        ...
        foreach ($fields as $row) {
            ...
            $local_buffer = $col_as . "\x00" . $type . "\x00" . ($row['Null'] == '' || $row['Null'] === 'NO' ? __('No') : __('Yes')) . "\x00" . ($row['Default'] ?? '');
            if ($do_relation && $have_rel) {
                $local_buffer .= "\x00";
            ...
            if ($row['Key'] === 'PRI') {
                $pos = (int) mb_strpos($local_buffer, "\x00");
            ...
            $buffer = str_replace("\x00", ' & ', $local_buffer);
            $buffer .= ' \\\\ \\hline ' . $crlf;
            if (!$this->export->outputHandler($buffer)) {
                return false;
            }
