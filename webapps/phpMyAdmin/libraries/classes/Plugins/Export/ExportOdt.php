...
class ExportOdt extends ExportPlugin
{
    ...
    {
        $GLOBALS['odt_buffer'] .= '<?xml version="1.0" encoding="utf-8"?' . '>' . '<office:document-content ' . OpenDocument::NS . ' office:version="1.0">' . '<office:body>' . '<office:text>';
        return true;
    }
    /**
    ...
    public function exportFooter()
    {
        ...
        return $this->export->outputHandler(OpenDocument::create('application/vnd.oasis.opendocument.text', $GLOBALS['odt_buffer']));
    }
    /**
    ...
    public function exportDBHeader($db, $db_alias = '')
    {
        ...
        $GLOBALS['odt_buffer'] .= '<text:h text:outline-level="1" text:style-name="Heading_1"' . ' text:is-list-header="true">' . __('Database') . ' ' . htmlspecialchars($db_alias) . '</text:h>';
        return true;
    }
    /**
    ...
    public function exportData($db, $table, $crlf, $error_url, $sql_query, array $aliases = [])
    {
        ...
        $table_alias != '' ? $GLOBALS['odt_buffer'] .= __('Dumping data for table') . ' ' . htmlspecialchars($table_alias) : ($GLOBALS['odt_buffer'] .= __('Dumping data for query result'));
        $GLOBALS['odt_buffer'] .= '</text:h>' . '<table:table' . ' table:name="' . htmlspecialchars($table_alias) . '_structure">' . '<table:table-column' . ' table:number-columns-repeated="' . $fields_cnt . '"/>';
        ...
        if (isset($GLOBALS[$what . '_columns'])) {
            ...
            for ($i = 0; $i < $fields_cnt; $i++) {
                ...
                $GLOBALS['odt_buffer'] .= '<table:table-cell office:value-type="string">' . '<text:p>' . htmlspecialchars(stripslashes($col_as)) . '</text:p>' . '</table:table-cell>';
            }
            $GLOBALS['odt_buffer'] .= '</table:table-row>';
        }
        // Format the data
        while ($row = $dbi->fetchRow($result)) {
            ...
            for ($j = 0; $j < $fields_cnt; $j++) {
                ...
                } elseif (stripos($field_flags[$j], 'BINARY') !== false && $fields_meta[$j]->blob) {
                    // ignore BLOB
    ...
    public function getTableDefStandIn($db, $view, $crlf, $aliases = [])
    {
         ...
         */
        $GLOBALS['odt_buffer'] .= '<table:table table:name="' . htmlspecialchars($view_alias) . '_data">';
        $columns_cnt = 4;
    ...
    public function getTableDef($db, $table, $crlf, $error_url, $do_relation, $do_comments, $do_mime, $show_dates = false, $add_semicolon = true, $view = false, array $aliases = [])
    {
        ...
        if ($do_mime && $cfgRelation['mimework']) {
            $GLOBALS['odt_buffer'] .= '<table:table-cell office:value-type="string">' . '<text:p>' . __('Media type') . '</text:p>' . '</table:table-cell>';
