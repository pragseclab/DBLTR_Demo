...
class ExportTexytext extends ExportPlugin
{
    ...
    public function exportDBHeader($db, $db_alias = '')
    {
        ...
        return $this->export->outputHandler('===' . __('Database') . ' ' . $db_alias . "\n\n");
    }
    /**
    ...
    public function exportData($db, $table, $crlf, $error_url, $sql_query, array $aliases = [])
    {
        ...
        if (!$this->export->outputHandler($table_alias != '' ? '== ' . __('Dumping data for table') . ' ' . $table_alias . "\n\n" : '==' . __('Dumping data for query result') . "\n\n")) {
            return false;
        }
        ...
        if (isset($GLOBALS[$what . '_columns'])) {
            ...
            for ($i = 0; $i < $fields_cnt; $i++) {
                ...
                $text_output .= '|' . htmlspecialchars(stripslashes($col_as));
            }
            $text_output .= "\n|------\n";
    ...
    public function formatOneColumnDefinition($column, $unique_keys, $col_alias = '')
    {
        ...
        $definition .= '|' . htmlspecialchars($column['Default'] ?? '');
        return $definition;
    }
}