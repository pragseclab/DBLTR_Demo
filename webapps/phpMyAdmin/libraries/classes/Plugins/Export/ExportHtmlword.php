...
class ExportHtmlword extends ExportPlugin
{
    ...
    {
        global $charset;
        return $this->export->outputHandler('<html xmlns:o="urn:schemas-microsoft-com:office:office"
    ...
    public function exportDBHeader($db, $db_alias = '')
    {
        ...
        return $this->export->outputHandler('<h1>' . __('Database') . ' ' . htmlspecialchars($db_alias) . '</h1>');
    }
    /**
    ...
    public function exportData($db, $table, $crlf, $error_url, $sql_query, array $aliases = [])
    {
        ...
        if (!$this->export->outputHandler('<h2>' . __('Dumping data for table') . ' ' . htmlspecialchars($table_alias) . '</h2>')) {
            return false;
        }
        if (!$this->export->outputHandler('<table class="pma-table w-100" cellspacing="1">')) {
            return false;
        }
        // Gets the data from the database
        $result = $dbi->query($sql_query, DatabaseInterface::CONNECT_USER, DatabaseInterface::QUERY_UNBUFFERED);
        ...
        if (isset($GLOBALS['htmlword_columns'])) {
            ...
            for ($i = 0; $i < $fields_cnt; $i++) {
                ...
                $schema_insert .= '<td class="print"><strong>' . htmlspecialchars($col_as) . '</strong></td>';
            }
            $schema_insert .= '</tr>';
        ...
        while ($row = $dbi->fetchRow($result)) {
            ...
            for ($j = 0; $j < $fields_cnt; $j++) {
                ...
                $schema_insert .= '<td class="print">' . htmlspecialchars((string) $value) . '</td>';
            }
            $schema_insert .= '</tr>';
            if (!$this->export->outputHandler($schema_insert)) {
                return false;
    ...
    {
        global $dbi;
        $schema_insert = '<table class="pma-table w-100" cellspacing="1">' . '<tr class="print-category">' . '<th class="print">' . __('Column') . '</th>' . '<td class="print"><strong>' . __('Type') . '</strong></td>' . '<td class="print"><strong>' . __('Null') . '</strong></td>' . '<td class="print"><strong>' . __('Default') . '</strong></td>' . '</tr>';
        /**
    ...
    public function getTableDef($db, $table, $do_relation, $do_comments, $do_mime, $view = false, array $aliases = [])
    {
        ...
        if ($do_mime && $cfgRelation['mimework']) {
            $schema_insert .= '<td class="print"><strong>' . __('Media type') . '</strong></td>';
    ...
    protected function getTriggers($db, $table)
    {
        ...
        foreach ($triggers as $trigger) {
            $dump .= '<tr class="print-category">';
            $dump .= '<td class="print">' . htmlspecialchars($trigger['name']) . '</td>' . '<td class="print">' . htmlspecialchars($trigger['action_timing']) . '</td>' . '<td class="print">' . htmlspecialchars($trigger['event_manipulation']) . '</td>' . '<td class="print">' . htmlspecialchars($trigger['definition']) . '</td>' . '</tr>';
        }
        $dump .= '</table>';
        return $dump;
    }
    ...
    public function exportStructure($db, $table, $crlf, $error_url, $export_mode, $export_type, $do_relation = false, $do_comments = false, $do_mime = false, $dates = false, array $aliases = [])
    {
        ...
        switch ($export_mode) {
            case 'create_table':
                $dump .= '<h2>' . __('Table structure for table') . ' ' . htmlspecialchars($table_alias) . '</h2>';
            ...
            case 'triggers':
                ...
                $triggers = $dbi->getTriggers($db, $table);
                if ($triggers) {
                    $dump .= '<h2>' . __('Triggers') . ' ' . htmlspecialchars($table_alias) . '</h2>';
                    $dump .= $this->getTriggers($db, $table);
                }
                break;
            case 'create_view':
                $dump .= '<h2>' . __('Structure for view') . ' ' . htmlspecialchars($table_alias) . '</h2>';
                $dump .= $this->getTableDef($db, $table, $do_relation, $do_comments, $do_mime, true, $aliases);
                break;
            case 'stand_in':
                $dump .= '<h2>' . __('Stand-in structure for view') . ' ' . htmlspecialchars($table_alias) . '</h2>';
    ...
    protected function formatOneColumnDefinition(array $column, array $unique_keys, $col_alias = '')
    {
        ...
        $definition .= '<td class="print">' . htmlspecialchars($column['Default'] ?? '') . '</td>';
        return $definition;
    }
}