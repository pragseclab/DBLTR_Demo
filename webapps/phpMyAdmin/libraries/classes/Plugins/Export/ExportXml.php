...
class ExportXml extends ExportPlugin
{
    ...
    public function exportHeader()
    {
        ...
        $head .= $crlf . '- ' . __('Generation Time:') . ' ' . Util::localisedDate() . $crlf . '- ' . __('Server version:') . ' ' . $dbi->getVersionString() . $crlf . '- ' . __('PHP Version:') . ' ' . PHP_VERSION . $crlf . '-->' . $crlf . $crlf;
        $head .= '<pma_xml_export version="1.0"' . ($export_struct ? ' xmlns:pma="https://www.phpmyadmin.net/some_doc_url/"' : '') . '>' . $crlf;
        if ($export_struct) {
            $result = $dbi->fetchResult('SELECT `DEFAULT_CHARACTER_SET_NAME`, `DEFAULT_COLLATION_NAME`' . ' FROM `information_schema`.`SCHEMATA` WHERE `SCHEMA_NAME`' . ' = \'' . $dbi->escapeString($db) . '\' LIMIT 1');
            ...
            foreach ($tables as $table) {
                ...
                $result = $dbi->fetchResult('SHOW CREATE TABLE ' . Util::backquote($db) . '.' . Util::backquote($table), 0);
                $tbl = (string) $result[$table][1];
                $is_view = $dbi->getTable($db, $table)->isView();
                if ($is_view) {
                    $type = 'view';
                } else {
                    $type = 'table';
            ...
            if (isset($GLOBALS['xml_export_events']) && $GLOBALS['xml_export_events']) {
                // Export events
                $events = $dbi->fetchResult('SELECT EVENT_NAME FROM information_schema.EVENTS ' . "WHERE EVENT_SCHEMA='" . $dbi->escapeString($db) . "'");
    ...
    public function exportDBHeader($db, $db_alias = '')
    {
        ...
        if (isset($GLOBALS['xml_export_contents']) && $GLOBALS['xml_export_contents']) {
            $head = '    <!--' . $crlf . '    - ' . __('Database:') . ' \'' . htmlspecialchars($db_alias) . '\'' . $crlf . '    -->' . $crlf . '    <database name="' . htmlspecialchars($db_alias) . '">' . $crlf;
            return $this->export->outputHandler($head);
        }
        return true;
    }
