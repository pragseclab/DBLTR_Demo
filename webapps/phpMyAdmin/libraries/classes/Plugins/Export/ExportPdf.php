...
class ExportPdf extends ExportPlugin
{
    ...
    public function exportData($db, $table, $crlf, $error_url, $sql_query, array $aliases = array())
    {
        ...
        $pdf = $this->getPdf();
        $attr = ['currentDb' => $db, 'currentTable' => $table, 'dbAlias' => $db_alias, 'tableAlias' => $table_alias, 'aliases' => $aliases, 'purpose' => __('Dumping data')];
        $pdf->setAttributes($attr);
    ...
    {
        $pdf = $this->getPdf();
        $attr = ['dbAlias' => '----', 'tableAlias' => '----', 'purpose' => __('Query result data')];
        $pdf->setAttributes($attr);
        $pdf->mysqlReport($sql_query);
        return true;
    }
    ...
    public function exportStructure($db, $table, $crlf, $error_url, $export_mode, $export_type, $do_relation = false, $do_comments = false, $do_mime = false, $dates = false, array $aliases = array())
    {
        ...
        $attr = ['currentDb' => $db, 'currentTable' => $table, 'dbAlias' => $db_alias, 'tableAlias' => $table_alias, 'aliases' => $aliases, 'purpose' => $purpose];
        $pdf->setAttributes($attr);
        /**
