...
class ExportRelationSchema
{
    ...
    protected function getFileName($extension) : string
    {
        ...
        if ($this->pageNumber != -1 && !$this->offline) {
            $_name_sql = 'SELECT page_descr FROM ' . Util::backquote($GLOBALS['cfgRelation']['db']) . '.' . Util::backquote($GLOBALS['cfgRelation']['pdf_pages']) . ' WHERE page_nr = ' . $this->pageNumber;
    ...
    public static function dieSchema($pageNumber, $type = '', $error_message = '')
    {
        ...
        echo '</p>', "\n";
        echo '<a href="';
        echo Url::getFromRoute('/database/designer', ['db' => $GLOBALS['db'], 'server' => $GLOBALS['server'], 'page' => $pageNumber]);
        echo '">' . __('Back') . '</a>';
        echo "\n";
        exit;
    }
