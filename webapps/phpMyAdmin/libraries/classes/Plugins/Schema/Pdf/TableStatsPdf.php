...
class TableStatsPdf extends TableStats
{
    ...
    {
        ExportRelationSchema::dieSchema($this->pageNumber, 'PDF', sprintf(__('The %s table doesn\'t exist!'), $this->tableName));
    }
    /**
    ...
    public function tableDraw($fontSize, $withDoc, $setColor = 0)
    {
        ...
        foreach ($this->fields as $field) {
            ...
            $this->diagram->cellScale($this->width, $this->heightCell, ' ' . $field, 1, 1, 'L', $setColor, $this->diagram->customLinks['doc'][$this->tableName][$field]);
            $this->diagram->setXScale($this->x);
            $this->diagram->SetFillColor(255);
        }
    }
