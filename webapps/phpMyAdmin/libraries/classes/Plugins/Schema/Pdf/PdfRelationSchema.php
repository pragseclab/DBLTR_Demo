...
class PdfRelationSchema extends ExportRelationSchema
{
    ...
    public function __construct($db)
    {
        ...
        parent::__construct($db, new Pdf($this->orientation, 'mm', $this->paper, $this->pageNumber, $this->withDoc, $db));
        ...
        foreach ($alltables as $table) {
            if (!isset($this->tables[$table])) {
                $this->tables[$table] = new TableStatsPdf($this->diagram, $this->db, $table, null, $this->pageNumber, $this->tablewidth, $this->showKeys, $this->tableDimension, $this->offline);
            }
            if ($this->sameWide) {
        ...
        $innerWidth = $this->diagram->getPageWidth() - $this->rightMargin - $this->leftMargin;
        $innerHeight = $this->diagram->getPageHeight() - $this->topMargin - $this->bottomMargin;
        $this->scale = ceil(max(($this->xMax - $this->xMin) / $innerWidth, ($this->yMax - $this->yMin) / $innerHeight) * 100) / 100;
            ...
            foreach ($exist_rel as $master_field => $rel) {
                ...
                foreach ($rel as $one_key) {
                    ...
                    foreach ($one_key['index_list'] as $index => $one_field) {
                        $this->addRelation($one_table, $one_field, $one_key['ref_table_name'], $one_key['ref_index_list'][$index]);
                    }
                }
            }
        }
     ...
     * @return void
     */
    private function addRelation($masterTable, $masterField, $foreignTable, $foreignField)
    {
        if (!isset($this->tables[$masterTable])) {
            $this->tables[$masterTable] = new TableStatsPdf($this->diagram, $this->db, $masterTable, null, $this->pageNumber, $this->tablewidth, $this->showKeys, $this->tableDimension);
            $this->setMinMax($this->tables[$masterTable]);
        }
        if (!isset($this->tables[$foreignTable])) {
            $this->tables[$foreignTable] = new TableStatsPdf($this->diagram, $this->db, $foreignTable, null, $this->pageNumber, $this->tablewidth, $this->showKeys, $this->tableDimension);
            $this->setMinMax($this->tables[$foreignTable]);
        }
        $this->relations[] = new RelationStatsPdf($this->diagram, $this->tables[$masterTable], $masterField, $this->tables[$foreignTable], $foreignField);
    }
    /**
     * Draws the grid
     *
    ...
    private function strokeGrid()
    {
        ...
        for ($l = 0, $size = intval($innerHeight / $gridSize); $l <= $size; $l++) {
            $this->diagram->line(0, $l * $gridSize + $topSpace, $this->diagram->getPageWidth(), $l * $gridSize + $topSpace);
            // Avoid duplicates
            if ($l <= 0 || $l > intval(($innerHeight - $labelHeight) / $gridSize)) {
                continue;
            }
            $this->diagram->SetXY(0, $l * $gridSize + $topSpace);
            $label = (string) sprintf('%.0f', ($l * $gridSize + $topSpace - $this->topMargin) * $this->scale + $this->yMin);
            $this->diagram->Cell($labelWidth, $labelHeight, ' ' . $label);
        }
        // Draws vertical lines
        for ($j = 0, $size = intval($this->diagram->getPageWidth() / $gridSize); $j <= $size; $j++) {
            $this->diagram->line($j * $gridSize, $topSpace, $j * $gridSize, $this->diagram->getPageHeight() - $bottomSpace);
    ...
    public function dataDictionaryDoc(array $alltables)
    {
        ...
        foreach ($alltables as $table) {
            ...
            // $this->diagram->Ln(1);
            $this->diagram->Cell(0, 6, __('Page number:') . ' {' . sprintf('%02d', $i) . '}', 0, 0, 'R', 0, $this->diagram->customLinks['doc'][$table]['-']);
            $this->diagram->SetX(10);
            $this->diagram->Cell(0, 6, $i . ' ' . $table, 0, 1, 'L', 0, $this->diagram->customLinks['doc'][$table]['-']);
            // $this->diagram->Ln(1);
        ...
        $this->diagram->SetX(10);
        $this->diagram->Cell(0, 6, __('Page number:') . ' {00}', 0, 0, 'R', 0, $this->diagram->customLinks['RT']['-']);
        $this->diagram->SetX(10);
        $this->diagram->Cell(0, 6, $i . ' ' . __('Relational schema'), 0, 1, 'L', 0, $this->diagram->customLinks['RT']['-']);
        $z = 0;
        foreach ($alltables as $table) {
            $z++;
            ...
            $this->diagram->Cell(0, 8, $z . ' ' . $table, 1, 1, 'C', 0, $this->diagram->customLinks['RT'][$table]['-']);
             ...
             */
            $showtable = $dbi->getTable($this->db, $table)->getStatusInfo();
            ...
            if (isset($this->orientation) && $this->orientation === 'L') {
                ...
                $this->diagram->setWidths([25, 20, 20, 10, 20, 25, 45, $comments_width, 45]);
            } else {
            ...
            foreach ($columns as $row) {
                ...
                $diagram_row = [$field_name, $type, $attribute, $row['Null'] == '' || $row['Null'] === 'NO' ? __('No') : __('Yes'), $row['Default'] ?? '', $row['Extra'], $linksTo, $comments[$field_name] ?? '', isset($mime_map, $mime_map[$field_name]) ? str_replace('_', '/', $mime_map[$field_name]['mimetype']) : ''];
                $links = [];
                $links[0] = $this->diagram->customLinks['RT'][$table][$field_name];
                if ($foreigner && isset($this->diagram->customLinks['doc'][$foreigner['foreign_table']][$foreigner['foreign_field']])) {
