...
class Pdf extends PdfLib
{
     ...
     * @return void
     */
    public function setScale($scale = 1, $xMin = 0, $yMin = 0, $leftMargin = -1, $topMargin = -1)
    {
        $this->scale = $scale;
        $this->xMin = $xMin;
        $this->yMin = $yMin;
    ...
    public function Header()
    {
        ...
        if ($this->offline || $this->pageNumber == -1) {
            ...
            $test_query = 'SELECT * FROM ' . Util::backquote($GLOBALS['cfgRelation']['db']) . '.' . Util::backquote($GLOBALS['cfgRelation']['pdf_pages']) . ' WHERE db_name = \'' . $dbi->escapeString($this->db) . '\' AND page_nr = \'' . $this->pageNumber . '\'';
