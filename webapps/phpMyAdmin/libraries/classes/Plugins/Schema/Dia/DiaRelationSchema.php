...
class DiaRelationSchema extends ExportRelationSchema
{
    ...
    /** @var float */
    private $topMargin = 2.822200059890747;
    /** @var float */
    private $bottomMargin = 2.822200059890747;
    /** @var float */
    private $leftMargin = 2.822200059890747;
    /** @var float */
    private $rightMargin = 2.822200059890747;
    /** @var int */
    ...
    public function __construct($db)
    {
        ...
        $this->diagram->startDiaDoc($this->paper, $this->topMargin, $this->bottomMargin, $this->leftMargin, $this->rightMargin, $this->orientation);
        $alltables = $this->getTablesFromRequest();
        foreach ($alltables as $table) {
            if (isset($this->tables[$table])) {
                continue;
            }
            $this->tables[$table] = new TableStatsDia($this->diagram, $this->db, $table, $this->pageNumber, $this->showKeys, $this->offline);
        }
            ...
            foreach ($exist_rel as $master_field => $rel) {
                ...
                if ($master_field !== 'foreign_keys_data') {
                    if (in_array($rel['foreign_table'], $alltables)) {
                        $this->addRelation($one_table, $master_field, $rel['foreign_table'], $rel['foreign_field'], $this->showKeys);
                    }
                    continue;
                }
                foreach ($rel as $one_key) {
                    ...
                    foreach ($one_key['index_list'] as $index => $one_field) {
                        $this->addRelation($one_table, $one_field, $one_key['ref_table_name'], $one_key['ref_index_list'][$index], $this->showKeys);
                    }
                }
            }
        }
     ...
     * @access private
     */
    private function addRelation($masterTable, $masterField, $foreignTable, $foreignField, $showKeys)
    {
        if (!isset($this->tables[$masterTable])) {
            $this->tables[$masterTable] = new TableStatsDia($this->diagram, $this->db, $masterTable, $this->pageNumber, $showKeys);
        }
        if (!isset($this->tables[$foreignTable])) {
            $this->tables[$foreignTable] = new TableStatsDia($this->diagram, $this->db, $foreignTable, $this->pageNumber, $showKeys);
        }
        $this->relations[] = new RelationStatsDia($this->diagram, $this->tables[$masterTable], $masterField, $this->tables[$foreignTable], $foreignField);
    }
    /**
