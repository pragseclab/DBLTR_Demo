...
class Text_Plain_Iptolong extends IOTransformationsPlugin
{
    ...
    public function getInputHtml(array $column, $row_id, $column_name_appendix, array $options, $value, $text_dir, $tabindex, $tabindex_for_value, $idindex)
    {
        ...
        if (!empty($value)) {
            ...
            if ($value !== $val) {
                $html = '<input type="hidden" name="fields_prev' . $column_name_appendix . '" value="' . htmlspecialchars($val) . '"/>';
            }
        }
        return $html . '<input type="text" name="fields' . $column_name_appendix . '"' . ' value="' . htmlspecialchars($val) . '"' . ' size="40"' . ' dir="' . $text_dir . '"' . ' class="transform_IPToLong"' . ' id="field_' . $idindex . '_3"' . ' tabindex="' . ($tabindex + $tabindex_for_value) . '" />';
    }
    /* ~~~~~~~~~~~~~~~~~~~~ Getters and Setters ~~~~~~~~~~~~~~~~~~~~ */
    /**
