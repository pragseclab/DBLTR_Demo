...
abstract class RegexValidationTransformationsPlugin extends IOTransformationsPlugin
{
    ...
    {
        return __('Validates the string using regular expression ' . 'and performs insert only if string matches it. ' . 'The first option is the Regular Expression.');
    }
    /**
    ...
    public function applyTransformation($buffer, array $options = [], ?stdClass $meta = null)
    {
        ...
        if (!empty($options[0]) && !preg_match($options[0], $buffer)) {
            $this->success = false;
            $this->error = sprintf(__('Validation failed for the input string %s.'), htmlspecialchars($buffer));
        }
        return $buffer;
    }
    /* ~~~~~~~~~~~~~~~~~~~~ Getters and Setters ~~~~~~~~~~~~~~~~~~~~ */
