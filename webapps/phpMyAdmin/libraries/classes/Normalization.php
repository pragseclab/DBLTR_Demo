...
class Normalization
{
     ...
     */
    public function getHtmlForColumnsList($db, $table, $colTypeCategory = 'all', $listType = 'dropdown')
    {
        $columnTypeList = [];
     ...
     * @return string HTML
     */
    public function getHtmlForCreateNewColumn($numFields, $db, $table, array $columnMeta = array())
    {
        ...
        for ($columnNumber = 0; $columnNumber < $numFields; $columnNumber++) {
            $contentCells[$columnNumber] = ['column_number' => $columnNumber, 'column_meta' => $columnMeta, 'type_upper' => '', 'length_values_input_size' => 8, 'length' => '', 'extracted_columnspec' => [], 'submit_attribute' => null, 'comments_map' => $commentsMap, 'fields_meta' => null, 'is_backup' => true, 'move_columns' => [], 'cfg_relation' => $cfgRelation, 'available_mime' => $availableMime, 'mime_map' => $mimeMap];
        }
        ...
        foreach ($charsets as $charset) {
            ...
            $charsetsList[] = ['name' => $charset->getName(), 'description' => $charset->getDescription(), 'collations' => $collationsList];
        }
        return $this->template->render('columns_definitions/table_fields_definitions', ['is_backup' => true, 'fields_meta' => null, 'mimework' => $cfgRelation['mimework'], 'content_cells' => $contentCells, 'change_column' => $_POST['change_column'] ?? $_GET['change_column'] ?? null, 'is_virtual_columns_supported' => Util::isVirtualColumnsSupported(), 'browse_mime' => $GLOBALS['cfg']['BrowseMIME'], 'server_type' => Util::getServerType(), 'server_version' => $this->dbi->getVersion(), 'max_rows' => intval($GLOBALS['cfg']['MaxRows']), 'char_editing' => $GLOBALS['cfg']['CharEditing'], 'attribute_types' => $this->dbi->types->getAttributes(), 'privs_available' => $GLOBALS['col_priv'] && $GLOBALS['is_reload_priv'], 'max_length' => $this->dbi->getVersion() >= 50503 ? 1024 : 255, 'charsets' => $charsetsList]);
    }
    /**
    ...
    public function getHtmlContentsFor1NFStep2($db, $table)
    {
        ...
        if ($primary) {
            ...
            $subText = '<a href="#" id="createPrimaryKey">' . Generator::getIcon('b_index_add', __('Add a primary key on existing column(s)')) . '</a>';
        ...
        return ['legendText' => $legendText, 'headText' => $headText, 'subText' => $subText, 'hasPrimaryKey' => $hasPrimaryKey, 'extra' => $extra];
    }
    /**
    ...
    public function getHtmlContentsFor1NFStep4($db, $table)
    {
        ...
        $legendText = __('Step 1.') . $step . ' ' . $stepTxt;
        $headText = __('Do you have a group of columns which on combining gives an existing' . ' column? For example, if you have first_name, last_name and' . ' full_name then combining first_name and last_name gives full_name' . ' which is redundant.');
        $subText = __('Check the columns which are redundant and click on remove. ' . "If no redundant column, click on 'No redundant column'");
        $extra = $this->getHtmlForColumnsList($db, $table, 'all', 'checkbox') . '<br>' . '<input class="btn btn-secondary" type="submit" id="removeRedundant" value="' . __('Remove selected') . '">' . '<input class="btn btn-secondary" type="submit" value="' . __('No redundant column') . '" onclick="goToFinish1NF();">';
    ...
    public function getHtmlContentsFor1NFStep3($db, $table)
    {
        ...
        $extra = $this->getHtmlForColumnsList($db, $table, 'all', 'checkbox') . '<br>' . '<input class="btn btn-secondary" type="submit" id="moveRepeatingGroup" value="' . __('Done') . '">' . '<input class="btn btn-secondary" type="submit" value="' . __('No repeating group') . '" onclick="goToStep4();">';
        $primary = Index::getPrimary($table, $db);
        ...
        return ['legendText' => $legendText, 'headText' => $headText, 'subText' => $subText, 'extra' => $extra, 'primary_key' => json_encode($pk)];
    }
    /**
    ...
    public function getHtmlFor2NFstep1($db, $table)
    {
        ...
        foreach ($primarycols as $col) {
            $pk[] = $col->getName();
            $selectPkForm .= '<input type="checkbox" name="pd" value="' . htmlspecialchars($col->getName()) . '">' . htmlspecialchars($col->getName());
        }
        $key = implode(', ', $pk);
        if (count($primarycols) > 1) {
            ...
            if (count($pk) == count($columns)) {
                $headText = sprintf(__('No partial dependencies possible as ' . 'no non-primary column exists since primary key ( %1$s ) ' . 'is composed of all the columns in the table.'), htmlspecialchars($key)) . '<br>';
                $extra = '<h3>' . __('Table is already in second normal form.') . '</h3>';
            } else {
                $headText = sprintf(__('The primary key ( %1$s ) consists of more than one column ' . 'so we need to find the partial dependencies.'), htmlspecialchars($key)) . '<br>' . __('Please answer the following question(s) ' . 'carefully to obtain a correct normalization.') . '<br><a href="#" id="showPossiblePd">' . __('+ Show me the possible partial dependencies ' . 'based on data in the table') . '</a>';
                $subText = __('For each column below, ' . 'please select the <b>minimal set</b> of columns among given set ' . 'whose values combined together are sufficient' . ' to determine the value of the column.');
                $cnt = 0;
                foreach ($columns as $column) {
                    ...
                    $cnt++;
                    $extra .= '<b>' . sprintf(__('\'%1$s\' depends on:'), htmlspecialchars($column)) . '</b><br>';
                    $extra .= '<form id="pk_' . $cnt . '" data-colname="' . htmlspecialchars($column) . '" class="smallIndent">' . $selectPkForm . '</form><br><br>';
                }
            }
        } else {
            $headText = sprintf(__('No partial dependencies possible as the primary key' . ' ( %1$s ) has just one column.'), htmlspecialchars($key)) . '<br>';
            $extra = '<h3>' . __('Table is already in second normal form.') . '</h3>';
        }
        return ['legendText' => $legendText, 'headText' => $headText, 'subText' => $subText, 'extra' => $extra, 'primary_key' => $key];
    }
    /**
    ...
    {
        $html = '<p><b>' . sprintf(__('In order to put the ' . 'original table \'%1$s\' into Second normal form we need ' . 'to create the following tables:'), htmlspecialchars($table)) . '</b></p>';
        $tableName = $table;
        $i = 1;
        foreach ($partialDependencies as $key => $dependents) {
            $html .= '<p><input type="text" name="' . htmlspecialchars($key) . '" value="' . htmlspecialchars($tableName) . '">' . '( <u>' . htmlspecialchars($key) . '</u>' . (count($dependents) > 0 ? ', ' : '') . htmlspecialchars(implode(', ', $dependents)) . ' )';
            $i++;
            $tableName = 'table' . $i;
        }
        return $html;
    ...
    public function createNewTablesFor2NF(array $partialDependencies, $tablesName, $table, $db)
    {
        ...
        $queries = [];
        $error = false;
        $headText = '<h3>' . sprintf(__('The second step of normalization is complete for table \'%1$s\'.'), htmlspecialchars($table)) . '</h3>';
        if (count((array) $partialDependencies) === 1) {
            return ['legendText' => __('End of step'), 'headText' => $headText, 'queryError' => $error];
        }
        $message = '';
        $this->dbi->selectDb($db);
        foreach ($partialDependencies as $key => $dependents) {
            if ($tablesName->{$key} != $table) {
                ...
                $queries[] = 'CREATE TABLE ' . Util::backquote($tablesName->{$key}) . ' SELECT DISTINCT ' . $backquotedKey . (count($dependents) > 0 ? ', ' : '') . implode(',', Util::backquote($dependents)) . ' FROM ' . Util::backquote($table) . ';';
                $queries[] = 'ALTER TABLE ' . Util::backquote($tablesName->{$key}) . ' ADD PRIMARY KEY(' . $backquotedKey . ');';
        ...
        foreach ($queries as $query) {
            if (!$this->dbi->tryQuery($query)) {
                ...
                $message->addMessage(Message::rawError((string) $this->dbi->getError()), '<br><br>');
                $error = true;
                break;
            }
        }
        return ['legendText' => __('End of step'), 'headText' => $headText, 'queryError' => $error, 'extra' => $message];
    }
    /**
    ...
    public function getHtmlForNewTables3NF($dependencies, array $tables, $db)
    {
        ...
        foreach ($tables as $table => $arrDependson) {
            ...
            $html .= '<p><b>' . sprintf(__('In order to put the ' . 'original table \'%1$s\' into Third normal form we need ' . 'to create the following tables:'), htmlspecialchars($table)) . '</b></p>';
            $tableName = $table;
            $columnList = [];
            foreach ($arrDependson as $key) {
                ...
                $html .= '<p><input type="text" name="' . htmlspecialchars($tableName) . '" value="' . htmlspecialchars($tableName) . '">' . '( <u>' . htmlspecialchars($key) . '</u>' . (count($dependents) > 0 ? ', ' : '') . htmlspecialchars(implode(', ', $dependents)) . ' )';
        ...
        foreach ($newTables as $originalTable => $tablesList) {
            foreach ($tablesList as $table => $cols) {
                if ($table != $originalTable) {
                    $quotedPk = implode(', ', Util::backquote(explode(', ', $cols->pk)));
                    $quotedNonpk = implode(', ', Util::backquote(explode(', ', $cols->nonpk)));
                    $queries[] = 'CREATE TABLE ' . Util::backquote($table) . ' SELECT DISTINCT ' . $quotedPk . ', ' . $quotedNonpk . ' FROM ' . Util::backquote($originalTable) . ';';
                    $queries[] = 'ALTER TABLE ' . Util::backquote($table) . ' ADD PRIMARY KEY(' . $quotedPk . ');';
                } else {
                    $dropCols = $cols;
                }
            }
            if ($dropCols) {
                ...
                $colPresent = array_merge(explode(', ', $dropCols->pk), explode(', ', $dropCols->nonpk));
     ...
     * @return array
     */
    public function moveRepeatingGroup($repeatingColumns, $primaryColumns, $newTable, $newColumn, $table, $db)
    {
        $repeatingColumnsArr = (array) Util::backquote(explode(', ', $repeatingColumns));
        $primaryColumns = implode(',', Util::backquote(explode(',', $primaryColumns)));
        ...
        foreach ($repeatingColumnsArr as $repeatingColumn) {
            ...
            $first = false;
            $query1 .= ' SELECT ' . $primaryColumns . ',' . $repeatingColumn . ' as ' . Util::backquote($newColumn) . ' FROM ' . Util::backquote($table);
    ...
    public function getHtmlFor3NFstep1($db, array $tables)
    {
        ...
        $subText = __('For each column below, ' . 'please select the <b>minimal set</b> of columns among given set ' . 'whose values combined together are sufficient' . ' to determine the value of the column.<br>' . 'Note: A column may have no transitive dependency, ' . 'in that case you don\'t have to select any.');
        $cnt = 0;
        foreach ($tables as $table) {
            ...
            foreach ($columns as $column) {
                ...
                $selectTdForm .= '<input type="checkbox" name="pd" value="' . htmlspecialchars($column) . '">' . '<span>' . htmlspecialchars($column) . '</span>';
            }
            foreach ($columns as $column) {
                if (in_array($column, $pk)) {
                    continue;
                ...
                $cnt++;
                $extra .= '<b>' . sprintf(__('\'%1$s\' depends on:'), htmlspecialchars($column)) . '</b><br>';
                $extra .= '<form id="td_' . $cnt . '" data-colname="' . htmlspecialchars($column) . '" data-tablename="' . htmlspecialchars($table) . '" class="smallIndent">' . $selectTdForm . '</form><br><br>';
            }
        }
        if ($extra == '') {
            $headText = __('No Transitive dependencies possible as the table ' . "doesn't have any non primary key columns");
            $subText = '';
    ...
    {
        $htmlOutput = '<form method="post" action="' . Url::getFromRoute('/normalization') . '" name="normalize" ' . 'id="normalizeTable" ' . '>' . Url::getHiddenInputs($GLOBALS['db'], $GLOBALS['table']) . '<input type="hidden" name="step1" value="1">';
        $htmlOutput .= '<fieldset>';
        $htmlOutput .= '<legend>' . __('Improve table structure (Normalization):') . '</legend>';
        $htmlOutput .= '<h3>' . __('Select up to what step you want to normalize') . '</h3>';
        ...
        $htmlOutput .= '</label></div>';
        $htmlOutput .= '</fieldset><fieldset class="tblFooters">' . "<span class='floatleft'>" . __('Hint: Please follow the procedure carefully in order ' . 'to obtain correct normalization') . '</span>' . '<input class="btn btn-primary" type="submit" name="submit_normalize" value="' . __('Go') . '">' . '</fieldset>' . '</form>' . '</div>';
        return $htmlOutput;
    }
    /**
    ...
    public function findPartialDependencies($table, $db)
    {
        ...
        $totalRowsRes = $this->dbi->fetchResult('SELECT COUNT(*) FROM (SELECT * FROM ' . Util::backquote($table) . ' LIMIT 500) as dt;');
        ...
        $distinctValCount = $this->findDistinctValuesCount(array_unique(array_merge($columns, $partialKeys)), $table);
        foreach ($columns as $column) {
            if (in_array($column, $pk)) {
                continue;
            }
            foreach ($partialKeys as $partialKey) {
                if (!$partialKey || !$this->checkPartialDependency($partialKey, $column, $table, $distinctValCount[$partialKey], $distinctValCount[$column], $totalRows)) {
                    continue;
                }
        ...
        foreach ($dependencyList as $dependon => $colList) {
            $html .= '<span class="displayblock">' . '<input type="button" class="btn btn-secondary pickPd" value="' . __('Pick') . '">' . '<span class="determinants">' . htmlspecialchars(str_replace('`', '', (string) $dependon)) . '</span> -> ' . '<span class="dependents">' . htmlspecialchars(str_replace('`', '', implode(', ', $colList))) . '</span>' . '</span>';
        }
     ...
     */
    private function checkPartialDependency($partialKey, $column, $table, $pkCnt, $colCnt, $totalRows)
    {
        $query = 'SELECT ' . 'COUNT(DISTINCT ' . $partialKey . ',' . $column . ') as pkColCnt ' . 'FROM (SELECT * FROM ' . Util::backquote($table) . ' LIMIT 500) as dt;';
    ...
    private function getAllCombinationPartialKeys(array $primaryKey)
    {
        ...
        array_pop($results);
        //remove key which consist of all primary key columns
        return $results;
    }
}