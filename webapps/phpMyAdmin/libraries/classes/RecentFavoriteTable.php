...
class RecentFavoriteTable
{
    ...
    public function getFromDb()
    {
        ...
        $sql_query = ' SELECT `tables` FROM ' . $this->getPmaTable() . " WHERE `username` = '" . $dbi->escapeString($GLOBALS['cfg']['Server']['user']) . "'";
        $return = [];
    ...
    public function saveToDb()
    {
        ...
        $sql_query = ' REPLACE INTO ' . $this->getPmaTable() . ' (`username`, `tables`)' . " VALUES ('" . $dbi->escapeString($username) . "', '" . $dbi->escapeString(json_encode($this->tables)) . "')";
