...
class SavedSearches
{
    ...
    public function setCriterias($criterias, $json = false)
    {
        ...
        $aListFieldsToGet = ['criteriaColumn', 'criteriaSort', 'criteriaShow', 'criteria', 'criteriaAndOrRow', 'criteriaAndOrColumn', 'rows', 'TableList'];
        $data = [];
    ...
    public function save()
    {
        ...
        if ($this->getUsername() == null || $this->getDbname() == null || $this->getSearchName() == null || $this->getCriterias() == null) {
        ...
        $savedSearchesTbl = Util::backquote($this->config['cfgRelation']['db']) . '.' . Util::backquote($this->config['cfgRelation']['savedsearches']);
        //If it's an insert.
        if ($this->getId() === null) {
            $wheres = ["search_name = '" . $dbi->escapeString($this->getSearchName()) . "'"];
            ...
            $sqlQuery = 'INSERT INTO ' . $savedSearchesTbl . '(`username`, `db_name`, `search_name`, `search_data`)' . ' VALUES (' . "'" . $dbi->escapeString($this->getUsername()) . "'," . "'" . $dbi->escapeString($this->getDbname()) . "'," . "'" . $dbi->escapeString($this->getSearchName()) . "'," . "'" . $dbi->escapeString(json_encode($this->getCriterias())) . "')";
        ...
        //Else, it's an update.
        $wheres = ['id != ' . $this->getId(), "search_name = '" . $dbi->escapeString($this->getSearchName()) . "'"];
        $existingSearches = $this->getList($wheres);
        if (!empty($existingSearches)) {
            $message = Message::error(__('An entry with this name already exists.'));
            $response = Response::getInstance();
        ...
        $sqlQuery = 'UPDATE ' . $savedSearchesTbl . "SET `search_name` = '" . $dbi->escapeString($this->getSearchName()) . "', " . "`search_data` = '" . $dbi->escapeString(json_encode($this->getCriterias())) . "' " . 'WHERE id = ' . $this->getId();
    ...
    public function delete()
    {
        ...
        $savedSearchesTbl = Util::backquote($this->config['cfgRelation']['db']) . '.' . Util::backquote($this->config['cfgRelation']['savedsearches']);
        $sqlQuery = 'DELETE FROM ' . $savedSearchesTbl . "WHERE id = '" . $dbi->escapeString((string) $this->getId()) . "'";
        return (bool) $this->relation->queryAsControlUser($sqlQuery);
    }
    /**
    ...
    public function load()
    {
        ...
        $savedSearchesTbl = Util::backquote($this->config['cfgRelation']['db']) . '.' . Util::backquote($this->config['cfgRelation']['savedsearches']);
        $sqlQuery = 'SELECT id, search_name, search_data ' . 'FROM ' . $savedSearchesTbl . ' ' . "WHERE id = '" . $dbi->escapeString((string) $this->getId()) . "' ";
    ...
    public function getList(array $wheres = array())
    {
        ...
        $savedSearchesTbl = Util::backquote($this->config['cfgRelation']['db']) . '.' . Util::backquote($this->config['cfgRelation']['savedsearches']);
        $sqlQuery = 'SELECT id, search_name ' . 'FROM ' . $savedSearchesTbl . ' ' . 'WHERE ' . "username = '" . $dbi->escapeString($this->getUsername()) . "' " . "AND db_name = '" . $dbi->escapeString($this->getDbname()) . "' ";
