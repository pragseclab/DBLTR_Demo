...
class NodeTableContainer extends NodeDatabaseChildContainer
{
    ...
    public function __construct()
    {
        ...
        $this->links = ['text' => Url::getFromRoute('/database/structure', ['server' => $GLOBALS['server'], 'tbl_type' => 'table']) . '&amp;db=%1$s', 'icon' => Url::getFromRoute('/database/structure', ['server' => $GLOBALS['server'], 'tbl_type' => 'table']) . '&amp;db=%1$s'];
        ...
        $new->links = ['text' => Url::getFromRoute('/table/create', ['server' => $GLOBALS['server']]) . '&amp;db=%2$s', 'icon' => Url::getFromRoute('/table/create', ['server' => $GLOBALS['server']]) . '&amp;db=%2$s'];
        $this->addChild($new);
    }
}