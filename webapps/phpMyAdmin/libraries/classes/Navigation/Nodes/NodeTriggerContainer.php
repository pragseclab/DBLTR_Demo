...
class NodeTriggerContainer extends Node
{
    ...
    public function __construct()
    {
        ...
        $this->links = ['text' => Url::getFromRoute('/database/triggers', ['server' => $GLOBALS['server']]) . '&amp;db=%2$s&amp;table=%1$s', 'icon' => Url::getFromRoute('/database/triggers', ['server' => $GLOBALS['server']]) . '&amp;db=%2$s&amp;table=%1$s'];
        ...
        $new->links = ['text' => Url::getFromRoute('/database/triggers', ['server' => $GLOBALS['server'], 'add_item' => 1]) . '&amp;db=%3$s', 'icon' => Url::getFromRoute('/database/triggers', ['server' => $GLOBALS['server'], 'add_item' => 1]) . '&amp;db=%3$s'];
        $this->addChild($new);
    }
}