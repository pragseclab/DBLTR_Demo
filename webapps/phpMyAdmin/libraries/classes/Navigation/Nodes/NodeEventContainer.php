...
class NodeEventContainer extends NodeDatabaseChildContainer
{
    ...
    public function __construct()
    {
        ...
        $this->links = ['text' => Url::getFromRoute('/database/events', ['server' => $GLOBALS['server']]) . '&amp;db=%1$s', 'icon' => Url::getFromRoute('/database/events', ['server' => $GLOBALS['server']]) . '&amp;db=%1$s'];
        ...
        $new->links = ['text' => Url::getFromRoute('/database/events', ['server' => $GLOBALS['server'], 'add_item' => 1]) . '&amp;db=%2$s', 'icon' => Url::getFromRoute('/database/events', ['server' => $GLOBALS['server'], 'add_item' => 1]) . '&amp;db=%2$s'];
        $this->addChild($new);
    }
}