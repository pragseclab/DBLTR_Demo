...
class NavigationTree
{
    ...
    public function renderDbSelect() : string
    {
        ...
        $listNavigator = Generator::getListNavigator($this->tree->getPresence('databases', ''), $this->pos, ['server' => $GLOBALS['server']], Url::getFromRoute('/navigation'), 'frame_navigation', $GLOBALS['cfg']['FirstLevelNavigationItems'], 'pos', ['dbselector']);
        $children = $this->tree->children;
        $selected = $GLOBALS['db'];
        $options = [];
        foreach ($children as $node) {
            ...
            $options[] = ['title' => $title, 'name' => $node->realName, 'data' => ['apath' => $paths['aPath'], 'vpath' => $paths['vPath'], 'pos' => $this->pos], 'isSelected' => $node->realName === $selected];
        }
        $children = $this->tree->children;
        usort($children, [self::class, 'sortNode']);
        $this->setVisibility();
        ...
        return $this->template->render('navigation/tree/database_select', ['quick_warp' => $quickWarp, 'list_navigator' => $listNavigator, 'server' => $GLOBALS['server'], 'options' => $options, 'nodes' => $nodes]);
    }
    /**
