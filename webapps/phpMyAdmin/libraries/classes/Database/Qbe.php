...
class Qbe
{
     ...
     */
    public function __construct(Relation $relation, Template $template, $dbi, $dbname, array $savedSearchList = [], $currentSearch = null)
    {
        $this->db = $dbname;
    ...
    private function setSearchParams()
    {
        ...
        $this->criteriaColumnInsert = Core::ifSetOr($_POST['criteriaColumnInsert'], null, 'array');
        $this->criteriaColumnDelete = Core::ifSetOr($_POST['criteriaColumnDelete'], null, 'array');
    ...
    private function setCriteriaTablesAndColumns()
    {
        ...
        $all_tables = $this->dbi->query('SHOW TABLES FROM ' . Util::backquote($this->db) . ';', DatabaseInterface::CONNECT_USER, DatabaseInterface::QUERY_STORE);
        ...
        while ([$table] = $this->dbi->fetchRow($all_tables)) {
            ...
            foreach ($columns as $each_column) {
                ...
                $this->formColumnWidth = max(mb_strlen($each_column), $this->formColumnWidth);
            }
        }
    ...
    {
        return $this->template->render('database/qbe/column_select_cell', ['column_number' => $column_number, 'column_names' => $this->columnNames, 'selected' => $selected]);
    }
    /**
     ...
     * @return string HTML for select options
     */
    private function getSortSelectCell($columnNumber, $selected = '')
    {
        return $this->template->render('database/qbe/sort_select_cell', ['real_width' => $this->realwidth, 'column_number' => $columnNumber, 'selected' => $selected]);
    }
    /**
    ...
    private function getSortOrderSelectCell($columnNumber, $sortOrder)
    {
        ...
        return $this->template->render('database/qbe/sort_order_select_cell', ['total_column_count' => $totalColumnCount, 'column_number' => $columnNumber, 'sort_order' => $sortOrder]);
    }
    /**
    ...
    private function getColumnNamesRow()
    {
        ...
        for ($column_index = 0; $column_index < $this->criteriaColumnCount; $column_index++) {
            if (isset($this->criteriaColumnInsert[$column_index]) && $this->criteriaColumnInsert[$column_index] === 'on') {
                $html_output .= $this->showColumnSelectCell($new_column_count);
                $new_column_count++;
            }
            if (!empty($this->criteriaColumnDelete) && isset($this->criteriaColumnDelete[$column_index]) && $this->criteriaColumnDelete[$column_index] === 'on') {
                continue;
            }
            $selected = '';
            if (isset($_POST['criteriaColumn'][$column_index])) {
                $selected = $_POST['criteriaColumn'][$column_index];
                $this->formColumns[$new_column_count] = $_POST['criteriaColumn'][$column_index];
            }
            $html_output .= $this->showColumnSelectCell($new_column_count, $selected);
            $new_column_count++;
        }
    ...
    private function getColumnAliasRow()
    {
        ...
        for ($colInd = 0; $colInd < $this->criteriaColumnCount; $colInd++) {
            if (!empty($this->criteriaColumnInsert) && isset($this->criteriaColumnInsert[$colInd]) && $this->criteriaColumnInsert[$colInd] === 'on') {
                $html_output .= '<td class="text-center">';
                $html_output .= '<input type="text"' . ' name="criteriaAlias[' . $new_column_count . ']">';
                $html_output .= '</td>';
                $new_column_count++;
            }
            if (!empty($this->criteriaColumnDelete) && isset($this->criteriaColumnDelete[$colInd]) && $this->criteriaColumnDelete[$colInd] === 'on') {
                continue;
            }
            $tmp_alias = '';
            if (!empty($_POST['criteriaAlias'][$colInd])) {
                $tmp_alias = $this->formAliases[$new_column_count] = $_POST['criteriaAlias'][$colInd];
            }
            $html_output .= '<td class="text-center">';
            $html_output .= '<input type="text"' . ' name="criteriaAlias[' . $new_column_count . ']"' . ' value="' . htmlspecialchars($tmp_alias) . '">';
            $html_output .= '</td>';
            $new_column_count++;
        }
        return $html_output;
    ...
    private function getSortRow()
    {
        ...
        for ($colInd = 0; $colInd < $this->criteriaColumnCount; $colInd++) {
            ...
            if (isset($_POST['criteriaSort'][$colInd], $_POST['criteriaColumn'][$colInd]) && mb_substr($_POST['criteriaColumn'][$colInd], -2) === '.*') {
                $_POST['criteriaSort'][$colInd] = '';
            }
            $selected = '';
            if (isset($_POST['criteriaSort'][$colInd])) {
                $this->formSorts[$new_column_count] = $_POST['criteriaSort'][$colInd];
            ...
            $html_output .= $this->getSortSelectCell($new_column_count, $selected);
            $new_column_count++;
        }
        return $html_output;
    }
    ...
    private function getSortOrder()
    {
        ...
        for ($colInd = 0; $colInd < $this->criteriaColumnCount; $colInd++) {
            if (!empty($this->criteriaColumnInsert) && isset($this->criteriaColumnInsert[$colInd]) && $this->criteriaColumnInsert[$colInd] === 'on') {
                $html_output .= $this->getSortOrderSelectCell($new_column_count, null);
                $new_column_count++;
            }
            if (!empty($this->criteriaColumnDelete) && isset($this->criteriaColumnDelete[$colInd]) && $this->criteriaColumnDelete[$colInd] === 'on') {
                continue;
            ...
            if (!empty($_POST['criteriaSortOrder'][$colInd])) {
                $sortOrder = $this->formSortOrders[$new_column_count] = $_POST['criteriaSortOrder'][$colInd];
            }
            $html_output .= $this->getSortOrderSelectCell($new_column_count, $sortOrder);
            $new_column_count++;
        }
        return $html_output;
    }
    ...
    private function getShowRow()
    {
        ...
        $new_column_count = 0;
        for ($column_index = 0; $column_index < $this->criteriaColumnCount; $column_index++) {
            if (!empty($this->criteriaColumnInsert) && isset($this->criteriaColumnInsert[$column_index]) && $this->criteriaColumnInsert[$column_index] === 'on') {
                $html_output .= '<td class="text-center">';
            ...
            if (isset($_POST['criteriaShow'][$column_index])) {
                ...
                $this->formShows[$new_column_count] = $_POST['criteriaShow'][$column_index];
            } else {
                $checked_options = '';
            }
            $html_output .= '<td class="text-center">';
    ...
    private function getCriteriaInputboxRow()
    {
        ...
        for ($column_index = 0; $column_index < $this->criteriaColumnCount; $column_index++) {
            if (!empty($this->criteriaColumnInsert) && isset($this->criteriaColumnInsert[$column_index]) && $this->criteriaColumnInsert[$column_index] === 'on') {
                $html_output .= '<td class="text-center">';
                $html_output .= '<input type="text"' . ' name="criteria[' . $new_column_count . ']"' . ' class="textfield"' . ' style="width: ' . $this->realwidth . '"' . ' size="20">';
                $html_output .= '</td>';
                $new_column_count++;
            }
            if (!empty($this->criteriaColumnDelete) && isset($this->criteriaColumnDelete[$column_index]) && $this->criteriaColumnDelete[$column_index] === 'on') {
            ...
            if (empty($this->prevCriteria) || !isset($this->prevCriteria[$column_index]) || $this->prevCriteria[$column_index] != htmlspecialchars($tmp_criteria)) {
                $this->formCriterions[$new_column_count] = $tmp_criteria;
            } else {
                $this->formCriterions[$new_column_count] = $this->prevCriteria[$column_index];
            }
            $html_output .= '<td class="text-center">';
            $html_output .= '<input type="hidden"' . ' name="prev_criteria[' . $new_column_count . ']"' . ' value="' . htmlspecialchars($this->formCriterions[$new_column_count]) . '">';
            $html_output .= '<input type="text"' . ' name="criteria[' . $new_column_count . ']"' . ' value="' . htmlspecialchars($tmp_criteria) . '"' . ' class="textfield"' . ' style="width: ' . $this->realwidth . '"' . ' size="20">';
            $html_output .= '</td>';
            $new_column_count++;
        }
        return $html_output;
     ...
     */
    private function getAndOrColCell($column_number, $selected = null, $last_column = false)
    {
        $html_output = '<td class="text-center">';
        if (!$last_column) {
            $html_output .= '<strong>' . __('Or:') . '</strong>';
            $html_output .= '<input type="radio"' . ' name="criteriaAndOrColumn[' . $column_number . ']"' . ' value="or"' . ($selected['or'] ?? '') . '>';
            $html_output .= '&nbsp;&nbsp;<strong>' . __('And:') . '</strong>';
            $html_output .= '<input type="radio"' . ' name="criteriaAndOrColumn[' . $column_number . ']"' . ' value="and"' . ($selected['and'] ?? '') . '>';
        }
    ...
    private function getModifyColumnsRow()
    {
        ...
        for ($column_index = 0; $column_index < $this->criteriaColumnCount; $column_index++) {
            ...
            if (isset($this->criteriaAndOrColumn[$column_index])) {
                $this->formAndOrCols[$new_column_count] = $this->criteriaAndOrColumn[$column_index];
            }
            $checked_options = [];
            if (isset($this->criteriaAndOrColumn[$column_index]) && $this->criteriaAndOrColumn[$column_index] === 'or') {
            ...
            $html_output .= $this->getAndOrColCell($new_column_count, $checked_options, $column_index + 1 == $this->criteriaColumnCount);
            $new_column_count++;
        }
        return $html_output;
    }
    ...
    private function getInputboxRow($new_row_index)
    {
        ...
        for ($column_index = 0; $column_index < $this->criteriaColumnCount; $column_index++) {
            if (!empty($this->criteriaColumnInsert) && isset($this->criteriaColumnInsert[$column_index]) && $this->criteriaColumnInsert[$column_index] === 'on') {
                ...
                $html_output .= '<td class="text-center">';
                $html_output .= '<input type="text"' . ' name="Or' . $orFieldName . '" class="textfield"' . ' style="width: ' . $this->realwidth . '" size="20">';
                $html_output .= '</td>';
                $new_column_count++;
            }
            if (!empty($this->criteriaColumnDelete) && isset($this->criteriaColumnDelete[$column_index]) && $this->criteriaColumnDelete[$column_index] === 'on') {
            ...
            $html_output .= '<td class="text-center">';
            $html_output .= '<input type="text"' . ' name="Or' . $new_row_index . '[' . $new_column_count . ']"' . ' value="' . htmlspecialchars($tmp_or) . '" class="textfield"' . ' style="width: ' . $this->realwidth . '" size="20">';
            $html_output .= '</td>';
    ...
    private function getInsDelAndOrCriteriaRows()
    {
        ...
        for ($row_index = 0; $row_index <= $this->criteriaRowCount; $row_index++) {
            if (isset($this->criteriaRowInsert[$row_index]) && $this->criteriaRowInsert[$row_index] === 'on') {
                $checked_options['or'] = true;
                $checked_options['and'] = false;
                $html_output .= '<tr class="noclick">';
                $html_output .= $this->template->render('database/qbe/ins_del_and_or_cell', ['row_index' => $new_row_count, 'checked_options' => $checked_options]);
                $html_output .= $this->getInputboxRow($new_row_count);
                $new_row_count++;
                $html_output .= '</tr>';
            }
            if (isset($this->criteriaRowDelete[$row_index]) && $this->criteriaRowDelete[$row_index] === 'on') {
                continue;
            }
            if (isset($this->criteriaAndOrRow[$row_index])) {
                $this->formAndOrRows[$new_row_count] = $this->criteriaAndOrRow[$row_index];
            }
            if (isset($this->criteriaAndOrRow[$row_index]) && $this->criteriaAndOrRow[$row_index] === 'and') {
    ...
    private function getSelectClause()
    {
        ...
        $select_clauses = [];
        for ($column_index = 0; $column_index < $this->criteriaColumnCount; $column_index++) {
            if (empty($this->formColumns[$column_index]) || !isset($this->formShows[$column_index]) || $this->formShows[$column_index] !== 'on') {
                continue;
            }
            $select = $this->formColumns[$column_index];
            if (!empty($this->formAliases[$column_index])) {
                $select .= ' AS ' . Util::backquote($this->formAliases[$column_index]);
            }
            $select_clauses[] = $select;
        }
        if (!empty($select_clauses)) {
            $select_clause = 'SELECT ' . htmlspecialchars(implode(', ', $select_clauses)) . "\n";
        }
    ...
    private function getWhereClause()
    {
        ...
        $criteria_cnt = 0;
        for ($column_index = 0; $column_index < $this->criteriaColumnCount; $column_index++) {
            if (isset($last_where, $this->formAndOrCols) && !empty($this->formColumns[$column_index]) && !empty($this->formCriterions[$column_index]) && $column_index) {
                $where_clause .= ' ' . mb_strtoupper($this->formAndOrCols[$last_where]) . ' ';
            }
            if (empty($this->formColumns[$column_index]) || empty($this->formCriterions[$column_index])) {
                continue;
            }
            $where_clause .= '(' . $this->formColumns[$column_index] . ' ' . $this->formCriterions[$column_index] . ')';
        ...
        for ($row_index = 0; $row_index <= $this->criteriaRowCount; $row_index++) {
            ...
            $last_orwhere = '';
            for ($column_index = 0; $column_index < $this->criteriaColumnCount; $column_index++) {
                if (!empty($this->formColumns[$column_index]) && !empty($_POST['Or' . $row_index][$column_index]) && $column_index) {
                    $qry_orwhere .= ' ' . mb_strtoupper($this->formAndOrCols[$last_orwhere]) . ' ';
                }
                if (empty($this->formColumns[$column_index]) || empty($_POST['Or' . $row_index][$column_index])) {
                    continue;
                }
                $qry_orwhere .= '(' . $this->formColumns[$column_index] . ' ' . $_POST['Or' . $row_index][$column_index] . ')';
            ...
            $where_clause .= "\n" . mb_strtoupper(isset($this->formAndOrRows[$row_index]) ? $this->formAndOrRows[$row_index] . ' ' : '') . $qry_orwhere;
        }
     ...
     */
    private function getIndexes(array $search_tables, array $search_columns, array $where_clause_columns)
    {
        $unique_columns = [];
        $index_columns = [];
        ...
        return ['unique' => $unique_columns, 'index' => $index_columns];
    }
    /**
     * Provides UNIQUE columns and INDEX columns present in criteria tables
     *
     ...
     * @return array having UNIQUE and INDEX columns
     */
    private function getLeftJoinColumnCandidates(array $search_tables, array $search_columns, array $where_clause_columns)
    {
        $this->dbi->selectDb($this->db);
        // Get unique columns and index columns
        $indexes = $this->getIndexes($search_tables, $search_columns, $where_clause_columns);
        $unique_columns = $indexes['unique'];
        $index_columns = $indexes['index'];
        [$candidate_columns, $needsort] = $this->getLeftJoinColumnCandidatesBest($search_tables, $where_clause_columns, $unique_columns, $index_columns);
     ...
     */
    private function getMasterTable(array $search_tables, array $search_columns, array $where_clause_columns, array $where_clause_tables)
    {
        ...
        $candidate_columns = $this->getLeftJoinColumnCandidates($search_tables, $search_columns, $where_clause_columns);
    ...
    private function getWhereClauseTablesAndColumns()
    {
        ...
        for ($column_index = 0, $nb = count($this->criteria); $column_index < $nb; $column_index++) {
            ...
            if (mb_substr($this->criteria[$column_index], 0, 1) !== '=' && stripos($this->criteria[$column_index], 'is') === false) {
                continue;
            }
            $where_clause_columns[$column] = $column;
            $where_clause_tables[$table] = $table;
        }
        return ['where_clause_tables' => $where_clause_tables, 'where_clause_columns' => $where_clause_columns];
    }
    /**
    ...
    private function getFromClause(array $formColumns)
    {
        ...
        $from_clause = $this->getJoinForFromClause($search_tables, $search_columns);
        ...
        if (empty($from_clause)) {
            ...
            $from_clause = implode(', ', array_map([Util::class, 'backquote'], $search_tables));
        }
        return $from_clause;
    }
    /**
    ...
    private function getJoinForFromClause(array $searchTables, array $searchColumns)
    {
        ...
        if (count($unfinalized) > 0) {
            ...
            if (count($unfinalized) > 0) {
                ...
                $join .= implode(', ', array_map([Util::class, 'backquote'], $unfinalized));
            }
        }
        $first = true;
        // Add joined tables
    ...
    private function loadRelationsForTable(array &$relations, $oneTable)
    {
        ...
        foreach ($foreigners as $field => $foreigner) {
            ...
            if ($field === 'foreign_keys_data') {
                ...
                // Internal relations
                $relations[$oneTable][$foreigner['foreign_table']] = Util::backquote($oneTable) . '.' . Util::backquote((string) $field) . ' = ' . Util::backquote($foreigner['foreign_table']) . '.' . Util::backquote($foreigner['foreign_field']);
            }
        }
    }
    /**
            ...
            foreach ($searchTables as $masterTable) {
                ...
                foreach ($foreignData as $foreignTable => $clause) {
                    if (!isset($finalized[$masterTable]) && isset($finalized[$foreignTable])) {
                        ...
                        $added = true;
                    } elseif (!isset($finalized[$foreignTable]) && isset($finalized[$masterTable]) && in_array($foreignTable, $searchTables)) {
    ...
    public function getSelectionForm() : string
    {
        ...
        return $this->template->render('database/qbe/selection_form', ['db' => $this->db, 'url_params' => $url_params, 'db_link' => Generator::getDbLink($this->db), 'criteria_tables' => $this->criteriaTables, 'saved_searches_field' => $savedSearchesField, 'column_names_row' => $columnNamesRow, 'column_alias_row' => $columnAliasRow, 'show_row' => $showRow, 'sort_row' => $sortRow, 'sort_order' => $sortOrder, 'criteria_input_box_row' => $criteriaInputBoxRow, 'ins_del_and_or_criteria_rows' => $insDelAndOrCriteriaRows, 'modify_columns_row' => $modifyColumnsRow, 'sql_query' => $sqlQuery]);
    }
    /**
    ...
    private function getSavedSearchesField()
    {
        ...
        foreach ($this->savedSearchList as $id => $name) {
            $html_output .= '<option value="' . htmlspecialchars((string) $id) . '" ' . ($id == $currentSearchId ? 'selected="selected" ' : '') . '>' . htmlspecialchars($name) . '</option>';
        }
        $html_output .= '</select>';
        $html_output .= '<input type="text" name="searchName" id="searchName" ' . 'value="' . htmlspecialchars((string) $currentSearchName) . '">';
        $html_output .= '<input type="hidden" name="action" id="action" value="">';
        $html_output .= '<input class="btn btn-secondary" type="submit" name="saveSearch" id="saveSearch" ' . 'value="' . __('Create bookmark') . '">';
        if ($currentSearchId !== null) {
            $html_output .= '<input class="btn btn-secondary" type="submit" name="updateSearch" ' . 'id="updateSearch" value="' . __('Update bookmark') . '">';
            $html_output .= '<input class="btn btn-secondary" type="submit" name="deleteSearch" ' . 'id="deleteSearch" value="' . __('Delete bookmark') . '">';
        }
        return $html_output;
    }
    /**
    ...
    {
        // sets column count
        $criteriaColumnCount = Core::ifSetOr($_POST['criteriaColumnCount'], 3, 'numeric');
        $criteriaColumnAdd = Core::ifSetOr($_POST['criteriaColumnAdd'], 0, 'numeric');
        ...
        $this->criteriaRowCount = min(100, max($rows + $criteriaRowAdd, 0));
     ...
     * @return array
     */
    private function getLeftJoinColumnCandidatesBest(array $search_tables, ?array $where_clause_columns, ?array $unique_columns, ?array $index_columns)
    {
