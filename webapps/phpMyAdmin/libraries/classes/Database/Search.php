...
class Search
{
    ...
    public function __construct(DatabaseInterface $dbi, $db, Template $template)
    {
        $this->db = $db;
        $this->dbi = $dbi;
        $this->searchTypes = ['1' => __('at least one of the words'), '2' => __('all of the words'), '3' => __('the exact phrase as substring'), '4' => __('the exact phrase as whole field'), '5' => __('as regular expression')];
    ...
    private function setSearchParams()
    {
        ...
        if (empty($_POST['criteriaSearchType']) || !is_string($_POST['criteriaSearchType']) || !array_key_exists($_POST['criteriaSearchType'], $this->searchTypes)) {
    ...
    private function getWhereClause($table)
    {
        ...
        $search_words = $this->criteriaSearchType > 2 ? [$criteriaSearchStringEscaped] : explode(' ', $criteriaSearchStringEscaped);
        foreach ($search_words as $search_word) {
            ...
            foreach ($allColumns as $column) {
                if (isset($this->criteriaColumnName) && strlen($this->criteriaColumnName) !== 0 && $column['Field'] != $this->criteriaColumnName) {
                    continue;
                }
    ...
    public function getSearchResults()
    {
        ...
        foreach ($this->criteriaTables as $eachTable) {
            ...
            $resultCount = intval($this->dbi->fetchValue($newSearchSqls['select_count']));
            $resultTotal += $resultCount;
            // Gets the result row's HTML for a table
            $rows[] = ['table' => htmlspecialchars($eachTable), 'new_search_sqls' => $newSearchSqls, 'result_count' => $resultCount];
        }
        return $this->template->render('database/search/results', ['db' => $this->db, 'rows' => $rows, 'result_total' => $resultTotal, 'criteria_tables' => $this->criteriaTables, 'criteria_search_string' => htmlspecialchars($this->criteriaSearchString), 'search_type_description' => $this->searchTypeDescription]);
    }
    /**
    ...
    {
        return $this->template->render('database/search/main', ['db' => $this->db, 'criteria_search_string' => $this->criteriaSearchString, 'criteria_search_type' => $this->criteriaSearchType, 'criteria_tables' => $this->criteriaTables, 'tables_names_only' => $this->tablesNamesOnly, 'criteria_column_name' => $this->criteriaColumnName ?? null]);
    }
}