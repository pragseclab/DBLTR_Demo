...
class Common
{
    ...
    public function getPageName($pg)
    {
        ...
        $query = 'SELECT `page_descr`' . ' FROM ' . Util::backquote($cfgRelation['db']) . '.' . Util::backquote($cfgRelation['pdf_pages']) . ' WHERE ' . Util::backquote('page_nr') . ' = ' . intval($pg);
        $page_name = $this->dbi->fetchResult($query, null, null, DatabaseInterface::CONNECT_CONTROL, DatabaseInterface::QUERY_STORE);
    ...
    public function deletePage($pg)
    {
        ...
        $query = 'DELETE FROM ' . Util::backquote($cfgRelation['db']) . '.' . Util::backquote($cfgRelation['table_coords']) . ' WHERE ' . Util::backquote('pdf_page_number') . ' = ' . intval($pg);
        $success = $this->relation->queryAsControlUser($query, true, DatabaseInterface::QUERY_STORE);
        if ($success) {
            $query = 'DELETE FROM ' . Util::backquote($cfgRelation['db']) . '.' . Util::backquote($cfgRelation['pdf_pages']) . ' WHERE ' . Util::backquote('page_nr') . ' = ' . intval($pg);
            $success = $this->relation->queryAsControlUser($query, true, DatabaseInterface::QUERY_STORE);
        }
        return (bool) $success;
    }
    ...
    public function getDefaultPage($db) : ?int
    {
        ...
        $query = 'SELECT `page_nr`' . ' FROM ' . Util::backquote($cfgRelation['db']) . '.' . Util::backquote($cfgRelation['pdf_pages']) . " WHERE `db_name` = '" . $this->dbi->escapeString($db) . "'" . " AND `page_descr` = '" . $this->dbi->escapeString($db) . "'";
        $default_page_no = $this->dbi->fetchResult($query, null, null, DatabaseInterface::CONNECT_CONTROL, DatabaseInterface::QUERY_STORE);
    ...
    public function getPageExists(string $pg) : bool
    {
        ...
        $query = 'SELECT `page_nr`' . ' FROM ' . Util::backquote($cfgRelation['db']) . '.' . Util::backquote($cfgRelation['pdf_pages']) . " WHERE `page_descr` = '" . $this->dbi->escapeString($pg) . "'";
        $pageNos = $this->dbi->fetchResult($query, null, null, DatabaseInterface::CONNECT_CONTROL, DatabaseInterface::QUERY_STORE);
    ...
    public function createNewPage($pageName, $db)
    {
        $cfgRelation = $this->relation->getRelationsParam();
        if ($cfgRelation['pdfwork']) {
            return $this->relation->createPage($pageName, $cfgRelation, $db);
        }
        return null;
    }
    /**
    ...
    public function saveTablePositions($pg)
    {
        ...
        $query = 'DELETE FROM ' . Util::backquote($cfgRelation['db']) . '.' . Util::backquote($cfgRelation['table_coords']) . " WHERE `pdf_page_number` = '" . $pageId . "'";
        ...
        foreach ($_POST['t_h'] as $key => $value) {
            ...
            $query = 'INSERT INTO ' . Util::backquote($cfgRelation['db']) . '.' . Util::backquote($cfgRelation['table_coords']) . ' (`db_name`, `table_name`, `pdf_page_number`, `x`, `y`)' . ' VALUES (' . "'" . $this->dbi->escapeString($DB) . "', " . "'" . $this->dbi->escapeString($TAB) . "', " . "'" . $pageId . "', " . "'" . $this->dbi->escapeString($_POST['t_x'][$key]) . "', " . "'" . $this->dbi->escapeString($_POST['t_y'][$key]) . "')";
            $res = $this->relation->queryAsControlUser($query, true, DatabaseInterface::QUERY_STORE);
        }
        return (bool) $res;
    }
    ...
    public function saveDisplayField($db, $table, $field) : array
    {
        ...
        if (!$cfgRelation['displaywork']) {
            return [false, _pgettext('phpMyAdmin configuration storage is not configured for' . ' "Display Features" on designer when user tries to set a display field.', 'phpMyAdmin configuration storage is not configured for "Display Features".')];
        }
    ...
    public function addNewRelation($db, $T1, $F1, $T2, $F2, $on_delete, $on_update, $DB1, $DB2) : array
    {
        ...
        // native foreign key
        if (Util::isForeignKeySupported($type_T1) && Util::isForeignKeySupported($type_T2) && $type_T1 == $type_T2) {
            // relation exists?
            ...
            $result = $this->dbi->query('SHOW INDEX FROM ' . Util::backquote($DB1) . '.' . Util::backquote($T1) . ';');
            ...
            $result = $this->dbi->query('SHOW INDEX FROM ' . Util::backquote($DB2) . '.' . Util::backquote($T2) . ';');
            ...
            if (!empty($index_array1[$F1]) && !empty($index_array2[$F2])) {
                $upd_query = 'ALTER TABLE ' . Util::backquote($DB2) . '.' . Util::backquote($T2) . ' ADD FOREIGN KEY (' . Util::backquote($F2) . ')' . ' REFERENCES ' . Util::backquote($DB1) . '.' . Util::backquote($T1) . '(' . Util::backquote($F1) . ')';
                if ($on_delete !== 'nix') {
