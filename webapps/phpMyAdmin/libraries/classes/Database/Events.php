...
class Events
{
    ...
    {
        $retval = [];
        $indices = ['item_name', 'item_original_name', 'item_status', 'item_execute_at', 'item_interval_value', 'item_interval_field', 'item_starts', 'item_ends', 'item_definition', 'item_preserve', 'item_comment', 'item_definer'];
    ...
    public function getDataFromName($name) : ?array
    {
        ...
        $where = 'EVENT_SCHEMA ' . Util::getCollateForIS() . '=' . "'" . $this->dbi->escapeString($db) . "' " . "AND EVENT_NAME='" . $this->dbi->escapeString($name) . "'";
    ...
    public function getEditorForm($mode, $operation, array $item)
    {
        ...
        return $this->template->render('database/events/editor_form', ['db' => $db, 'event' => $item, 'mode' => $mode, 'is_ajax' => $this->response->isAjax(), 'status_display' => $this->status['display'], 'event_type' => $this->type, 'event_interval' => $this->interval]);
    }
    /**
        ...
        if (!empty($_POST['item_type']) && in_array($_POST['item_type'], $this->type)) {
            if ($_POST['item_type'] === 'RECURRING') {
                ...
                if (!empty($_POST['item_starts'])) {
                    $query .= "STARTS '" . $this->dbi->escapeString($_POST['item_starts']) . "' ";
                }
                if (!empty($_POST['item_ends'])) {
                    $query .= "ENDS '" . $this->dbi->escapeString($_POST['item_ends']) . "' ";
                }
            } else {
                if (!empty($_POST['item_execute_at'])) {
                    $query .= "AT '" . $this->dbi->escapeString($_POST['item_execute_at']) . "' ";
                } else {
    ...
    private function checkResult($result, $createStatement, array $errors)
    {
        ...
        // this just in case.
        $errors[] = __('Sorry, we failed to restore the dropped event.') . '<br>' . __('The backed up query was:') . '"' . htmlspecialchars((string) $createStatement) . '"<br>' . __('MySQL said: ') . $this->dbi->getError();
        return $errors;
    }
    /**
    ...
    private function sendEditor($mode, ?array $item, $title, $db, $operation)
    {
        ...
        $message .= sprintf(__('No event with name %1$s found in database %2$s.'), htmlspecialchars(Util::backquote($_REQUEST['item_name'])), htmlspecialchars(Util::backquote($db)));
