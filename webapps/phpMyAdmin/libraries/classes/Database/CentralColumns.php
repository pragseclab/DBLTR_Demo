...
class CentralColumns
{
    ...
    public function getColumnsList(string $db, int $from = 0, int $num = 25) : array
    {
        ...
        if ($num == 0) {
            $query = 'SELECT * FROM ' . Util::backquote($central_list_table) . ' ' . 'WHERE db_name = \'' . $this->dbi->escapeString($db) . '\';';
        } else {
            $query = 'SELECT * FROM ' . Util::backquote($central_list_table) . ' ' . 'WHERE db_name = \'' . $this->dbi->escapeString($db) . '\' ' . 'LIMIT ' . $from . ', ' . $num . ';';
        }
    ...
    public function getCount(string $db) : int
    {
        ...
        $this->dbi->selectDb($pmadb, DatabaseInterface::CONNECT_CONTROL);
        $central_list_table = $cfgCentralColumns['table'];
        $query = 'SELECT count(db_name) FROM ' . Util::backquote($central_list_table) . ' ' . 'WHERE db_name = \'' . $this->dbi->escapeString($db) . '\';';
    ...
    private function findExistingColNames(string $db, string $cols, bool $allFields = false) : array
    {
        ...
        $central_list_table = $cfgCentralColumns['table'];
        if ($allFields) {
            $query = 'SELECT * FROM ' . Util::backquote($central_list_table) . ' ' . 'WHERE db_name = \'' . $this->dbi->escapeString($db) . '\' AND col_name IN (' . $cols . ');';
            $has_list = (array) $this->dbi->fetchResult($query, null, null, DatabaseInterface::CONNECT_CONTROL);
            $this->handleColumnExtra($has_list);
        } else {
            $query = 'SELECT col_name FROM ' . Util::backquote($central_list_table) . ' ' . 'WHERE db_name = \'' . $this->dbi->escapeString($db) . '\' AND col_name IN (' . $cols . ');';
            $has_list = (array) $this->dbi->fetchResult($query, null, null, DatabaseInterface::CONNECT_CONTROL);
        }
        return $has_list;
    }
    ...
    private function getInsertQuery(string $column, array $def, string $db, string $central_list_table) : string
    {
        ...
        $extra = $def['Extra'] ?? '';
        $default = $def['Default'] ?? '';
        return 'INSERT INTO ' . Util::backquote($central_list_table) . ' ' . 'VALUES ( \'' . $this->dbi->escapeString($db) . '\' ,' . '\'' . $this->dbi->escapeString($column) . '\',\'' . $this->dbi->escapeString($type) . '\',' . '\'' . $this->dbi->escapeString((string) $length) . '\',\'' . $this->dbi->escapeString($collation) . '\',' . '\'' . $this->dbi->escapeString($isNull) . '\',' . '\'' . implode(',', [$extra, $attribute]) . '\',\'' . $this->dbi->escapeString($default) . '\');';
    }
    /**
    ...
    public function syncUniqueColumns(array $field_select, bool $isTable = true, ?string $table = null)
    {
        $cfgCentralColumns = $this->getParams();
        if (!is_array($cfgCentralColumns)) {
            return Message::error(__('The configuration storage is not ready for the central list of columns feature.'));
        }
        $db = $_POST['db'];
        $pmadb = $cfgCentralColumns['db'];
        $central_list_table = $cfgCentralColumns['table'];
        ...
        if (!empty($existingCols)) {
            ...
            $message = Message::notice(sprintf(__('Could not add %1$s as they already exist in central list!'), htmlspecialchars($existingCols)));
            $message->addMessage(Message::notice('Please remove them first ' . 'from central list if you want to update above columns'));
        }
        $this->dbi->selectDb($pmadb, DatabaseInterface::CONNECT_CONTROL);
        if (!empty($insQuery)) {
            foreach ($insQuery as $query) {
                if (!$this->dbi->tryQuery($query, DatabaseInterface::CONNECT_CONTROL)) {
                    ...
                    $message->addMessage(Message::rawError((string) $this->dbi->getError(DatabaseInterface::CONNECT_CONTROL)));
                    break;
                }
            }
        }
    ...
    public function deleteColumnsFromList(string $database, array $field_select, bool $isTable = true)
    {
        ...
        $this->dbi->selectDb($pmadb, DatabaseInterface::CONNECT_CONTROL);
        $query = 'DELETE FROM ' . Util::backquote($central_list_table) . ' ' . 'WHERE db_name = \'' . $this->dbi->escapeString($database) . '\' AND col_name IN (' . $cols . ');';
        if (!$this->dbi->tryQuery($query, DatabaseInterface::CONNECT_CONTROL)) {
    ...
    public function makeConsistentWithList(string $db, array $selected_tables)
    {
        ...
        foreach ($selected_tables as $table) {
            ...
            foreach ($has_list as $column) {
                ...
                $query .= ' MODIFY ' . Util::backquote($column['col_name']) . ' ' . $this->dbi->escapeString($column['col_type']);
                ...
                if ($column['col_default']) {
                    if ($column['col_default'] !== 'CURRENT_TIMESTAMP' && $column['col_default'] !== 'current_timestamp()') {
    ...
    public function updateOneColumn(string $db, string $orig_col_name, string $col_name, string $col_type, string $col_attribute, string $col_length, int $col_isNull, string $collation, string $col_extra, string $col_default)
    {
        ...
        if (!$this->dbi->tryQuery($query, DatabaseInterface::CONNECT_CONTROL)) {
            return Message::error((string) $this->dbi->getError(DatabaseInterface::CONNECT_CONTROL));
        }
        return true;
    }
    /**
    ...
    public function updateMultipleColumn(array $params)
    {
        ...
        for ($i = 0; $i < $numberCentralFields; $i++) {
            ...
            $message = $this->updateOneColumn($params['db'], $params['orig_col_name'][$i], $params['field_name'][$i], $params['field_type'][$i], $params['field_attribute'][$i], $params['field_length'][$i], $columnIsNull[$i], $params['field_collation'][$i], $columnExtra[$i], $columnDefault[$i]);
            if (!is_bool($message)) {
                return $message;
            }
        }
    ...
    private function getHtmlForEditTableRow(array $row, int $row_num) : string
    {
        ...
        return $this->template->render('database/central_columns/edit_table_row', ['row_num' => $row_num, 'row' => $row, 'max_rows' => $this->maxRows, 'meta' => $meta, 'default_value' => $defaultValue, 'char_editing' => $this->charEditing, 'charsets' => $charsets, 'collations' => $collations, 'attribute_types' => $this->dbi->types->getAttributes()]);
    }
    /**
    ...
    public function getListRaw(string $db, string $table) : array
    {
        ...
        if (empty($table) || $table == '') {
            $query = 'SELECT * FROM ' . Util::backquote($centralTable) . ' ' . 'WHERE db_name = \'' . $this->dbi->escapeString($db) . '\';';
        } else {
            $this->dbi->selectDb($db);
    ...
    public function getColumnsCount(string $db, int $from = 0, int $num = 25) : int
    {
        ...
        $central_list_table = $cfgCentralColumns['table'];
        //get current values of $db from central column list
        $query = 'SELECT COUNT(db_name) FROM ' . Util::backquote($central_list_table) . ' ' . 'WHERE db_name = \'' . $this->dbi->escapeString($db) . '\'' . ($num === 0 ? '' : 'LIMIT ' . $from . ', ' . $num) . ';';
    ...
    public function getTemplateVariablesForMain(string $db, int $total_rows, int $pos, string $themeImagePath, string $text_dir) : array
    {
        ...
        $tn_page_selector = $tn_nbTotalPage > 1 ? Util::pageselector('pos', $this->maxRows, $tn_pageNow, $tn_nbTotalPage) : '';
        $this->dbi->selectDb($db);
        ...
        return ['db' => $db, 'total_rows' => $total_rows, 'max_rows' => $max_rows, 'pos' => $pos, 'char_editing' => $this->charEditing, 'attribute_types' => $attribute_types, 'tn_nbTotalPage' => $tn_nbTotalPage, 'tn_page_selector' => $tn_page_selector, 'tables' => $tables, 'rows_list' => $rows_list, 'rows_meta' => $rows_meta, 'default_values' => $defaultValues, 'types_upper' => $types_upper, 'theme_image_path' => $themeImagePath, 'text_dir' => $text_dir, 'charsets' => $charsetsList];
    }
}