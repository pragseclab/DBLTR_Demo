...
class Routines
{
    ...
    public function backupPrivileges()
    {
        ...
        $privilegesBackupQuery = 'SELECT * FROM ' . Util::backquote('mysql') . '.' . Util::backquote('procs_priv') . ' where Routine_name = "' . $_POST['item_original_name'] . '" AND Routine_type = "' . $_POST['item_original_type'] . '";';
    ...
    public function create($routine_query, $create_routine, array $privilegesBackup)
    {
        ...
        if (!$result) {
            $errors = [];
            $errors[] = sprintf(__('The following query has failed: "%s"'), htmlspecialchars($routine_query)) . '<br>' . __('MySQL said: ') . $this->dbi->getError();
        ...
        if ($GLOBALS['proc_priv'] && $GLOBALS['is_reload_priv']) {
            ...
            foreach ($privilegesBackup as $priv) {
                $adjustProcPrivilege = 'INSERT INTO ' . Util::backquote('mysql') . '.' . Util::backquote('procs_priv') . ' VALUES("' . $priv[0] . '", "' . $priv[1] . '", "' . $priv[2] . '", "' . $_POST['item_name'] . '", "' . $_POST['item_type'] . '", "' . $priv[5] . '", "' . $priv[6] . '", "' . $priv[7] . '");';
    ...
    public function flushPrivileges($flushPrivileges)
    {
        if ($flushPrivileges) {
            ...
            $message = Message::success(__('Routine %1$s has been modified. Privileges have been adjusted.'));
        } else {
            $message = Message::success(__('Routine %1$s has been modified.'));
        }
        $message->addParam(Util::backquote($_POST['item_name']));
        return $message;
    }
    ...
    {
        $retval = [];
        $indices = ['item_name', 'item_original_name', 'item_returnlength', 'item_returnopts_num', 'item_returnopts_text', 'item_definition', 'item_comment', 'item_definer'];
    ...
    public function getDataFromName($name, $type, $all = true) : ?array
    {
        ...
        $where = 'ROUTINE_SCHEMA ' . Util::getCollateForIS() . '=' . "'" . $this->dbi->escapeString($db) . "' " . "AND SPECIFIC_NAME='" . $this->dbi->escapeString($name) . "'" . "AND ROUTINE_TYPE='" . $this->dbi->escapeString($type) . "'";
    ...
    public function getParameterRow(array $routine = [], $index = null, $class = '')
    {
        if ($index === null) {
            ...
            $index = '%s';
            $drop_class = '';
            $routine = ['item_param_dir' => [0 => ''], 'item_param_name' => [0 => ''], 'item_param_type' => [0 => ''], 'item_param_length' => [0 => ''], 'item_param_opts_num' => [0 => ''], 'item_param_opts_text' => [0 => '']];
        ...
        foreach ($allCharsets as $charset) {
            $charsets[] = ['name' => $charset->getName(), 'description' => $charset->getDescription(), 'is_selected' => $charset->getName() === $routine['item_param_opts_text'][$i]];
        }
        return $this->template->render('database/routines/parameter_row', ['class' => $class, 'index' => $index, 'param_directions' => $this->directions, 'param_opts_num' => $this->numericOptions, 'item_param_dir' => $routine['item_param_dir'][$i] ?? '', 'item_param_name' => $routine['item_param_name'][$i] ?? '', 'item_param_length' => $routine['item_param_length'][$i] ?? '', 'item_param_opts_num' => $routine['item_param_opts_num'][$i] ?? '', 'supported_datatypes' => Util::getSupportedDatatypes(true, $routine['item_param_type'][$i]), 'charsets' => $charsets, 'drop_class' => $drop_class]);
    }
    /**
    ...
    public function getEditorForm($mode, $operation, array $routine)
    {
        ...
        $need_escape = ['item_original_name', 'item_name', 'item_returnlength', 'item_definition', 'item_definer', 'item_comment'];
        ...
        foreach ($charsets as $charset) {
            $retval .= '<option value="' . $charset->getName() . '" title="' . $charset->getDescription() . '"' . ($routine['item_returnopts_text'] == $charset->getName() ? ' selected' : '') . '>' . $charset->getName() . '</option>' . "\n";
        }
        $retval .= '</select>' . "\n";
        $retval .= '    </div>';
    ...
    private function processParamsAndBuild(array $itemParamName, array $itemParamDir, array $itemParamType, array $itemParamLength, array $itemParamOpsText, array $itemParamOpsNum, string $itemType, bool &$warnedAboutLength) : string
    {
        ...
        for ($i = 0, $nb = count($itemParamName); $i < $nb; $i++) {
            ...
            if ($itemType === 'PROCEDURE' && !empty($itemParamDir[$i]) && in_array($itemParamDir[$i], $this->directions)) {
                $params .= $itemParamDir[$i] . ' ' . Util::backquote($itemParamName[$i]) . ' ' . $itemParamType[$i];
            } elseif ($itemType === 'FUNCTION') {
                $params .= Util::backquote($itemParamName[$i]) . ' ' . $itemParamType[$i];
            } elseif (!$warnedAboutDir) {
                $warnedAboutDir = true;
                $errors[] = sprintf(__('Invalid direction "%s" given for parameter.'), htmlspecialchars($itemParamDir[$i]));
            }
            if ($itemParamLength[$i] != '' && !preg_match('@^(DATE|TINYBLOB|TINYTEXT|BLOB|TEXT|' . 'MEDIUMBLOB|MEDIUMTEXT|LONGBLOB|LONGTEXT|' . 'SERIAL|BOOLEAN)$@i', $itemParamType[$i])) {
                $params .= '(' . $itemParamLength[$i] . ')';
            } elseif ($itemParamLength[$i] == '' && preg_match('@^(ENUM|SET|VARCHAR|VARBINARY)$@i', $itemParamType[$i])) {
            ...
            if (!empty($itemParamOpsText[$i])) {
                if ($dbi->types->getTypeClass($itemParamType[$i]) === 'CHAR') {
                    if (!in_array($itemParamType[$i], ['VARBINARY', 'BINARY'])) {
                        $params .= ' CHARSET ' . mb_strtolower($itemParamOpsText[$i]);
                    }
                }
            }
            if (!empty($itemParamOpsNum[$i])) {
                if ($dbi->types->getTypeClass($itemParamType[$i]) === 'NUMBER') {
                    $params .= ' ' . mb_strtoupper($itemParamOpsNum[$i]);
                }
            }
    ...
    private function processFunctionSpecificParameters(string $query, bool $warnedAboutLength) : string
    {
        ...
        if (!empty($_POST['item_returnlength']) && !preg_match('@^(DATE|DATETIME|TIME|TINYBLOB|TINYTEXT|BLOB|TEXT|' . 'MEDIUMBLOB|MEDIUMTEXT|LONGBLOB|LONGTEXT|SERIAL|BOOLEAN)$@i', $itemReturnType)) {
            $query .= '(' . $_POST['item_returnlength'] . ')';
        } elseif (empty($_POST['item_returnlength']) && preg_match('@^(ENUM|SET|VARCHAR|VARBINARY)$@i', $itemReturnType)) {
            if (!$warnedAboutLength) {
                $errors[] = __('You must provide length/values for routine parameters' . ' of type ENUM, SET, VARCHAR and VARBINARY.');
            }
        }
        if (!empty($_POST['item_returnopts_text'])) {
            if ($dbi->types->getTypeClass($itemReturnType) === 'CHAR') {
                $query .= ' CHARSET ' . mb_strtolower($_POST['item_returnopts_text']);
            }
        }
        if (!empty($_POST['item_returnopts_num'])) {
            if ($dbi->types->getTypeClass($itemReturnType) === 'NUMBER') {
                $query .= ' ' . mb_strtoupper($_POST['item_returnopts_num']);
            }
        }
        return $query . ' ';
    }
    ...
    public function getQueryFromRequest() : string
    {
        ...
        if ($itemType === 'FUNCTION' || $itemType === 'PROCEDURE') {
            ...
            $errors[] = sprintf(__('Invalid routine type: "%s"'), htmlspecialchars($itemType));
        }
        if (!empty($itemName)) {
        ...
        if (!empty($itemParamName) && !empty($itemParamType) && !empty($itemParamLength) && is_array($itemParamName) && is_array($itemParamType) && is_array($itemParamLength)) {
            $params = $this->processParamsAndBuild($itemParamName, $itemParamDir, $itemParamType, $itemParamLength, $itemParamOpsText, $itemParamOpsNum, $itemType, $warnedAboutLength);
        }
        $query .= '(' . $params . ') ';
    ...
    private function getQueriesFromRoutineForm(array $routine) : array
    {
        ...
        for ($i = 0; $i < $routine['item_num_params']; $i++) {
            if (isset($_POST['params'][$routine['item_param_name'][$i]])) {
                ...
                if (!empty($_POST['funcs'][$routine['item_param_name'][$i]]) && in_array($_POST['funcs'][$routine['item_param_name'][$i]], $all_functions)) {
                    $queries[] = 'SET @p' . $i . '=' . $_POST['funcs'][$routine['item_param_name'][$i]] . "('" . $value . "');\n";
                } else {
            ...
            $end_query[] = '@p' . $i . ' AS ' . Util::backquote($routine['item_param_name'][$i]);
        }
        if ($routine['item_type'] === 'PROCEDURE') {
            ...
            $queries[] = 'SELECT ' . Util::backquote($routine['item_name']) . '(' . implode(', ', $args) . ') ' . 'AS ' . Util::backquote($routine['item_name']) . ";\n";
        }
        return $queries;
    }
    private function handleExecuteRoutine() : void
    {
        ...
        if ($routine === null) {
            $message = __('Error in processing request:') . ' ';
            $message .= sprintf(__('No routine with name %1$s found in database %2$s.'), htmlspecialchars(Util::backquote($_POST['item_name'])), htmlspecialchars(Util::backquote($db)));
            $message = Message::error($message);
            if ($this->response->isAjax()) {
                $this->response->setRequestStatus(false);
                $this->response->addJSON('message', $message);
        ...
        if ($outcome) {
            ...
            $output .= sprintf(__('Execution results of routine %s'), Util::backquote(htmlspecialchars($routine['item_name'])));
            $output .= '</legend>';
            do {
        ...
        if ($outcome) {
            ...
            if ($routine['item_type'] === 'PROCEDURE') {
                ...
                $message .= sprintf(_ngettext('%d row affected by the last statement inside the ' . 'procedure.', '%d rows affected by the last statement inside the ' . 'procedure.', (int) $affected), $affected);
            }
            ...
            $output = '';
            $message = Message::error(sprintf(__('The following query has failed: "%s"'), htmlspecialchars($multiple_query)) . '<br><br>' . __('MySQL said: ') . $this->dbi->getError());
        }
        // Print/send output
        if ($this->response->isAjax()) {
    ...
    public function getExecuteForm(array $routine) : string
    {
        ...
        for ($i = 0; $i < $routine['item_num_params']; $i++) {
            ...
            if ($cfg['ShowFunctionFields']) {
                $retval .= "<td>\n";
                if (stripos($routine['item_param_type'][$i], 'enum') !== false || stripos($routine['item_param_type'][$i], 'set') !== false || in_array(mb_strtolower($routine['item_param_type'][$i]), $no_support_types)) {
                    $retval .= "--\n";
                } else {
                    $field = ['True_Type' => mb_strtolower($routine['item_param_type'][$i]), 'Type' => '', 'Key' => '', 'Field' => '', 'Default' => '', 'first_timestamp' => false];
    ...
    {
        global $db, $table;
        $sqlDrop = sprintf('DROP %s IF EXISTS %s', $routine['type'], Util::backquote($routine['name']));
        // this is for our purpose to decide whether to
        // show the edit link or not, so we need the DEFINER for the routine
        $where = 'ROUTINE_SCHEMA ' . Util::getCollateForIS() . '=' . "'" . $this->dbi->escapeString($db) . "' " . "AND SPECIFIC_NAME='" . $this->dbi->escapeString($routine['name']) . "'" . "AND ROUTINE_TYPE='" . $this->dbi->escapeString($routine['type']) . "'";
        ...
        $hasEditPrivilege = Util::currentUserHasPrivilege('CREATE ROUTINE', $db) && $currentUser == $routineDefiner || $this->dbi->isSuperUser();
        ...
        $hasExportPrivilege = Util::currentUserHasPrivilege('CREATE ROUTINE', $db) && $currentUser == $routineDefiner || $this->dbi->isSuperUser();
        return $this->template->render('database/routines/row', ['db' => $db, 'table' => $table, 'sql_drop' => $sqlDrop, 'routine' => $routine, 'row_class' => $rowClass, 'has_edit_privilege' => $hasEditPrivilege, 'has_export_privilege' => $hasExportPrivilege, 'has_execute_privilege' => $hasExecutePrivilege, 'execute_action' => $executeAction]);
    }
    /**
    ...
    private function checkResult($result, $createStatement, array $errors)
    {
        ...
        // this just in case.
        $errors[] = __('Sorry, we failed to restore the dropped routine.') . '<br>' . __('The backed up query was:') . '"' . htmlspecialchars($createStatement) . '"<br>' . __('MySQL said: ') . $this->dbi->getError();
        return $errors;
    }
