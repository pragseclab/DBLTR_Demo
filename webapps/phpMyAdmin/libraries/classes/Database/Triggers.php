...
class Triggers
{
    ...
    {
        $retval = [];
        $indices = ['item_name', 'item_table', 'item_original_name', 'item_action_timing', 'item_event_manipulation', 'item_definition', 'item_definer'];
    ...
    public function getEditorForm($mode, array $item)
    {
        ...
        $need_escape = ['item_original_name', 'item_name', 'item_definition', 'item_definer'];
    ...
    private function checkResult($result, $createStatement, array $errors)
    {
        ...
        // this just in case.
        $errors[] = __('Sorry, we failed to restore the dropped trigger.') . '<br>' . __('The backed up query was:') . '"' . htmlspecialchars($createStatement) . '"<br>' . __('MySQL said: ') . $this->dbi->getError();
        return $errors;
    }
    /**
    ...
    private function sendEditor($mode, ?array $item, $title, $db)
    {
        ...
        $message .= sprintf(__('No trigger with name %1$s found in database %2$s.'), htmlspecialchars(Util::backquote($_REQUEST['item_name'])), htmlspecialchars(Util::backquote($db)));
