...
class Monitor
{
    ...
    public function getJsonForChartingData(string $requiredData) : array
    {
        ...
        [$serverVars, $statusVars, $ret] = $this->getJsonForChartingDataGet($ret, $serverVars, $statusVars, $sysinfo, $cpuload, $memory);
        // Retrieve all required status variables
        $statusVarValues = [];
        if (count($statusVars)) {
            $statusVarValues = $this->dbi->fetchResult("SHOW GLOBAL STATUS WHERE Variable_name='" . implode("' OR Variable_name='", $statusVars) . "'", 0, 1);
        }
        // Retrieve all required server variables
        $serverVarValues = [];
        if (count($serverVars)) {
            $serverVarValues = $this->dbi->fetchResult("SHOW GLOBAL VARIABLES WHERE Variable_name='" . implode("' OR Variable_name='", $serverVars) . "'", 0, 1);
        }
        // ...and now assign them
     ...
     * @return array
     */
    private function getJsonForChartingDataSet(array $ret, array $statusVarValues, array $serverVarValues) : array
    {
     ...
     * @return array
     */
    private function getJsonForChartingDataGet(array $ret, array $serverVars, array $statusVars, $sysinfo, $cpuload, $memory)
    {
        // For each chart
        foreach ($ret as $chartId => $chartNodes) {
            ...
            foreach ($chartNodes as $nodeId => $nodeDataPoints) {
                ...
                foreach ($nodeDataPoints as $pointId => $dataPoint) {
                    [$serverVars, $statusVars, $ret[$chartId][$nodeId][$pointId]] = $this->getJsonForChartingDataSwitch($dataPoint['type'], $dataPoint['name'], $serverVars, $statusVars, $ret[$chartId][$nodeId][$pointId], $sysinfo, $cpuload, $memory);
                }
                /* foreach */
            }
            /* foreach */
     ...
     * @return array
     */
    private function getJsonForChartingDataSwitch($type, $pName, array $serverVars, array $statusVars, array $ret, $sysinfo, $cpuload, $memory)
    {
        switch ($type) {
    ...
    public function getJsonForLogDataTypeSlow(int $start, int $end) : array
    {
        ...
        while ($row = $this->dbi->fetchAssoc($result)) {
            $type = mb_strtolower(mb_substr($row['sql_text'], 0, (int) mb_strpos($row['sql_text'], ' ')));
            switch ($type) {
                case 'insert':
                case 'update':
                    //Cut off big inserts and updates, but append byte count instead
                    if (mb_strlen($row['sql_text']) > 220) {
                        $implodeSqlText = implode(' ', (array) Util::formatByteDown(mb_strlen($row['sql_text']), 2, 2));
                        $row['sql_text'] = mb_substr($row['sql_text'], 0, 200) . '... [' . $implodeSqlText . ']';
                    }
                    break;
                default:
                    break;
     ...
     * @return array
     */
    public function getJsonForLogDataTypeGeneral(int $start, int $end, bool $isTypesLimited, bool $removeVariables) : array
    {
        $limitTypes = '';
        if ($isTypesLimited) {
        ...
        while ($row = $this->dbi->fetchAssoc($result)) {
            ...
            switch ($type) {
                ...
                case 'insert':
                    ...
                    if ($removeVariables && preg_match('/^INSERT INTO (`|\'|"|)([^\\s\\1]+)\\1/i', $row['argument'], $matches)) {
                        ...
                        if ($insertTables[$matches[2]] > 1) {
                            ...
                            $return['rows'][$insertTablesFirst]['argument'] .= $this->getSuspensionPoints($temp[strlen($temp) - 1]);
                ...
                case 'update':
                    ...
                    if (mb_strlen($row['argument']) > 220) {
                        $row['argument'] = mb_substr($row['argument'], 0, 200) . '... [' . implode(' ', (array) Util::formatByteDown(mb_strlen($row['argument']), 2, 2)) . ']';
                    }
                    break;
                default:
                    break;
    ...
    public function getJsonForLoggingVars(?string $name, ?string $value) : array
    {
        ...
        return $this->dbi->fetchResult('SHOW GLOBAL VARIABLES WHERE Variable_name IN' . ' ("general_log","slow_query_log","long_query_time","log_output")', 0, 1);
    }
    /**
     ...
     * @return array JSON
     */
    public function getJsonForQueryAnalyzer(string $database, string $query) : array
    {
        ...
        if ($profiling) {
            $return['profiling'] = [];
            $result = $this->dbi->tryQuery('SELECT seq,state,duration FROM INFORMATION_SCHEMA.PROFILING' . ' WHERE QUERY_ID=1 ORDER BY seq');
            while ($row = $this->dbi->fetchAssoc($result)) {
