...
class Privileges
{
    ...
    public function wildcardEscapeForGrant(string $dbname, string $tablename) : string
    {
        ...
        if (strlen($tablename) > 0) {
            return Util::backquote(Util::unescapeMysqlWildcards($dbname)) . '.' . Util::backquote($tablename);
        }
    ...
    public function rangeOfUsers($initial = '')
    {
        ...
        return " WHERE `User` LIKE '" . $this->dbi->escapeString($initial) . "%'" . " OR `User` LIKE '" . $this->dbi->escapeString(mb_strtolower($initial)) . "%'";
    }
    /**
    ...
    {
        return [['Delete', 'DELETE', __('Allows deleting data.')], ['Create', 'CREATE', __('Allows creating new tables.')], ['Drop', 'DROP', __('Allows dropping tables.')], ['Index', 'INDEX', __('Allows creating and dropping indexes.')], ['Alter', 'ALTER', __('Allows altering the structure of existing tables.')], ['Create View', 'CREATE_VIEW', __('Allows creating new views.')], ['Show view', 'SHOW_VIEW', __('Allows performing SHOW CREATE VIEW queries.')], ['Trigger', 'TRIGGER', __('Allows creating and dropping triggers.')]];
    }
    /**
    ...
    public function getHtmlToChooseUserGroup($username)
    {
        ...
        if (isset($GLOBALS['username'])) {
            $sql_query = 'SELECT `usergroup` FROM ' . $userTable . " WHERE `username` = '" . $this->dbi->escapeString($username) . "'";
        ...
        return $this->template->render('server/privileges/choose_user_group', ['all_user_groups' => $allUserGroups, 'user_group' => $userGroup, 'params' => ['username' => $username]]);
    }
    /**
    ...
    public function setUserGroup($username, $userGroup)
    {
        ...
        $userTable = Util::backquote($cfgRelation['db']) . '.' . Util::backquote($cfgRelation['users']);
        $sql_query = 'SELECT `usergroup` FROM ' . $userTable . " WHERE `username` = '" . $this->dbi->escapeString($username) . "'";
        $oldUserGroup = $this->dbi->fetchValue($sql_query, 0, 0, DatabaseInterface::CONNECT_CONTROL);
        if ($oldUserGroup === false) {
            $upd_query = 'INSERT INTO ' . $userTable . '(`username`, `usergroup`)' . " VALUES ('" . $this->dbi->escapeString($username) . "', " . "'" . $this->dbi->escapeString($userGroup) . "')";
        } else {
            if (empty($userGroup)) {
                $upd_query = 'DELETE FROM ' . $userTable . " WHERE `username`='" . $this->dbi->escapeString($username) . "'";
            } elseif ($oldUserGroup != $userGroup) {
                $upd_query = 'UPDATE ' . $userTable . " SET `usergroup`='" . $this->dbi->escapeString($userGroup) . "'" . " WHERE `username`='" . $this->dbi->escapeString($username) . "'";
            }
        }
        if (!isset($upd_query)) {
            return;
    ...
    public function getHtmlForRoutineSpecificPrivileges($username, $hostname, $db, $routine, $url_dbname)
    {
        ...
        return $this->template->render('server/privileges/edit_routine_privileges', ['username' => $username, 'hostname' => $hostname, 'database' => $db, 'routine' => $routine, 'privileges' => $privileges, 'dbname' => $url_dbname, 'current_user' => $this->dbi->getCurrentUser()]);
    }
    /**
    ...
    {
        $grants = $this->dbi->fetchResult("SHOW GRANTS FOR '" . $this->dbi->escapeString($user) . "'@'" . $this->dbi->escapeString($host) . "'");
        $response = '';
    ...
    public function updatePassword($err_url, $username, $hostname)
    {
        ...
        if ($message === null) {
            ...
            $serverType = Util::getServerType();
            $serverVersion = $this->dbi->getVersion();
            $authentication_plugin = $_POST['authentication_plugin'] ?? $this->getCurrentAuthenticationPlugin('change', $username, $hostname);
            // Use 'ALTER USER ...' syntax for MySQL 5.7.6+
            if ($serverType === 'MySQL' && $serverVersion >= 50706) {
                if ($authentication_plugin !== 'mysql_old_password') {
                    $query_prefix = "ALTER USER '" . $this->dbi->escapeString($username) . "'@'" . $this->dbi->escapeString($hostname) . "'" . ' IDENTIFIED WITH ' . $authentication_plugin . " BY '";
                } else {
                    $query_prefix = "ALTER USER '" . $this->dbi->escapeString($username) . "'@'" . $this->dbi->escapeString($hostname) . "'" . " IDENTIFIED BY '";
                }
                // in $sql_query which will be displayed, hide the password
                $sql_query = $query_prefix . "*'";
                $local_query = $query_prefix . $this->dbi->escapeString($_POST['pma_pw']) . "'";
                ...
                $query_prefix = "SET PASSWORD FOR  '" . $this->dbi->escapeString($username) . "'@'" . $this->dbi->escapeString($hostname) . "'" . " = PASSWORD ('";
                $sql_query = $local_query = $query_prefix . $this->dbi->escapeString($_POST['pma_pw']) . "')";
            } elseif ($serverType === 'MariaDB' && $serverVersion >= 50200 && $this->dbi->isSuperUser()) {
                ...
                $sql_query = 'SET PASSWORD FOR \'' . $this->dbi->escapeString($username) . '\'@\'' . $this->dbi->escapeString($hostname) . '\' = ' . ($_POST['pma_pw'] == '' ? '\'\'' : $hashing_function . '(\'' . preg_replace('@.@s', '*', $_POST['pma_pw']) . '\')');
                $local_query = 'UPDATE `mysql`.`user` SET ' . " `authentication_string` = '" . $hashedPassword . "', `Password` = '', " . " `plugin` = '" . $authentication_plugin . "'" . " WHERE `User` = '" . $dbi->escapeString($username) . "' AND Host = '" . $dbi->escapeString($hostname) . "';";
            } else {
                ...
                $update_plugin_query = 'UPDATE `mysql`.`user` SET' . " `plugin` = '" . $authentication_plugin . "'" . " WHERE `User` = '" . $dbi->escapeString($username) . "' AND Host = '" . $dbi->escapeString($hostname) . "';";
                // Update the plugin for the user
                if (!$this->dbi->tryQuery($update_plugin_query)) {
                    Generator::mysqlDie($this->dbi->getError(), $update_plugin_query, false, $err_url);
                }
                ...
                $sql_query = 'SET PASSWORD FOR \'' . $this->dbi->escapeString($username) . '\'@\'' . $this->dbi->escapeString($hostname) . '\' = ' . ($_POST['pma_pw'] == '' ? '\'\'' : $hashing_function . '(\'' . preg_replace('@.@s', '*', $_POST['pma_pw']) . '\')');
                $local_query = 'SET PASSWORD FOR \'' . $this->dbi->escapeString($username) . '\'@\'' . $this->dbi->escapeString($hostname) . '\' = ' . ($_POST['pma_pw'] == '' ? '\'\'' : $hashing_function . '(\'' . $this->dbi->escapeString($_POST['pma_pw']) . '\')');
            }
            if (!$this->dbi->tryQuery($local_query)) {
                Generator::mysqlDie($this->dbi->getError(), $sql_query, false, $err_url);
            }
            ...
            if (isset($orig_value)) {
                $this->dbi->tryQuery('SET `old_passwords` = ' . $orig_value . ';');
            }
        }
        return $message;
    }
    ...
    public function getMessageAndSqlQueryForPrivilegesRevoke(string $dbname, string $tablename, $username, $hostname, $itemType)
    {
        ...
        $sql_query0 = 'REVOKE ALL PRIVILEGES ON ' . $itemType . ' ' . $db_and_table . ' FROM \'' . $this->dbi->escapeString($username) . '\'@\'' . $this->dbi->escapeString($hostname) . '\';';
        $sql_query1 = 'REVOKE GRANT OPTION ON ' . $itemType . ' ' . $db_and_table . ' FROM \'' . $this->dbi->escapeString($username) . '\'@\'' . $this->dbi->escapeString($hostname) . '\';';
        ...
        $message = Message::success(__('You have revoked the privileges for %s.'));
        $message->addParam('\'' . $username . '\'@\'' . $hostname . '\'');
    ...
    {
        return Message::error(__('Not enough privilege to view users.'))->getDisplay();
    }
    /**
    ...
    public function getUserGroupCount()
    {
        ...
        return $this->dbi->fetchValue($sql_query, 0, 0, DatabaseInterface::CONNECT_CONTROL);
    }
    /**
    ...
    public function getHtmlForInitials(array $array_initials)
    {
        ...
        $initials = $this->dbi->tryQuery('SELECT DISTINCT UPPER(LEFT(`User`,1)) FROM `user`' . ' ORDER BY UPPER(LEFT(`User`,1)) ASC', DatabaseInterface::CONNECT_USER, DatabaseInterface::QUERY_STORE);
        if ($initials) {
        ...
        return $this->template->render('server/privileges/initials_row', ['array_initials' => $array_initials, 'initial' => $_GET['initial'] ?? null]);
    }
    /**
    ...
    public function deleteUser(array $queries)
    {
        ...
        if (empty($queries)) {
            ...
            if (!empty($drop_user_error)) {
                ...
                $message = Message::success(__('The selected users have been deleted successfully.'));
            }
        }
    ...
    public function updatePrivileges(string $username, string $hostname, string $tablename, string $dbname, string $itemType) : array
    {
        ...
        if (!(strlen($tablename) > 0 && implode('', $this->extractPrivInfo()) === 'USAGE')) {
            [$grantBackQuery, $alterUserQuery] = $this->generateQueriesForUpdatePrivileges($itemType, $db_and_table, $username, $hostname, $dbname);
        }
    ...
    {
        $alterUserQuery = null;
        $grantBackQuery = 'GRANT ' . implode(', ', $this->extractPrivInfo()) . ' ON ' . $itemType . ' ' . $db_and_table . ' TO \'' . $this->dbi->escapeString($username) . '\'@\'' . $this->dbi->escapeString($hostname) . '\'';
        ...
        if ($needsToUseAlter) {
            $alterUserQuery = 'ALTER USER \'' . $this->dbi->escapeString($username) . '\'@\'' . $this->dbi->escapeString($hostname) . '\' ';
        }
        if (strlen($dbname) === 0) {
            // add REQUIRE clause
            if ($needsToUseAlter) {
        ...
        if (isset($_POST['Grant_priv']) && $_POST['Grant_priv'] === 'Y' || strlen($dbname) === 0 && (isset($_POST['max_questions']) || isset($_POST['max_connections']) || isset($_POST['max_updates']) || isset($_POST['max_user_connections']))) {
            if ($needsToUseAlter) {
    ...
    {
        if (isset($_POST['change_copy'])) {
            $selected_usr = [$_POST['old_username'] . '&amp;#27;' . $_POST['old_hostname']];
        } else {
        ...
        foreach ($selected_usr as $each_user) {
            ...
            $queries[] = 'DROP USER \'' . $this->dbi->escapeString($this_user) . '\'@\'' . $this->dbi->escapeString($this_host) . '\';';
    ...
    public function getListForExportUserDefinition($username, $hostname)
    {
        ...
        if (isset($_POST['selected_usr'])) {
            ...
            foreach ($_POST['selected_usr'] as $export_user) {
                $export_username = mb_substr($export_user, 0, (int) mb_strpos($export_user, '&'));
    ...
    public function getAddUserHtmlFieldset($db = '', $table = '')
    {
        ...
        return $this->template->render('server/privileges/add_user_fieldset', ['url_params' => $url_params, 'rel_params' => $rel_params]);
    }
    /**
    ...
    {
        $res = $this->dbi->query('SELECT `Db`, `Table_name`, `Table_priv` FROM `mysql`.`tables_priv`' . $user_host_condition, DatabaseInterface::CONNECT_USER, DatabaseInterface::QUERY_STORE);
        while ($row = $this->dbi->fetchAssoc($res)) {
            $res2 = $this->dbi->query('SELECT `Column_name`, `Column_priv`' . ' FROM `mysql`.`columns_priv`' . ' WHERE `User`' . ' = \'' . $this->dbi->escapeString($_POST['old_username']) . "'" . ' AND `Host`' . ' = \'' . $this->dbi->escapeString($_POST['old_username']) . '\'' . ' AND `Db`' . ' = \'' . $this->dbi->escapeString($row['Db']) . "'" . ' AND `Table_name`' . ' = \'' . $this->dbi->escapeString($row['Table_name']) . "'" . ';', DatabaseInterface::CONNECT_USER, DatabaseInterface::QUERY_STORE);
            $tmp_privs1 = $this->extractPrivInfo($row);
            $tmp_privs2 = ['Select' => [], 'Insert' => [], 'Update' => [], 'References' => []];
            ...
            $queries[] = 'GRANT ' . implode(', ', $tmp_privs1) . ' ON ' . Util::backquote($row['Db']) . '.' . Util::backquote($row['Table_name']) . ' TO \'' . $this->dbi->escapeString($username) . '\'@\'' . $this->dbi->escapeString($hostname) . '\'' . (in_array('Grant', explode(',', $row['Table_priv'])) ? ' WITH GRANT OPTION;' : ';');
        }
        return $queries;
    }
    /**
    ...
    {
        $user_host_condition = ' WHERE `User`' . ' = \'' . $this->dbi->escapeString($_POST['old_username']) . "'" . ' AND `Host`' . ' = \'' . $this->dbi->escapeString($_POST['old_hostname']) . '\';';
        $res = $this->dbi->query('SELECT * FROM `mysql`.`db`' . $user_host_condition);
        while ($row = $this->dbi->fetchAssoc($res)) {
            $queries[] = 'GRANT ' . implode(', ', $this->extractPrivInfo($row)) . ' ON ' . Util::backquote($row['Db']) . '.*' . ' TO \'' . $this->dbi->escapeString($username) . '\'@\'' . $this->dbi->escapeString($hostname) . '\'' . ($row['Grant_priv'] === 'Y' ? ' WITH GRANT OPTION;' : ';');
        }
        $this->dbi->freeResult($res);
        $queries = $this->getTablePrivsQueriesForChangeOrCopyUser($user_host_condition, $queries, $username, $hostname);
        return $queries;
    }
    /**
    ...
    {
        $sql = 'SELECT `Proc_priv`' . ' FROM `mysql`.`procs_priv`' . " WHERE `User` = '" . $this->dbi->escapeString($username) . "'" . " AND `Host` = '" . $this->dbi->escapeString($hostname) . "'" . " AND `Db` = '" . $this->dbi->escapeString(Util::unescapeMysqlWildcards($database)) . "'" . " AND `Routine_name` LIKE '" . $this->dbi->escapeString($routine) . "';";
