...
class Select
{
    ...
    public static function render($not_only_options, $omit_fieldset)
    {
        ...
        if ($not_only_options) {
            $retval .= '<form method="post" action="' . Util::getScriptNameForOption($GLOBALS['cfg']['DefaultTabServer'], 'server') . '" class="disableAjax">';
            if (!$omit_fieldset) {
                $retval .= '<fieldset>';
            }
        ...
        foreach ($GLOBALS['cfg']['Servers'] as $key => $server) {
            ...
            if ($list) {
                ...
                if ($selected) {
                    ...
                    $retval .= '<a class="disableAjax item" href="' . $scriptName . Url::getCommon(['server' => $key], strpos($scriptName, '?') === false ? '?' : '&') . '" >' . htmlspecialchars($label) . '</a>';
                }
                $retval .= '</li>';
            } else {
