...
class UserGroups
{
    ...
    public static function getHtmlForListingUsersofAGroup(string $userGroup) : string
    {
        ...
        $usersTable = Util::backquote($cfgRelation['db']) . '.' . Util::backquote($cfgRelation['users']);
        $sql_query = 'SELECT `username` FROM ' . $usersTable . " WHERE `usergroup`='" . $dbi->escapeString($userGroup) . "'";
        ...
        return $template->render('/server/user_groups/user_listings', ['user_group_special_chars' => $userGroupSpecialChars, 'num_rows' => $numRows, 'users' => $users]);
    }
    /**
    ...
    public static function getHtmlForUserGroupsTable() : string
    {
        ...
        if ($result && $numRows) {
            ...
            foreach ($userGroups as $groupName => $tabs) {
                ...
                $userGroupVal['viewUsersUrl'] = Url::getCommon(['viewUsers' => 1, 'userGroup' => $groupName], '');
                $userGroupVal['viewUsersIcon'] = Generator::getIcon('b_usrlist', __('View users'));
                $userGroupVal['editUsersUrl'] = Url::getCommon(['editUserGroup' => 1, 'userGroup' => $groupName], '');
                $userGroupVal['editUsersIcon'] = Generator::getIcon('b_edit', __('Edit'));
                $userGroupVal['deleteUsersUrl'] = Url::getCommon(['deleteUserGroup' => 1, 'userGroup' => $groupName], '');
        ...
        $dbi->freeResult($result);
        $template = new Template();
        return $template->render('server/user_groups/user_groups', ['action' => $action, 'hidden_inputs' => $hidden_inputs ?? '', 'result' => $result, 'has_rows' => $numRows, 'user_groups_values' => $userGroupsValues, 'add_user_url' => $addUserUrl, 'add_user_icon' => $addUserIcon]);
    }
    /**
    ...
    public static function delete(string $userGroup) : void
    {
        ...
        $groupTable = Util::backquote($cfgRelation['db']) . '.' . Util::backquote($cfgRelation['usergroups']);
        $sql_query = 'DELETE FROM ' . $userTable . " WHERE `usergroup`='" . $dbi->escapeString($userGroup) . "'";
        $relation->queryAsControlUser($sql_query, true);
        $sql_query = 'DELETE FROM ' . $groupTable . " WHERE `usergroup`='" . $dbi->escapeString($userGroup) . "'";
        $relation->queryAsControlUser($sql_query, true);
    }
    /**
    ...
    public static function getHtmlToEditUserGroup(?string $userGroup = null) : string
    {
        ...
        if ($userGroup !== null) {
            $cfgRelation = $relation->getRelationsParam();
            $groupTable = Util::backquote($cfgRelation['db']) . '.' . Util::backquote($cfgRelation['usergroups']);
            $sql_query = 'SELECT * FROM ' . $groupTable . " WHERE `usergroup`='" . $dbi->escapeString($userGroup) . "'";
            $result = $relation->queryAsControlUser($sql_query, false);
            if ($result) {
                while ($row = $dbi->fetchAssoc($result)) {
                    $key = $row['tab'];
        ...
        $tabList = self::getTabList(__('Server-level tabs'), 'server', $allowedTabs['server']);
        $tabList .= self::getTabList(__('Database-level tabs'), 'db', $allowedTabs['db']);
        $tabList .= self::getTabList(__('Table-level tabs'), 'table', $allowedTabs['table']);
        $template = new Template();
        return $template->render('/server/user_groups/edit_user_groups', ['user_group' => $userGroup, 'edit_user_group_special_chars' => $editUserGroupSpecialChars, 'user_group_url' => Url::getFromRoute('/server/user-groups'), 'hidden_inputs' => Url::getHiddenInputs($urlParams), 'tab_list' => $tabList]);
    }
    /**
    ...
    public static function getTabList(string $title, string $level, array $selected) : string
    {
        ...
        $template = new Template();
        return $template->render('/server/user_groups/tab_list', ['title' => $title, 'level' => $level, 'tab_details' => $tabDetails]);
    }
    /**
    ...
    public static function edit(string $userGroup, bool $new = false) : void
    {
        ...
        foreach ($tabs as $tabGroupName => $tabGroup) {
            foreach ($tabGroup as $tab => $tabName) {
                ...
                $sql_query .= "('" . $dbi->escapeString($userGroup) . "', '" . $tabName . "', '" . ($allowed ? 'Y' : 'N') . "')";
                $first = false;
            }
        }
        $sql_query .= ';';
