...
class Tracker
{
    ...
    public static function isTracked($dbname, $tablename)
    {
        ...
        $sql_query = ' SELECT tracking_active FROM ' . self::getTrackingTable() . " WHERE db_name = '" . $dbi->escapeString($dbname) . "' " . " AND table_name = '" . $dbi->escapeString($tablename) . "' " . ' ORDER BY version DESC LIMIT 1';
    ...
    public static function createVersion($dbname, $tablename, $version, $tracking_set = '', bool $is_view = false)
    {
         ...
         */
        $export_sql_plugin = Plugins::getPlugin('export', 'sql', 'libraries/classes/Plugins/Export/', ['export_type' => $export_type, 'single_table' => false]);
        $sql_backquotes = true;
        $date = Util::date('Y-m-d H:i:s');
    ...
    public static function deleteTracking($dbname, $tablename, $version = '')
    {
        global $dbi;
        $relation = new Relation($dbi);
        $sql_query = "/*NOTRACK*/\n" . 'DELETE FROM ' . self::getTrackingTable() . " WHERE `db_name` = '" . $dbi->escapeString($dbname) . "'" . " AND `table_name` = '" . $dbi->escapeString($tablename) . "'";
        if ($version) {
    ...
    private static function changeTracking($dbname, $tablename, $version, $new_state)
    {
        global $dbi;
        $relation = new Relation($dbi);
        $sql_query = ' UPDATE ' . self::getTrackingTable() . " SET `tracking_active` = '" . $new_state . "' " . " WHERE `db_name` = '" . $dbi->escapeString($dbname) . "' " . " AND `table_name` = '" . $dbi->escapeString($tablename) . "' " . " AND `version` = '" . $dbi->escapeString((string) $version) . "' ";
        return $relation->queryAsControlUser($sql_query);
    }
    /**
    ...
    public static function changeTrackingData($dbname, $tablename, $version, $type, $new_data)
    {
        ...
        if (is_array($new_data)) {
            foreach ($new_data as $data) {
                $new_data_processed .= '# log ' . $date . ' ' . $data['username'] . $dbi->escapeString($data['statement']) . "\n";
            }
        } else {
            $new_data_processed = $new_data;
        }
        $sql_query = ' UPDATE ' . self::getTrackingTable() . ' SET `' . $save_to . "` = '" . $new_data_processed . "' " . " WHERE `db_name` = '" . $dbi->escapeString($dbname) . "' " . " AND `table_name` = '" . $dbi->escapeString($tablename) . "' " . " AND `version` = '" . $dbi->escapeString($version) . "' ";
        $result = $relation->queryAsControlUser($sql_query);
        return (bool) $result;
    }
    /**
    ...
    public static function getVersion($dbname, $tablename, $statement = null)
    {
        ...
        global $dbi;
        $relation = new Relation($dbi);
        $sql_query = ' SELECT MAX(version) FROM ' . self::getTrackingTable() . " WHERE `db_name` = '" . $dbi->escapeString($dbname) . "' " . " AND `table_name` = '" . $dbi->escapeString($tablename) . "' ";
        if ($statement != '') {
    ...
    public static function getTrackedData($dbname, $tablename, $version)
    {
        global $dbi;
        $relation = new Relation($dbi);
        $sql_query = ' SELECT * FROM ' . self::getTrackingTable() . " WHERE `db_name` = '" . $dbi->escapeString($dbname) . "' ";
        if (!empty($tablename)) {
            $sql_query .= " AND `table_name` = '" . $dbi->escapeString($tablename) . "' ";
        }
        $sql_query .= " AND `version` = '" . $dbi->escapeString($version) . "' ORDER BY `version` DESC LIMIT 1";
    ...
    public static function handleQuery($query)
    {
        ...
        //     3. the statements
        // we want to track
        $sql_query .= " WHERE FIND_IN_SET('" . $result['identifier'] . "',tracking) > 0" . " AND `db_name` = '" . $dbi->escapeString($dbname ?? '') . "' " . " AND `table_name` = '" . $dbi->escapeString($result['tablename']) . "' " . " AND `version` = '" . $dbi->escapeString($version ?? '') . "' ";
