...
class ErrorReport
{
    ...
    public function getData(string $exceptionType = 'js') : array
    {
        ...
        if ($exceptionType === 'js') {
            ...
            foreach ($_SESSION['prev_errors'] as $errorObj) {
                ...
                $errors[$i++] = ['lineNum' => $errorObj->getLine(), 'file' => $errorObj->getFile(), 'type' => $errorObj->getType(), 'msg' => $errorObj->getOnlyMessage(), 'stackTrace' => $errorObj->getBacktrace(5), 'stackhash' => $errorObj->getHash()];
            }
    ...
    {
        return $this->httpRequest->create($this->submissionUrl, 'POST', false, json_encode($report), 'Content-Type: application/json');
    }
    /**
    ...
    {
        $datas = ['report_data' => $this->getPrettyData(), 'hidden_inputs' => Url::getHiddenInputs(), 'hidden_fields' => null];
