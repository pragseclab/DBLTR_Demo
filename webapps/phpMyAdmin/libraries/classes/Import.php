...
class Import
{
    ...
    {
        return (int) substr($last_cumulative_size, 0, (int) strpos($last_cumulative_size, ','));
    }
    /**
        ...
        if ($curr_type == self::DECIMAL) {
            ...
            if ($last_cumulative_type == self::DECIMAL) {
                ...
                if ($size[self::M] > $oldM || $size[self::D] > $oldD) {
                    ...
                    return (string) (($size[self::M] > $oldM ? $size[self::M] : $oldM) . ',' . ($size[self::D] > $oldD ? $size[self::D] : $oldD));
                }
                return $last_cumulative_size;
            }
            if ($last_cumulative_type == self::BIGINT || $last_cumulative_type == self::INT) {
    ...
    public function detectType(?int $last_cumulative_type, ?string $cell) : int
    {
        ...
        if ($cell == (string) (float) $cell && mb_strpos((string) $cell, '.') !== false && mb_substr_count((string) $cell, '.') === 1) {
            return self::DECIMAL;
        }
    ...
    public function analyzeTable(array &$table)
    {
        ...
        if (!is_array($table) || is_array($table[self::TBL_NAME]) || !is_array($table[self::COL_NAMES]) || !is_array($table[self::ROWS])) {
            /**
        ...
        for ($i = 0; $i < $numCols; ++$i) {
            ...
            for ($j = 0; $j < $numRows; ++$j) {
                ...
                $sizes[$i] = $this->detectSize($sizes[$i], $types[$i], $curr_type, (string) $table[self::ROWS][$j][$i]);
                /**
    ...
    public function buildSql(string $db_name, array &$tables, ?array &$analyses = null, ?array &$additional_sql = null, ?array $options = null, array &$sql_data) : void
    {
        ...
        if ($analyses != null) {
            $type_array = [self::NONE => 'NULL', self::VARCHAR => 'varchar', self::INT => 'int', self::DECIMAL => 'decimal', self::BIGINT => 'bigint', self::GEOMETRY => 'geometry'];
            ...
            for ($i = 0; $i < $num_tables; ++$i) {
                ...
                $tempSQLStr = 'CREATE TABLE IF NOT EXISTS ' . Util::backquote($db_name) . '.' . Util::backquote($tables[$i][self::TBL_NAME]) . ' (';
                for ($j = 0; $j < $num_cols; ++$j) {
                    $size = $analyses[$i][self::SIZES][$j];
                    if ((int) $size == 0) {
                        $size = 10;
                    }
                    $tempSQLStr .= Util::backquote($tables[$i][self::COL_NAMES][$j]) . ' ' . $type_array[$analyses[$i][self::TYPES][$j]];
        ...
        for ($i = 0; $i < $num_tables; ++$i) {
            ...
            $tempSQLStr = 'INSERT INTO ' . Util::backquote($db_name) . '.' . Util::backquote($tables[$i][self::TBL_NAME]) . ' (';
            ...
            for ($j = 0; $j < $num_rows; ++$j) {
                ...
                for ($k = 0; $k < $num_cols; ++$k) {
                    ...
                    if ($analyses != null && isset($analyses[$i][self::FORMATTEDSQL][$col_count]) && $analyses[$i][self::FORMATTEDSQL][$col_count] == true) {
        ...
        $message .= sprintf('<br><li><a href="%s" title="%s">%s</a> (<a href="%s" title="%s">' . __('Options') . '</a>)</li>', $db_url, sprintf(__('Go to database: %s'), htmlspecialchars(Util::backquote($db_name))), htmlspecialchars($db_name), $db_ops_url, sprintf(__('Edit settings for %s'), htmlspecialchars(Util::backquote($db_name))));
        $message .= '<ul>';
        unset($params);
        foreach ($tables as $table) {
            $params = ['db' => $db_name, 'table' => (string) $table[self::TBL_NAME]];
            ...
            if (!$_table->isView()) {
                ...
                $message .= sprintf('<li><a href="%s" title="%s">%s</a></li>', $tbl_url, sprintf(__('Go to view: %s'), htmlspecialchars(Util::backquote($table[self::TBL_NAME]))), htmlspecialchars($table[self::TBL_NAME]));
            }
        }
        $message .= '</ul></ul>';
    ...
    public function handleSimulateDmlRequest() : void
    {
        ...
        foreach ($queries as $sql_query) {
            ...
            $analyzed_sql_results = ['query' => $sql_query, 'parser' => $parser, 'statement' => $statement];
            if (!($statement instanceof UpdateStatement || $statement instanceof DeleteStatement) || !empty($statement->join)) {
                $error = $error_msg;
                break;
            }
    ...
    public function getMatchedRows(array $analyzed_sql_results = []) : array
    {
        ...
        // URL to matched rows.
        $_url_params = ['db' => $GLOBALS['db'], 'sql_query' => $matched_row_query];
        $matched_rows_url = Url::getFromRoute('/sql', $_url_params);
        return ['sql_query' => Html\Generator::formatSql($analyzed_sql_results['query']), 'matched_rows' => $matched_rows, 'matched_rows_url' => $matched_rows_url];
    }
    /**
    ...
    {
        $table_references = Query::getTables($analyzed_sql_results['statement']);
        $where = Query::getClause($analyzed_sql_results['statement'], $analyzed_sql_results['parser']->list, 'WHERE');
        if (empty($where)) {
            $where = '1';
        }
        $columns = [];
        ...
        if (!empty($analyzed_sql_results['statement']->order)) {
            $order_and_limit .= ' ORDER BY ' . Query::getClause($analyzed_sql_results['statement'], $analyzed_sql_results['parser']->list, 'ORDER BY');
        }
        if (!empty($analyzed_sql_results['statement']->limit)) {
            $order_and_limit .= ' LIMIT ' . Query::getClause($analyzed_sql_results['statement'], $analyzed_sql_results['parser']->list, 'LIMIT');
        }
        return 'SELECT ' . implode(', ', $columns) . ' FROM ' . implode(', ', $table_references) . ' WHERE ' . $where . $order_and_limit;
    }
    /**
    ...
    public function checkIfRollbackPossible(string $sql_query) : bool
    {
        ...
        if (!($statement instanceof InsertStatement || $statement instanceof UpdateStatement || $statement instanceof DeleteStatement || $statement instanceof ReplaceStatement)) {
            return false;
        }
    ...
    public function isTableTransactional(string $table) : bool
    {
        ...
        $check_table_query = 'SELECT * FROM ' . Util::backquote($db) . '.' . Util::backquote($table) . ' ' . 'LIMIT 1';
        $result = $dbi->tryQuery($check_table_query);
        if (!$result) {
            return false;
        }
        // List of Transactional Engines.
        $transactional_engines = ['INNODB', 'FALCON', 'NDB', 'INFINIDB', 'TOKUDB', 'XTRADB', 'SEQUENCE', 'BDB'];
