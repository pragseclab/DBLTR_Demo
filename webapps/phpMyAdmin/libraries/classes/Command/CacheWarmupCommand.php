...
final class CacheWarmupCommand extends Command
{
    ...
    private function warmUpRoutingCache(OutputInterface $output) : int
    {
        ...
        $output->writeln(sprintf('Warm up did not work, the folder "%s" is probably not writable.', CACHE_DIR), OutputInterface::VERBOSITY_NORMAL);
        return 1;
    }
    private function warmUpTwigCache(OutputInterface $output) : int
    {
        ...
        $twig = new Environment($loader, ['auto_reload' => true, 'cache' => $tmpDir]);
        $twig->setExtensions([new CoreExtension(), new I18nExtension(), new MessageExtension(), new PluginsExtension(), new RelationExtension(), new SanitizeExtension(), new TableExtension(), new TrackerExtension(), new TransformationsExtension(), new UrlExtension(), new UtilExtension()]);
        ...
        $replacements = [];
        $templates = new RecursiveIteratorIterator(new RecursiveDirectoryIterator($tplDir), RecursiveIteratorIterator::LEAVES_ONLY);
