...
class FormDisplay
{
    ...
    {
        $valueCmp = is_bool($value) ? (int) $value : $value;
    ...
    public function save($forms, $allowPartialSave = true)
    {
        ...
        foreach ($forms as $formName) {
            ...
            $changeIndex = $form->index === 0 ? $this->configFile->getServerCount() + 1 : false;
            // grab POST values
            foreach ($form->fields as $field => $systemPath) {
                ...
                if (!isset($_POST[$key])) {
                    ...
                    if ($type !== 'boolean') {
                        $this->errors[$form->name][] = sprintf(__('Missing data for %s'), '<i>' . Descriptions::get($systemPath) . '</i>');
                        $result = false;
                        continue;
                    }
                    $_POST[$key] = false;
                ...
                switch ($type) {
                    ...
                    case 'integer':
                        ...
                        break;
                    case 'select':
                        $successfullyValidated = $this->validateSelect($_POST[$key], $form->getOptionValueList($systemPath));
                    ...
                    case 'array':
                        ...
                        $postValues = is_array($_POST[$key]) ? $_POST[$key] : explode("\n", $_POST[$key]);
                        $_POST[$key] = [];
                ...
                if ($changeIndex !== false) {
                    $workPath = str_replace('Servers/' . $form->index . '/', 'Servers/' . $changeIndex . '/', $workPath);
                }
        ...
        if ($isSetupScript) {
            $this->configFile->set('UserprefsDisallow', array_keys($this->userprefsDisallow));
        }
        // don't look for non-critical errors
        $this->validate();
        return $result;
