...
class Sql
{
    ...
    public function getHtmlForRelationalColumnDropdown($db, $table, $column, $curr_value)
    {
        ...
        if ($foreignData['disp_row'] == null) {
            ...
            $_url_params = ['db' => $db, 'table' => $table, 'field' => $column];
            $dropdown = $this->template->render('sql/relational_column_dropdown', ['current_value' => $_POST['curr_value'], 'params' => $_url_params]);
        } else {
            $dropdown = $this->relation->foreignDropdown($foreignData['disp_row'], $foreignData['foreign_field'], $foreignData['foreign_display'], $curr_value, $GLOBALS['cfg']['ForeignKeyMaxLimit']);
    ...
    private function getDetailedProfilingStats(array $profilingResults) : array
    {
        ...
        foreach ($profilingResults as $oneResult) {
            ...
            $profiling['profile'][] = ['status' => $status, 'duration' => Util::formatNumber($oneResult['Duration'], 3, 1), 'duration_raw' => $oneResult['Duration']];
    ...
    {
        $row = $this->dbi->fetchSingleRow(sprintf('SELECT `%s` FROM `%s`.`%s` WHERE %s', $column, $db, $table, $whereClause));
        if ($row === null) {
            return '';
        }
        return $row[$column];
    ...
    public function getValuesForColumn($db, $table, $column)
    {
        ...
        $field_info_result = $this->dbi->fetchResult($field_info_query, null, null, DatabaseInterface::CONNECT_USER, DatabaseInterface::QUERY_STORE);
    ...
    public function setColumnProperty($pmatable, $request_index)
    {
        ...
        return $pmatable->setUiProp($property_to_set, $property_value, $_POST['table_create_time'] ?? null);
    }
    /**
    ...
    {
        $bfields = ['bkm_database' => $db, 'bkm_user' => $bkm_user, 'bkm_sql_query' => $sql_query_for_bookmark, 'bkm_label' => $bkm_label];
        ...
        $bookmark = Bookmark::createBookmark($this->dbi, $GLOBALS['cfg']['Server']['user'], $bfields, isset($_POST['bkm_all_users']));
        $bookmark->save();
    }
    /**
