...
final class Search
{
    ...
    {
        $geom_unary_functions = ['IsEmpty' => 1, 'IsSimple' => 1, 'IsRing' => 1, 'IsClosed' => 1];
        $where = '';
        ...
        if ($geom_funcs[$geom_func]['params'] > 1) {
            ...
            return $geom_func . '(' . Util::backquote($names) . ', ' . $gis_data . ')';
        }
    ...
    private function getEnumWhereClause($criteriaValues, $func_type) : string
    {
        ...
        $enum_where = '\'' . $this->dbi->escapeString($criteriaValues[0]) . '\'';
        for ($e = 1; $e < $enum_selected_count; $e++) {
            $enum_where .= ', \'' . $this->dbi->escapeString($criteriaValues[$e]) . '\'';
        }
