...
class UserPassword
{
    ...
    public function changePassword($password) : string
    {
        ...
        if ($serverType === 'MySQL' && $serverVersion >= 50706) {
            $sql_query = 'ALTER USER \'' . $dbi->escapeString($username) . '\'@\'' . $dbi->escapeString($hostname) . '\' IDENTIFIED WITH ' . $orig_auth_plugin . ' BY ' . ($password == '' ? '\'\'' : '\'***\'');
        } elseif ($serverType === 'MySQL' && $serverVersion >= 50507 || $serverType === 'MariaDB' && $serverVersion >= 50200) {
        ...
        $this->changePassUrlParamsAndSubmitQuery($username, $hostname, $password, $sql_query, $hashing_function, $orig_auth_plugin);
    ...
    {
        if (Core::isValid($_POST['authentication_plugin'], 'identical', 'mysql_old_password')) {
     ...
     * @return void
     */
    private function changePassUrlParamsAndSubmitQuery($username, $hostname, $password, $sql_query, $hashing_function, $orig_auth_plugin)
    {
        global $dbi;
        ...
        $serverVersion = $dbi->getVersion();
        if ($serverType === 'MySQL' && $serverVersion >= 50706) {
            $local_query = 'ALTER USER \'' . $dbi->escapeString($username) . '\'@\'' . $dbi->escapeString($hostname) . '\'' . ' IDENTIFIED with ' . $orig_auth_plugin . ' BY ' . ($password == '' ? '\'\'' : '\'' . $dbi->escapeString($password) . '\'');
        } elseif ($serverType === 'MariaDB' && $serverVersion >= 50200 && $serverVersion < 100100 && $orig_auth_plugin !== '') {
            ...
            $local_query = 'UPDATE `mysql`.`user` SET' . " `authentication_string` = '" . $hashedPassword . "', `Password` = '', " . " `plugin` = '" . $orig_auth_plugin . "'" . " WHERE `User` = '" . $dbi->escapeString($username) . "' AND Host = '" . $dbi->escapeString($hostname) . "';";
        } else {
            $local_query = 'SET password = ' . ($password == '' ? '\'\'' : $hashing_function . '(\'' . $dbi->escapeString($password) . '\')');
        }
        if (!@$dbi->tryQuery($local_query)) {
            Generator::mysqlDie($dbi->getError(), $sql_query, false, $err_url);
        }
