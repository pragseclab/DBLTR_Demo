...
class Innodb extends StorageEngine
{
    ...
    public function getPageBufferpool()
    {
        ...
        $sql = 'SHOW STATUS' . ' WHERE Variable_name LIKE \'Innodb\\_buffer\\_pool\\_%\'' . ' OR Variable_name = \'Innodb_page_size\';';
        $status = $dbi->fetchResult($sql, 0, 1);
        $output = '<table class="table table-light table-striped table-hover w-auto float-left">' . "\n" . '    <caption>' . "\n" . '        ' . __('Buffer Pool Usage') . "\n" . '    </caption>' . "\n" . '    <tfoot class="thead-light">' . "\n" . '        <tr>' . "\n" . '            <th colspan="2">' . "\n" . '                ' . __('Total:') . ' ' . Util::formatNumber($status['Innodb_buffer_pool_pages_total'], 0) . '&nbsp;' . __('pages') . ' / ' . implode('&nbsp;', Util::formatByteDown($status['Innodb_buffer_pool_pages_total'] * $status['Innodb_page_size'])) . "\n" . '            </th>' . "\n" . '        </tr>' . "\n" . '    </tfoot>' . "\n" . '    <tbody>' . "\n" . '        <tr>' . "\n" . '            <th scope="row">' . __('Free pages') . '</th>' . "\n" . '            <td class="text-monospace text-right">' . Util::formatNumber($status['Innodb_buffer_pool_pages_free'], 0) . '</td>' . "\n" . '        </tr>' . "\n" . '        <tr>' . "\n" . '            <th scope="row">' . __('Dirty pages') . '</th>' . "\n" . '            <td class="text-monospace text-right">' . Util::formatNumber($status['Innodb_buffer_pool_pages_dirty'], 0) . '</td>' . "\n" . '        </tr>' . "\n" . '        <tr>' . "\n" . '            <th scope="row">' . __('Pages containing data') . '</th>' . "\n" . '            <td class="text-monospace text-right">' . Util::formatNumber($status['Innodb_buffer_pool_pages_data'], 0) . "\n" . '</td>' . "\n" . '        </tr>' . "\n" . '        <tr>' . "\n" . '            <th scope="row">' . __('Pages to be flushed') . '</th>' . "\n" . '            <td class="text-monospace text-right">' . Util::formatNumber($status['Innodb_buffer_pool_pages_flushed'], 0) . "\n" . '</td>' . "\n" . '        </tr>' . "\n" . '        <tr>' . "\n" . '            <th scope="row">' . __('Busy pages') . '</th>' . "\n" . '            <td class="text-monospace text-right">' . Util::formatNumber($status['Innodb_buffer_pool_pages_misc'], 0) . "\n" . '</td>' . "\n" . '        </tr>';
        // not present at least since MySQL 5.1.40
        if (isset($status['Innodb_buffer_pool_pages_latched'])) {
            $output .= '        <tr>' . '            <th scope="row">' . __('Latched pages') . '</th>' . '            <td class="text-monospace text-right">' . Util::formatNumber($status['Innodb_buffer_pool_pages_latched'], 0) . '</td>' . '        </tr>';
        }
        $output .= '    </tbody>' . "\n" . '</table>' . "\n\n" . '<table class="table table-light table-striped table-hover w-auto ml-4 float-left">' . "\n" . '    <caption>' . "\n" . '        ' . __('Buffer Pool Activity') . "\n" . '    </caption>' . "\n" . '    <tbody>' . "\n" . '        <tr>' . "\n" . '            <th scope="row">' . __('Read requests') . '</th>' . "\n" . '            <td class="text-monospace text-right">' . Util::formatNumber($status['Innodb_buffer_pool_read_requests'], 0) . "\n" . '</td>' . "\n" . '        </tr>' . "\n" . '        <tr>' . "\n" . '            <th scope="row">' . __('Write requests') . '</th>' . "\n" . '            <td class="text-monospace text-right">' . Util::formatNumber($status['Innodb_buffer_pool_write_requests'], 0) . "\n" . '</td>' . "\n" . '        </tr>' . "\n" . '        <tr>' . "\n" . '            <th scope="row">' . __('Read misses') . '</th>' . "\n" . '            <td class="text-monospace text-right">' . Util::formatNumber($status['Innodb_buffer_pool_reads'], 0) . "\n" . '</td>' . "\n" . '        </tr>' . "\n" . '        <tr>' . "\n" . '            <th scope="row">' . __('Write waits') . '</th>' . "\n" . '            <td class="text-monospace text-right">' . Util::formatNumber($status['Innodb_buffer_pool_wait_free'], 0) . "\n" . '</td>' . "\n" . '        </tr>' . "\n" . '        <tr>' . "\n" . '            <th scope="row">' . __('Read misses in %') . '</th>' . "\n" . '            <td class="text-monospace text-right">' . ($status['Innodb_buffer_pool_read_requests'] == 0 ? '---' : htmlspecialchars(Util::formatNumber($status['Innodb_buffer_pool_reads'] * 100 / $status['Innodb_buffer_pool_read_requests'], 3, 2)) . ' %') . "\n" . '</td>' . "\n" . '        </tr>' . "\n" . '        <tr>' . "\n" . '            <th scope="row">' . __('Write waits in %') . '</th>' . "\n" . '            <td class="text-monospace text-right">' . ($status['Innodb_buffer_pool_write_requests'] == 0 ? '---' : htmlspecialchars(Util::formatNumber($status['Innodb_buffer_pool_wait_free'] * 100 / $status['Innodb_buffer_pool_write_requests'], 3, 2)) . ' %') . "\n" . '</td>' . "\n" . '        </tr>' . "\n" . '    </tbody>' . "\n" . '</table>' . "\n";
        return $output;
    }
    /**
    ...
    {
        global $dbi;
        return '<pre id="pre_innodb_status">' . "\n" . htmlspecialchars((string) $dbi->fetchValue('SHOW ENGINE INNODB STATUS;', 0, 'Status')) . "\n" . '</pre>' . "\n";
    }
    /**
    ...
    {
        global $dbi;
        return $dbi->fetchValue("SHOW GLOBAL VARIABLES LIKE 'innodb_file_format';", 0, 1);
    }
    /**
