...
class Util
{
    ...
    private static function checkRowCount($db, array $table)
    {
        ...
        if ($table['Rows'] === null) {
            ...
            if ($tbl_is_view || Utilities::isSystemSchema($db)) {
                $rowCount = $dbi->getTable($db, $table['Name'])->countRecords();
            }
        }
        return $rowCount;
    }
    ...
    public static function getTableList($db) : array
    {
        ...
        $default = ['Name' => '', 'Rows' => 0, 'Comment' => '', 'disp_name' => ''];
        $table_groups = [];
    ...
    public static function extractValueFromFormattedSize($formatted_size) : int
    {
        ...
        if (preg_match('/^[0-9]+GB$/', $formatted_size)) {
            $return_value = (int) mb_substr($formatted_size, 0, -2) * pow(1024, 3);
        } elseif (preg_match('/^[0-9]+MB$/', $formatted_size)) {
            $return_value = (int) mb_substr($formatted_size, 0, -2) * pow(1024, 2);
        } elseif (preg_match('/^[0-9]+K$/', $formatted_size)) {
            $return_value = (int) mb_substr($formatted_size, 0, -1) * pow(1024, 1);
        }
        return $return_value;
    }
    /**
    ...
    public static function getFirstOccurringRegularExpression(array $regex_array, $query) : string
    {
        ...
        foreach ($regex_array as $test_regex) {
            ...
            if ($minimum_first_occurence_index !== null && $matches[0][1] >= $minimum_first_occurence_index) {
                continue;
            }
            $regex = $test_regex;
    ...
    public static function getTablesWhenOpen($db, $db_info_result) : array
    {
        ...
        if (count($sot_cache) > 0) {
            ...
            if (Core::isValid($_REQUEST['tbl_group'])) {
                ...
                $groupWithSeparator = self::escapeMysqlWildcards($_REQUEST['tbl_group'] . $GLOBALS['cfg']['NavigationTreeTableSeparator']);
                $tblGroupSql .= ' WHERE (' . self::backquote('Tables_in_' . $db) . " LIKE '" . $groupWithSeparator . "%'" . ' OR ' . self::backquote('Tables_in_' . $db) . " LIKE '" . $group . "')";
                $whereAdded = true;
            }
            ...
            $db_info_result = $dbi->query('SHOW FULL TABLES FROM ' . self::backquote($db) . $tblGroupSql, DatabaseInterface::CONNECT_USER, DatabaseInterface::QUERY_STORE);
            ...
            if ($db_info_result && $dbi->numRows($db_info_result) > 0) {
                ...
                while ($tmp = $dbi->fetchRow($db_info_result)) {
                    if (!isset($sot_cache[$tmp[0]])) {
                        ...
                        // table in use
                        $tables[$tmp[0]] = ['TABLE_NAME' => $tmp[0], 'ENGINE' => '', 'TABLE_TYPE' => '', 'TABLE_ROWS' => 0, 'TABLE_COMMENT' => ''];
                    }
                }
                if (count($names) > 0) {
                    $tables = array_merge($tables, $dbi->getTablesFull($db, $names));
                }
                if ($GLOBALS['cfg']['NaturalOrder']) {
                    uksort($tables, 'strnatcasecmp');
                }
