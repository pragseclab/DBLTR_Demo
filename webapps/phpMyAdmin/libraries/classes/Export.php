...
class Export
{
    ...
    public function gzencodeNeeded() : bool
    {
        ...
        // see bug #4942
        return function_exists('gzencode') && (!ini_get('zlib.output_compression') && !$this->isGzHandlerEnabled() || $GLOBALS['save_on_server'] || $chromeAndGreaterThan43);
    }
    /**
    ...
    public function getHtmlForDisplayedExportFooter(string $back_button, string $refreshButton) : string
    {
         ...
         */
        return '</textarea>' . '    </form>' . '<br>' . $back_button . $refreshButton . '</div>' . '<script type="text/javascript">' . "\n" . '//<![CDATA[' . "\n" . 'var $body = $("body");' . "\n" . '$("#textSQLDUMP")' . "\n" . '.width($body.width() - 50)' . "\n" . '.height($body.height() - 100);' . "\n" . '//]]>' . "\n" . '</script>' . "\n";
    }
    /**
    ...
    public function openFile(string $filename, bool $quick_export) : array
    {
        ...
        $save_filename = Util::userDir($GLOBALS['cfg']['SaveDir']) . preg_replace('@[/\\\\]@', '_', $filename);
        if (@file_exists($save_filename) && (!$quick_export && empty($_POST['onserver_overwrite']) || $quick_export && $doNotSaveItOver)) {
    ...
    public function closeFile($file_handle, string $dump_buffer, string $save_filename) : Message
    {
        ...
        if (strlen($dump_buffer) > 0 && (!$write_result || $write_result != strlen($dump_buffer))) {
            $message = new Message(__('Insufficient space to save the file %s.'), Message::ERROR, [$save_filename]);
        } else {
            $message = new Message(__('Dump has been saved to file %s.'), Message::SUCCESS, [$save_filename]);
        }
        return $message;
    }
    /**
    ...
    public function getHtmlForDisplayedExportHeader(string $export_type, string $db, string $table) : array
    {
        ...
        if ($export_type === 'server') {
            ...
            $back_button .= Url::getFromRoute('/table/export') . '" data-post="' . Url::getCommon(['db' => $db, 'table' => $table], '');
        }
        ...
        $refreshButton .= '</form>';
        $html .= $refreshButton . '<br>' . '<form name="nofunction">' . '<textarea name="sqldump" cols="50" rows="30" ' . 'id="textSQLDUMP" wrap="OFF">';
    ...
    public static function exportRaw(string $whatStrucOrData, ExportPlugin $export_plugin, string $crlf, string $err_url, string $sql_query, string $export_type) : void
    {
        ...
        if (!$export_plugin->exportRawQuery($err_url, $sql_query, $crlf)) {
    ...
    public function mergeAliases(array $aliases1, array $aliases2) : array
    {
        ...
        foreach ($aliases as $db_name => $db) {
            ...
            if (isset($db['alias']) && is_array($db['alias'])) {
                ...
                $aliases[$db_name]['alias'] = empty($val2) ? $val1 : $val2;
            }
            if (!isset($db['tables'])) {
                continue;
            }
            foreach ($db['tables'] as $tbl_name => $tbl) {
                if (isset($tbl['alias']) && is_array($tbl['alias'])) {
                    ...
                    $val2 = $tbl['alias'][1];
                    // Use aliases2 alias if non empty
                    $aliases[$db_name]['tables'][$tbl_name]['alias'] = empty($val2) ? $val1 : $val2;
                }
                ...
                foreach ($tbl['columns'] as $col => $col_as) {
                    ...
                    $val2 = $col_as[1];
                    // Use aliases2 alias if non empty
                    $aliases[$db_name]['tables'][$tbl_name]['columns'][$col] = empty($val2) ? $val1 : $val2;
                }
            }
        }
        return $aliases;
    ...
    public function lockTables(string $db, array $tables, string $lockType = 'WRITE')
    {
        $locks = [];
        foreach ($tables as $table) {
            $locks[] = Util::backquote($db) . '.' . Util::backquote($table) . ' ' . $lockType;
        }
    ...
    {
        return ['column_info', 'table_uiprefs', 'tracking', 'bookmark', 'relation', 'table_coords', 'pdf_pages', 'savedsearches', 'central_columns', 'export_templates'];
    }
    /**
    ...
    public function processExportSchema(?string $export_type) : void
    {
        ...
        $export_plugin = Plugins::getPlugin('schema', $export_type, 'libraries/classes/Plugins/Schema/');
