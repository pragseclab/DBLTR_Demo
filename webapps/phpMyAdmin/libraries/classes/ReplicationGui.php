...
class ReplicationGui
{
    ...
    public function getHtmlForMasterReplication()
    {
        ...
        return $this->template->render('server/replication/master_replication', ['clear_screen' => isset($_POST['repl_clear_scr']), 'master_status_table' => $masterStatusTable ?? '', 'slaves' => $slaves ?? [], 'url_params' => $urlParams ?? [], 'master_add_user' => isset($_POST['mr_adduser']), 'master_add_slave_user' => $masterAddSlaveUser ?? '']);
    }
    /**
    ...
    public function getHtmlForMasterConfiguration()
    {
        ...
        return $this->template->render('server/replication/master_configuration', ['database_multibox' => $databaseMultibox]);
    }
    /**
    ...
    public function getHtmlForReplicationChangeMaster($submitName)
    {
        ...
        return $this->template->render('server/replication/change_master', ['server_id' => time(), 'username_length' => $usernameLength, 'hostname_length' => $hostnameLength, 'submit_name' => $submitName]);
    }
    /**
    ...
    public function getHtmlForReplicationStatusTable($type, $isHidden = false, $hasTitle = true) : string
    {
        ...
        foreach ($replicationVariables as $variable) {
            ...
            $variablesWrap = ['Replicate_Do_DB', 'Replicate_Ignore_DB', 'Replicate_Do_Table', 'Replicate_Ignore_Table', 'Replicate_Wild_Do_Table', 'Replicate_Wild_Ignore_Table'];
            if (!in_array($variable, $variablesWrap)) {
                continue;
            }
            $variables[$variable]['value'] = str_replace(',', ', ', $serverReplicationVariable);
        }
        return $this->template->render('server/replication/status_table', ['type' => $type, 'is_hidden' => $isHidden, 'has_title' => $hasTitle, 'variables' => $variables]);
    }
    /**
    ...
    public function getHtmlForReplicationMasterAddSlaveUser()
    {
        ...
        if (!empty($currentUser)) {
            $userHost = str_replace("'", '', mb_substr($currentUser, mb_strrpos($currentUser, '@') + 1));
        ...
        return $this->template->render('server/replication/master_add_slave_user', ['username_length' => $usernameLength, 'hostname_length' => $hostnameLength, 'has_username' => isset($_POST['username']), 'username' => $username, 'hostname' => $_POST['hostname'] ?? '', 'predefined_username' => $GLOBALS['pred_username'] ?? '', 'predefined_hostname' => $GLOBALS['pred_hostname'] ?? '', 'this_host' => $thisHost ?? null]);
    }
    /**
    ...
    public function handleRequestForSlaveChangeMaster()
    {
        ...
        global $dbi;
        $sr = ['username' => $dbi->escapeString($_POST['username']), 'pma_pw' => $dbi->escapeString($_POST['pma_pw']), 'hostname' => $dbi->escapeString($_POST['hostname']), 'port' => (int) $dbi->escapeString($_POST['text_port'])];
        ...
        $link_to_master = $this->replication->connectToMaster($sr['username'], $sr['pma_pw'], $sr['hostname'], $sr['port']);
        if (!$link_to_master) {
            $_SESSION['replication']['sr_action_status'] = 'error';
            ...
            if (empty($position)) {
                ...
                if (!$this->replication->slaveChangeMaster($sr['username'], $sr['pma_pw'], $sr['hostname'], $sr['port'], $position, true, false)) {
                    $_SESSION['replication']['sr_action_status'] = 'error';
    ...
    public function handleRequestForSlaveServerControl()
    {
        ...
        if ($_POST['sr_slave_action'] === 'reset') {
            ...
            $qControl = $this->replication->slaveControl($_POST['sr_slave_action'], $_POST['sr_slave_control_parm'], DatabaseInterface::CONNECT_USER);
    ...
    public function handleRequestForSlaveSkipError()
    {
        ...
        $qStart = $this->replication->slaveControl('START', null, DatabaseInterface::CONNECT_USER);
        return $qStop !== false && $qStop !== -1 && $qSkip !== false && $qSkip !== -1 && $qStart !== false && $qStart !== -1;
    }
}