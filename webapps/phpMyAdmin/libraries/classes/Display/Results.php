...
class Results
{
    ...
    {
        return $this->template->render('display/results/value_display', ['class' => $class, 'condition_field' => $conditionField, 'value' => $value]);
    }
    /**
    ...
    private function getRepeatingHeaders(array $display_params)
    {
        ...
        if ($display_params['emptypre'] > 0) {
            $header_html .= '    <th colspan="' . $display_params['emptypre'] . '">' . "\n" . '        &nbsp;</th>' . "\n";
        } elseif ($GLOBALS['cfg']['RowActionLinks'] === self::POSITION_NONE) {
            $header_html .= '    <th></th>' . "\n";
        }
        ...
        if ($display_params['emptyafter'] > 0) {
            $header_html .= '    <th colspan="' . $display_params['emptyafter'] . '">' . "\n" . '        &nbsp;</th>' . "\n";
        }
        $header_html .= '</tr>' . "\n";
        return $header_html;
    }
    ...
    private function getDataCellForGeometryColumns(?string $column, $class, $meta, array $map, array $_url_params, $condition_field, $transformation_plugin, $default_function, $transform_options, array $analyzed_sql_results)
    {
        ...
        if ($_SESSION['tmpval']['geoOption'] === self::GEOMETRY_DISP_GEOM) {
            $geometry_text = $this->handleNonPrintableContents(strtoupper(self::GEOMETRY_FIELD), $column, $transformation_plugin, $transform_options, $default_function, $meta, $_url_params);
            return $this->buildValueDisplay($class, $condition_field, $geometry_text);
        }
        if ($_SESSION['tmpval']['geoOption'] === self::GEOMETRY_DISP_WKT) {
            ...
            [$is_field_truncated, $displayedColumn] = $this->getPartialText($wktval);
            return $this->getRowData($class, $condition_field, $analyzed_sql_results, $meta, $map, $wktval, $displayedColumn, $transformation_plugin, $default_function, '', $where_comparison, $transform_options, $is_field_truncated, '');
        }
        // Prepare in  Well Known Binary (WKB) format.
        if ($_SESSION['tmpval']['display_binary']) {
            $where_comparison = ' = ' . $column;
            $wkbval = substr(bin2hex($column), 8);
            [$is_field_truncated, $displayedColumn] = $this->getPartialText($wkbval);
            return $this->getRowData($class, $condition_field, $analyzed_sql_results, $meta, $map, $wkbval, $displayedColumn, $transformation_plugin, $default_function, '', $where_comparison, $transform_options, $is_field_truncated, '');
        }
        $wkbval = $this->handleNonPrintableContents(self::BINARY_FIELD, $column, $transformation_plugin, $transform_options, $default_function, $meta, $_url_params);
        return $this->buildValueDisplay($class, $condition_field, $wkbval);
    }
    /**
    ...
    private function handleNonPrintableContents($category, ?string $content, $transformation_plugin, $transform_options, $default_function, $meta, array $url_params = [], &$is_truncated = null)
    {
        ...
        if ($_SESSION['tmpval']['display_binary'] && $meta->type === self::STRING_FIELD || $_SESSION['tmpval']['display_blob'] && stripos($meta->type, self::BLOB_FIELD) !== false) {
            ...
            [$is_truncated, $result] = $this->getPartialText($result);
        }
        ...
        if (count($url_params) > 0 && (!empty($tmpdb) && !empty($meta->orgtable))) {
            ...
            $result = '<a href="' . Url::getFromRoute('/table/get-field', $url_params) . '" class="disableAjax">' . $result . '</a>';
        }
        return $result;
    }
    /**
