...
class SystemDatabase
{
    ...
    public function getExistingTransformationData($db)
    {
        ...
        $pma_transformation_sql = sprintf("SELECT * FROM %s.%s WHERE `db_name` = '%s'", Util::backquote($cfgRelation['db']), Util::backquote($cfgRelation['column_info']), $this->dbi->escapeString($db));
     ...
     */
    public function getNewTransformationDataSql($pma_transformation_data, array $column_map, $view_name, $db)
    {
        $cfgRelation = $this->relation->getRelationsParam();
        ...
        while ($data_row = $this->dbi->fetchAssoc($pma_transformation_data)) {
            foreach ($column_map as $column) {
                ...
                $new_transformations_sql .= sprintf("%s ('%s', '%s', '%s', '%s', '%s', '%s', '%s')", $add_comma ? ', ' : '', $db, $view_name, $column['real_column'] ?? $column['refering_column'], $data_row['comment'], $data_row['mimetype'], $data_row['transformation'], $GLOBALS['dbi']->escapeString($data_row['transformation_options']));
                $add_comma = true;
                $column_count++;
                break;
            }
