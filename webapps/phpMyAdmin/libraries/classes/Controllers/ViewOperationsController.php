    ...
    public function index() : void
    {
        ...
        if (isset($result)) {
            ...
            if (empty($message->getString())) {
                if ($result) {
                    $message->addText(__('Your SQL query has been executed successfully.'));
                } else {
            ...
            $this->response->addHTML(Generator::getMessage($message, $sql_query, $type));
        }
        $this->render('table/operations/view', ['db' => $db, 'table' => $table, 'url_params' => $url_params]);
    }
}