...
class TransformationWrapperController extends AbstractController
{
     ...
     */
    public function __construct($response, Template $template, Transformations $transformations, Relation $relation, $dbi)
    {
    ...
    public function index() : void
    {
         ...
         */
        $request_params = ['cn', 'ct', 'sql_query', 'transform_key', 'where_clause'];
        $size_params = ['newHeight', 'newWidth'];
        ...
        if (isset($where_clause)) {
            ...
            $result = $this->dbi->query('SELECT * FROM ' . Util::backquote($table) . ' WHERE ' . $where_clause . ';', DatabaseInterface::CONNECT_USER, DatabaseInterface::QUERY_STORE);
            $row = $this->dbi->fetchAssoc($result);
        } else {
            $result = $this->dbi->query('SELECT * FROM ' . Util::backquote($table) . ' LIMIT 1;', DatabaseInterface::CONNECT_USER, DatabaseInterface::QUERY_STORE);
            $row = $this->dbi->fetchAssoc($result);
        }
        // No row returned
        if (!$row) {
        ...
        if ($cfgRelation['commwork'] && $cfgRelation['mimework']) {
            ...
            $mime_options = $this->transformations->getOptions($mime_map[$transform_key]['transformation_options'] ?? '');
        ...
        if (isset($ct) && !empty($ct)) {
            ...
            $mime_type = (!empty($mime_map[$transform_key]['mimetype']) ? str_replace('_', '/', $mime_map[$transform_key]['mimetype']) : $default_ct) . ($mime_options['charset'] ?? '');
        }
