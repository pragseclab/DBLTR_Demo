...
class DataDictionaryController extends AbstractController
{
    ...
    public function index() : void
    {
        ...
        foreach ($tablesNames as $tableName) {
            ...
            [, $primaryKeys] = Util::processIndexData($this->dbi->getTableIndexes($this->db, $tableName));
            [$foreigners, $hasRelation] = $this->relation->getRelationsAndStatus(!empty($cfgRelation['relation']), $this->db, $tableName);
            ...
            foreach ($columns as $row) {
                ...
                $rows[$row['Field']] = ['name' => $row['Field'], 'has_primary_key' => isset($primaryKeys[$row['Field']]), 'type' => $extractedColumnSpec['type'], 'print_type' => $extractedColumnSpec['print_type'], 'is_nullable' => $row['Null'] !== '' && $row['Null'] !== 'NO', 'default' => $row['Default'] ?? null, 'comment' => $columnsComments[$row['Field']] ?? '', 'mime' => $mime, 'relation' => $relation];
            }
            $tables[$tableName] = ['name' => $tableName, 'comment' => $showComment, 'has_relation' => $hasRelation, 'has_mime' => $cfgRelation['mimework'], 'columns' => $rows, 'indexes' => Index::getFromTable($tableName, $this->db)];
        }
        $this->render('database/data_dictionary/index', ['database' => $this->db, 'comment' => $comment, 'tables' => $tables]);
    }
}