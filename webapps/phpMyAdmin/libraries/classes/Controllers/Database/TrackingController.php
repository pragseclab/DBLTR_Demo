...
class TrackingController extends AbstractController
{
    ...
    public function index() : void
    {
        ...
        if (isset($_POST['delete_tracking'], $_POST['table'])) {
            ...
            echo Message::success(__('Tracking data deleted successfully.'))->getDisplay();
        } elseif (isset($_POST['submit_create_version'])) {
            $this->tracking->createTrackingForMultipleTables($_POST['selected']);
            echo Message::success(sprintf(__('Version %1$s was created for selected tables,' . ' tracking is active for them.'), htmlspecialchars($_POST['version'])))->getDisplay();
            ...
            if (!empty($_POST['selected_tbl'])) {
                if ($_POST['submit_mult'] === 'delete_tracking') {
                    ...
                    echo $this->template->render('create_tracking_version', ['route' => '/database/tracking', 'url_params' => $url_params, 'last_version' => 0, 'db' => $db, 'selected' => $_POST['selected_tbl'], 'type' => 'both', 'default_statements' => $cfg['Server']['tracking_default_statements']]);
                    return;
                }
            } else {
                echo Message::notice(__('No tables selected.'))->getDisplay();
            }
        }
        ...
        echo $this->tracking->getHtmlForDbTrackingTables($db, $url_params, $PMA_Theme->getImgPath(), $text_dir);
