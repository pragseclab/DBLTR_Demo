...
class ErrorReportController extends AbstractController
{
     ...
     */
    public function __construct($response, Template $template, ErrorReport $errorReport, ErrorHandler $errorHandler)
    {
    ...
    {
        global $cfg;
        if (!isset($_POST['exception_type']) || !in_array($_POST['exception_type'], ['js', 'php'])) {
            return;
        }
        if (isset($_POST['send_error_report']) && ($_POST['send_error_report'] == true || $_POST['send_error_report'] == '1')) {
            if ($_POST['exception_type'] === 'php') {
                 ...
                 */
                if (isset($_SESSION['prev_error_subm_time'], $_SESSION['error_subm_count']) && $_SESSION['error_subm_count'] >= 3 && $_SESSION['prev_error_subm_time'] - time() <= 3000) {
                    $_SESSION['error_subm_count'] = 0;
                    $_SESSION['prev_errors'] = '';
                    $this->response->addJSON('stopErrorReportLoop', '1');
                } else {
                    $_SESSION['prev_error_subm_time'] = time();
                    $_SESSION['error_subm_count'] = isset($_SESSION['error_subm_count']) ? $_SESSION['error_subm_count'] + 1 : 0;
                }
            }
            ...
            if (count($reportData) > 0) {
                ...
                if ($success) {
                    if (isset($_POST['automatic']) && $_POST['automatic'] === 'true' || $cfg['SendErrorReports'] === 'always') {
                    ...
                    $msg = __('An error has been detected and an error report has been ' . 'generated but failed to be sent.');
                    $msg .= ' ';
                ...
                if ($this->response->isAjax()) {
                    ...
                    $jsCode = 'Functions.ajaxShowMessage(\'<div class="alert alert-danger" role="alert">' . $msg . '</div>\', false);';
                ...
                if (isset($_POST['always_send']) && $_POST['always_send'] === 'true') {
