...
class BinlogController extends AbstractController
{
    ...
    public function __construct($response, Template $template, $dbi)
    {
        ...
        $this->dbi = $dbi;
        $this->binaryLogs = $this->dbi->fetchResult('SHOW MASTER LOGS', 'Log_name', null, DatabaseInterface::CONNECT_USER, DatabaseInterface::QUERY_STORE);
    }
    public function index() : void
    {
        global $cfg, $PMA_Theme, $err_url;
        $params = ['log' => $_POST['log'] ?? null, 'pos' => $_POST['pos'] ?? null, 'is_full_query' => $_POST['is_full_query'] ?? null];
        ...
        $this->render('server/binlog/index', ['url_params' => $urlParams, 'binary_logs' => $this->binaryLogs, 'log' => $params['log'], 'sql_message' => Generator::getMessage(Message::success(), $sqlQuery), 'values' => $values, 'has_previous' => $position > 0, 'has_next' => $numRows >= $cfg['MaxRows'], 'previous_params' => $previousParams, 'full_queries_params' => $fullQueriesParams, 'next_params' => $nextParams, 'has_icons' => Util::showIcons('TableNavigationLinksMode'), 'is_full_query' => $isFullQuery, 'image_path' => $PMA_Theme->getImgPath()]);
    }
    /**
     ...
     */
    private function getSqlQuery(string $log, int $position, int $maxRows) : string
    {
