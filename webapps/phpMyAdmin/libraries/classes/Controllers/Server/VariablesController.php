...
class VariablesController extends AbstractController
{
    ...
    public function getValue(array $params) : void
    {
        ...
        $varValue = $this->dbi->fetchSingleRow('SHOW GLOBAL VARIABLES WHERE Variable_name=\'' . $this->dbi->escapeString($params['name']) . '\';', 'NUM');
        ...
        if ($variableType === 'byte') {
            $json['message'] = implode(' ', Util::formatByteDown($varValue[1], 3, 3));
        }
    ...
    {
        $params = ['varName' => $vars['name'], 'varValue' => $_POST['varValue'] ?? null];
        if (!$this->response->isAjax()) {
            return;
        }
        ...
        if ($variableType === 'byte' && preg_match('/^\\s*(\\d+(\\.\\d+)?)\\s*(mb|kb|mib|kib|gb|gib)\\s*$/i', $value, $matches)) {
            $exp = ['kb' => 1, 'kib' => 1, 'mb' => 2, 'mib' => 2, 'gb' => 3, 'gib' => 3];
            $value = (float) $matches[1] * pow(1024, $exp[mb_strtolower($matches[3])]);
        } else {
        ...
        $json = [];
        if (!preg_match('/[^a-zA-Z0-9_]+/', $params['varName']) && $this->dbi->query('SET GLOBAL ' . $params['varName'] . ' = ' . $value)) {
            // Some values are rounded down etc.
            $varValue = $this->dbi->fetchSingleRow('SHOW GLOBAL VARIABLES WHERE Variable_name="' . $this->dbi->escapeString($params['varName']) . '";', 'NUM');
            [$formattedValue, $isHtmlFormatted] = $this->formatVariable($params['varName'], $varValue[1]);
