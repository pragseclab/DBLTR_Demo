...
class ImportStatusController
{
    ...
    public function index() : void
    {
        ...
        [$SESSION_KEY, $upload_id, $plugins] = Ajax::uploadProgressSetup();
        ...
        if (isset($_GET['message']) && $_GET['message']) {
            ...
            while (($_SESSION['Import_message']['message'] ?? null) == null) {
                ...
                if (time() - $timestamp > $maximumTime) {
                    $_SESSION['Import_message']['message'] = Message::error(__('Could not load the progress of the import.'))->getDisplay();
                    break;
                }
            }
            echo $_SESSION['Import_message']['message'] ?? '';
            if (isset($_SESSION['Import_message']['go_back_url'])) {
                echo $this->template->render('import_status', ['go_back_url' => $_SESSION['Import_message']['go_back_url']]);
            }
        } else {
            Ajax::status($_GET['id']);
        }
