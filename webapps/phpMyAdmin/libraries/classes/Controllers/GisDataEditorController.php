...
class GisDataEditorController extends AbstractController
{
    public function index() : void
    {
        ...
        $gis_types = ['POINT', 'MULTIPOINT', 'LINESTRING', 'MULTILINESTRING', 'POLYGON', 'MULTIPOLYGON', 'GEOMETRYCOLLECTION'];
        ...
        if (!isset($gis_data['gis_type'])) {
            ...
            if (isset($_POST['value']) && trim($_POST['value']) != '') {
                ...
                $gis_data['gis_type'] = mb_substr($_POST['value'], $start, mb_strpos($_POST['value'], '(') - $start);
            }
        ...
        if (isset($_POST['value'])) {
            $gis_data = array_merge($gis_data, $gis_obj->generateParams($_POST['value']));
        }
        ...
        $visualizationSettings = ['width' => 450, 'height' => 300, 'spatialColumn' => 'wkt', 'mysqlVersion' => $dbi->getVersion(), 'isMariaDB' => $dbi->isMariaDB()];
        ...
        if (isset($_POST['generate']) && $_POST['generate'] == true) {
            $this->response->addJSON(['result' => $result, 'visualization' => $visualization, 'openLayers' => $open_layers]);
            return;
        }
        $geom_count = 1;
        ...
        $templateOutput = $this->template->render('gis_data_editor_form', ['width' => $visualizationSettings['width'], 'height' => $visualizationSettings['height'], 'theme_image_path' => $PMA_Theme->getImgPath(), 'field' => $_POST['field'], 'input_name' => $_POST['input_name'], 'srid' => $srid, 'visualization' => $visualization, 'open_layers' => $open_layers, 'gis_types' => $gis_types, 'geom_type' => $geom_type, 'geom_count' => $geom_count, 'gis_data' => $gis_data, 'result' => $result]);
