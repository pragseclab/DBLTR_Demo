...
class FindReplaceController extends AbstractController
{
    ...
    public function displaySelectionFormAction() : void
    {
        ...
        $this->render('table/find_replace/index', ['db' => $this->db, 'table' => $this->table, 'goto' => $goto, 'column_names' => $column_names, 'types' => $types, 'sql_types' => $this->dbi->types]);
    }
    public function findAction() : void
    {
        $useRegex = array_key_exists('useRegex', $_POST) && $_POST['useRegex'] === 'on';
        $preview = $this->getReplacePreview($_POST['columnIndex'], $_POST['find'], $_POST['replaceWith'], $useRegex, $this->connectionCharSet);
        $this->response->addJSON('preview', $preview);
    }
    public function replaceAction() : void
    {
        $this->replace($_POST['columnIndex'], $_POST['findString'], $_POST['replaceWith'], $_POST['useRegex'], $this->connectionCharSet);
     ...
     */
    public function getReplacePreview($columnIndex, $find, $replaceWith, $useRegex, $charSet)
    {
        ...
        return $this->template->render('table/find_replace/replace_preview', ['db' => $this->db, 'table' => $this->table, 'column_index' => $columnIndex, 'find' => $find, 'replace_with' => $replaceWith, 'use_regex' => $useRegex, 'result' => $result]);
    }
    /**
     ...
     */
    private function getRegexReplaceRows($columnIndex, $find, $replaceWith, $charSet)
    {
        $column = $this->columnNames[$columnIndex];
        $sql_query = 'SELECT ' . Util::backquote($column) . ',' . ' 1,' . ' COUNT(*)' . ' FROM ' . Util::backquote($this->db) . '.' . Util::backquote($this->table) . ' WHERE ' . Util::backquote($column) . " RLIKE '" . $this->dbi->escapeString($find) . "' COLLATE " . $charSet . '_bin';
        // here we
        // change the collation of the 2nd operand to a case sensitive
        ...
        if ($useRegex) {
            ...
            if (is_array($toReplace)) {
                foreach ($toReplace as $row) {
                    $sql_query .= "\n WHEN " . Util::backquote($column) . " = '" . $this->dbi->escapeString($row[0]) . "' THEN '" . $this->dbi->escapeString($row[1]) . "'";
                }
            }
            $sql_query .= ' END' . ' WHERE ' . Util::backquote($column) . " RLIKE '" . $this->dbi->escapeString($find) . "' COLLATE " . $charSet . '_bin';
            // here we
            // change the collation of the 2nd operand to a case sensitive
            // binary collation to make sure that the comparison
            // is case sensitive
