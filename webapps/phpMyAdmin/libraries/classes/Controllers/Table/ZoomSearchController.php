...
class ZoomSearchController extends AbstractController
{
    ...
    public function index() : void
    {
        ...
        DbTableExists::check();
        $this->addScriptFiles(['makegrid.js', 'sql.js', 'vendor/jqplot/jquery.jqplot.js', 'vendor/jqplot/plugins/jqplot.canvasTextRenderer.js', 'vendor/jqplot/plugins/jqplot.canvasAxisLabelRenderer.js', 'vendor/jqplot/plugins/jqplot.dateAxisRenderer.js', 'vendor/jqplot/plugins/jqplot.highlighter.js', 'vendor/jqplot/plugins/jqplot.cursor.js', 'table/zoom_plot_jqplot.js', 'table/change.js']);
        /**
    ...
    public function displaySelectionFormAction($dataLabel = null)
    {
        ...
        $this->render('table/zoom_search/index', ['db' => $this->db, 'table' => $this->table, 'goto' => $goto, 'self' => $this, 'geom_column_flag' => $this->geomColumnFlag, 'column_names' => $column_names, 'data_label' => $dataLabel, 'keys' => $keys, 'criteria_column_names' => $criteria_column_names, 'criteria_column_types' => $_POST['criteriaColumnTypes'] ?? null, 'max_plot_limit' => !empty($_POST['maxPlotLimit']) ? intval($_POST['maxPlotLimit']) : intval($GLOBALS['cfg']['maxRowPlotLimit'])]);
    }
    /**
     * Get data row action
     *
    ...
    public function getDataRowAction()
    {
        ...
        $extra_data = [];
        $row_info_query = 'SELECT * FROM ' . Util::backquote($_POST['db']) . '.' . Util::backquote($_POST['table']) . ' WHERE ' . $_POST['where_clause'];
        $result = $this->dbi->query($row_info_query . ';', DatabaseInterface::CONNECT_USER, DatabaseInterface::QUERY_STORE);
    ...
    public function changeTableInfoAction()
    {
        ...
        $search_index = isset($_POST['it']) && is_numeric($_POST['it']) ? intval($_POST['it']) : 0;
    ...
    public function zoomSubmitAction($dataLabel, $goto)
    {
        ...
        while ($row = $this->dbi->fetchAssoc($result)) {
            ...
            $uniqueCondition = Util::getUniqueCondition($result, count($this->columnNames), $fields_meta, $tmpRow, true);
            //Append it to row array as where_clause
            $row['where_clause'] = $uniqueCondition[0];
            $row['where_clause_sign'] = Core::signSqlQuery($uniqueCondition[0]);
            $tmpData = [$_POST['criteriaColumnNames'][0] => $row[$_POST['criteriaColumnNames'][0]], $_POST['criteriaColumnNames'][1] => $row[$_POST['criteriaColumnNames'][1]], 'where_clause' => $uniqueCondition[0], 'where_clause_sign' => Core::signSqlQuery($uniqueCondition[0])];
        ...
        $this->render('table/zoom_search/result_form', ['db' => $this->db, 'table' => $this->table, 'column_names' => $this->columnNames, 'column_names_hashes' => $column_names_hashes, 'foreigners' => $this->foreigners, 'column_null_flags' => $this->columnNullFlags, 'column_types' => $this->columnTypes, 'goto' => $goto, 'data' => $data, 'data_json' => json_encode($data), 'zoom_submit' => isset($_POST['zoom_submit']), 'foreign_max_limit' => $GLOBALS['cfg']['ForeignKeyMaxLimit']]);
    }
    /**
    ...
    public function getColumnProperties($search_index, $column_index)
    {
        ...
        $htmlAttributes .= ' onfocus="return ' . 'verifyAfterSearchFieldChange(' . $search_index . ', \'#zoom_search_form\')"';
