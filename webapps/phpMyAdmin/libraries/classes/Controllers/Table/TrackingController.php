...
final class TrackingController extends AbstractController
{
     ...
     */
    public function __construct($response, Template $template, $db, $table, Tracking $tracking)
    {
    ...
    public function index() : void
    {
        ...
        if (Tracker::isActive() && Tracker::isTracked($GLOBALS['db'], $GLOBALS['table']) && !(isset($_POST['toggle_activation']) && $_POST['toggle_activation'] === 'deactivate_now') && !(isset($_POST['report_export']) && $_POST['export_type'] === 'sqldumpfile')) {
            $msg = Message::notice(sprintf(__('Tracking of %s is activated.'), htmlspecialchars($GLOBALS['db'] . '.' . $GLOBALS['table'])));
        ...
        if (isset($_POST['submit_mult'])) {
            if (!empty($_POST['selected_versions'])) {
                if ($_POST['submit_mult'] === 'delete_version') {
                    ...
                    $actionMessage = Message::success(__('Tracking versions deleted successfully.'))->getDisplay();
                }
            } else {
                $actionMessage = Message::notice(__('No versions selected.'))->getDisplay();
            }
        }
        $deleteVersion = '';
        ...
        $trackingReport = '';
        if (isset($_POST['report']) || isset($_POST['report_export'])) {
            $trackingReport = $this->tracking->getHtmlForTrackingReport($data, $url_params, $selection_schema, $selection_data, $selection_both, (int) $filter_ts_to, (int) $filter_ts_from, $filter_users);
        }
        $main = $this->tracking->getHtmlForMainPage($url_params, $PMA_Theme->getImgPath(), $text_dir);
        $this->render('table/tracking/index', ['active_message' => $activeMessage, 'action_message' => $actionMessage, 'delete_version' => $deleteVersion, 'create_version' => $createVersion, 'deactivate_tracking' => $deactivateTracking, 'activate_tracking' => $activateTracking, 'message' => $message, 'sql_dump' => $sqlDump, 'schema_snapshot' => $schemaSnapshot, 'tracking_report_rows' => $trackingReportRows, 'tracking_report' => $trackingReport, 'main' => $main]);
    }
}