...
final class RelationController extends AbstractController
{
    ...
    private function updateForDisplayField(Table $table, array $cfgRelation) : void
    {
        ...
        $this->response->addHTML(Generator::getMessage(__('Display column was successfully updated.'), '', 'success'));
    }
    /**
     * Update for FK
     *
    ...
    private function updateForInternalRelation(Table $table, array $cfgRelation, array $relations) : void
    {
        ...
        if (!$table->updateInternalRelations($multi_edit_columns_name, $_POST['destination_db'], $_POST['destination_table'], $_POST['destination_column'], $cfgRelation, $relations)) {
            return;
        }
        $this->response->addHTML(Generator::getMessage(__('Internal relationships were successfully updated.'), '', 'success'));
    }
    /**
    ...
    public function getDropdownValueForDatabase(string $storageEngine) : void
    {
        ...
        if ($foreign) {
            ...
            $query = 'SHOW TABLES FROM ' . Util::backquote($_POST['foreignDb']);
            $tables_rs = $this->dbi->query($query, DatabaseInterface::CONNECT_USER, DatabaseInterface::QUERY_STORE);
            while ($row = $this->dbi->fetchArray($tables_rs)) {
