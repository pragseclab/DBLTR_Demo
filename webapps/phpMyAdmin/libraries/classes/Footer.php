...
class Footer
{
    ...
    private function getDemoMessage() : string
    {
        ...
        if (@file_exists(ROOT_PATH . 'revision-info.php')) {
            ...
            $message .= sprintf(__('Currently running Git revision %1$s from the %2$s branch.'), '<a target="_blank" rel="noopener noreferrer" href="' . htmlspecialchars($repobase . $fullrevision) . '">' . htmlspecialchars($revision) . '</a>', '<a target="_blank" rel="noopener noreferrer" href="' . htmlspecialchars($repobranchbase . $branch) . '">' . htmlspecialchars($branch) . '</a>');
        } else {
