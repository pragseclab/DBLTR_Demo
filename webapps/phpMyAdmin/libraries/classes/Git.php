...
class Git
{
    ...
    private function getPackOffset(string $packFile, string $hash) : ?int
    {
        ...
        // check format
        if (substr($index_data, 0, 4) != "\xfftOc") {
            return null;
        }
        // check version
        ...
        $packOffsets = unpack('N', substr($index_data, $offset + $position * 4, 4));
    ...
    {
        $commit = false;
        $gitFileName = $gitFolder . '/objects/' . substr($hash, 0, 2) . '/' . substr($hash, 2);
        if (@file_exists($gitFileName)) {
            ...
            $commit = explode("\x00", $commitData, 2);
            $commit = explode("\n", $commit[1]);
    ...
    private function extractDataFormTextBody(array $commit) : array
    {
        ...
        do {
            ...
            $user2 = ['name' => trim($user[1]), 'email' => trim($user[2]), 'date' => date('Y-m-d H:i:s', (int) $user[3])];
            if (isset($user[4])) {
                $user2['date'] .= $user[4];
            }
            ${$linetype} = $user2;
    ...
    public function checkGitRevision() : ?array
    {
        ...
        $commit_json = $this->isRemoteCommit(
            $commit,
            // Will be modified if necessary by the function
            $is_remote_commit,
            // Will be modified if necessary by the function
            $hash
        );
        ...
        if ($commit !== false) {
            ...
            $author = ['name' => $commit_json->author->name, 'email' => $commit_json->author->email, 'date' => $commit_json->author->date];
            $committer = ['name' => $commit_json->committer->name, 'email' => $commit_json->committer->email, 'date' => $commit_json->committer->date];
        ...
        return ['hash' => $hash, 'branch' => $branch, 'message' => $message, 'author' => $author, 'committer' => $committer, 'is_remote_commit' => $is_remote_commit, 'is_remote_branch' => $is_remote_branch];
    }
}