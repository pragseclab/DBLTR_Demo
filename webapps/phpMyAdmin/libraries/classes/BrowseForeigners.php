...
class BrowseForeigners
{
     ...
     */
    private function getHtmlForOneKey(int $horizontal_count, string $header, array $keys, int $indexByKeyname, array $descriptions, int $indexByDescription, string $current_value) : array
    {
        $horizontal_count++;
        $output = '';
        ...
        [$rightDescription, $rightDescriptionTitle] = $this->getDescriptionAndTitle($descriptions[$indexByDescription]);
        $indexByDescription++;
        ...
        $output .= $this->template->render('table/browse_foreigners/column_element', ['keyname' => $leftKeyname, 'description' => $leftDescription, 'title' => $leftDescriptionTitle, 'is_selected' => $leftKeynameIsSelected, 'nowrap' => true]);
        $output .= $this->template->render('table/browse_foreigners/column_element', ['keyname' => $leftKeyname, 'description' => $leftDescription, 'title' => $leftDescriptionTitle, 'is_selected' => $leftKeynameIsSelected, 'nowrap' => false]);
        $output .= '<td width="20%">' . '<img src="' . $this->themeImage . 'spacer.png" alt=""' . ' width="1" height="1"></td>';
        $output .= $this->template->render('table/browse_foreigners/column_element', ['keyname' => $rightKeyname, 'description' => $rightDescription, 'title' => $rightDescriptionTitle, 'is_selected' => $rightKeynameIsSelected, 'nowrap' => false]);
        $output .= $this->template->render('table/browse_foreigners/column_element', ['keyname' => $rightKeyname, 'description' => $rightDescription, 'title' => $rightDescriptionTitle, 'is_selected' => $rightKeynameIsSelected, 'nowrap' => true]);
        $output .= '</tr>';
     ...
     */
    public function getHtmlForRelationalFieldSelection(string $db, string $table, string $field, array $foreignData, ?string $fieldkey, string $current_value) : string
    {
        $gotopage = $this->getHtmlForGotoPage($foreignData);
        $foreignShowAll = $this->template->render('table/browse_foreigners/show_all', ['foreign_data' => $foreignData, 'show_all' => $this->showAll, 'max_rows' => $this->maxRows]);
        $output = '<form class="ajax" ' . 'id="browse_foreign_form" name="browse_foreign_from" action="' . Url::getFromRoute('/browse-foreigners') . '" method="post"><fieldset>' . Url::getHiddenInputs($db, $table) . '<input type="hidden" name="field" value="' . htmlspecialchars($field) . '">' . '<input type="hidden" name="fieldkey" value="' . (isset($fieldkey) ? htmlspecialchars($fieldkey) : '') . '">';
        if (isset($_POST['rownumber'])) {
            $output .= '<input type="hidden" name="rownumber" value="' . htmlspecialchars((string) $_POST['rownumber']) . '">';
        }
        $filter_value = isset($_POST['foreign_filter']) ? htmlspecialchars($_POST['foreign_filter']) : '';
        $output .= '<span class="formelement">' . '<label for="input_foreign_filter">' . __('Search:') . '</label>' . '<input type="text" name="foreign_filter" ' . 'id="input_foreign_filter" ' . 'value="' . $filter_value . '" data-old="' . $filter_value . '" ' . '>' . '<input class="btn btn-primary" type="submit" name="submit_foreign_filter" value="' . __('Go') . '">' . '</span>' . '<span class="formelement">' . $gotopage . '</span>' . '<span class="formelement">' . $foreignShowAll . '</span>' . '</fieldset>' . '</form>';
        ...
        foreach ($keys as $indexByKeyname => $value) {
            [$html, $horizontal_count, $indexByDescription] = $this->getHtmlForOneKey($horizontal_count, $header, $keys, $indexByKeyname, $descriptions, $indexByDescription, $current_value);
            $output .= $html;
        }
        $output .= '</tbody>' . '</table>';
        return $output;
    ...
    private function getDescriptionAndTitle(string $description) : array
    {
        if (mb_strlen($description) <= $this->limitChars) {
            ...
            $descriptionTitle = htmlspecialchars($description);
            $description = htmlspecialchars(mb_substr($description, 0, $this->limitChars) . '...');
        }
    ...
    private function getHtmlForGotoPage(?array $foreignData) : string
    {
        ...
        if ($foreignData['the_total'] > $this->maxRows) {
            $gotopage = Util::pageselector('pos', $this->maxRows, $pageNow, $nbTotalPage, 200, 5, 5, 20, 10, __('Page number:'));
        }
        return $gotopage;
    }
    /**
