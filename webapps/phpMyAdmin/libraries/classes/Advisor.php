...
class Advisor
{
    ...
    public function __construct(DatabaseInterface $dbi, ExpressionLanguage $expression)
    {
        ...
        $this->globals = ['PMA_MYSQL_INT_VERSION' => $this->dbi->getVersion()];
    }
    ...
    {
        $this->runResult['errors'][] = $description . ' ' . sprintf(__('Error when evaluating: %s'), $exception->getMessage());
    }
    /**
        ...
        foreach ($this->rules as $rule) {
            ...
            if (isset($rule['precondition'])) {
                try {
                    ...
                    $this->storeError(sprintf(__('Failed evaluating precondition for rule \'%s\'.'), $rule['name']), $e);
                    continue;
                }
            }
            if (!$precondition) {
            ...
            try {
                ...
                $this->storeError(sprintf(__('Failed calculating value for rule \'%s\'.'), $rule['name']), $e);
                continue;
            }
            ...
            try {
                ...
                $this->storeError(sprintf(__('Failed running test for rule \'%s\'.'), $rule['name']), $e);
            }
        }
    }
    /**
    ...
    public function addRule(string $type, array $rule) : void
    {
        ...
        $rule['recommendation'] = preg_replace_callback('/\\{([a-z_0-9]+)\\}/Ui', function (array $matches) {
            return $this->replaceVariable($matches);
        }, $rule['recommendation']);
        $rule['issue'] = preg_replace_callback('/\\{([a-z_0-9]+)\\}/Ui', function (array $matches) {
            return $this->replaceVariable($matches);
        }, $rule['issue']);
        // Replaces external Links with Core::linkURL() generated links
        $rule['recommendation'] = preg_replace_callback('#href=("|\')(https?://[^"\']+)\\1#i', function (array $matches) {
    ...
    {
        return '<a href="' . Url::getFromRoute('/server/variables', ['filter' => $matches[1]]) . '">' . htmlspecialchars($matches[1]) . '</a>';
    }
    /**
