...
class Generator
{
    ...
    public static function getDbLink($database = '') : string
    {
        ...
        return '<a href="' . $scriptName . Url::getCommon(['db' => $database], strpos($scriptName, '?') === false ? '?' : '&') . '" title="' . htmlspecialchars(sprintf(__('Jump to database “%s”.'), $database)) . '">' . htmlspecialchars($database) . '</a>';
    }
    /**
    ...
    public static function getExternalBug($functionality, $component, $minimum_version, $bugref) : string
    {
        ...
        if ($component === 'mysql' && $dbi->getVersion() < $minimum_version) {
            $ext_but_html .= self::showHint(sprintf(__('The %s functionality is affected by a known bug, see %s'), $functionality, Core::linkURL('https://bugs.mysql.com/') . $bugref));
        }
        return $ext_but_html;
    }
    /**
    ...
    public static function getStartAndNumberOfRowsPanel($sql_query) : string
    {
        ...
        return $template->render('start_and_number_of_rows_panel', ['pos' => $pos, 'unlim_num_rows' => (int) $_REQUEST['unlim_num_rows'], 'rows' => $rows, 'sql_query' => $sql_query]);
    }
    /**
