...
class GisMultiPolygon extends GisGeometry
{
    ...
    public function scaleRow($spatial)
    {
        ...
        $multipolygon = mb_substr($spatial, 15, mb_strlen($spatial) - 18);
     ...
     * @access public
     */
    public function prepareRowAsPng($spatial, ?string $label, $fill_color, array $scale_data, $image)
    {
        // allocate colors
        ...
        if (isset($label_point)) {
            imagestring($image, 1, $points_arr[2], $points_arr[3], trim((string) $label), $black);
        }
        return $image;
    }
    /**
    ...
    {
        $polygon_options = ['name' => $label, 'class' => 'multipolygon vector', 'stroke' => 'black', 'stroke-width' => 0.5, 'fill' => $fill_color, 'fill-rule' => 'evenodd', 'fill-opacity' => 0.8];
        $row = '';
        // Trim to remove leading 'MULTIPOLYGON(((' and trailing ')))'
        $multipolygon = mb_substr($spatial, 15, mb_strlen($spatial) - 18);
        // Separate each polygon
    ...
    public function prepareRowAsOl($spatial, $srid, $label, $fill_color, array $scale_data)
    {
        ...
        $stroke_style = ['color' => [0, 0, 0], 'width' => 0.5];
        $row = 'var style = new ol.style.Style({' . 'fill: new ol.style.Fill(' . json_encode($fill_style) . '),' . 'stroke: new ol.style.Stroke(' . json_encode($stroke_style) . ')';
        if ($label) {
        ...
        // Separate each polygon
        $polygons = explode(')),((', $multipolygon);
        return $row . $this->getPolygonArrayForOpenLayers($polygons, $srid) . 'var multiPolygon = new ol.geom.MultiPolygon(polygonArray);' . 'var feature = new ol.Feature(multiPolygon);' . 'feature.setStyle(style);' . 'vectorLayer.addFeature(feature);';
    }
    /**
        ...
        for ($k = 0; $k < $no_of_polygons; $k++) {
            ...
            for ($i = 0; $i < $no_of_lines; $i++) {
                ...
                for ($j = 0; $j < $no_of_points; $j++) {
                    $wkt .= (isset($data_row[$k][$i][$j]['x']) && trim((string) $data_row[$k][$i][$j]['x']) != '' ? $data_row[$k][$i][$j]['x'] : $empty) . ' ' . (isset($data_row[$k][$i][$j]['y']) && trim((string) $data_row[$k][$i][$j]['y']) != '' ? $data_row[$k][$i][$j]['y'] : $empty) . ',';
                }
    ...
    public function generateParams($value, $index = -1)
    {
        ...
        // Trim to remove leading 'MULTIPOLYGON(((' and trailing ')))'
        $multipolygon = mb_substr($wkt, 15, mb_strlen($wkt) - 18);
        // Separate each polygon
        $polygons = explode(')),((', $multipolygon);
