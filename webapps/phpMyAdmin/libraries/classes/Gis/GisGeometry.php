...
abstract class GisGeometry
{
     ...
     * @access public
     */
    public abstract function prepareRowAsPng($spatial, ?string $label, $color, array $scale_data, $image);
    /**
     ...
     * @access public
     */
    public abstract function prepareRowAsPdf($spatial, ?string $label, $color, array $scale_data, $pdf);
    /**
     ...
     * @access public
     */
    public abstract function prepareRowAsOl($spatial, $srid, $label, $color, array $scale_data);
    /**
     * Scales each row.
     *
    ...
    {
        return sprintf('var minLoc = [%s, %s];' . 'var maxLoc = [%s, %s];' . 'var ext = ol.extent.boundingExtent([minLoc, maxLoc]);' . 'ext = ol.proj.transformExtent(ext, ol.proj.get("EPSG:%s"), ol.proj.get(\'EPSG:3857\'));' . 'map.getView().fit(ext, map.getSize());', $scale_data['minX'], $scale_data['minY'], $scale_data['maxX'], $scale_data['maxY'], intval($srid));
    }
    /**
        ...
        foreach ($points as $point) {
            ...
            if (isset($coordinates[0], $coordinates[1]) && trim($coordinates[0]) != '' && trim($coordinates[1]) != '') {
                if ($scale_data != null) {
                    ...
                    $y = $scale_data['height'] - ($coordinates[1] - $scale_data['y']) * $scale_data['scale'];
                } else {
    ...
    {
        return $this->getLineArrayForOpenLayers($polygon, $srid, false) . 'var polygon = new ol.geom.Polygon(arr);';
    }
    /**
     ...
     * @access protected
     */
    protected function getLineArrayForOpenLayers(array $lines, $srid, $is_line_string = true)
    {
        $ol_array = 'var arr = [];';
     ...
     * @access protected
     */
    protected function getLineForOpenLayers(array $points_arr, $srid, $is_line_string = true)
    {
        return 'new ol.geom.' . ($is_line_string ? 'LineString' : 'LinearRing') . '(' . $this->getPointsArrayForOpenLayers($points_arr, $srid) . ')';
    }
    /**
    ...
    protected function getPointsArrayForOpenLayers(array $points_arr, $srid)
    {
        ...
        $ol_array = mb_substr($ol_array, 0, mb_strlen($ol_array) - 2);
        return $ol_array . ')';
    }
    /**
    ...
    {
        return '(new ol.geom.Point([' . $point[0] . ',' . $point[1] . '])' . '.transform(ol.proj.get("EPSG:' . (int) $srid . '")' . ', ol.proj.get(\'EPSG:3857\')))';
    }
