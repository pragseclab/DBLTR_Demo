...
class GisMultiLineString extends GisGeometry
{
    ...
    {
        $line_options = ['name' => $label, 'class' => 'linestring vector', 'fill' => 'none', 'stroke' => $line_color, 'stroke-width' => 2];
        // Trim to remove leading 'MULTILINESTRING((' and trailing '))'
        $multilinestirng = mb_substr($spatial, 17, mb_strlen($spatial) - 19);
        // Separate each linestring
        $linestirngs = explode('),(', $multilinestirng);
    ...
    public function prepareRowAsOl($spatial, $srid, $label, $line_color, array $scale_data)
    {
        ...
        $row = 'var style = new ol.style.Style({' . 'stroke: new ol.style.Stroke(' . json_encode($stroke_style) . ')';
        if ($label) {
        ...
        // Separate each linestring
        $linestirngs = explode('),(', $multilinestirng);
        return $row . $this->getLineArrayForOpenLayers($linestirngs, $srid) . 'var multiLineString = new ol.geom.MultiLineString(arr);' . 'var feature = new ol.Feature({geometry: multiLineString});' . 'feature.setStyle(style);' . 'vectorLayer.addFeature(feature);';
    }
    /**
    ...
    public function generateWkt(array $gis_data, $index, $empty = '')
    {
        ...
        for ($i = 0; $i < $no_of_lines; $i++) {
            ...
            for ($j = 0; $j < $no_of_points; $j++) {
                $wkt .= (isset($data_row[$i][$j]['x']) && trim((string) $data_row[$i][$j]['x']) != '' ? $data_row[$i][$j]['x'] : $empty) . ' ' . (isset($data_row[$i][$j]['y']) && trim((string) $data_row[$i][$j]['y']) != '' ? $data_row[$i][$j]['y'] : $empty) . ',';
            }
    ...
    public function generateParams($value, $index = -1)
    {
        ...
        // Trim to remove leading 'MULTILINESTRING((' and trailing '))'
        $multilinestirng = mb_substr($wkt, 17, mb_strlen($wkt) - 19);
        // Separate each linestring
        $linestirngs = explode('),(', $multilinestirng);
