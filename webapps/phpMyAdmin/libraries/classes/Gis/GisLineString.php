...
class GisLineString extends GisGeometry
{
    ...
    public function scaleRow($spatial)
    {
        ...
        $linestring = mb_substr($spatial, 11, mb_strlen($spatial) - 12);
     ...
     * @access public
     */
    public function prepareRowAsPng($spatial, ?string $label, $line_color, array $scale_data, $image)
    {
        // allocate colors
        ...
        // Trim to remove leading 'LINESTRING(' and trailing ')'
        $linesrting = mb_substr($spatial, 11, mb_strlen($spatial) - 12);
    ...
    {
        $line_options = ['name' => $label, 'id' => $label . $this->getRandomId(), 'class' => 'linestring vector', 'fill' => 'none', 'stroke' => $line_color, 'stroke-width' => 2];
        // Trim to remove leading 'LINESTRING(' and trailing ')'
        $linesrting = mb_substr($spatial, 11, mb_strlen($spatial) - 12);
        $points_arr = $this->extractPoints($linesrting, $scale_data);
        $row = '<polyline points="';
    ...
    {
        $stroke_style = ['color' => $line_color, 'width' => 2];
        $result = 'var style = new ol.style.Style({' . 'stroke: new ol.style.Stroke(' . json_encode($stroke_style) . ')';
        if ($label) {
        ...
        return $result . 'var line = new ol.Feature({geometry: ' . $this->getLineForOpenLayers($points_arr, $srid) . '});' . 'line.setStyle(style);' . 'vectorLayer.addFeature(line);';
    }
    /**
    ...
    public function generateWkt(array $gis_data, $index, $empty = '')
    {
        ...
        for ($i = 0; $i < $no_of_points; $i++) {
            $wkt .= (isset($gis_data[$index]['LINESTRING'][$i]['x']) && trim((string) $gis_data[$index]['LINESTRING'][$i]['x']) != '' ? $gis_data[$index]['LINESTRING'][$i]['x'] : $empty) . ' ' . (isset($gis_data[$index]['LINESTRING'][$i]['y']) && trim((string) $gis_data[$index]['LINESTRING'][$i]['y']) != '' ? $gis_data[$index]['LINESTRING'][$i]['y'] : $empty) . ',';
        }
    ...
    public function generateParams($value, $index = -1)
    {
        ...
        // Trim to remove leading 'LINESTRING(' and trailing ')'
        $linestring = mb_substr($wkt, 11, mb_strlen($wkt) - 12);
