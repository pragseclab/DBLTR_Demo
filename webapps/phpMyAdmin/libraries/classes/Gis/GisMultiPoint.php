...
class GisMultiPoint extends GisGeometry
{
    ...
    public function scaleRow($spatial)
    {
        ...
        $multipoint = mb_substr($spatial, 11, mb_strlen($spatial) - 12);
     ...
     * @access public
     */
    public function prepareRowAsPng($spatial, ?string $label, $point_color, array $scale_data, $image)
    {
        // allocate colors
     ...
     * @access public
     */
    public function prepareRowAsPdf($spatial, ?string $label, $point_color, array $scale_data, $pdf)
    {
        // allocate colors
        $red = hexdec(mb_substr($point_color, 1, 2));
        $green = hexdec(mb_substr($point_color, 3, 2));
    ...
    {
        $point_options = ['name' => $label, 'class' => 'multipoint vector', 'fill' => 'white', 'stroke' => $point_color, 'stroke-width' => 2];
        // Trim to remove leading 'MULTIPOINT(' and trailing ')'
        $multipoint = mb_substr($spatial, 11, mb_strlen($spatial) - 12);
        $points_arr = $this->extractPoints($multipoint, $scale_data);
        $row = '';
     ...
     * @access public
     */
    public function prepareRowAsOl($spatial, $srid, $label, $point_color, array $scale_data)
    {
        $fill_style = ['color' => 'white'];
        $stroke_style = ['color' => $point_color, 'width' => 2];
        $result = 'var fill = new ol.style.Fill(' . json_encode($fill_style) . ');' . 'var stroke = new ol.style.Stroke(' . json_encode($stroke_style) . ');' . 'var style = new ol.style.Style({' . 'image: new ol.style.Circle({' . 'fill: fill,' . 'stroke: stroke,' . 'radius: 3' . '}),' . 'fill: fill,' . 'stroke: stroke';
        if ($label) {
            $text_style = ['text' => $label, 'offsetY' => -9];
        ...
        return $result . 'var multiPoint = new ol.geom.MultiPoint(' . $this->getPointsArrayForOpenLayers($points_arr, $srid) . ');' . 'var feature = new ol.Feature({geometry: multiPoint});' . 'feature.setStyle(style);' . 'vectorLayer.addFeature(feature);';
    }
    /**
    ...
    public function generateWkt(array $gis_data, $index, $empty = '')
    {
        ...
        for ($i = 0; $i < $no_of_points; $i++) {
            $wkt .= (isset($gis_data[$index]['MULTIPOINT'][$i]['x']) && trim((string) $gis_data[$index]['MULTIPOINT'][$i]['x']) != '' ? $gis_data[$index]['MULTIPOINT'][$i]['x'] : '') . ' ' . (isset($gis_data[$index]['MULTIPOINT'][$i]['y']) && trim((string) $gis_data[$index]['MULTIPOINT'][$i]['y']) != '' ? $gis_data[$index]['MULTIPOINT'][$i]['y'] : '') . ',';
        }
    ...
    public function generateParams($value, $index = -1)
    {
        ...
        // Trim to remove leading 'MULTIPOINT(' and trailing ')'
        $points = mb_substr($wkt, 11, mb_strlen($wkt) - 12);
