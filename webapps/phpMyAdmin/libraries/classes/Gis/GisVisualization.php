...
class GisVisualization
{
    ...
    private $settings = array(
        ...
        'colors' => array('#B02EE0', '#E0642E', '#E0D62E', '#2E97E0', '#BCE02E', '#E02E75', '#5CE02E', '#E0B02E', '#0022E0', '#726CB1', '#481A36', '#BAC658', '#127224', '#825119', '#238C74', '#4C489B', '#87C9BF'),
        // Hex values for abovementioned colours
        'colors_hex' => array(array(176, 46, 224), array(224, 100, 46), array(224, 214, 46), array(46, 151, 224), array(188, 224, 46), array(224, 46, 117), array(92, 224, 46), array(224, 176, 46), array(0, 34, 224), array(114, 108, 177), array(72, 26, 54), array(186, 198, 88), array(18, 114, 36), array(130, 81, 25), array(35, 140, 116), array(76, 72, 155), array(135, 201, 191)),
    ...
    private function modifySqlQuery($sql_query, $rows, $pos)
    {
        ...
        $modified_query .= $spatialAsText . '(' . Util::backquote($this->userSpecifiedSettings['spatialColumn']) . $axisOrder . ') AS ' . Util::backquote($this->userSpecifiedSettings['spatialColumn']) . ', ';
        // Get the SRID
        $modified_query .= $spatialSrid . '(' . Util::backquote($this->userSpecifiedSettings['spatialColumn']) . ') AS ' . Util::backquote('srid') . ' ';
    ...
    {
        $this->init();
        $output = '<?xml version="1.0" encoding="UTF-8" standalone="no"?>' . "\n" . '<svg version="1.1" xmlns:svg="http://www.w3.org/2000/svg"' . ' xmlns="http://www.w3.org/2000/svg"' . ' width="' . intval($this->settings['width']) . '"' . ' height="' . intval($this->settings['height']) . '">' . '<g id="groupPanel">';
    ...
    private function png()
    {
        ...
        imagefilledrectangle($image, 0, 0, $this->settings['width'] - 1, $this->settings['height'] - 1, $bg);
        $scale_data = $this->scaleDataSet($this->data);
    ...
    public function asOl()
    {
        $this->init();
        $scale_data = $this->scaleDataSet($this->data);
        $output = 'if (typeof ol !== "undefined") {' . 'var olCss = "js/vendor/openlayers/theme/ol.css";' . '$(\'head\').append(\'<link rel="stylesheet" type="text/css" href=\'+olCss+\'>\');' . 'var vectorLayer = new ol.source.Vector({});' . 'var map = new ol.Map({' . 'target: \'openlayersmap\',' . 'layers: [' . 'new ol.layer.Tile({' . 'source: new ol.source.OSM()' . '}),' . 'new ol.layer.Vector({' . 'source: vectorLayer' . '})' . '],' . 'view: new ol.View({' . 'center: ol.proj.fromLonLat([37.41, 8.82]),' . 'zoom: 4' . '}),' . 'controls: [new ol.control.MousePosition({' . 'coordinateFormat: ol.coordinate.createStringXY(4),' . 'projection: \'EPSG:4326\'}),' . 'new ol.control.Zoom,' . 'new ol.control.Attribution]' . '});';
    ...
    {
        $min_max = ['maxX' => 0.0, 'maxY' => 0.0, 'minX' => 0.0, 'minY' => 0.0];
        $border = 15;
        ...
        return ['scale' => $scale, 'x' => $x, 'y' => $y, 'minX' => $min_max['minX'], 'maxX' => $min_max['maxX'], 'minY' => $min_max['minY'], 'maxY' => $min_max['maxY'], 'height' => $this->settings['height']];
    }
    /**
    ...
    private function prepareDataSet(array $data, array $scale_data, $format, $results)
    {
        ...
        foreach ($data as $row) {
            ...
            if ($format === 'svg') {
                $results .= $gis_obj->prepareRowAsSvg($row[$this->settings['spatialColumn']], $label, $this->settings['colors'][$index], $scale_data);
            } elseif ($format === 'png') {
                $results = $gis_obj->prepareRowAsPng($row[$this->settings['spatialColumn']], $label, $this->settings['colors'][$index], $scale_data, $results);
            } elseif ($format === 'pdf' && $results instanceof TCPDF) {
                $results = $gis_obj->prepareRowAsPdf($row[$this->settings['spatialColumn']], $label, $this->settings['colors'][$index], $scale_data, $results);
            } elseif ($format === 'ol') {
                $results .= $gis_obj->prepareRowAsOl($row[$this->settings['spatialColumn']], $row['srid'], $label, $this->settings['colors_hex'][$index], $scale_data);
            }
            $color_number++;
        }
        return $results;
