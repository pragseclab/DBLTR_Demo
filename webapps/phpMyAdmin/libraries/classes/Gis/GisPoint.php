...
class GisPoint extends GisGeometry
{
    ...
    {
        $point_options = ['name' => $label, 'id' => $label . $this->getRandomId(), 'class' => 'point vector', 'fill' => 'white', 'stroke' => $point_color, 'stroke-width' => 2];
        // Trim to remove leading 'POINT(' and trailing ')'
        $point = mb_substr($spatial, 6, mb_strlen($spatial) - 7);
        $points_arr = $this->extractPoints($point, $scale_data);
        $row = '';
    ...
    public function prepareRowAsOl($spatial, $srid, $label, $point_color, array $scale_data)
    {
        ...
        if ($points_arr[0][0] != '' && $points_arr[0][1] != '') {
            $result .= 'var point = new ol.Feature({geometry: ' . $this->getPointForOpenLayers($points_arr[0], $srid) . '});' . 'point.setStyle(style);' . 'vectorLayer.addFeature(point);';
        }
        return $result;
    }
    /**
    ...
    {
        return 'POINT(' . (isset($gis_data[$index]['POINT']['x']) && trim((string) $gis_data[$index]['POINT']['x']) != '' ? $gis_data[$index]['POINT']['x'] : '') . ' ' . (isset($gis_data[$index]['POINT']['y']) && trim((string) $gis_data[$index]['POINT']['y']) != '' ? $gis_data[$index]['POINT']['y'] : '') . ')';
    }
    /**
    ...
    {
        return 'POINT(' . ($row_data['x'] ?? '') . ' ' . ($row_data['y'] ?? '') . ')';
    }
    /**
    ...
    public function generateParams($value, $index = -1)
    {
        ...
        // Trim to remove leading 'POINT(' and trailing ')'
        $point = mb_substr($wkt, 6, mb_strlen($wkt) - 7);
        $points_arr = $this->extractPoints($point, null);
