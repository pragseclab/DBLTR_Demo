...
class GisPolygon extends GisGeometry
{
    ...
    {
        $polygon_options = ['name' => $label, 'id' => $label . $this->getRandomId(), 'class' => 'polygon vector', 'stroke' => 'black', 'stroke-width' => 0.5, 'fill' => $fill_color, 'fill-rule' => 'evenodd', 'fill-opacity' => 0.8];
        // Trim to remove leading 'POLYGON((' and trailing '))'
        $polygon = mb_substr($spatial, 9, mb_strlen($spatial) - 11);
        $row = '<path d="';
        // If the polygon doesn't have an inner polygon
    ...
    public function prepareRowAsOl($spatial, $srid, $label, $fill_color, array $scale_data)
    {
        ...
        // Separate outer and inner polygons
        $parts = explode('),(', $polygon);
        return $row . $this->getPolygonForOpenLayers($parts, $srid) . 'var feature = new ol.Feature({geometry: polygon});' . 'feature.setStyle(style);' . 'vectorLayer.addFeature(feature);';
    }
    /**
    ...
    public function generateWkt(array $gis_data, $index, $empty = '')
    {
        ...
        for ($i = 0; $i < $no_of_lines; $i++) {
            ...
            for ($j = 0; $j < $no_of_points; $j++) {
                $wkt .= (isset($gis_data[$index]['POLYGON'][$i][$j]['x']) && trim((string) $gis_data[$index]['POLYGON'][$i][$j]['x']) != '' ? $gis_data[$index]['POLYGON'][$i][$j]['x'] : $empty) . ' ' . (isset($gis_data[$index]['POLYGON'][$i][$j]['y']) && trim((string) $gis_data[$index]['POLYGON'][$i][$j]['y']) != '' ? $gis_data[$index]['POLYGON'][$i][$j]['y'] : $empty) . ',';
            }
    ...
    public function generateParams($value, $index = -1)
    {
        ...
        // Trim to remove leading 'POLYGON((' and trailing '))'
        $polygon = mb_substr($wkt, 9, mb_strlen($wkt) - 11);
