...
class AnnotationHelper
{
     ...
     * @return string
     */
    public static function fixAnnotationConstantFetchNode(File $phpcsFile, Annotation $annotation, ConstFetchNode $node, ConstFetchNode $fixedNode) : string
    {
        if ($annotation instanceof MethodAnnotation) {
            ...
            $fixedAnnotation = new MethodAnnotation($annotation->getName(), $annotation->getStartPointer(), $annotation->getEndPointer(), $annotation->getContent(), $fixedContentNode);
        } else {
    ...
    public static function isAnnotationUseless(File $phpcsFile, int $functionPointer, $typeHint, Annotation $annotation, array $traversableTypeHints) : bool
    {
        ...
        $annotationTypeNode = $annotation->getType();
        if ($annotationTypeNode instanceof IdentifierTypeNode && in_array(strtolower($annotationTypeNode->name), ['true', 'false', 'class-string', 'trait-string', 'callable-string', 'numeric-string'], true)) {
            return false;
        }
    ...
    private static function fixAnnotation(Annotation $annotation, TypeNode $typeNode, TypeNode $fixedTypeNode) : Annotation
    {
        ...
        } elseif ($annotation instanceof ExtendsAnnotation || $annotation instanceof ImplementsAnnotation || $annotation instanceof UseAnnotation) {
            $fixedContentNode = clone $annotation->getContentNode();
