                ...
                if ($tokens[$nextTokenAfterEndPointer]['code'] === T_OPEN_PARENTHESIS) {
                    ...
                    if (!in_array($tokens[$previousTokenBeforeStartPointer]['code'], [
                        ...
                        if ($tokens[$previousTokenBeforeStartPointer]['code'] === T_COMMA) {
                            $precedingTokenPointer = TokenHelper::findPreviousExcluding($phpcsFile, array_merge([T_COMMA], $nameTokenCodes, TokenHelper::$ineffectiveTokenCodes), $previousTokenBeforeStartPointer - 1);
                            if (!in_array($tokens[$precedingTokenPointer]['code'], [T_IMPLEMENTS, T_EXTENDS, T_USE], true)) {
                                $type = ReferencedName::TYPE_CONSTANT;
                            }
                            ...
                            $catchPointer = TokenHelper::findPreviousExcluding($phpcsFile, array_merge($exclude, $nameTokenCodes, TokenHelper::$ineffectiveTokenCodes), $previousTokenBeforeStartPointer - 1);
                            $exclude = [T_BITWISE_OR];
                            $openParenthesisPointer = TokenHelper::findPreviousExcluding($phpcsFile, array_merge($exclude, $nameTokenCodes, TokenHelper::$ineffectiveTokenCodes), $previousTokenBeforeStartPointer);
