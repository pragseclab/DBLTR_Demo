...
class DocCommentSpacingSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, $docCommentOpenerPointer) : void
    {
        ...
        $firstContentStartPointer = TokenHelper::findNextExcluding($phpcsFile, [T_DOC_COMMENT_WHITESPACE, T_DOC_COMMENT_STAR], $docCommentOpenerPointer + 1, $tokens[$docCommentOpenerPointer]['comment_closer']);
        ...
        do {
            ...
            $actualPointer = TokenHelper::findNextExcluding($phpcsFile, [T_DOC_COMMENT_STAR, T_DOC_COMMENT_WHITESPACE], $actualPointer + 1, $tokens[$docCommentOpenerPointer]['comment_closer'] + 1);
    ...
    private function checkLinesBetweenDescriptionAndFirstAnnotation(File $phpcsFile, int $docCommentOpenerPointer, int $firstContentStartPointer, int $firstContentEndPointer, ?Annotation $firstAnnotation) : void
    {
    ...
    private function checkAnnotationsGroupsOrder(File $phpcsFile, int $docCommentOpenerPointer, array $annotationsGroups, array $annotations) : void
    {
        ...
        foreach ($sortedAnnotationsGroups as $sortedAnnotationsGroup) {
            ...
            foreach ($sortedAnnotationsGroup as $sortedAnnotation) {
                $fixedAnnotations .= sprintf('%s * %s%s', $indentation, TokenHelper::getContent($phpcsFile, $sortedAnnotation->getStartPointer(), $sortedAnnotation->getEndPointer()), $phpcsFile->eolChar);
            }
        }
