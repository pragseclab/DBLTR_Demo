...
abstract class AbstractControlStructureSpacing implements Sniff
{
    ...
    protected function checkLinesBefore(File $phpcsFile, int $controlStructurePointer) : void
    {
        ...
        if ($isFirstControlStructure && in_array($tokens[$controlStructurePointer]['code'], [T_CASE, T_DEFAULT], true) && array_key_exists('scope_condition', $tokens[$pointerBefore]) && in_array($tokens[$tokens[$pointerBefore]['scope_condition']]['code'], [T_CASE, T_DEFAULT], true)) {
    ...
    protected function checkLinesAfter(File $phpcsFile, int $controlStructurePointer) : void
    {
        ...
        $isControlStructureEndAfterPointer = static function (int $pointer) use($tokens, $controlStructurePointer) : bool {
            return in_array($tokens[$controlStructurePointer]['code'], [T_CASE, T_DEFAULT], true) ? $tokens[$pointer]['code'] === T_CLOSE_CURLY_BRACKET : in_array($tokens[$pointer]['code'], [T_CLOSE_CURLY_BRACKET, T_CASE, T_DEFAULT], true);
        };
        if ($hasCommentAfter) {
    ...
    private function findControlStructureEnd(File $phpcsFile, int $controlStructurePointer) : int
    {
        ...
        if (in_array($tokens[$controlStructurePointer]['code'], [T_CASE, T_DEFAULT], true)) {
            ...
            $pointers = TokenHelper::findNextAll($phpcsFile, [T_CASE, T_DEFAULT], $controlStructurePointer + 1, $tokens[$switchPointer]['scope_closer']);
        ...
        $nextPointer = $tokens[$nextPointer]['code'] === T_OPEN_SHORT_ARRAY ? (int) TokenHelper::findNext($phpcsFile, T_SEMICOLON, $tokens[$nextPointer]['bracket_closer'] + 1) : (int) TokenHelper::findNext($phpcsFile, T_SEMICOLON, $tokens[$nextPointer]['scope_closer'] + 1);
