...
class RequireSingleLineCallSniff extends AbstractLineCall
{
    ...
    public function process(File $phpcsFile, $stringPointer) : void
    {
        ...
        if ($this->ignoreWithComplexParameter) {
            if (TokenHelper::findNext($phpcsFile, [T_CLOSURE, T_FN, T_OPEN_SHORT_ARRAY], $parenthesisOpenerPointer + 1, $parenthesisCloserPointer) !== null) {
                return;
            }
            // Contains inner call
            foreach (TokenHelper::findNextAll($phpcsFile, TokenHelper::getOnlyNameTokenCodes(), $parenthesisOpenerPointer + 1, $parenthesisCloserPointer) as $innerStringPointer) {
