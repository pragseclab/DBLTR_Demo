            ...
            foreach ($annotationByName as $annotation) {
                ...
                foreach (AnnotationHelper::getAnnotationTypes($annotation) as $annotationType) {
                    foreach (AnnotationTypeHelper::getUnionTypeNodes($annotationType) as $unionTypeNode) {
                        ...
                        $fix = $phpcsFile->addFixableError(sprintf('Null type hint should be on last position in "%s".', AnnotationTypeHelper::export($unionTypeNode)), $annotation->getStartPointer(), self::CODE_NULL_TYPE_HINT_NOT_ON_LAST_POSITION);
                        if (!$fix) {
                            continue;
                        }
