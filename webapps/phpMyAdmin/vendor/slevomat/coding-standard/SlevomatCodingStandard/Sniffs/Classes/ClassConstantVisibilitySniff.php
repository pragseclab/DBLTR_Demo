...
class ClassConstantVisibilitySniff implements Sniff
{
    ...
    public function process(File $phpcsFile, $constantPointer) : void
    {
        ...
        $message = sprintf('Constant %s::%s visibility missing.', ClassHelper::getFullyQualifiedName($phpcsFile, $classPointer), $tokens[TokenHelper::findNextEffective($phpcsFile, $constantPointer + 1)]['content']);
        if ($this->fixable) {
