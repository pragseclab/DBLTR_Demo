...
class UnusedPrivateElementsSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, $classPointer) : void
    {
        ...
        $checkPropertyUsage = function (int $propertyNameTokenPointer, int $thisOrSelfTokenPointer) use($phpcsFile, $tokens, &$reportedProperties, &$writeOnlyProperties) : void {
            ...
            if (in_array($possibleAssignToken['code'], [T_EQUAL, T_PLUS_EQUAL, T_MINUS_EQUAL, T_MUL_EQUAL, T_DIV_EQUAL, T_POW_EQUAL, T_MOD_EQUAL, T_AND_EQUAL, T_OR_EQUAL, T_XOR_EQUAL, T_SL_EQUAL, T_SR_EQUAL, T_CONCAT_EQUAL], true)) {
    ...
    private function findVisibilityPointer(File $phpcsFile, int $pointer) : ?int
    {
        ...
        if (in_array($tokens[$visibilityPointer]['code'], array_merge([T_SELF], TokenHelper::getNameTokenCodes()), true)) {
            $visibilityPointer = TokenHelper::findPreviousExcluding($phpcsFile, array_merge(TokenHelper::getNameTokenCodes(), TokenHelper::$ineffectiveTokenCodes), $visibilityPointer - 1);
        ...
        if (in_array($tokens[$visibilityPointer]['code'], [T_ABSTRACT, T_STATIC], true)) {
            $visibilityPointer = TokenHelper::findPreviousExcluding($phpcsFile, array_merge([T_ABSTRACT, T_STATIC], TokenHelper::$ineffectiveTokenCodes), $visibilityPointer - 1);
        }
