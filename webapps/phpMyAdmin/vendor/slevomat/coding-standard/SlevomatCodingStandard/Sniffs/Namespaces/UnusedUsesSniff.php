...
class UnusedUsesSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, $openTagPointer) : void
    {
        ...
        foreach ($referencedNames as $referencedName) {
            ...
            $uniqueId = $nameReferencedWithoutSubNamespace ? UseStatement::getUniqueId($referencedName->getType(), $nameAsReferencedInFile) : UseStatement::getUniqueId(ReferencedName::TYPE_DEFAULT, $nameAsReferencedInFile);
            ...
            while (true) {
                ...
                foreach ($fileUnusedNames[$pointerBeforeUseStatements] as $useStatement) {
                    ...
                    foreach ($annotations as $annotationName => $annotationsByName) {
                        ...
                        if (!in_array($annotationName, $this->getIgnoredAnnotationNames(), true) && preg_match('~^@(' . preg_quote($nameAsReferencedInFile, '~') . ')(?=[^-a-z\\d]|$)~i', $annotationName, $matches) !== 0) {
                            $allUsedNames[$pointerBeforeUseStatements][$uniqueId] = true;
                            ...
                            if (!$annotation instanceof GenericAnnotation) {
                                foreach (AnnotationHelper::getAnnotationTypes($annotation) as $annotationType) {
                                    foreach (AnnotationTypeHelper::getIdentifierTypeNodes($annotationType) as $typeNode) {
                                        ...
                                        if (TypeHintHelper::isSimpleTypeHint($typeNode->name) || TypeHintHelper::isSimpleUnofficialTypeHints($typeNode->name) || !TypeHelper::isTypeName($typeNode->name)) {
                                            continue;
                                        }
    ...
    private function getIgnoredAnnotationNames() : array
    {
        if ($this->normalizedIgnoredAnnotationNames === null) {
            $this->normalizedIgnoredAnnotationNames = array_merge(SniffSettingsHelper::normalizeArray($this->ignoredAnnotationNames), ['@param', '@throws', '@property', '@method']);
        }
