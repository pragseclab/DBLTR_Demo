...
class ReferenceUsedNamesOnlySniff implements Sniff
{
    ...
    public function process(File $phpcsFile, $openTagPointer) : void
    {
        ...
        foreach ($references as $reference) {
            ...
            if ($isFullyQualified) {
                if ($reference->isClass && $this->allowFullyQualifiedNameForCollidingClasses) {
                    ...
                    if (array_key_exists($lowerCasedUnqualifiedClassName, $useStatements) && $canonicalName !== NamespaceHelper::normalizeToCanonicalName($useStatements[$lowerCasedUnqualifiedClassName]->getFullyQualifiedTypeName())) {
                        continue;
                    }
            ...
            if ($isFullyQualified || $isGlobalFunctionFallback || $isGlobalConstantFallback) {
                ...
                $previousKeywordPointer = TokenHelper::findPreviousExcluding($phpcsFile, array_merge(TokenHelper::getNameTokenCodes(), [T_WHITESPACE, T_COMMA]), $startPointer - 1);
                    ...
                    if ($isFullyQualified && !NamespaceHelper::hasNamespace($name) && $namespacePointers === []) {
                        ...
                        if ($fix) {
                            ...
                            if ($reference->source === self::SOURCE_ANNOTATION) {
                                $fixedAnnotationContent = AnnotationHelper::fixAnnotationType($phpcsFile, $reference->annotation, $reference->nameNode, new IdentifierTypeNode(substr($reference->name, 1)));
                                ...
                                $fixedAnnotationContent = AnnotationHelper::fixAnnotationConstantFetchNode($phpcsFile, $reference->annotation, $reference->constantFetchNode, new ConstFetchNode(substr($reference->name, 1), $reference->constantFetchNode->name));
                                $phpcsFile->fixer->replaceToken($startPointer, $fixedAnnotationContent);
                                for ($i = $startPointer + 1; $i <= $reference->endPointer; $i++) {
                                    $phpcsFile->fixer->replaceToken($i, '');
                                }
        ...
        $alreadyAddedUses = [UseStatement::TYPE_DEFAULT => [], UseStatement::TYPE_FUNCTION => [], UseStatement::TYPE_CONSTANT => []];
        $phpcsFile->fixer->beginChangeset();
        foreach ($referenceErrors as $referenceData) {
            ...
            $canBeFixed = array_reduce($alreadyAddedUses[$reference->type], static function (bool $carry, string $use) use($canonicalName) : bool {
                return NamespaceHelper::getLastNamePart($use) === NamespaceHelper::getLastNamePart($canonicalName) ? false : $carry;
            }, true);
            foreach ($useStatements as $useStatement) {
                ...
                if (!($useStatement->getCanonicalNameAsReferencedInFile() === $canonicalNameToReference || $reference->isClass && array_key_exists($canonicalNameToReference, $definedClassesIndex) && $canonicalName !== NamespaceHelper::normalizeToCanonicalName($definedClassesIndex[$canonicalNameToReference]) || $reference->isFunction && array_key_exists($canonicalNameToReference, $definedFunctionsIndex) || $reference->isConstant && array_key_exists($canonicalNameToReference, $definedConstantsIndex))) {
                    continue;
                }
                $canBeFixed = false;
                break;
            ...
            if ($reference->source === self::SOURCE_ANNOTATION) {
                $fixedAnnotationContent = AnnotationHelper::fixAnnotationType($phpcsFile, $reference->annotation, $reference->nameNode, new IdentifierTypeNode($nameToReference));
                $phpcsFile->fixer->replaceToken($startPointer, $fixedAnnotationContent);
            } elseif ($reference->source === self::SOURCE_ANNOTATION_CONSTANT_FETCH) {
                $fixedAnnotationContent = AnnotationHelper::fixAnnotationConstantFetchNode($phpcsFile, $reference->annotation, $reference->constantFetchNode, new ConstFetchNode($nameToReference, $reference->constantFetchNode->name));
                $phpcsFile->fixer->replaceToken($startPointer, $fixedAnnotationContent);
            } else {
