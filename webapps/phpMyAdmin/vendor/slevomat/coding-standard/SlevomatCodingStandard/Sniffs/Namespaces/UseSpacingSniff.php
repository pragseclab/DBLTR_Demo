...
class UseSpacingSniff implements Sniff
{
    ...
    private function checkLinesAfterLastUse(File $phpcsFile, UseStatement $lastUse) : void
    {
        ...
        if (in_array($tokens[$pointerAfterWhitespaceEnd]['code'], Tokens::$commentTokens, true) && ($tokens[$useEndPointer]['line'] === $tokens[$pointerAfterWhitespaceEnd]['line'] || $tokens[$useEndPointer]['line'] + 1 === $tokens[$pointerAfterWhitespaceEnd]['line'])) {
            $useEndPointer = array_key_exists('comment_closer', $tokens[$pointerAfterWhitespaceEnd]) ? $tokens[$pointerAfterWhitespaceEnd]['comment_closer'] : CommentHelper::getMultilineCommentEndPointer($phpcsFile, $pointerAfterWhitespaceEnd);
    ...
    private function checkLinesBetweenSameTypesOfUse(File $phpcsFile, array $useStatements) : void
    {
        ...
        foreach ($useStatements as $use) {
            ...
            if (in_array($tokens[$pointerBeforeUse]['code'], Tokens::$commentTokens, true) && TokenHelper::findFirstNonWhitespaceOnLine($phpcsFile, $pointerBeforeUse) === $pointerBeforeUse && $tokens[$pointerBeforeUse]['line'] + 1 === $tokens[$useStartPointer]['line']) {
                $useStartPointer = array_key_exists('comment_opener', $tokens[$pointerBeforeUse]) ? $tokens[$pointerBeforeUse]['comment_opener'] : CommentHelper::getMultilineCommentStartPointer($phpcsFile, $pointerBeforeUse);
            }
