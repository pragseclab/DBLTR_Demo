...
class UnusedVariableSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, $variablePointer) : void
    {
        ...
        if (in_array($variableName, ['$this', '$GLOBALS', '$_SERVER', '$_GET', '$_POST', '$_FILES', '$_COOKIE', '$_SESSION', '$_REQUEST', '$_ENV'], true)) {
            return;
        }
        ...
        if (in_array($tokens[$previousPointer]['code'], [T_EQUAL, T_PLUS_EQUAL, T_MINUS_EQUAL, T_MUL_EQUAL, T_DIV_EQUAL, T_POW_EQUAL, T_MOD_EQUAL, T_AND_EQUAL, T_OR_EQUAL, T_XOR_EQUAL, T_SL_EQUAL, T_SR_EQUAL, T_CONCAT_EQUAL], true)) {
            return;
        }
    ...
    private function isAssignment(File $phpcsFile, int $variablePointer) : bool
    {
        ...
        if (in_array($tokens[$nextPointer]['code'], [T_EQUAL, T_PLUS_EQUAL, T_MINUS_EQUAL, T_MUL_EQUAL, T_DIV_EQUAL, T_POW_EQUAL, T_MOD_EQUAL, T_AND_EQUAL, T_OR_EQUAL, T_XOR_EQUAL, T_SL_EQUAL, T_SR_EQUAL, T_CONCAT_EQUAL], true)) {
        ...
        $possibleShortListCloserPointer = TokenHelper::findNextExcluding($phpcsFile, array_merge(TokenHelper::$ineffectiveTokenCodes, [T_VARIABLE, T_COMMA]), $variablePointer + 1);
    ...
    private function isDefinedInDoConditionAndUsedInLoop(File $phpcsFile, int $variablePointer, string $variableName) : bool
    {
        ...
        return TokenHelper::findNextContent($phpcsFile, T_VARIABLE, $variableName, $tokens[$loopCloserPointer]['bracket_opener'] + 1, $loopCloserPointer) !== null;
    }
    private function isUsedInLoopCycle(File $phpcsFile, int $variablePointer, string $variableName) : bool
    {
        ...
        for ($i = $tokens[$loopPointer]['scope_opener'] + 1; $i < $tokens[$loopPointer]['scope_closer']; $i++) {
            if (in_array($tokens[$i]['code'], [T_DOUBLE_QUOTED_STRING, T_HEREDOC], true) && VariableHelper::isUsedInScopeInString($phpcsFile, $variableName, $i)) {
                return true;
            }
            if ($tokens[$i]['code'] !== T_VARIABLE) {
                continue;
            ...
            if (!in_array($tokens[$nextPointer]['code'], [T_INC, T_DEC, T_PLUS_EQUAL, T_MINUS_EQUAL, T_MUL_EQUAL, T_DIV_EQUAL, T_POW_EQUAL, T_MOD_EQUAL, T_AND_EQUAL, T_OR_EQUAL, T_XOR_EQUAL, T_SL_EQUAL, T_SR_EQUAL, T_CONCAT_EQUAL], true)) {
                continue;
            }
