...
class U2FServer
{
    ...
    public static function checkOpenSSLVersion()
    {
        if (OPENSSL_VERSION_NUMBER < 0x10000000) {
            throw new U2FException('OpenSSL has to be at least version 1.0.0, this is ' . OPENSSL_VERSION_TEXT, U2FException::OLD_OPENSSL);
        }
        return true;
    }
    /**
    ...
    public static function makeRegistration($appId, array $registrations = array())
    {
        ...
        return ["request" => $request, "signatures" => $signatures];
    }
    /**
    ...
    public static function register(RegistrationRequest $request, $response, $attestDir = null, $includeCert = true)
    {
        ...
        if (property_exists($response, 'errorCode') && $response->errorCode !== 0) {
            throw new U2FException('User-agent returned error. Error code: ' . $response->errorCode, U2FException::BAD_UA_RETURNING);
        }
        ...
        if ($clientToken->challenge !== $request->challenge()) {
            throw new U2FException('Registration challenge does not match', U2FException::UNMATCHED_CHALLENGE);
        }
        ...
        if (static::publicKeyToPem($pubKey) === null) {
            throw new U2FException('Decoding of public key failed', U2FException::PUBKEY_DECODE);
        }
        ...
        if ($attestDir) {
            if (openssl_x509_checkpurpose($pemCert, -1, static::get_certs($attestDir)) !== true) {
                throw new U2FException('Attestation certificate can not be validated', U2FException::ATTESTATION_VERIFICATION);
            }
        }
        ...
        if (openssl_verify($dataToVerify, $signature, $pemCert, 'sha256') === 1) {
            ...
            throw new U2FException('Attestation signature does not match', U2FException::ATTESTATION_SIGNATURE);
        }
    }
    /**
    ...
    public static function makeAuthentication(array $registrations, $appId)
    {
        ...
        foreach ($registrations as $reg) {
            ...
            $signatures[] = new SignRequest(['appId' => $appId, 'keyHandle' => $reg->keyHandle, 'challenge' => $challenge]);
        }
        return $signatures;
    }
    /**
    ...
    public static function authenticate(array $requests, array $registrations, $response)
    {
        ...
        if ($req === null) {
            throw new U2FException('No matching request found', U2FException::NO_MATCHING_REQUEST);
        }
        ...
        if ($reg === null) {
            throw new U2FException('No matching registration found', U2FException::NO_MATCHING_REGISTRATION);
        }
        ...
        if (openssl_verify($dataToVerify, $signature, $pemKey, 'sha256') === 1) {
            ...
            if ($counter > $reg->counter) {
                ...
                throw new U2FException('Counter too low.', U2FException::COUNTER_TOO_LOW);
            }
        } else {
            throw new U2FException('Authentication failed', U2FException::AUTHENTICATION_FAILURE);
        }
    }
    /**
     * @param string $attestDir
    ...
    private static function publicKeyToPem($key)
    {
         ...
         */
        $der = "0Y0\x13\x06\x07*\x86H\xce=\x02\x01";
        $der .= "\x06\x08*\x86H\xce=\x03\x01\x07\x03B";
        $der .= "\x00" . $key;
    ...
    private static function createChallenge()
    {
        ...
        if ($crypto_strong !== true) {
            throw new U2FException('Unable to obtain a good source of randomness', U2FException::BAD_RANDOM);
        }
