...
class Plugin implements PluginInterface, EventSubscriberInterface
{
    ...
    private function updateInstalledPaths()
    {
        ...
        $finder = new Finder();
        $finder->files()->depth('<= ' . $this->getMaxDepth())->depth('>= ' . $this->getMinDepth())->ignoreUnreadableDirs()->ignoreVCS(true)->in($searchPaths)->name('ruleset.xml');
        ...
        foreach ($finder as $ruleset) {
            ...
            if ($this->isRunningGlobally() === false) {
                $standardsPath = $this->filesystem->findShortestPath($this->getPHPCodeSnifferInstallPath(), $standardsPath, true);
            }
    ...
    private function getMaxDepth()
    {
        ...
        if (array_key_exists(self::KEY_MAX_DEPTH, $extra)) {
            ...
            if ((string) (int) $maxDepth !== (string) $maxDepth || $maxDepth <= $minDepth || is_float($maxDepth) === true) {
                $message = vsprintf(self::MESSAGE_ERROR_WRONG_MAX_DEPTH, array('key' => self::KEY_MAX_DEPTH, 'min' => $minDepth, 'given' => var_export($maxDepth, true)));
