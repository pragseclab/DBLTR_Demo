    ...
    public static function parse(string $docblock, ?int $line_number = null, bool $preserve_format = false) : array
    {
        ...
        foreach ($lines as $k => $line) {
            if (preg_match('/^\\s?@\\w/i', $line)) {
                ...
                $lines[$last] = rtrim($old_last_line) . ($preserve_format || trim($old_last_line) === '@return' ? "\n" . $line : ' ' . trim($line));
                if ($line_number) {
        ...
        return ['description' => $docblock, 'specials' => $special];
    }
    /**
     * Parse a docblock comment into its parts.
     */
