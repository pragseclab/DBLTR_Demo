...
abstract class Type
{
     ...
     */
    public static function parseString(string $type_string, ?array $php_version = null, array $template_type_map = []) : Union
    {
        return TypeParser::parseTokens(TypeTokenizer::tokenize($type_string), $php_version, $template_type_map);
    }
    public static function getFQCLNFromString(string $class, Aliases $aliases) : string
    {
        if ($class === '') {
     ...
     * @psalm-pure
     */
    public static function getStringFromFQCLN(string $value, ?string $namespace, array $aliased_classes, ?string $this_class, bool $allow_self = false, bool $was_static = false) : string
    {
    ...
    {
        $type = new TArray([new Type\Union([new TArrayKey()]), new Type\Union([new TMixed()])]);
        return new Union([$type]);
    }
    public static function getEmptyArray() : Union
    {
        $array_type = new TArray([new Type\Union([new TEmpty()]), new Type\Union([new TEmpty()])]);
     ...
     *
     */
    public static function combineUnionTypes(Union $type_1, Union $type_2, ?Codebase $codebase = null, bool $overwrite_empty_array = false, bool $allow_mixed_union = true, int $literal_limit = 500) : Union
    {
        if ($type_1 === $type_2) {
            return $type_1;
        }
     ...
     *
     */
    public static function intersectUnionTypes(Union $type_1, Union $type_2, Codebase $codebase) : ?Union
    {
                    ...
                    foreach ($type_2->getAtomicTypes() as $t2_key => $type_2_atomic) {
                        if ($type_1_atomic instanceof TNamedObject && $type_2_atomic instanceof TNamedObject) {
                            if ($type_1_atomic->value === $type_2_atomic->value && get_class($type_1_atomic) === TNamedObject::class && get_class($type_2_atomic) !== TNamedObject::class) {
                                ...
                                $intersection_performed = true;
                            } elseif (AtomicTypeComparator::isContainedBy($codebase, $type_2_atomic, $type_1_atomic)) {
                                $combined_type->removeType($t1_key);
                                $combined_type->addType(clone $type_2_atomic);
                                $intersection_performed = true;
                        ...
                        if (($type_1_atomic instanceof TIterable || $type_1_atomic instanceof TNamedObject || $type_1_atomic instanceof TTemplateParam || $type_1_atomic instanceof TObjectWithProperties) && ($type_2_atomic instanceof TIterable || $type_2_atomic instanceof TNamedObject || $type_2_atomic instanceof TTemplateParam || $type_2_atomic instanceof TObjectWithProperties)) {
                            ...
                            if ($type_2_atomic_intersection_types) {
                                foreach ($type_2_atomic_intersection_types as $type_2_intersection_type) {
                                    $type_1_atomic->extra_types[$type_2_intersection_type->getKey()] = clone $type_2_intersection_type;
                                }
                            }
                        }
