...
class IssueBuffer
{
     ...
     */
    protected static $issues_data = array();
    /**
     * @var array<int, array>
     */
    protected static $console_issues = array();
    /**
     * @var array<string, int>
     */
    protected static $fixable_issue_counts = array();
    /**
     * @var int
     */
    ...
    protected static $recorded_issues = array();
    /**
     * @var array<string, array<int, int>>
     */
    protected static $unused_suppressions = array();
    /**
     * @var array<string, array<int, bool>>
     */
    protected static $used_suppressions = array();
    /**
     * @param   string[]  $suppressed_issues
     *
     */
    public static function accepts(CodeIssue $e, array $suppressed_issues = array(), bool $is_fixable = false) : bool
    {
     ...
     *
     */
    public static function isSuppressed(CodeIssue $e, array $suppressed_issues = array()) : bool
    {
    ...
    public static function add(CodeIssue $e, bool $is_fixable = false) : bool
    {
        ...
        $emitted_key = $issue_type . '-' . $e->getShortLocation() . ':' . $e->code_location->getColumn() . ' ' . $e->dupe_key;
        ...
        if ($config->throw_exception) {
            ...
            throw new Exception\CodeException($issue_type . ' - ' . $e->getShortLocationWithPrevious() . ':' . $e->code_location->getColumn() . ' - ' . $message);
        }
        if ($is_tainted || !self::alreadyEmitted($emitted_key)) {
            ++self::$error_count;
    ...
    public static function processUnusedSuppressions(\Psalm\Internal\Provider\FileProvider $file_provider) : void
    {
        ...
        foreach (self::$unused_suppressions as $file_path => $offsets) {
            ...
            foreach ($offsets as $start => $end) {
                ...
                self::add(new UnusedPsalmSuppress('This suppression is never used', new CodeLocation\Raw($file_contents, $file_path, $config->shortenFileName($file_path), $start, $end)));
            }
        }
    }
    ...
    public static function addIssues(array $issues_data) : void
    {
        foreach ($issues_data as $file_path => $file_issues) {
            foreach ($file_issues as $issue) {
                $emitted_key = $issue->type . '-' . $issue->file_name . ':' . $issue->line_from . ':' . $issue->column_from . ' ' . $issue->dupe_key;
     ...
     *
     */
    public static function finish(ProjectAnalyzer $project_analyzer, bool $is_full, float $start_time, bool $add_stats = false, array $issue_baseline = array()) : void
    {
        ...
        if (self::$issues_data) {
            ...
            foreach (self::$issues_data as $file_path => $file_issues) {
                usort($file_issues, function (IssueData $d1, IssueData $d2) : int {
                    ...
                    foreach ($file_issues as $key => $issue_data) {
                        ...
                        if (isset($issue_baseline[$file][$type]) && $issue_baseline[$file][$type]['o'] > 0) {
                            if ($issue_baseline[$file][$type]['o'] === count($issue_baseline[$file][$type]['s'])) {
                                $position = array_search($issue_data->selected_text, $issue_baseline[$file][$type]['s'], true);
                                if ($position !== false) {
        ...
        if ($codebase->config->eventDispatcher->after_analysis || $codebase->config->eventDispatcher->legacy_after_analysis) {
            try {
     ...
     *
     */
    public static function getOutput(array $issues_data, \Psalm\Report\ReportOptions $report_options, array $mixed_counts = array(0, 0)) : string
    {
