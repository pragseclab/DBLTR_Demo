...
class Shepherd implements \Psalm\Plugin\EventHandler\AfterAnalysisInterface
{
    ...
    public static function afterAnalysis(AfterAnalysisEvent $event) : void
    {
        ...
        if ($build_info) {
            $normalized_data = $issues === [] ? [] : array_filter(array_merge(...array_values($issues)), static function (IssueData $i) : bool {
                return $i->severity === 'error';
            });
            $data = ['build' => $build_info, 'git' => $source_control_data, 'issues' => $normalized_data, 'coverage' => $codebase->analyzer->getTotalTypeCoverage($codebase)];
            ...
            if ($return !== '') {
                ...
                if ($return === false) {
                    ...
                    echo $return . PHP_EOL;
                    echo 'Git args: ' . var_export($source_control_data, true) . PHP_EOL;
