...
class FileFilter
{
     ...
     * @var array<string>
     */
    protected $fq_classlike_names = array();
    /**
     * @var array<string>
     */
    protected $fq_classlike_patterns = array();
    /**
     * @var array<string>
     */
    protected $method_ids = array();
    /**
     * @var array<string>
     */
    protected $property_ids = array();
    /**
     * @var array<string>
     */
     ...
     * @var array<string>
     */
    protected $files_lowercase = array();
    /**
     * @var bool
     */
    protected $inclusive;
     ...
     */
    protected $ignore_type_stats = array();
    /**
     * @var array<string, bool>
     */
    protected $declare_strict_types = array();
    ...
    public static function loadFromXMLElement(SimpleXMLElement $e, string $base_dir, bool $inclusive)
    {
        ...
        if ($e->directory) {
            ...
            foreach ($e->directory as $directory) {
                ...
                $ignore_type_stats = strtolower(isset($directory['ignoreTypeStats']) ? (string) $directory['ignoreTypeStats'] : '') === 'true';
                $declare_strict_types = strtolower(isset($directory['useStrictTypes']) ? (string) $directory['useStrictTypes'] : '') === 'true';
                ...
                if (strpos($prospective_directory_path, '*') !== false) {
                    ...
                    if (empty($globs)) {
                        ...
                        throw new ConfigException('Could not resolve config path to ' . $base_dir . DIRECTORY_SEPARATOR . (string) $directory['name']);
                    }
                    foreach ($globs as $glob_index => $directory_path) {
                        if (!$directory_path) {
                            ...
                            throw new ConfigException('Could not resolve config path to ' . $base_dir . DIRECTORY_SEPARATOR . (string) $directory['name'] . ':' . $glob_index);
                        }
                ...
                if (!is_dir($directory_path)) {
                    throw new ConfigException($base_dir . DIRECTORY_SEPARATOR . (string) $directory['name'] . ' is not a directory');
                }
        ...
        if ($e->file) {
            ...
            foreach ($e->file as $file) {
                ...
                if (strpos($prospective_file_path, '*') !== false) {
                    ...
                    if (empty($globs)) {
                        throw new ConfigException('Could not resolve config path to ' . $base_dir . DIRECTORY_SEPARATOR . (string) $file['name']);
                    }
                    foreach ($globs as $glob_index => $file_path) {
                        if (!$file_path) {
                            throw new ConfigException('Could not resolve config path to ' . $base_dir . DIRECTORY_SEPARATOR . (string) $file['name'] . ':' . $glob_index);
                        }
