...
';
    public static function getContents(string $current_dir, ?string $suggested_dir, int $level, string $vendor_dir) : string
    {
        ...
        $template = str_replace('errorLevel="1"', 'errorLevel="' . $level . '"', $template);
        return $template;
    }
    public static function createBareConfig(string $current_dir, ?string $suggested_dir, string $vendor_dir) : \Psalm\Config
    {
    ...
    public static function getLevel(array $issues, int $counted_types) : int
    {
        ...
        foreach ($issues_at_level as $level => $issues) {
            ...
            $filtered_issues = array_filter($issues, function ($amount) : bool {
                return $amount > 0.1;
            });
    ...
    public static function getPaths(string $current_dir, ?string $suggested_dir) : array
    {
        ...
        if ($suggested_dir) {
            ...
            if (!file_exists($composer_json_location)) {
                throw new ConfigCreationException('Problem during config autodiscovery - could not find composer.json during initialization.');
            }
    ...
    private static function getPsr4Or0Paths(string $current_dir, array $composer_json) : array
    {
        ...
        foreach ($psr_paths as $paths) {
            ...
            foreach ($paths as $path) {
                if ($path === '') {
                    $nodes = array_merge($nodes, self::guessPhpFileDirs($current_dir));
                    continue;
                }
