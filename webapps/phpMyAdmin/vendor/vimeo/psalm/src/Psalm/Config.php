...
class Config
{
     ...
     */
    protected $universal_object_crates = [\stdClass::class, SimpleXMLElement::class];
    /**
     * @var static|null
     */
    ...
    private static function fromXmlAndPaths(string $base_dir, string $file_contents, string $current_dir) : self
    {
        ...
        if (isset($config_xml['errorLevel'])) {
            ...
            if (!in_array($attribute_text, [1, 2, 3, 4, 5, 6, 7, 8], true)) {
                throw new Exception\ConfigException('Invalid error level ' . $config_xml['errorLevel']);
            }
        ...
        if (isset($config_xml->stubs) && isset($config_xml->stubs->file)) {
            ...
            foreach ($config_xml->stubs->file as $stub_file) {
                ...
                if (!$file_path) {
                    throw new Exception\ConfigException('Cannot resolve stubfile path ' . $config->base_dir . DIRECTORY_SEPARATOR . $stub_file['name']);
                }
    ...
    public function initializePlugins(ProjectAnalyzer $project_analyzer) : void
    {
        ...
        foreach ($this->plugin_classes as $plugin_class_entry) {
            ...
            try {
                ...
                if ($this->composer_class_loader && ($plugin_class_path = $this->composer_class_loader->findFile($plugin_class_name))) {
    ...
    {
        if (($this->show_mixed_issues === false || $this->level > 2) && in_array($issue_type, self::MIXED_ISSUES, true)) {
            return false;
        }
    ...
    public function visitStubFiles(Codebase $codebase, ?Progress $progress = null) : void
    {
        ...
        $core_generic_files = [$generic_stubs_path, $generic_classes_path, $generic_iterator_path, $immutable_classes_path];
        if (\PHP_VERSION_ID >= 80000 && $codebase->php_major_version >= 8) {
            $stringable_path = dirname(__DIR__, 2) . '/stubs/Php80.phpstub';
            if (!file_exists($stringable_path)) {
                throw new \UnexpectedValueException('Cannot locate PHP 8.0 classes');
    ...
    public function visitComposerAutoloadFiles(ProjectAnalyzer $project_analyzer, ?Progress $progress = null) : void
    {
        ...
        if (file_exists($vendor_autoload_files_path)) {
            $this->include_collector->runAndCollect(function () use($vendor_autoload_files_path) {
                /**
                 * @psalm-suppress UnresolvableInclude
                 * @var string[]
                 */
        ...
        if ($this->autoloader) {
            ...
            $this->include_collector->runAndCollect(function () {
