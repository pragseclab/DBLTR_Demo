...
class CodeLocation
{
    ...
    public function __construct(FileSource $file_source, PhpParser\Node $stmt, ?CodeLocation $previous_location = null, bool $single_line = false, ?int $regex_type = null, ?string $selected_text = null)
    {
    ...
    private function calculateRealLocation() : void
    {
        ...
        if ($this->docblock_line_number && $this->docblock_start_line_number && $this->preview_start < $this->selection_start) {
            $preview_lines = explode("\n", substr($file_contents, $this->preview_start, $this->selection_start - $this->preview_start - 1));
            $preview_offset = 0;
        ...
        if ($this->regex_type !== null) {
            ...
            $preview_snippet = substr($file_contents, $this->selection_start, $this->selection_end - $this->selection_start);
            if ($this->text) {
