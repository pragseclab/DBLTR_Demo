...
class SarifReport extends Report
{
    ...
    {
        $report = ['version' => '2.1.0', 'runs' => [['tool' => ['driver' => ['name' => 'Psalm', 'informationUri' => 'https://psalm.dev', 'version' => PSALM_VERSION]], 'results' => []]]];
        $rules = [];
        foreach ($this->issues_data as $issue_data) {
            $rules[$issue_data->shortcode] = ['id' => (string) $issue_data->shortcode, 'name' => $issue_data->type, 'shortDescription' => ['text' => $issue_data->type], 'properties' => ['tags' => [\substr($issue_data->type, 0, 7) === 'Tainted' ? 'security' : 'maintainability']], 'helpUri' => $issue_data->link];
            ...
            if ($issue_data->taint_trace != null) {
                ...
                foreach ($issue_data->taint_trace as $trace) {
                    if ($trace instanceof \Psalm\Internal\Analyzer\DataFlowNodeData && $trace->line_from > 0) {
