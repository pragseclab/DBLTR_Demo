...
class Codebase
{
    ...
    public function __construct(Config $config, Providers $providers, ?Progress $progress = null)
    {
        if ($progress === null) {
        ...
        $this->scanner = new Internal\Codebase\Scanner($this, $config, $providers->file_storage_provider, $providers->file_provider, $reflection, $providers->file_reference_provider, $progress);
        $this->loadAnalyzer();
        $this->functions = new Internal\Codebase\Functions($providers->file_storage_provider, $reflection);
        $this->classlikes = new Internal\Codebase\ClassLikes($this->config, $providers->classlike_storage_provider, $providers->file_reference_provider, $providers->statements_provider, $this->scanner);
        $this->properties = new Internal\Codebase\Properties($providers->classlike_storage_provider, $providers->file_reference_provider, $this->classlikes);
        $this->methods = new Internal\Codebase\Methods($providers->classlike_storage_provider, $providers->file_reference_provider, $this->classlikes);
        $this->populator = new Internal\Codebase\Populator($config, $providers->classlike_storage_provider, $providers->file_storage_provider, $this->classlikes, $providers->file_reference_provider, $progress);
        $this->loadAnalyzer();
    }
    private function loadAnalyzer() : void
    {
        $this->analyzer = new Internal\Codebase\Analyzer($this->config, $this->file_provider, $this->file_storage_provider, $this->progress);
    }
    /**
    ...
    {
        return $this->statements_provider->getStatementsForFile($file_path, $this->php_major_version . '.' . $this->php_minor_version, $this->progress);
    }
    ...
    {
        $doesMethodExist = \Psalm\Internal\MethodIdentifier::isValidMethodIdReference($function_id) && $this->methodExists($function_id);
        if ($doesMethodExist) {
    ...
    *
    @return bool
    */
    public function methodExists($method_id, ?CodeLocation $code_location = null, $calling_method_id = null, ?string $file_path = null) : bool
    {
        return $this->methods->methodExists(Internal\MethodIdentifier::wrap($method_id), is_string($calling_method_id) ? strtolower($calling_method_id) : strtolower((string) $calling_method_id), $code_location, null, $file_path);
    }
    /**
     * @param  string|\Psalm\Internal\MethodIdentifier $method_id
     *
    ...
    {
        return $this->methods->getMethodReturnType(Internal\MethodIdentifier::wrap($method_id), $self_class, null, $call_args);
    }
    /**
     * @param  string|\Psalm\Internal\MethodIdentifier $method_id
     *
     ...
     *
     */
    public function getMethodReturnTypeLocation($method_id, CodeLocation &$defined_location = null) : ?CodeLocation
    {
        return $this->methods->getMethodReturnTypeLocation(Internal\MethodIdentifier::wrap($method_id), $defined_location);
    }
    /**
     * @param  string|\Psalm\Internal\MethodIdentifier $method_id
     *
    ...
    public function getSymbolLocation(string $file_path, string $symbol) : ?CodeLocation
    {
        if (\is_numeric($symbol[0])) {
            ...
            return new CodeLocation\Raw($file_contents, $file_path, $this->config->shortenFileName($file_path), (int) $symbol_parts[0], (int) $symbol_parts[1]);
        }
        try {
            if (strpos($symbol, '::')) {
                if (strpos($symbol, '()')) {
    ...
    public function getSignatureInformation(string $function_symbol) : ?\LanguageServerProtocol\SignatureInformation
    {
        ...
        foreach ($params as $i => $param) {
            ...
            $parameters[] = new \LanguageServerProtocol\ParameterInformation([strlen($signature_label), strlen($signature_label) + strlen($parameter_label)]);
        ...
        $signature_label .= ')';
        return new \LanguageServerProtocol\SignatureInformation($signature_label, $parameters);
    }
    /**
    ...
    public function getCompletionItemsForClassishThing(string $type_string, string $gap) : array
    {
            ...
            if ($atomic_type instanceof Type\Atomic\TNamedObject) {
                try {
                    ...
                    foreach ($class_storage->declaring_property_ids as $property_name => $declaring_class) {
                        ...
                        $completion_item = new \LanguageServerProtocol\CompletionItem('$' . $property_name, \LanguageServerProtocol\CompletionItemKind::PROPERTY, $property_storage->getInfo(), null, (string) $property_storage->visibility, $property_name, ($gap === '::' ? '$' : '') . $property_name);
                    ...
                    foreach ($class_storage->constants as $const_name => $_) {
                        $static_completion_items[] = new \LanguageServerProtocol\CompletionItem($const_name, \LanguageServerProtocol\CompletionItemKind::VARIABLE, 'const ' . $const_name, null, null, $const_name, $const_name);
                    }
                } catch (\Exception $e) {
                    error_log($e->getMessage());
                    continue;
        ...
        if ($gap === '->') {
            ...
            $completion_items = array_merge($instance_completion_items, $static_completion_items);
        }
    ...
    public function getCompletionItemsForPartialSymbol(string $type_string, int $offset, string $file_path) : array
    {
        ...
        foreach ($matching_classlike_names as $fq_class_name) {
            $extra_edits = [];
            $insertion_text = Type::getStringFromFQCLN($fq_class_name, $aliases && $aliases->namespace ? $aliases->namespace : null, $aliases ? $aliases->uses_flipped : [], null);
            if ($aliases && !$fq_suggestion && $aliases->namespace && $insertion_text === '\\' . $fq_class_name && $aliases->namespace_first_stmt_start) {
                ...
                if ($aliases->uses_end) {
                    ...
                    $extra_edits[] = new \LanguageServerProtocol\TextEdit(new Range($position, $position), "\n" . 'use ' . $fq_class_name . ';');
                } else {
                    $position = self::getPositionFromOffset($aliases->namespace_first_stmt_start, $file_contents);
                    $extra_edits[] = new \LanguageServerProtocol\TextEdit(new Range($position, $position), 'use ' . $fq_class_name . ';' . "\n" . "\n");
                }
                $insertion_text = $class_name;
            }
            $completion_items[] = new \LanguageServerProtocol\CompletionItem($fq_class_name, \LanguageServerProtocol\CompletionItemKind::CLASS_, null, null, null, $fq_class_name, $insertion_text, null, $extra_edits);
        }
        ...
        foreach ($functions as $function_lowercase => $function) {
            ...
            $completion_items[] = new \LanguageServerProtocol\CompletionItem($function_name, \LanguageServerProtocol\CompletionItemKind::FUNCTION, $function->getSignature(false), null, null, $function_name, $function_name . (count($function->params) !== 0 ? '($0)' : '()'), null, null, null, null, 2);
        }
        return $completion_items;
    }
    /**
        ...
        foreach ($type->getAtomicTypes() as $atomic_type) {
            if ($atomic_type instanceof Type\Atomic\TBool) {
                ...
                foreach ($bools as $property_name) {
                    $completion_items[] = new \LanguageServerProtocol\CompletionItem($property_name, \LanguageServerProtocol\CompletionItemKind::VALUE, 'bool', null, null, null, $property_name);
                }
            } elseif ($atomic_type instanceof Type\Atomic\TLiteralString) {
                $completion_items[] = new \LanguageServerProtocol\CompletionItem($atomic_type->value, \LanguageServerProtocol\CompletionItemKind::VALUE, $atomic_type->getId(), null, null, null, "'{$atomic_type->value}'");
            } elseif ($atomic_type instanceof Type\Atomic\TLiteralInt) {
                $completion_items[] = new \LanguageServerProtocol\CompletionItem((string) $atomic_type->value, \LanguageServerProtocol\CompletionItemKind::VALUE, $atomic_type->getId(), null, null, null, (string) $atomic_type->value);
            } elseif ($atomic_type instanceof Type\Atomic\TScalarClassConstant) {
                $const = $atomic_type->fq_classlike_name . '::' . $atomic_type->const_name;
                $completion_items[] = new \LanguageServerProtocol\CompletionItem($const, \LanguageServerProtocol\CompletionItemKind::VALUE, $atomic_type->getId(), null, null, null, $const);
            }
        }
        return $completion_items;
    }
        ...
        foreach ($type->getAtomicTypes() as $atomic_type) {
            if ($atomic_type instanceof Type\Atomic\TKeyedArray) {
                foreach ($atomic_type->properties as $property_name => $property) {
                    $completion_items[] = new \LanguageServerProtocol\CompletionItem((string) $property_name, \LanguageServerProtocol\CompletionItemKind::PROPERTY, (string) $property, null, null, null, "'{$property_name}'");
                }
            }
        }
        return $completion_items;
     ...
     */
    public function isTypeContainedByType(Type\Union $input_type, Type\Union $container_type) : bool
    {
     ...
     */
    public function canTypeBeContainedByType(Type\Union $input_type, Type\Union $container_type) : bool
    {
    ...
    public function getKeyValueParamsForTraversableObject(Type\Atomic $type) : array
    {
        ...
        return [$key_type ?? Type::getMixed(), $value_type ?? Type::getMixed()];
    }
    /**
     * @param array<string, mixed> $phantom_classes
     * @psalm-suppress PossiblyUnusedMethod part of the public API
     */
    public function queueClassLikeForScanning(string $fq_classlike_name, bool $analyze_too = false, bool $store_failure = true, array $phantom_classes = []) : void
    {
     ...
     */
    public function addTaintSource(Type\Union $expr_type, string $taint_id, array $taints = \Psalm\Type\TaintKindGroup::ALL_INPUT, ?CodeLocation $code_location = null) : void
    {
        if (!$this->taint_flow_graph) {
            return;
        }
        $source = new \Psalm\Internal\DataFlow\TaintSource($taint_id, $taint_id, $code_location, null, $taints);
     ...
     * @psalm-suppress PossiblyUnusedMethod
     */
    public function addTaintSink(string $taint_id, array $taints = \Psalm\Type\TaintKindGroup::ALL_INPUT, ?CodeLocation $code_location = null) : void
    {
        if (!$this->taint_flow_graph) {
            return;
        }
        $sink = new \Psalm\Internal\DataFlow\TaintSink($taint_id, $taint_id, $code_location, null, $taints);
