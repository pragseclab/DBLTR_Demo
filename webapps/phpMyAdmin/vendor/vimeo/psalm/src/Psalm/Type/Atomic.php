...
abstract class Atomic implements TypeNode
{
     ...
     */
    public static function create(string $value, ?array $php_version = null, array $template_type_map = [], array $type_aliases = []) : Atomic
    {
        switch ($value) {
            case 'int':
                return new TInt();
            ...
            case 'bool':
                return new TBool();
            case 'void':
                if ($php_version === null || $php_version[0] > 7 || $php_version[0] === 7 && $php_version[1] >= 1) {
                    return new TVoid();
                }
                break;
            case 'array-key':
            ...
            case 'no-return':
                return new TNever();
            case 'object':
                if ($php_version === null || $php_version[0] > 7 || $php_version[0] === 7 && $php_version[1] >= 2) {
                    return new TObject();
                }
                break;
            case 'callable':
    ...
    public abstract function getKey(bool $include_extra = true) : string;
    ...
    {
        return $this instanceof TNamedObject || $this instanceof TTemplateParam && ($this->as->hasNamedObjectType() || array_filter($this->extra_types ?: [], function ($extra_type) {
            return $extra_type->isNamedObjectType();
        }));
    }
    public function isCallableType() : bool
    {
        return $this instanceof TCallable || $this instanceof TCallableObject || $this instanceof TCallableString || $this instanceof TCallableArray || $this instanceof TCallableList || $this instanceof TCallableKeyedArray;
    }
    public function isIterable(Codebase $codebase) : bool
    {
        return $this instanceof TIterable || $this->hasTraversableInterface($codebase) || $this instanceof TArray || $this instanceof TKeyedArray || $this instanceof TList;
    }
    ...
    {
        return $this instanceof TNamedObject && (strtolower($this->value) === 'traversable' || $codebase->classOrInterfaceExists($this->value) && ($codebase->classExtendsOrImplements($this->value, 'Traversable') || $codebase->interfaceExtends($this->value, 'Traversable')) || $this->extra_types && array_filter($this->extra_types, function (Atomic $a) use($codebase) : bool {
            return $a->hasTraversableInterface($codebase);
        }));
    }
    public function hasCountableInterface(Codebase $codebase) : bool
    {
        return $this instanceof TNamedObject && (strtolower($this->value) === 'countable' || $codebase->classOrInterfaceExists($this->value) && ($codebase->classExtendsOrImplements($this->value, 'Countable') || $codebase->interfaceExtends($this->value, 'Countable')) || $this->extra_types && array_filter($this->extra_types, function (Atomic $a) use($codebase) : bool {
            return $a->hasCountableInterface($codebase);
        }));
    }
    public function isArrayAccessibleWithStringKey(Codebase $codebase) : bool
    {
        return $this instanceof TArray || $this instanceof TKeyedArray || $this instanceof TList || $this instanceof Atomic\TClassStringMap || $this->hasArrayAccessInterface($codebase) || $this instanceof TNamedObject && $this->value === 'SimpleXMLElement';
    }
    ...
    public function replaceClassLike(string $old, string $new) : void
    {
        ...
        if ($this instanceof TNamedObject || $this instanceof TIterable || $this instanceof TTemplateParam) {
            if ($this->extra_types) {
        ...
        if ($this instanceof Type\Atomic\TArray || $this instanceof Type\Atomic\TGenericObject || $this instanceof Type\Atomic\TIterable) {
        ...
        if ($this instanceof Type\Atomic\TClosure || $this instanceof Type\Atomic\TCallable) {
            if ($this->params) {
    ...
    {
        if ($this instanceof TNamedObject || $this instanceof TTemplateParam || $this instanceof TIterable || $this instanceof Type\Atomic\TObjectWithProperties) {
            if ($this->extra_types) {
     ...
     * @param  array<lowercase-string, string> $aliased_classes
     */
    public abstract function toPhpString(?string $namespace, array $aliased_classes, ?string $this_class, int $php_major_version, int $php_minor_version) : ?string;
    public abstract function canBeFullyExpressedInPhp(int $php_major_version, int $php_minor_version) : bool;
    public function replaceTemplateTypesWithStandins(TemplateResult $template_result, ?Codebase $codebase = null, ?StatementsAnalyzer $statements_analyzer = null, Type\Atomic $input_type = null, ?int $input_arg_offset = null, ?string $calling_class = null, ?string $calling_function = null, bool $replace = true, bool $add_upper_bound = false, int $depth = 0) : self
    {
        return $this;
    }
    public function replaceTemplateTypesWithArgTypes(TemplateResult $template_result, ?Codebase $codebase) : void
    {
        // do nothing
    }
