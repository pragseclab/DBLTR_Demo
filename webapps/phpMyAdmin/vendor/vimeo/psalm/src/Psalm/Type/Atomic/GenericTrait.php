...
trait GenericTrait
{
    ...
    public function getId(bool $nested = false) : string
    {
        ...
        if ($this instanceof TNamedObject) {
            if ($this->extra_types) {
                $extra_types = '&' . implode('&', array_map(function ($type) {
    ...
    {
        $base_value = $this instanceof TNamedObject ? parent::toNamespacedString($namespace, $aliased_classes, $this_class, $use_phpdoc_format) : $this->value;
        ...
        return $base_value . '<' . implode(', ', array_map(
            /**
             * @return string
             */
    ...
    public function replaceTemplateTypesWithStandins(TemplateResult $template_result, ?Codebase $codebase = null, ?StatementsAnalyzer $statements_analyzer = null, ?Atomic $input_type = null, ?int $input_arg_offset = null, ?string $calling_class = null, ?string $calling_function = null, bool $replace = true, bool $add_upper_bound = false, int $depth = 0) : Atomic
    {
        ...
        if ($input_type instanceof Atomic\TGenericObject && ($this instanceof Atomic\TGenericObject || $this instanceof Atomic\TIterable) && $codebase) {
            $input_object_type_params = TemplateStandinTypeReplacer::getMappedGenericTypeParams($codebase, $input_type, $this);
        }
        $atomic = clone $this;
        foreach ($atomic->type_params as $offset => $type_param) {
            $input_type_param = null;
            if (($input_type instanceof Atomic\TIterable || $input_type instanceof Atomic\TArray) && isset($input_type->type_params[$offset])) {
            ...
            } elseif ($input_type instanceof Atomic\TNamedObject && isset($input_object_type_params[$offset])) {
                $input_type_param = $input_object_type_params[$offset];
            }
            /** @psalm-suppress PropertyTypeCoercion */
            $atomic->type_params[$offset] = TemplateStandinTypeReplacer::replace($type_param, $template_result, $codebase, $statements_analyzer, $input_type_param, $input_arg_offset, $calling_class, $calling_function, $replace, $add_upper_bound, $depth + 1);
        }
        return $atomic;
    }
    public function replaceTemplateTypesWithArgTypes(TemplateResult $template_result, ?Codebase $codebase) : void
    {
        foreach ($this->type_params as $offset => $type_param) {
            TemplateInferredTypeReplacer::replace($type_param, $template_result, $codebase);
