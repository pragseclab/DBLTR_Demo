...
class TKeyedArray extends \Psalm\Type\Atomic
{
    ...
    {
        $property_strings = array_map(function ($name, Union $type) : string {
    ...
    public function getId(bool $nested = false) : string
    {
        ...
        /** @psalm-suppress MixedOperand */
        return static::KEY . '{' . implode(', ', $property_strings) . '}' . ($this->previous_value_type && (!$this->previous_value_type->isMixed() || $this->previous_key_type && !$this->previous_key_type->isArrayKey()) ? '<' . ($this->previous_key_type ? $this->previous_key_type->getId() . ', ' : '') . $this->previous_value_type->getId() . '>' : '');
    }
    /**
    ...
    public function toNamespacedString(?string $namespace, array $aliased_classes, ?string $this_class, bool $use_phpdoc_format) : string
    {
        if ($use_phpdoc_format) {
            return $this->getGenericArrayType()->toNamespacedString($namespace, $aliased_classes, $this_class, $use_phpdoc_format);
        }
        /** @psalm-suppress MixedOperand */
        return static::KEY . '{' . implode(', ', array_map(function ($name, Union $type) use($namespace, $aliased_classes, $this_class, $use_phpdoc_format) : string {
            if (\is_string($name) && \preg_match('/[ "\'\\\\.\\n:]/', $name)) {
                $name = '\'' . \str_replace("\n", '\\n', \addslashes($name)) . '\'';
            }
            return $name . ($type->possibly_undefined ? '?' : '') . ': ' . $type->toNamespacedString($namespace, $aliased_classes, $this_class, $use_phpdoc_format);
        }, array_keys($this->properties), $this->properties)) . '}';
    }
    /**
     * @param  array<lowercase-string, string> $aliased_classes
    ...
    public function replaceTemplateTypesWithStandins(TemplateResult $template_result, ?Codebase $codebase = null, ?StatementsAnalyzer $statements_analyzer = null, ?Atomic $input_type = null, ?int $input_arg_offset = null, ?string $calling_class = null, ?string $calling_function = null, bool $replace = true, bool $add_upper_bound = false, int $depth = 0) : Atomic
    {
        ...
        foreach ($this->properties as $offset => $property) {
            ...
            if ($input_type instanceof Atomic\TKeyedArray && isset($input_type->properties[$offset])) {
                $input_type_param = $input_type->properties[$offset];
            }
            $object_like->properties[$offset] = TemplateStandinTypeReplacer::replace($property, $template_result, $codebase, $statements_analyzer, $input_type_param, $input_arg_offset, $calling_class, $calling_function, $replace, $add_upper_bound, $depth);
        }
        return $object_like;
    }
    ...
    {
        foreach ($this->properties as $property) {
            TemplateInferredTypeReplacer::replace($property, $template_result, $codebase);
        }
    }
