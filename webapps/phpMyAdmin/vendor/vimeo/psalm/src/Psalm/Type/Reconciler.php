...
class Reconciler
{
     ...
     */
    public static function reconcileKeyedTypes(array $new_types, array $active_new_types, array $existing_types, array &$changed_var_ids, array $referenced_var_ids, StatementsAnalyzer $statements_analyzer, array $template_type_map = [], bool $inside_loop = false, ?CodeLocation $code_location = null, bool $negated = false) : array
    {
        if (!$new_types) {
        ...
        foreach ($new_types as $key => $new_type_parts) {
            ...
            foreach ($new_type_parts as $new_type_part_parts) {
                foreach ($new_type_part_parts as $new_type_part_part) {
                    ...
                    $has_isset = $has_isset || $new_type_part_part === 'isset' || $new_type_part_part === '=isset' || $new_type_part_part === 'array-key-exists' || $new_type_part_part === '=string-array-access';
                    $has_empty = $has_empty || $new_type_part_part === 'empty';
                    $has_falsyish = $has_falsyish || $new_type_part_part === 'empty' || $new_type_part_part === 'falsy';
                    $is_equality = $is_equality && $new_type_part_part[0] === '=' && $new_type_part_part !== '=isset';
                    $has_inverted_isset = $has_inverted_isset || $new_type_part_part === '!isset';
                    $has_count_check = $has_count_check || $new_type_part_part === 'non-empty-countable';
                }
            }
            $did_type_exist = isset($existing_types[$key]);
            $has_object_array_access = false;
            $result_type = isset($existing_types[$key]) ? clone $existing_types[$key] : self::getValueForKey($codebase, $key, $existing_types, $new_types, $code_location, $has_isset, $has_inverted_isset, $has_empty, $inside_loop, $has_object_array_access);
            ...
            foreach ($new_type_parts as $offset => $new_type_part_parts) {
                ...
                foreach ($new_type_part_parts as $new_type_part_part) {
                    if ($new_type_part_part[0] === '>') {
                        ...
                        $existing_types = self::reconcileKeyedTypes($data, $data, $existing_types, $changed_var_ids, $referenced_var_ids, $statements_analyzer, $template_type_map, $inside_loop, $code_location, $negated);
                        $new_type_part_part = '!falsy';
                    }
                    $result_type_candidate = AssertionReconciler::reconcile($new_type_part_part, $result_type ? clone $result_type : null, $key, $statements_analyzer, $inside_loop, $template_type_map, $code_location && isset($referenced_var_ids[$key]) && isset($active_new_types[$key][$offset]) ? $code_location : null, $suppressed_issues, $failed_reconciliation, $negated);
                    if (!$result_type_candidate->getAtomicTypes()) {
                        $result_type_candidate->addType(new TEmpty());
                    }
                    $orred_type = $orred_type ? Type::combineUnionTypes($result_type_candidate, $orred_type, $codebase) : $result_type_candidate;
                }
            ...
            if ($statements_analyzer->data_flow_graph instanceof \Psalm\Internal\Codebase\TaintFlowGraph && !$result_type->hasScalarType() || $result_type->hasString() && !$result_type->hasLiteralString() || $statements_analyzer->data_flow_graph instanceof \Psalm\Internal\Codebase\VariableUseGraph) {
                if ($before_adjustment && $before_adjustment->parent_nodes) {
                    ...
                    $result_type->parent_nodes = $statements_analyzer->getParentNodesForPossiblyUndefinedVariable($key);
                }
            }
            ...
            if ($type_changed || $failed_reconciliation) {
                ...
                if (substr($key, -1) === ']' && !$has_inverted_isset && !$has_empty && !$is_equality) {
                    ...
                    foreach ($existing_types as $new_key => $_) {
                        ...
                        if (!isset($new_types[$new_key]) && preg_match('/' . preg_quote($key, '/') . '[\\]\\[\\-]/', $new_key)) {
    ...
    private static function addNestedAssertions(array $new_types, array $existing_types) : array
    {
        foreach ($new_types as $nk => $type) {
            if (strpos($nk, '[') || strpos($nk, '->')) {
                if ($type[0][0] === '=isset' || $type[0][0] === '!=empty' || $type[0][0] === 'isset' || $type[0][0] === '!empty') {
                ...
                if ($type[0][0] === 'array-key-exists') {
                    $key_parts = Reconciler::breakUpPathIntoParts($nk);
                    if (count($key_parts) === 4 && $key_parts[1] === '[' && $key_parts[2][0] !== '\'' && !\is_numeric($key_parts[2])) {
     ...
     */
    private static function getValueForKey(Codebase $codebase, string $key, array &$existing_keys, array $new_assertions, ?CodeLocation $code_location, bool $has_isset, bool $has_inverted_isset, bool $has_empty, bool $inside_loop, bool &$has_object_array_access) : ?Union
    {
        $key_parts = self::breakUpPathIntoParts($key);
            ...
            if ($divider === '[') {
                ...
                if (!isset($existing_keys[$new_base_key])) {
                    ...
                    while ($atomic_types) {
                        ...
                        } elseif ($existing_key_type_part instanceof Type\Atomic\TNull || $existing_key_type_part instanceof Type\Atomic\TFalse) {
                        ...
                        } elseif ($existing_key_type_part instanceof Type\Atomic\TEmpty || $existing_key_type_part instanceof Type\Atomic\TMixed && $existing_key_type_part->from_loop_isset) {
                            return Type::getMixed($inside_loop);
                        } elseif ($existing_key_type_part instanceof TString) {
                            $new_base_type_candidate = Type::getString();
                        } elseif ($existing_key_type_part instanceof Type\Atomic\TNamedObject && ($has_isset || $has_inverted_isset)) {
                ...
                if (!isset($existing_keys[$new_base_key])) {
                    ...
                    while ($atomic_types) {
                        ...
                        if ($existing_key_type_part instanceof TNull) {
                        ...
                        } elseif ($existing_key_type_part instanceof TMixed || $existing_key_type_part instanceof TTemplateParam || $existing_key_type_part instanceof TObject || $existing_key_type_part instanceof TNamedObject && strtolower($existing_key_type_part->value) === 'stdclass') {
                            ...
                            if (!$codebase->classOrInterfaceExists($existing_key_type_part->value)) {
                                ...
                                if (substr($property_name, -2) === '()') {
                                    $method_id = new \Psalm\Internal\MethodIdentifier($existing_key_type_part->value, strtolower(substr($property_name, 0, -2)));
                                    ...
                                    if ($method_return_type) {
                                        $class_property_type = \Psalm\Internal\Type\TypeExpander::expandUnion($codebase, clone $method_return_type, $declaring_class, $declaring_class, null);
                                    } else {
                                        $class_property_type = Type::getMixed();
                                    }
                                } else {
    ...
    private static function getPropertyType(Codebase $codebase, string $fq_class_name, string $property_name) : ?Type\Union
    {
        ...
        if ($class_property_type) {
            return \Psalm\Internal\Type\TypeExpander::expandUnion($codebase, clone $class_property_type, $declaring_class_storage->name, $declaring_class_storage->name, null);
        }
        return Type::getMixed();
    }
    /**
     ...
     *
     */
    protected static function triggerIssueForImpossible(Union $existing_var_type, string $old_var_type_string, string $key, string $assertion, bool $redundant, bool $negated, CodeLocation $code_location, array $suppressed_issues) : void
    {
        $not = $assertion[0] === '!';
        if ($not) {
        ...
        $from_docblock = $existing_var_type->from_docblock || isset($existing_var_atomic_types[$assertion]) && $existing_var_atomic_types[$assertion]->from_docblock;
        if ($redundant) {
            if ($existing_var_type->from_property && $assertion === 'isset') {
                if (IssueBuffer::accepts(new RedundantPropertyInitializationCheck('Property type ' . $key . ' with type ' . $old_var_type_string . ' should already be set in the constructor', $code_location), $suppressed_issues)) {
                    // fall through
                }
            } elseif ($from_docblock) {
                if (IssueBuffer::accepts(new RedundantConditionGivenDocblockType('Docblock-defined type ' . $old_var_type_string . ' for ' . $key . ' is ' . ($not ? 'never ' : 'always ') . $assertion, $code_location, $old_var_type_string . ' ' . $assertion), $suppressed_issues)) {
                    // fall through
                }
            } else {
                if (IssueBuffer::accepts(new RedundantCondition('Type ' . $old_var_type_string . ' for ' . $key . ' is ' . ($not ? 'never ' : 'always ') . $assertion, $code_location, $old_var_type_string . ' ' . $assertion), $suppressed_issues)) {
                    // fall through
                }
            }
        } else {
            if ($from_docblock) {
                if (IssueBuffer::accepts(new DocblockTypeContradiction('Docblock-defined type ' . $old_var_type_string . ' for ' . $key . ' is ' . ($not ? 'always ' : 'never ') . $assertion, $code_location, $old_var_type_string . ' ' . $assertion), $suppressed_issues)) {
                    // fall through
                }
            } else {
                if ($assertion === 'null' && !$not) {
                    $issue = new TypeDoesNotContainNull('Type ' . $old_var_type_string . ' for ' . $key . ' is never ' . $assertion, $code_location, $old_var_type_string . ' ' . $assertion);
                } else {
     ...
     * @param  array<string, bool>       $changed_var_ids
     */
    private static function adjustTKeyedArrayType(array $key_parts, array &$existing_types, array &$changed_var_ids, Type\Union $result_type) : void
    {
        array_pop($key_parts);
        ...
        if (isset($existing_types[$base_key]) && $array_key_offset !== false) {
            foreach ($existing_types[$base_key]->getAtomicTypes() as $base_atomic_type) {
                if ($base_atomic_type instanceof Type\Atomic\TKeyedArray || $base_atomic_type instanceof Type\Atomic\TArray && !$base_atomic_type->type_params[1]->isEmpty() || $base_atomic_type instanceof Type\Atomic\TList || $base_atomic_type instanceof Type\Atomic\TClassStringMap) {
                    ...
                    if ($base_atomic_type instanceof Type\Atomic\TArray) {
                        ...
                        $base_atomic_type = new Type\Atomic\TKeyedArray([$array_key_offset => clone $result_type], null);
