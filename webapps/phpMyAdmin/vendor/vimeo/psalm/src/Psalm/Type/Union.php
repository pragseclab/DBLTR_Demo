...
class Union implements TypeNode
{
    ...
    public function __construct(array $types)
    {
        ...
        foreach ($types as $type) {
            ...
            } elseif ($type instanceof Type\Atomic\TClassString && ($type->as_type || $type instanceof Type\Atomic\TTemplateParamClass)) {
    ...
    public function addType(Atomic $type) : void
    {
        ...
        if ($type instanceof TLiteralString) {
            ...
            if (!$type instanceof Type\Atomic\TClassString || !$type->as_type && !$type instanceof Type\Atomic\TTemplateParamClass) {
    ...
    {
        $types = [];
        $multi_ints = count($this->literal_int_types) > 1 || $this->hasPositiveInt();
     ...
     * @param  array<lowercase-string, string> $aliased_classes
     */
    public function toPhpString(?string $namespace, array $aliased_classes, ?string $this_class, int $php_major_version, int $php_minor_version) : ?string
    {
        if (!$this->isSingleAndMaybeNullable()) {
            if ($php_major_version < 8) {
                return null;
            }
        } elseif ($php_major_version < 7 || isset($this->types['null']) && $php_major_version === 7 && $php_minor_version < 1) {
            return null;
        }
        $types = $this->types;
        $nullable = false;
        ...
        foreach ($types as $atomic_type) {
            $php_type = $atomic_type->toPhpString($namespace, $aliased_classes, $this_class, $php_major_version, $php_minor_version);
            if (!$php_type) {
                return null;
            }
            $php_types[] = $php_type;
    ...
    public function canBeFullyExpressedInPhp(int $php_major_version, int $php_minor_version) : bool
    {
        ...
        return !array_filter($types, function ($atomic_type) use($php_major_version, $php_minor_version) {
    ...
    {
        return $this->isSingle() && count(array_filter($this->types, function ($type) : bool {
            return $type instanceof Atomic\TTemplateParamClass;
        })) === 1;
    }
    public function hasEmptyArray() : bool
    {
        return isset($this->types['array']) && $this->types['array'] instanceof Atomic\TArray && $this->types['array']->type_params[1]->isEmpty();
    }
    public function hasArrayAccessInterface(Codebase $codebase) : bool
    {
        return !!array_filter($this->types, function ($type) use($codebase) {
    ...
    {
        return array_filter($this->types, function ($type) : bool {
    ...
    {
        foreach ($this->types as $type) {
            if (!$type instanceof TNamedObject || !$type->was_static) {
                return false;
            }
        }
        return true;
    ...
    {
        return isset($this->types['string']) || isset($this->types['class-string']) || isset($this->types['trait-string']) || isset($this->types['numeric-string']) || isset($this->types['callable-string']) || $this->literal_string_types || $this->typed_class_strings;
    }
    public function hasLowercaseString() : bool
    {
        return isset($this->types['string']) && ($this->types['string'] instanceof Atomic\TLowercaseString || $this->types['string'] instanceof Atomic\TNonEmptyLowercaseString);
    }
    ...
    {
        return isset($this->types['int']) || isset($this->types['float']) || isset($this->types['numeric-string']) || $include_literal_int && $this->literal_int_types || $this->literal_float_types;
    }
    public function hasPossiblyNumericType() : bool
    {
        return isset($this->types['int']) || isset($this->types['float']) || isset($this->types['string']) || isset($this->types['numeric-string']) || $this->literal_int_types || $this->literal_float_types || $this->literal_string_types;
    }
    ...
    {
        return (bool) array_filter($this->types, function (Atomic $type) : bool {
            return $type instanceof Type\Atomic\TTemplateParam || $type instanceof Type\Atomic\TNamedObject && $type->extra_types && array_filter($type->extra_types, function ($t) : bool {
    ...
    {
        return (bool) array_filter($this->types, function (Atomic $type) : bool {
            return $type instanceof Type\Atomic\TTemplateParam || $type instanceof Type\Atomic\TNamedObject && ($type->was_static || $type->extra_types && array_filter($type->extra_types, function ($t) : bool {
                return $t instanceof Type\Atomic\TTemplateParam;
            }));
        });
    }
    ...
    {
        return isset($this->types['mixed']) && $this->types['mixed'] instanceof Type\Atomic\TEmptyMixed;
    }
    public function isVanillaMixed() : bool
    {
        return isset($this->types['mixed']) && get_class($this->types['mixed']) === Type\Atomic\TMixed::class && !$this->types['mixed']->from_loop_isset && count($this->types) === 1;
    }
    ...
    public function substitute(Union $old_type, ?Union $new_type = null) : void
    {
        ...
        foreach ($old_type->types as $old_type_part) {
            if (!$this->removeType($old_type_part->getKey())) {
                if ($old_type_part instanceof Type\Atomic\TFalse && isset($this->types['bool']) && !isset($this->types['true'])) {
                    $this->removeType('bool');
                    $this->types['true'] = new Type\Atomic\TTrue();
                } elseif ($old_type_part instanceof Type\Atomic\TTrue && isset($this->types['bool']) && !isset($this->types['false'])) {
                    $this->removeType('bool');
                    $this->types['false'] = new Type\Atomic\TFalse();
                } elseif (isset($this->types['iterable'])) {
                    if ($old_type_part instanceof Type\Atomic\TNamedObject && $old_type_part->value === 'Traversable' && !isset($this->types['array'])) {
                        $this->removeType('iterable');
                        $this->types['array'] = new Type\Atomic\TArray([Type::getArrayKey(), Type::getMixed()]);
                    }
                    if ($old_type_part instanceof Type\Atomic\TArray && !isset($this->types['traversable'])) {
                        $this->removeType('iterable');
                        $this->types['traversable'] = new Type\Atomic\TNamedObject('Traversable');
                    }
                } elseif (isset($this->types['array-key'])) {
                    if ($old_type_part instanceof Type\Atomic\TString && !isset($this->types['int'])) {
                        $this->removeType('array-key');
                        $this->types['int'] = new Type\Atomic\TInt();
                    }
                    if ($old_type_part instanceof Type\Atomic\TInt && !isset($this->types['string'])) {
                        $this->removeType('array-key');
        ...
        if ($new_type) {
            foreach ($new_type->types as $key => $new_type_part) {
                if (!isset($this->types[$key]) || $new_type_part instanceof Type\Atomic\Scalar && get_class($new_type_part) === get_class($this->types[$key])) {
    ...
    {
        return count(array_filter($this->types, function ($type) use($check_templates) : bool {
            return $type instanceof TInt || $check_templates && $type instanceof TTemplateParam && $type->as->isInt();
        })) === count($this->types);
    }
    /**
    ...
    {
        return count(array_filter($this->types, function ($type) use($check_templates) : bool {
            return $type instanceof TString || $check_templates && $type instanceof TTemplateParam && $type->as->isString();
        })) === count($this->types);
    }
    /**
     ...
     *
     */
    public function check(StatementsSource $source, CodeLocation $code_location, array $suppressed_issues, array $phantom_classes = [], bool $inferred = true, bool $inherited = false, bool $prevent_template_covariance = false, ?string $calling_method_id = null) : bool
    {
        if ($this->checked) {
            return true;
        }
        $checker = new \Psalm\Internal\TypeVisitor\TypeChecker($source, $code_location, $suppressed_issues, $phantom_classes, $inferred, $inherited, $prevent_template_covariance, $calling_method_id);
    ...
    {
        $scanner_visitor = new \Psalm\Internal\TypeVisitor\TypeScanner($codebase->scanner, $file_storage, $phantom_classes);
