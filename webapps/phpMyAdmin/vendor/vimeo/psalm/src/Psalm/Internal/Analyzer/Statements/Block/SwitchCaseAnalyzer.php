...
class SwitchCaseAnalyzer
{
    ...
    public static function analyze(StatementsAnalyzer $statements_analyzer, Codebase $codebase, PhpParser\Node\Stmt\Switch_ $stmt, ?string $switch_var_id, PhpParser\Node\Stmt\Case_ $case, Context $context, Context $original_context, string $case_exit_type, array $case_actions, bool $is_last, SwitchScope $switch_scope) : ?bool
    {
        ...
        $has_leaving_statements = $has_ending_statements || count($case_actions) && !in_array(ScopeAnalyzer::ACTION_NONE, $case_actions, true);
        ...
        if ($case->cond) {
            ...
            $traverser->addVisitor(new \Psalm\Internal\PhpVisitor\ConditionCloningVisitor($statements_analyzer->node_data));
            ...
            if (($switch_condition_type = $statements_analyzer->node_data->getType($switch_condition)) && ($case_cond_type = $statements_analyzer->node_data->getType($case->cond)) && ($switch_condition_type->isString() && $case_cond_type->isString() || $switch_condition_type->isInt() && $case_cond_type->isInt() || $switch_condition_type->isFloat() && $case_cond_type->isFloat())) {
        ...
        if ($case_equality_expr && $switch_condition instanceof PhpParser\Node\Expr\Variable && is_string($switch_condition->name) && isset($context->vars_in_scope['$' . $switch_condition->name])) {
            ...
            if ($new_case_equality_expr) {
                ExpressionAnalyzer::analyze($statements_analyzer, $new_case_equality_expr->args[1]->value, $case_context);
        ...
        if ($switch_scope->negated_clauses && count($switch_scope->negated_clauses) < 50) {
            $entry_clauses = Algebra::simplifyCNF(array_merge($original_context->clauses, $switch_scope->negated_clauses));
        } else {
        ...
        if ($case_clauses && $case_equality_expr) {
            ...
            $switch_scope->negated_clauses = array_merge($switch_scope->negated_clauses, $negated_case_clauses);
        }
        ...
        $traverser = new PhpParser\NodeTraverser();
        $traverser->addVisitor(new \Psalm\Internal\PhpVisitor\TypeMappingVisitor($statements_analyzer->node_data, $old_node_data));
        ...
        $case_context->possibly_assigned_var_ids = $pre_possibly_assigned_var_ids + $new_case_possibly_assigned_var_ids;
        ...
        if ($case_scope->break_vars !== null) {
                ...
                foreach ($case_scope->break_vars as $var_id => $type) {
                    if (isset($context->vars_in_scope[$var_id])) {
                        if (!isset($switch_scope->possibly_redefined_vars[$var_id])) {
                            ...
                            $switch_scope->possibly_redefined_vars[$var_id] = Type::combineUnionTypes(clone $type, $switch_scope->possibly_redefined_vars[$var_id]);
                        }
                    }
                }
            }
                ...
                foreach ($switch_scope->new_vars_in_scope as $var_id => $type) {
                    if (isset($case_scope->break_vars[$var_id])) {
                        if (!isset($case_context->vars_in_scope[$var_id])) {
                            ...
                            $switch_scope->new_vars_in_scope[$var_id] = Type::combineUnionTypes(clone $case_scope->break_vars[$var_id], $type);
                        }
                    } else {
                        unset($switch_scope->new_vars_in_scope[$var_id]);
                    }
            ...
            if ($switch_scope->redefined_vars !== null) {
                foreach ($switch_scope->redefined_vars as $var_id => $type) {
                    if (isset($case_scope->break_vars[$var_id])) {
                        $switch_scope->redefined_vars[$var_id] = Type::combineUnionTypes(clone $case_scope->break_vars[$var_id], $type);
                    } else {
    ...
    {
        if (!$case->cond && $switch_var_id && isset($case_context->vars_in_scope[$switch_var_id]) && $case_context->vars_in_scope[$switch_var_id]->isEmpty()) {
            ...
            if ($switch_scope->redefined_vars === null) {
                ...
                foreach ($switch_scope->redefined_vars as $var_id => $type) {
                    if (!isset($case_redefined_vars[$var_id])) {
                        ...
                        $switch_scope->redefined_vars[$var_id] = Type::combineUnionTypes($type, clone $case_redefined_vars[$var_id]);
                    }
                }
            }
            ...
            if ($switch_scope->new_vars_in_scope === null) {
                ...
                foreach ($switch_scope->new_vars_in_scope as $new_var => $type) {
                    if (!$case_context->hasVariable($new_var)) {
                        ...
                        $switch_scope->new_vars_in_scope[$new_var] = Type::combineUnionTypes(clone $case_context->vars_in_scope[$new_var], $type);
                    }
                }
