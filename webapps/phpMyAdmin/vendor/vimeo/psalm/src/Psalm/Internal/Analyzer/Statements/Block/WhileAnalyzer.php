...
class WhileAnalyzer
{
    ...
    {
        $while_true = $stmt->cond instanceof PhpParser\Node\Expr\ConstFetch && $stmt->cond->name->parts === ['true'] || $stmt->cond instanceof PhpParser\Node\Scalar\LNumber && $stmt->cond->value > 0;
        $pre_context = null;
        if ($while_true) {
        ...
        if (LoopAnalyzer::analyze($statements_analyzer, $stmt->stmts, self::getAndExpressions($stmt->cond), [], $loop_scope, $inner_loop_context) === false) {
            return false;
        }
        ...
        if ($stmt_cond_type = $statements_analyzer->node_data->getType($stmt->cond)) {
            ...
            foreach ($stmt_cond_type->getAtomicTypes() as $iterator_type) {
                if ($iterator_type instanceof Type\Atomic\TArray || $iterator_type instanceof Type\Atomic\TKeyedArray) {
                ...
                if ($iterator_type instanceof Type\Atomic\TLiteralString && $iterator_type->value) {
                    continue;
                }
                if ($iterator_type instanceof Type\Atomic\TLiteralInt && $iterator_type->value) {
                    continue;
                }
                $always_enters_loop = false;
                break;
        ...
        $can_leave_loop = !$while_true || in_array(ScopeAnalyzer::ACTION_BREAK, $loop_scope->final_actions, true);
        ...
        $context->referenced_var_ids = array_merge($context->referenced_var_ids, $while_context->referenced_var_ids);
        return null;
    }
    /**
     * @return list<PhpParser\Node\Expr>
     */
    private static function getAndExpressions(PhpParser\Node\Expr $expr) : array
    {
        if ($expr instanceof PhpParser\Node\Expr\BinaryOp\BooleanAnd) {
            return array_merge(self::getAndExpressions($expr->left), self::getAndExpressions($expr->right));
        }
        return [$expr];
    }
}