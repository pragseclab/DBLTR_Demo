...
class FunctionCallReturnTypeFetcher
{
     ...
     */
    public static function fetch(StatementsAnalyzer $statements_analyzer, \Psalm\Codebase $codebase, PhpParser\Node\Expr\FuncCall $stmt, PhpParser\Node\Name $function_name, string $function_id, bool $in_call_map, bool $is_stubbed, ?FunctionLikeStorage $function_storage, ?TCallable $callmap_callable, TemplateResult $template_result, Context $context) : Type\Union
    {
        $stmt_type = null;
        $config = $codebase->config;
        if ($codebase->functions->return_type_provider->has($function_id)) {
            $stmt_type = $codebase->functions->return_type_provider->getReturnType($statements_analyzer, $function_id, $stmt->args, $context, new CodeLocation($statements_analyzer->getSource(), $function_name));
        }
        if (!$stmt_type) {
            if (!$in_call_map || $is_stubbed) {
                    ...
                    foreach ($function_storage->template_types as $template_name => $_) {
                        if (!isset($template_result->upper_bounds[$template_name])) {
                            if ($template_name === 'TFunctionArgCount') {
                                $template_result->upper_bounds[$template_name] = ['fn-' . $function_id => new TemplateBound(Type::getInt(false, count($stmt->args)))];
                            } elseif ($template_name === 'TPhpMajorVersion') {
                                $template_result->upper_bounds[$template_name] = ['fn-' . $function_id => new TemplateBound(Type::getInt(false, $codebase->php_major_version))];
                            } else {
                                $template_result->upper_bounds[$template_name] = ['fn-' . $function_id => new TemplateBound(Type::getEmpty())];
                            }
                        }
                    }
                }
                ...
                try {
                    if ($function_storage && $function_storage->return_type) {
                        ...
                        if ($template_result->upper_bounds && $function_storage->template_types) {
                            ...
                            TemplateInferredTypeReplacer::replace($return_type, $template_result, $codebase);
                        }
                        $return_type = TypeExpander::expandUnion($codebase, $return_type, null, null, null, true, false, false, true);
                        ...
                        if ($return_type_location && !$is_stubbed && !$config->isInProjectDirs($return_type_location->file_path)) {
                            $return_type->check($statements_analyzer, new CodeLocation($statements_analyzer->getSource(), $stmt), $statements_analyzer->getSuppressedIssues(), $context->phantom_classes, true, false, false, $context->calling_method_id);
                        }
                    }
                ...
                $stmt_type = self::getReturnTypeFromCallMapWithArgs($statements_analyzer, $function_id, $stmt->args, $callmap_callable, $context);
            }
        }
        if (!$stmt_type) {
            $stmt_type = Type::getMixed();
        ...
        $return_node = self::taintReturnType($statements_analyzer, $stmt, $function_id, $function_storage, $stmt_type, $template_result);
     ...
     */
    private static function getReturnTypeFromCallMapWithArgs(StatementsAnalyzer $statements_analyzer, string $function_id, array $call_args, TCallable $callmap_callable, Context $context) : Type\Union
    {
        ...
        if (!$call_args) {
            switch ($call_map_key) {
                case 'hrtime':
                    return new Type\Union([new Type\Atomic\TKeyedArray([Type::getInt(), Type::getInt()])]);
                case 'get_called_class':
                    return new Type\Union([new Type\Atomic\TClassString($context->self ?: 'object', $context->self ? new Type\Atomic\TNamedObject($context->self, true) : null)]);
                case 'get_parent_class':
                    if ($context->self && $codebase->classExists($context->self)) {
                        $classlike_storage = $codebase->classlike_storage_provider->get($context->self);
                        if ($classlike_storage->parent_classes) {
                            return new Type\Union([new Type\Atomic\TClassString(\array_values($classlike_storage->parent_classes)[0])]);
                        }
                    }
            }
        } else {
            switch ($call_map_key) {
                    ...
                    if ($first_arg_type = $statements_analyzer->node_data->getType($call_args[0]->value)) {
                        ...
                        if (count($atomic_types) === 1) {
                            if (isset($atomic_types['array'])) {
                                if ($atomic_types['array'] instanceof Type\Atomic\TCallableArray || $atomic_types['array'] instanceof Type\Atomic\TCallableList || $atomic_types['array'] instanceof Type\Atomic\TCallableKeyedArray) {
                                    return Type::getInt(false, 2);
                                }
                                if ($atomic_types['array'] instanceof Type\Atomic\TNonEmptyArray) {
                                    return new Type\Union([$atomic_types['array']->count !== null ? new Type\Atomic\TLiteralInt($atomic_types['array']->count) : new Type\Atomic\TPositiveInt()]);
                                }
                                if ($atomic_types['array'] instanceof Type\Atomic\TNonEmptyList) {
                                    return new Type\Union([$atomic_types['array']->count !== null ? new Type\Atomic\TLiteralInt($atomic_types['array']->count) : new Type\Atomic\TPositiveInt()]);
                                }
                                if ($atomic_types['array'] instanceof Type\Atomic\TKeyedArray && $atomic_types['array']->sealed) {
                                    return new Type\Union([new Type\Atomic\TLiteralInt(count($atomic_types['array']->properties))]);
                                }
                                return new Type\Union([new Type\Atomic\TLiteralInt(0), new Type\Atomic\TPositiveInt()]);
                            }
                        }
                    }
                    break;
                case 'hrtime':
                    if ($first_arg_type = $statements_analyzer->node_data->getType($call_args[0]->value)) {
                        ...
                        return new Type\Union([new Type\Atomic\TKeyedArray([Type::getInt(), Type::getInt()]), new Type\Atomic\TInt()]);
                    }
                    $int = Type::getInt();
                    $int->from_calculation = true;
                    return $int;
                ...
                case 'fgetcsv':
                    ...
                    $call_map_return_type = new Type\Union([new Type\Atomic\TNonEmptyList($string_type), new Type\Atomic\TFalse(), new Type\Atomic\TNull()]);
        ...
        $stmt_type = $callmap_callable->return_type ? clone $callmap_callable->return_type : Type::getMixed();
        switch ($function_id) {
            case 'mb_strpos':
            case 'mb_strrpos':
            case 'mb_stripos':
    ...
    private static function taintReturnType(StatementsAnalyzer $statements_analyzer, PhpParser\Node\Expr\FuncCall $stmt, string $function_id, FunctionLikeStorage $function_storage, Type\Union $stmt_type, TemplateResult $template_result) : ?DataFlowNode
    {
        ...
        $function_call_node = DataFlowNode::getForMethodReturn($function_id, $function_id, $function_storage->signature_return_type_location ?: $function_storage->location, $function_storage->specialize_call ? $node_location : null);
        ...
        foreach ($function_storage->conditionally_removed_taints as $conditionally_removed_taint) {
            ...
            TemplateInferredTypeReplacer::replace($conditionally_removed_taint, $template_result, $codebase);
            $expanded_type = TypeExpander::expandUnion($statements_analyzer->getCodebase(), $conditionally_removed_taint, null, null, null, true, true);
        ...
        if ($conditionally_removed_taints && $function_storage->location) {
            ...
            $statements_analyzer->data_flow_graph->addPath($function_call_node, $assignment_node, 'conditionally-escaped', [], $conditionally_removed_taints);
        ...
        if ($function_storage->return_source_params) {
            ...
            self::taintUsingFlows($statements_analyzer, $function_storage, $statements_analyzer->data_flow_graph, $function_id, $stmt->args, $node_location, $function_call_node, $removed_taints);
        }
     ...
     */
    public static function taintUsingFlows(StatementsAnalyzer $statements_analyzer, FunctionLikeStorage $function_storage, TaintFlowGraph $graph, string $function_id, array $args, CodeLocation $node_location, DataFlowNode $function_call_node, array $removed_taints) : void
    {
        foreach ($function_storage->return_source_params as $i => $path_type) {
            ...
            foreach ($taintable_arg_index as $arg_index) {
                ...
                $graph->addPath($function_param_sink, $function_call_node, $path_type, $function_storage->added_taints, $removed_taints);
            }
        }
    }
    /**
