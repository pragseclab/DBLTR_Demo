...
class OrAnalyzer
{
    public static function analyze(StatementsAnalyzer $statements_analyzer, PhpParser\Node\Expr\BinaryOp $stmt, Context $context, bool $from_stmt = false) : bool
    {
        ...
        if ($left_context->reconciled_expression_clauses) {
            ...
            $negated_left_clauses = array_values(array_filter($negated_left_clauses, function ($c) use($reconciled_expression_clauses) : bool {
        ...
        $clauses_for_right_analysis = Algebra::simplifyCNF(array_merge($context->clauses, $negated_left_clauses));
        ...
        if ($stmt->left instanceof PhpParser\Node\Expr\BinaryOp\BooleanOr && $left_assigned_var_ids && $mic_drop_context) {
            IfAnalyzer::addConditionallyAssignedVarsToContext($statements_analyzer, $stmt->left, $mic_drop_context, $right_context, $left_assigned_var_ids);
        }
        if ($negated_type_assertions) {
            // while in an or, we allow scope to boil over to support
            // statements of the form if ($x === null || $x->foo())
            $right_vars_in_scope = Reconciler::reconcileKeyedTypes($negated_type_assertions, $active_negated_type_assertions, $right_context->vars_in_scope, $changed_var_ids, $left_referenced_var_ids, $statements_analyzer, [], $left_context->inside_loop, new CodeLocation($statements_analyzer->getSource(), $stmt->left), !$context->inside_negation);
        ...
        if ($changed_var_ids) {
            ...
            $right_context->reconciled_expression_clauses = array_merge($context->reconciled_expression_clauses, array_map(function ($c) {
                return $c->hash;
            ...
            $context->reconciled_expression_clauses = array_merge($context->reconciled_expression_clauses, array_map(function ($c) {
                return $c->hash;
            }, $partitioned_clauses[1]));
        }
        ...
        $combined_right_clauses = Algebra::simplifyCNF(array_merge($clauses_for_right_analysis, $right_clauses));
        ...
        if ($right_type_assertions) {
            ...
            Reconciler::reconcileKeyedTypes($right_type_assertions, $active_right_type_assertions, $right_context->vars_in_scope, $right_changed_var_ids, $right_referenced_var_ids, $statements_analyzer, [], $left_context->inside_loop, new CodeLocation($statements_analyzer->getSource(), $stmt->right), $context->inside_negation);
        }
        if (!$stmt->right instanceof PhpParser\Node\Expr\Exit_) {
            ...
            if ($var_id && isset($left_context->vars_in_scope[$var_id])) {
                $left_inferred_reconciled = AssertionReconciler::reconcile('!falsy', clone $left_context->vars_in_scope[$var_id], '', $statements_analyzer, $context->inside_loop, [], new CodeLocation($statements_analyzer->getSource(), $stmt->left), $statements_analyzer->getSuppressedIssues());
        ...
        $context->referenced_var_ids = array_merge($right_context->referenced_var_ids, $context->referenced_var_ids);
        ...
        if ($context->if_context) {
            ...
            foreach ($right_context->vars_in_scope as $var_id => $type) {
                if (isset($if_context->vars_in_scope[$var_id])) {
                    $if_context->vars_in_scope[$var_id] = Type::combineUnionTypes($type, $if_context->vars_in_scope[$var_id], $codebase);
                } elseif (isset($left_context->vars_in_scope[$var_id])) {
                    $if_context->vars_in_scope[$var_id] = $left_context->vars_in_scope[$var_id];
                }
            }
            $if_context->referenced_var_ids = array_merge($context->referenced_var_ids, $if_context->referenced_var_ids);
            $if_context->assigned_var_ids = array_merge($context->assigned_var_ids, $if_context->assigned_var_ids);
