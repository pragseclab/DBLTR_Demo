...
class ReturnAnalyzer
{
     ...
     * @return false|null
     */
    public static function analyze(StatementsAnalyzer $statements_analyzer, PhpParser\Node\Stmt\Return_ $stmt, Context $context) : ?bool
    {
        ...
        if ($doc_comment && ($parsed_docblock = $statements_analyzer->getParsedDocblock())) {
            ...
            try {
                $var_comments = CommentAnalyzer::arrayToDocblocks($doc_comment, $parsed_docblock, $statements_analyzer->getSource(), $statements_analyzer->getAliases(), $statements_analyzer->getTemplateTypeMap(), $file_storage->type_aliases);
            } catch (DocblockParseException $e) {
                if (IssueBuffer::accepts(new InvalidDocblock($e->getMessage(), new CodeLocation($source, $stmt)))) {
                    // fall through
                }
            }
        ...
        if ($stmt->expr) {
            ...
            if ($var_comment_type) {
                ...
                if ($stmt_type->isNever()) {
                    if (IssueBuffer::accepts(new NoValue('This function or method call never returns output', new CodeLocation($source, $stmt)), $statements_analyzer->getSuppressedIssues())) {
                        // fall through
                    }
        ...
        if ($source instanceof FunctionLikeAnalyzer && !$source->getSource() instanceof TraitAnalyzer) {
            ...
            if ($stmt->expr && $storage->location) {
                $inferred_type = \Psalm\Internal\Type\TypeExpander::expandUnion($codebase, $stmt_type, $source->getFQCLN(), $source->getFQCLN(), $source->getParentFQCLN());
                ...
                if ($storage instanceof \Psalm\Storage\MethodStorage && $context->self) {
                    ...
                    $declared_return_type = $codebase->methods->getMethodReturnType(\Psalm\Internal\MethodIdentifier::wrap($cased_method_id), $self_class, $statements_analyzer, null);
                } else {
                    $declared_return_type = $storage->return_type;
                }
                if ($declared_return_type && !$declared_return_type->hasMixed()) {
                    $local_return_type = $source->getLocalReturnType($declared_return_type, $storage instanceof \Psalm\Storage\MethodStorage && $storage->final);
                    if ($storage instanceof \Psalm\Storage\MethodStorage) {
                        [$fq_class_name, $method_name] = explode('::', $cased_method_id);
                        $class_storage = $codebase->classlike_storage_provider->get($fq_class_name);
                        $found_generic_params = ClassTemplateParamCollector::collect($codebase, $class_storage, $class_storage, strtolower($method_name), null, true);
                        if ($found_generic_params) {
                            ...
                            TemplateInferredTypeReplacer::replace($local_return_type, new TemplateResult([], $found_generic_params), $codebase);
                        }
                    }
                    ...
                    if ($stmt_type->hasMixed()) {
                        ...
                        if ($stmt_type->isMixed()) {
                            if (IssueBuffer::accepts(new MixedReturnStatement('Could not infer a return type', new CodeLocation($source, $stmt->expr)), $statements_analyzer->getSuppressedIssues())) {
                                // fall through
                            }
                            return null;
                        }
                        if (IssueBuffer::accepts(new MixedReturnStatement('Possibly-mixed return value', new CodeLocation($source, $stmt->expr)), $statements_analyzer->getSuppressedIssues())) {
                            // fall through
                        }
                    }
                    ...
                    if (!UnionTypeComparator::isContainedBy($codebase, $inferred_type, $local_return_type, true, true, $union_comparison_results)) {
                        ...
                        if ($union_comparison_results->type_coerced) {
                            if ($union_comparison_results->type_coerced_from_mixed) {
                                if (!$union_comparison_results->type_coerced_from_as_mixed) {
                                    if ($inferred_type->hasMixed()) {
                                        ...
                                        if (IssueBuffer::accepts(new MixedReturnTypeCoercion('The type \'' . $stmt_type->getId() . '\' is more general than the' . ' declared return type \'' . $local_return_type->getId() . '\'' . ' for ' . $cased_method_id, new CodeLocation($source, $stmt->expr)), $statements_analyzer->getSuppressedIssues())) {
                                            // fall through
                                        }
                                    }
                                }
                            } else {
                                if (IssueBuffer::accepts(new LessSpecificReturnStatement('The type \'' . $stmt_type->getId() . '\' is more general than the' . ' declared return type \'' . $local_return_type->getId() . '\'' . ' for ' . $cased_method_id, new CodeLocation($source, $stmt->expr)), $statements_analyzer->getSuppressedIssues())) {
                                    // fall throuhg
                                }
                            }
                            foreach ($local_return_type->getAtomicTypes() as $local_type_part) {
                                if ($local_type_part instanceof Type\Atomic\TClassString && $stmt->expr instanceof PhpParser\Node\Scalar\String_) {
                                    if (ClassLikeAnalyzer::checkFullyQualifiedClassLikeName($statements_analyzer, $stmt->expr->value, new CodeLocation($source, $stmt->expr), $context->self, $context->calling_method_id, $statements_analyzer->getSuppressedIssues()) === false) {
                                        return false;
                                    }
                                } elseif ($local_type_part instanceof Type\Atomic\TArray && $stmt->expr instanceof PhpParser\Node\Expr\Array_) {
                                        ...
                                        if ($local_array_type_part instanceof Type\Atomic\TClassString) {
                                            foreach ($stmt->expr->items as $item) {
                                                if ($item && $item->value instanceof PhpParser\Node\Scalar\String_) {
                                                    if (ClassLikeAnalyzer::checkFullyQualifiedClassLikeName($statements_analyzer, $item->value->value, new CodeLocation($source, $item->value), $context->self, $context->calling_method_id, $statements_analyzer->getSuppressedIssues()) === false) {
                                                        return false;
                                                    }
                                                }
                                            }
                            ...
                            if (IssueBuffer::accepts(new InvalidReturnStatement('The inferred type \'' . $inferred_type->getId() . '\' does not match the declared return ' . 'type \'' . $local_return_type->getId() . '\' for ' . $cased_method_id, new CodeLocation($source, $stmt->expr)), $statements_analyzer->getSuppressedIssues())) {
                                // fall through
                            }
                        }
                    }
                    if (!$stmt_type->ignore_nullable_issues && $inferred_type->isNullable() && !$local_return_type->isNullable() && !$local_return_type->hasTemplate()) {
                        if (IssueBuffer::accepts(new NullableReturnStatement('The declared return type \'' . $local_return_type->getId() . '\' for ' . $cased_method_id . ' is not nullable, but the function returns \'' . $inferred_type->getId() . '\'', new CodeLocation($source, $stmt->expr)), $statements_analyzer->getSuppressedIssues())) {
                            //fall through
                        }
                    }
                    if (!$stmt_type->ignore_falsable_issues && $inferred_type->isFalsable() && !$local_return_type->isFalsable() && (!$local_return_type->hasBool() || $local_return_type->isTrue()) && !$local_return_type->hasScalar()) {
                        if (IssueBuffer::accepts(new FalsableReturnStatement('The declared return type \'' . $local_return_type . '\' for ' . $cased_method_id . ' does not allow false, but the function returns \'' . $inferred_type . '\'', new CodeLocation($source, $stmt->expr)), $statements_analyzer->getSuppressedIssues())) {
                            // fall throughg
                        }
                    }
                }
                ...
                if ($storage->signature_return_type && !$storage->signature_return_type->isVoid() && !$storage->has_yield) {
                    if (IssueBuffer::accepts(new InvalidReturnStatement('Empty return statement is not expected in ' . $cased_method_id, new CodeLocation($source, $stmt)), $statements_analyzer->getSuppressedIssues())) {
                        // fall through
                    }
                }
            }
    ...
    private static function handleTaints(StatementsAnalyzer $statements_analyzer, PhpParser\Node\Stmt\Return_ $stmt, string $cased_method_id, Type\Union $inferred_type, \Psalm\Storage\FunctionLikeStorage $storage) : void
    {
    ...
    private static function potentiallyInferTypesOnClosureFromParentReturnType(StatementsAnalyzer $statements_analyzer, PhpParser\Node\FunctionLike $expr, Context $context) : void
    {
        ...
        foreach ($closure_storage->params as $key => $param) {
            ...
            $param->type = self::inferInnerClosureTypeFromParent($statements_analyzer->getCodebase(), $param->type, $parent_param ? $parent_param->type : null);
        }
     ...
     * - else, do nothing
     */
    private static function inferInnerClosureTypeFromParent(Codebase $codebase, ?Type\Union $return_type, ?Type\Union $parent_return_type) : ?Type\Union
    {
