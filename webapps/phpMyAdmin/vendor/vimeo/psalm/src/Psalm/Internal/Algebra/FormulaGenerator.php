...
class FormulaGenerator
{
     ...
     * @return list<Clause>
     */
    public static function getFormula(int $conditional_object_id, int $creating_object_id, PhpParser\Node\Expr $conditional, ?string $this_class_name, FileSource $source, ?Codebase $codebase = null, bool $inside_negation = false, bool $cache = true) : array
    {
        if ($conditional instanceof PhpParser\Node\Expr\BinaryOp\BooleanAnd || $conditional instanceof PhpParser\Node\Expr\BinaryOp\LogicalAnd) {
            ...
            return array_merge($left_assertions, $right_assertions);
        }
                ...
                foreach ($anded_assertions as $assertions) {
                    foreach ($assertions as $var => $anded_types) {
                        ...
                        foreach ($anded_types as $orred_types) {
                            $clauses[] = new Clause([$var => $orred_types], $conditional_object_id, \spl_object_id($conditional->expr), false, true, $orred_types[0][0] === '=' || $orred_types[0][0] === '~' || strlen($orred_types[0]) > 1 && ($orred_types[0][1] === '=' || $orred_types[0][1] === '~'), $redefined ? [$var => true] : []);
                        }
                    }
                }
        ...
        foreach ($anded_assertions as $assertions) {
            foreach ($assertions as $var => $anded_types) {
                ...
                foreach ($anded_types as $orred_types) {
                    $clauses[] = new Clause([$var => $orred_types], $conditional_object_id, $creating_object_id, false, true, $orred_types[0][0] === '=' || $orred_types[0][0] === '~' || strlen($orred_types[0]) > 1 && ($orred_types[0][1] === '=' || $orred_types[0][1] === '~'), $redefined ? [$var => true] : []);
                }
            }
        }
        if ($clauses) {
