...
class ArrayMergeReturnTypeProvider implements \Psalm\Plugin\EventHandler\FunctionReturnTypeProviderInterface
{
    ...
    public static function getFunctionReturnType(FunctionReturnTypeProviderEvent $event) : Type\Union
    {
        ...
        if (!$statements_source instanceof \Psalm\Internal\Analyzer\StatementsAnalyzer || !$call_args) {
            ...
            foreach ($call_arg_type->getAtomicTypes() as $type_part) {
                ...
                foreach ($unpacked_type_parts as $unpacked_type_part) {
                    if (!$unpacked_type_part instanceof Type\Atomic\TArray) {
                        if ($unpacked_type_part instanceof Type\Atomic\TFalse && $call_arg_type->ignore_falsable_issues || $unpacked_type_part instanceof Type\Atomic\TNull && $call_arg_type->ignore_nullable_issues) {
                            continue;
                        }
                        if ($unpacked_type_part instanceof Type\Atomic\TKeyedArray) {
                            foreach ($unpacked_type_part->properties as $key => $type) {
                                ...
                                if (!isset($generic_properties[$key]) || !$type->possibly_undefined) {
                                    ...
                                    $generic_properties[$key] = Type::combineUnionTypes($generic_properties[$key], $type, $codebase);
                        ...
                        if ($unpacked_type_part instanceof Type\Atomic\TList) {
                            ...
                            if ($unpacked_type_part instanceof Type\Atomic\TMixed && $unpacked_type_part->from_loop_isset) {
                                $unpacked_type_part = new Type\Atomic\TArray([Type::getArrayKey(), Type::getMixed(true)]);
                            } else {
                                return Type::getArray();
                            }
                        }
                    ...
                    $inner_key_types = array_merge($inner_key_types, $unpacked_type_part instanceof Type\Atomic\TList ? [new Type\Atomic\TInt()] : array_values($unpacked_type_part->type_params[0]->getAtomicTypes()));
                    $inner_value_types = array_merge($inner_value_types, $unpacked_type_part instanceof Type\Atomic\TList ? array_values($unpacked_type_part->type_param->getAtomicTypes()) : array_values($unpacked_type_part->type_params[1]->getAtomicTypes()));
                }
            }
        }
        $inner_key_type = null;
        ...
        if ($inner_value_type) {
            if ($all_int_offsets) {
                if ($any_nonempty) {
                    return new Type\Union([new Type\Atomic\TNonEmptyList($inner_value_type)]);
                }
                return new Type\Union([new Type\Atomic\TList($inner_value_type)]);
            }
            $inner_key_type = $inner_key_type ?: Type::getArrayKey();
            if ($any_nonempty) {
                return new Type\Union([new Type\Atomic\TNonEmptyArray([$inner_key_type, $inner_value_type])]);
            }
            return new Type\Union([new Type\Atomic\TArray([$inner_key_type, $inner_value_type])]);
        }
        return Type::getArray();
    }
}