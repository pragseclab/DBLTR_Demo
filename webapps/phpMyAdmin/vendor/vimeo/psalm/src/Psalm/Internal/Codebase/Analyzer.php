...
class Analyzer
{
    ...
    public function analyzeFiles(ProjectAnalyzer $project_analyzer, int $pool_size, bool $alter_code, bool $consolidate_analyzed_data = false) : void
    {
        ...
        $this->files_to_analyze = array_filter($this->files_to_analyze, function (string $file_path) : bool {
    ...
    private function doAnalysis(ProjectAnalyzer $project_analyzer, int $pool_size) : void
    {
        ...
        $analysis_worker = function (int $_, string $file_path) use($project_analyzer, $filetype_analyzers) : array {
                    ...
                    if (!isset($this->possible_method_param_types[$declaring_method_id])) {
                        ...
                        foreach ($possible_param_types as $offset => $possible_param_type) {
                            if (!isset($this->possible_method_param_types[$declaring_method_id][$offset])) {
                                $this->possible_method_param_types[$declaring_method_id][$offset] = $possible_param_type;
                            } else {
                                $this->possible_method_param_types[$declaring_method_id][$offset] = \Psalm\Type::combineUnionTypes($this->possible_method_param_types[$declaring_method_id][$offset], $possible_param_type, $codebase);
                            }
                        }
                    }
                }
    ...
    public function loadCachedResults(ProjectAnalyzer $project_analyzer) : void
    {
        ...
        foreach ($changed_members as $file_changed_members) {
            foreach ($file_changed_members as $member_id => $_) {
                ...
                unset($method_references_to_class_members[$member_id], $file_references_to_class_members[$member_id], $method_references_to_missing_class_members[$member_id], $file_references_to_missing_class_members[$member_id], $references_to_mixed_member_names[$member_id], $method_param_uses[$member_id]);
        ...
        foreach ($this->existing_issues as $file_path => &$file_issues) {
            ...
            foreach ($file_issues as $i => &$issue_data) {
                ...
                foreach ($file_diff_map as [$from, $to, $file_offset, $line_offset]) {
                    if ($issue_data->from >= $from && $issue_data->from <= $to && !$matched) {
            ...
            foreach ($reference_map as $reference_from => [$reference_to, $tag]) {
                ...
                foreach ($file_diff_map as [$from, $to, $file_offset]) {
                    if ($reference_from >= $from && $reference_from <= $to) {
                        ...
                        $reference_map[$reference_from += $file_offset] = [$reference_to += $file_offset, $tag];
                    }
                }
            }
        }
            ...
            foreach ($type_map as $type_from => [$type_to, $tag]) {
                ...
                foreach ($file_diff_map as [$from, $to, $file_offset]) {
                    if ($type_from >= $from && $type_from <= $to) {
                        ...
                        $type_map[$type_from += $file_offset] = [$type_to += $file_offset, $tag];
                    }
                }
            }
        }
            ...
            foreach ($argument_map as $argument_from => [$argument_to, $method_id, $argument_number]) {
                ...
                foreach ($file_diff_map as [$from, $to, $file_offset]) {
                    if ($argument_from >= $from && $argument_from <= $to) {
                        ...
                        $argument_map[$argument_from += $file_offset] = [$argument_to += $file_offset, $method_id, $argument_number];
                    }
                }
            }
        }
    ...
    public function getTypeInferenceSummary(\Psalm\Codebase $codebase) : string
    {
        ...
        return 'Psalm was able to infer types for ' . $percentage . '%' . ' of the codebase';
    }
        ...
        foreach ($all_deep_scanned_files as $file_path => $_) {
            if (isset($this->mixed_counts[$file_path])) {
                ...
                if ($path_mixed_count + $path_nonmixed_count) {
                    $stats .= number_format(100 * $path_nonmixed_count / ($path_mixed_count + $path_nonmixed_count), 0) . '% ' . $this->config->shortenFileName($file_path) . ' (' . $path_mixed_count . ' mixed)' . "\n";
                }
            }
        }
        return $stats;
    ...
    public function updateFile(string $file_path, bool $dry_run) : void
    {
        ...
        if ($dry_run) {
            echo $file_path . ':' . "\n";
            $differ = new \SebastianBergmann\Diff\Differ(new \SebastianBergmann\Diff\Output\StrictUnifiedDiffOutputBuilder(['fromFile' => $file_path, 'toFile' => $file_path]));
