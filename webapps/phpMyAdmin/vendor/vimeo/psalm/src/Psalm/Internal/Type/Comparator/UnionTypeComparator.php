...
class UnionTypeComparator
{
     ...
     */
    public static function isContainedBy(Codebase $codebase, Type\Union $input_type, Type\Union $container_type, bool $ignore_null = false, bool $ignore_false = false, ?TypeComparisonResult $union_comparison_result = null, bool $allow_interface_equality = false, bool $allow_float_int_equality = true) : bool
    {
        ...
        while ($input_type_part = \array_pop($input_atomic_types)) {
            ...
            foreach ($container_type->getAtomicTypes() as $container_type_part) {
                ...
                $is_atomic_contained_by = AtomicTypeComparator::isContainedBy($codebase, $input_type_part, $container_type_part, $allow_interface_equality, $allow_float_int_equality, $atomic_comparison_result);
                ...
                if ($atomic_comparison_result) {
                    ...
                    if ($is_atomic_contained_by && $union_comparison_result && $atomic_comparison_result->replacement_atomic_type) {
                        ...
                        $union_comparison_result->replacement_union_type->addType($atomic_comparison_result->replacement_atomic_type);
                    }
                }
     ...
     *
     */
    public static function isContainedByInPhp(?Type\Union $input_type, Type\Union $container_type) : bool
    {
        if (!$input_type) {
            return false;
        }
     ...
     * Does the input param type match the given param type
     */
    public static function canBeContainedBy(Codebase $codebase, Type\Union $input_type, Type\Union $container_type, bool $ignore_null = false, bool $ignore_false = false, array &$matching_input_keys = []) : bool
    {
        ...
        foreach ($container_type->getAtomicTypes() as $container_type_part) {
            ...
            foreach ($input_type->getAtomicTypes() as $input_type_part) {
                $atomic_comparison_result = new TypeComparisonResult();
                $is_atomic_contained_by = AtomicTypeComparator::isContainedBy($codebase, $input_type_part, $container_type_part, false, false, $atomic_comparison_result);
     ...
     *
     */
    public static function canExpressionTypesBeIdentical(Codebase $codebase, Type\Union $type1, Type\Union $type2, bool $allow_interface_equality = true) : bool
    {
        ...
        foreach ($type1->getAtomicTypes() as $type1_part) {
            foreach ($type2->getAtomicTypes() as $type2_part) {
                $either_contains = AtomicTypeComparator::canBeIdentical($codebase, $type1_part, $type2_part, $allow_interface_equality);
                if ($either_contains) {
                    return true;
                }
            }
