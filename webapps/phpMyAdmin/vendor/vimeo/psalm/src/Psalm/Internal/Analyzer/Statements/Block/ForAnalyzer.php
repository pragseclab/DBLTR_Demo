...
class ForAnalyzer
{
    ...
    public static function analyze(StatementsAnalyzer $statements_analyzer, PhpParser\Node\Stmt\For_ $stmt, Context $context) : ?bool
    {
        ...
        LoopAnalyzer::analyze($statements_analyzer, $stmt->stmts, $stmt->cond, $stmt->loop, $loop_scope, $inner_loop_context);
        ...
        foreach ($stmt->cond as $cond) {
            ...
            if (\count($stmt->init) === 1 && \count($stmt->cond) === 1 && $cond instanceof PhpParser\Node\Expr\BinaryOp && $cond->right instanceof PhpParser\Node\Scalar\LNumber && $cond->left instanceof PhpParser\Node\Expr\Variable && \is_string($cond->left->name) && isset($init_var_types[$cond->left->name]) && $init_var_types[$cond->left->name]->isSingleIntLiteral()) {
        ...
        $context->referenced_var_ids = array_intersect_key($for_context->referenced_var_ids, $context->referenced_var_ids);
