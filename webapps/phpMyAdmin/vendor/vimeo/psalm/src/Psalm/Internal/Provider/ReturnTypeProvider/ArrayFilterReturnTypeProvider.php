    ...
    public static function getFunctionReturnType(FunctionReturnTypeProviderEvent $event) : Type\Union
    {
        ...
        if ($first_arg_array instanceof Type\Atomic\TArray) {
            ...
            if (!isset($call_args[1]) && !$first_arg_array->previous_value_type) {
                ...
                $new_properties = \array_filter(array_map(function ($keyed_type) use($statements_source, $context) {
                    $prev_keyed_type = $keyed_type;
                    $keyed_type = \Psalm\Internal\Type\AssertionReconciler::reconcile('!falsy', clone $keyed_type, '', $statements_source, $context->inside_loop, [], null, $statements_source->getSuppressedIssues());
                    $keyed_type->possibly_undefined = $prev_keyed_type->hasInt() && !$prev_keyed_type->hasLiteralInt() || $prev_keyed_type->hasFloat() || $prev_keyed_type->getId() !== $keyed_type->getId();
                    return $keyed_type;
                }, $first_arg_array->properties), function ($keyed_type) {
        ...
        if (!isset($call_args[1])) {
            $inner_type = \Psalm\Internal\Type\AssertionReconciler::reconcile('!falsy', clone $inner_type, '', $statements_source, $context->inside_loop, [], null, $statements_source->getSuppressedIssues());
            if ($first_arg_array instanceof Type\Atomic\TKeyedArray && $first_arg_array->is_list && $key_type->isSingleIntLiteral() && $key_type->getSingleIntLiteral()->value === 0) {
                return new Type\Union([new Type\Atomic\TList($inner_type)]);
            }
            ...
            return new Type\Union([new Type\Atomic\TArray([$key_type, $inner_type])]);
        }
        if (!isset($call_args[2])) {
            ...
            if ($function_call_arg->value instanceof PhpParser\Node\Scalar\String_ || $function_call_arg->value instanceof PhpParser\Node\Expr\Array_ || $function_call_arg->value instanceof PhpParser\Node\Expr\BinaryOp\Concat) {
                ...
                if ($array_arg && $mapping_function_ids) {
                    $assertions = [];
                    ArrayMapReturnTypeProvider::getReturnTypeFromMappingIds($statements_source, $mapping_function_ids, $context, $function_call_arg, \array_slice($call_args, 0, 1), $assertions);
                    $array_var_id = ExpressionIdentifier::getArrayVarId($array_arg, null, $statements_source);
                    if (isset($assertions[$array_var_id . '[$__fake_offset_var__]'])) {
                        $changed_var_ids = [];
                        $assertions = ['$inner_type' => $assertions[$array_var_id . '[$__fake_offset_var__]']];
                        $reconciled_types = Reconciler::reconcileKeyedTypes($assertions, $assertions, ['$inner_type' => $inner_type], $changed_var_ids, ['$inner_type' => true], $statements_source, $statements_source->getTemplateTypeMap() ?: [], false, new CodeLocation($statements_source, $function_call_arg->value));
                ...
                if ($closure_return_type->isVoid()) {
                    IssueBuffer::accepts(new InvalidReturnType('No return type could be found in the closure passed to array_filter', $code_location), $statements_source->getSuppressedIssues());
                    return Type::getArray();
                }
                ...
                if (count($function_call_stmts) === 1 && count($function_call_arg->value->params)) {
                    ...
                    if ($first_param->variadic === false && $first_param->var instanceof PhpParser\Node\Expr\Variable && is_string($first_param->var->name) && $stmt instanceof PhpParser\Node\Stmt\Return_ && $stmt->expr) {
                        ...
                        $filter_clauses = \Psalm\Internal\Algebra\FormulaGenerator::getFormula($cond_object_id, $cond_object_id, $stmt->expr, $context->self, $statements_source, $codebase);
                        $assertions = \Psalm\Internal\Algebra::getTruthsFromFormula($filter_clauses, $cond_object_id);
                        if (isset($assertions['$' . $first_param->var->name])) {
                            $changed_var_ids = [];
                            $assertions = ['$inner_type' => $assertions['$' . $first_param->var->name]];
                            $reconciled_types = Reconciler::reconcileKeyedTypes($assertions, $assertions, ['$inner_type' => $inner_type], $changed_var_ids, ['$inner_type' => true], $statements_source, $statements_source->getTemplateTypeMap() ?: [], false, new CodeLocation($statements_source, $stmt));
                            if (isset($reconciled_types['$inner_type'])) {
                                $inner_type = $reconciled_types['$inner_type'];
                            }
                        }
