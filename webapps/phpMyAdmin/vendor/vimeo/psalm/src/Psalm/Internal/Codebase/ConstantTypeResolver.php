...
{
    public static function resolve(ClassLikes $classlikes, \Psalm\Internal\Scanner\UnresolvedConstantComponent $c, \Psalm\Internal\Analyzer\StatementsAnalyzer $statements_analyzer = null, array $visited_constant_ids = array()) : Type\Atomic
    {
        ...
        if ($c instanceof UnresolvedConstant\UnresolvedBinaryOp) {
            ...
            if ($c instanceof UnresolvedConstant\UnresolvedAdditionOp || $c instanceof UnresolvedConstant\UnresolvedSubtractionOp || $c instanceof UnresolvedConstant\UnresolvedDivisionOp || $c instanceof UnresolvedConstant\UnresolvedMultiplicationOp || $c instanceof UnresolvedConstant\UnresolvedBitwiseOr || $c instanceof UnresolvedConstant\UnresolvedBitwiseXor || $c instanceof UnresolvedConstant\UnresolvedBitwiseAnd) {
                if (($left instanceof Type\Atomic\TLiteralFloat || $left instanceof Type\Atomic\TLiteralInt) && ($right instanceof Type\Atomic\TLiteralFloat || $right instanceof Type\Atomic\TLiteralInt)) {
        ...
        if ($c instanceof UnresolvedConstant\UnresolvedTernary) {
            ...
            if ($cond instanceof Type\Atomic\TLiteralFloat || $cond instanceof Type\Atomic\TLiteralInt || $cond instanceof Type\Atomic\TLiteralString) {
                if ($cond->value) {
                    return $if ? $if : $cond;
                }
        ...
        if ($c instanceof UnresolvedConstant\ArrayValue) {
            ...
            foreach ($c->entries as $i => $entry) {
                if ($entry->key) {
                    ...
                    if (!$key_type instanceof Type\Atomic\TLiteralInt || $key_type->value !== $i) {
                        $is_list = false;
                    }
                } else {
                    $key_type = new Type\Atomic\TLiteralInt($i);
                }
                if ($key_type instanceof Type\Atomic\TLiteralInt || $key_type instanceof Type\Atomic\TLiteralString) {
                    $key_value = $key_type->value;
                } else {
                    return new Type\Atomic\TArray([Type::getArrayKey(), Type::getMixed()]);
                }
                $value_type = new Type\Union([self::resolve($classlikes, $entry->value, $statements_analyzer, $visited_constant_ids + [$c_id => true])]);
        ...
        if ($c instanceof UnresolvedConstant\ClassConstant) {
            ...
            $found_type = $classlikes->getClassConstantType($c->fqcln, $c->name, ReflectionProperty::IS_PRIVATE, $statements_analyzer, $visited_constant_ids + [$c_id => true]);
            if ($found_type) {
        ...
        if ($c instanceof UnresolvedConstant\ArrayOffsetFetch) {
            ...
            if ($var_type instanceof Type\Atomic\TKeyedArray && ($offset_type instanceof Type\Atomic\TLiteralInt || $offset_type instanceof Type\Atomic\TLiteralString)) {
