...
class ObjectComparator
{
     ...
     *
     */
    public static function isShallowlyContainedBy(Codebase $codebase, Type\Atomic $input_type_part, Type\Atomic $container_type_part, bool $allow_interface_equality, ?TypeComparisonResult $atomic_comparison_result) : bool
    {
        ...
        if ($input_type_part instanceof TTemplateParam) {
            foreach ($input_type_part->as->getAtomicTypes() as $g) {
                if ($g instanceof TNamedObject && $g->extra_types) {
                    $intersection_input_types = array_merge($intersection_input_types, $g->extra_types);
                }
            }
        }
        ...
        if ($container_type_part instanceof TTemplateParam) {
            foreach ($container_type_part->as->getAtomicTypes() as $g) {
                if ($g instanceof TNamedObject && $g->extra_types) {
                    $intersection_container_types = array_merge($intersection_container_types, $g->extra_types);
                }
            }
        }
        foreach ($intersection_container_types as $container_type_key => $intersection_container_type) {
            ...
            if ($intersection_container_type instanceof TIterable) {
                ...
                if (!$allow_interface_equality) {
                    ...
                    foreach ($intersection_input_types as $intersection_input_type) {
                        if ($intersection_input_type instanceof TTemplateParam && (\substr($intersection_container_type->defining_class, 0, 3) === 'fn-' || \substr($intersection_input_type->defining_class, 0, 3) === 'fn-')) {
                            if (\substr($intersection_input_type->defining_class, 0, 3) === 'fn-' && \substr($intersection_container_type->defining_class, 0, 3) === 'fn-' && $intersection_input_type->defining_class !== $intersection_container_type->defining_class) {
                                continue 2;
                            }
            ...
            foreach ($intersection_input_types as $intersection_input_key => $intersection_input_type) {
                ...
                if ($intersection_container_type instanceof TTemplateParam && $intersection_input_type instanceof TTemplateParam) {
                    if ($intersection_container_type->param_name !== $intersection_input_type->param_name || $intersection_container_type->defining_class !== $intersection_input_type->defining_class && \substr($intersection_input_type->defining_class, 0, 3) !== 'fn-' && \substr($intersection_container_type->defining_class, 0, 3) !== 'fn-') {
                ...
                if (!$intersection_container_type instanceof TTemplateParam || $intersection_input_type instanceof TTemplateParam) {
                    if ($intersection_container_type_lower === $intersection_input_type_lower) {
                        if ($container_was_static && !$input_was_static && !$intersection_input_type instanceof TTemplateParam) {
                    ...
                    if ($allow_interface_equality && $container_type_is_interface && ($input_type_is_interface || !isset($intersection_container_types[$intersection_input_key]))) {
                        continue 2;
                    }
                    if ($codebase->classExists($intersection_input_type_lower) && $codebase->classOrInterfaceExists($intersection_container_type_lower) && $codebase->classExtendsOrImplements($intersection_input_type_lower, $intersection_container_type_lower)) {
