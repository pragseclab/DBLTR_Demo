    ...
    public static function simplifyCNF(array $clauses) : array
    {
        ...
        foreach ($cloned_clauses as $clause_a_hash => $clause_a) {
                    ...
                    if (array_keys($clause_a->possibilities) === array_keys($clause_b->possibilities)) {
                        ...
                        foreach ($clause_a->possibilities as $key => $a_possibilities) {
                            ...
                            if (count($a_possibilities) === 1 && count($b_possibilities) === 1) {
                                if ($a_possibilities[0] === '!' . $b_possibilities[0] || $b_possibilities[0] === '!' . $a_possibilities[0]) {
                                    $opposing_keys[] = $key;
                                    continue;
                                }
                            }
            ...
            foreach ($cloned_clauses as $clause_b_hash => $clause_b) {
                ...
                if (isset($clause_b->possibilities[$clause_var]) && in_array($negated_clause_type, $clause_b->possibilities[$clause_var], true)) {
                    $clause_var_possibilities = array_values(array_filter($clause_b->possibilities[$clause_var], function (string $possible_type) use($negated_clause_type) : bool {
        ...
        foreach ($clauses as $clause) {
            ...
            foreach ($clause->possibilities as $var => $possible_types) {
                ...
                if (count($possible_types) === 1) {
                    ...
                    $things_that_can_be_said = array_filter($possible_types, function (string $possible_type) : bool {
        ...
        if (!$clause->wedge) {
            ...
            foreach ($clause->impossibilities as $var => $impossible_types) {
                foreach ($impossible_types as $impossible_type) {
                    $seed_clause = new Clause([$var => [$impossible_type]], $clause->creating_conditional_id, $clause->creating_object_id);
                ...
                foreach ($clause->impossibilities as $var => $impossible_types) {
                    foreach ($impossible_types as $impossible_type) {
                        ...
                        if (isset($grouped_clause->possibilities[$var])) {
                            $new_clause_possibilities[$var] = array_values(array_unique(array_merge([$impossible_type], $new_clause_possibilities[$var])));
                            $removed_indexes = [];
                            ...
                            if ($removed_indexes) {
                                $new_possibilities = array_values(\array_diff_key($new_clause_possibilities[$var], $removed_indexes));
            ...
            foreach ($right_clauses as $right_clause) {
                ...
                foreach ($possibilities as $var_possibilities) {
                    if (count($var_possibilities) === 2) {
                        if ($var_possibilities[0] === '!' . $var_possibilities[1] || $var_possibilities[1] === '!' . $var_possibilities[0]) {
                            continue 2;
                        }
                    }
                }
