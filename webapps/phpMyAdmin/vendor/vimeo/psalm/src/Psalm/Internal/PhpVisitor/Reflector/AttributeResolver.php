...
class AttributeResolver
{
    public static function resolve(Codebase $codebase, FileScanner $file_scanner, FileStorage $file_storage, Aliases $aliases, PhpParser\Node\Attribute $stmt, ?string $fq_classlike_name) : AttributeStorage
    {
        ...
        foreach ($stmt->args as $arg_node) {
            ...
            $const_type = SimpleTypeInferer::infer($codebase, new \Psalm\Internal\Provider\NodeDataProvider(), $arg_node->value, $aliases, null, [], $fq_classlike_name);
            if (!$const_type) {
                $const_type = ExpressionResolver::getUnresolvedClassConstExpr($arg_node->value, $aliases, $fq_classlike_name);
            }
            if (!$const_type) {
                $const_type = Type::getMixed();
            }
            $args[] = new \Psalm\Storage\AttributeArg($key, $const_type, new CodeLocation($file_scanner, $arg_node->value));
        }
