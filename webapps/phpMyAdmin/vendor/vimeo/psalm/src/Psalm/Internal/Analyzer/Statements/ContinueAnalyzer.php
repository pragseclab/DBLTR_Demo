...
class ContinueAnalyzer
{
     ...
     * @return false|null
     */
    public static function analyze(StatementsAnalyzer $statements_analyzer, PhpParser\Node\Stmt\Continue_ $stmt, Context $context) : ?bool
    {
        $count = $stmt->num && $stmt->num instanceof PhpParser\Node\Scalar\LNumber ? $stmt->num->value : 1;
        ...
        if ($loop_scope === null) {
            ...
            foreach ($redefined_vars as $var => $type) {
                if ($type->hasMixed()) {
                    ...
                    $loop_scope->possibly_redefined_loop_vars[$var] = Type::combineUnionTypes($type, $loop_scope->possibly_redefined_loop_vars[$var]);
                } else {
                    $loop_scope->possibly_redefined_loop_vars[$var] = $type;
                }
            }
