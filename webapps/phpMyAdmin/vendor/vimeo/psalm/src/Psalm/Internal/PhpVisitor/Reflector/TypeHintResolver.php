...
class TypeHintResolver
{
     ...
     */
    public static function resolve(PhpParser\NodeAbstract $hint, CodebaseScanner $scanner, FileStorage $file_storage, ?ClassLikeStorage $classlike_storage, Aliases $aliases, int $php_major_version, int $php_minor_version) : Type\Union
    {
        if ($hint instanceof PhpParser\Node\UnionType) {
            ...
            foreach ($hint->types as $atomic_typehint) {
                $resolved_type = self::resolve($atomic_typehint, $scanner, $file_storage, $classlike_storage, $aliases, $php_major_version, $php_minor_version);
                if (!$type) {
                    $type = $resolved_type;
                } else {
        ...
        $type = Type::parseString($fq_type_string, [$php_major_version, $php_minor_version], []);
        if ($type_string) {
