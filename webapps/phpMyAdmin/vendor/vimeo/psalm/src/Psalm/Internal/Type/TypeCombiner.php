...
class TypeCombiner
{
    ...
    public static function combine(array $types, ?Codebase $codebase = null, bool $overwrite_empty_array = false, bool $allow_mixed_union = true, int $literal_limit = 500) : Union
    {
        ...
        if (count($combination->value_types) === 1 && !count($combination->objectlike_entries) && !$combination->array_type_params && !$combination->builtin_type_params && !$combination->object_type_params && !$combination->named_object_types && !$combination->strings && !$combination->class_string_types && !$combination->ints && !$combination->floats) {
        ...
        if ($combination->array_type_params && (isset($combination->named_object_types['Traversable']) || isset($combination->builtin_type_params['Traversable'])) && ($codebase && !$codebase->config->allow_phpstorm_generics || isset($combination->builtin_type_params['Traversable']) || isset($combination->named_object_types['Traversable']) && $combination->named_object_types['Traversable']->from_docblock) && !$combination->extra_types) {
        ...
        if ($combination->objectlike_entries) {
            $new_types = self::handleKeyedArrayEntries($combination, $overwrite_empty_array);
        }
        if ($combination->array_type_params) {
            ...
            $new_types[] = self::getArrayTypeFromGenericParams($codebase, $combination, $overwrite_empty_array, $allow_mixed_union, $type, $combination->array_type_params);
        }
        ...
        foreach ($combination->value_types as $type) {
            if ($type instanceof TMixed && $combination->mixed_from_loop_isset && (count($combination->value_types) > 1 + $has_empty || count($new_types) > $has_empty)) {
                continue;
            }
            if ($type instanceof TEmpty && (count($combination->value_types) > 1 || count($new_types))) {
                continue;
            }
            $new_types[] = $type;
        }
    ...
    private static function scrapeTypeProperties(Atomic $type, TypeCombination $combination, ?Codebase $codebase, bool $overwrite_empty_array, bool $allow_mixed_union, int $literal_limit) : ?Union
    {
        ...
        if ($type instanceof TNamedObject || $type instanceof TTemplateParam || $type instanceof TIterable || $type instanceof Type\Atomic\TObjectWithProperties) {
            if ($type->extra_types) {
        ...
        if ($type instanceof TArray && $type_key === 'array') {
            ...
            foreach ($type->type_params as $i => $type_param) {
                if (isset($combination->array_type_params[$i])) {
                    /** @psalm-suppress PropertyTypeCoercion */
                    $combination->array_type_params[$i] = Type::combineUnionTypes($combination->array_type_params[$i], $type_param, $codebase, $overwrite_empty_array);
                } else {
                    /** @psalm-suppress PropertyTypeCoercion */
        ...
        if ($type instanceof TGenericObject && ($type->value === 'Traversable' || $type->value === 'Generator') || $type instanceof TIterable && $type->has_docblock_params || $type instanceof TArray && $type_key === 'iterable') {
            foreach ($type->type_params as $i => $type_param) {
                if (isset($combination->builtin_type_params[$type_key][$i])) {
                    /** @psalm-suppress PropertyTypeCoercion */
                    $combination->builtin_type_params[$type_key][$i] = Type::combineUnionTypes($combination->builtin_type_params[$type_key][$i], $type_param, $codebase, $overwrite_empty_array);
                } else {
                    /** @psalm-suppress PropertyTypeCoercion */
        ...
        if ($type instanceof TGenericObject) {
            foreach ($type->type_params as $i => $type_param) {
                if (isset($combination->object_type_params[$type_key][$i])) {
                    /** @psalm-suppress PropertyTypeCoercion */
                    $combination->object_type_params[$type_key][$i] = Type::combineUnionTypes($combination->object_type_params[$type_key][$i], $type_param, $codebase, $overwrite_empty_array);
                } else {
                    /** @psalm-suppress PropertyTypeCoercion */
        ...
        if ($type instanceof TKeyedArray) {
            ...
            if ($type->previous_value_type) {
                if (!$combination->objectlike_value_type) {
                    ...
                    $combination->objectlike_value_type = Type::combineUnionTypes($type->previous_value_type, $combination->objectlike_value_type, $codebase, $overwrite_empty_array);
                }
            }
            if ($type->previous_key_type) {
                if (!$combination->objectlike_key_type) {
                    ...
                    $combination->objectlike_key_type = Type::combineUnionTypes($type->previous_key_type, $combination->objectlike_key_type, $codebase, $overwrite_empty_array);
                }
            }
            $has_defined_keys = false;
            foreach ($type->properties as $candidate_property_name => $candidate_property_type) {
                $value_type = isset($combination->objectlike_entries[$candidate_property_name]) ? $combination->objectlike_entries[$candidate_property_name] : null;
                if (!$value_type) {
                    ...
                    $combination->objectlike_entries[$candidate_property_name]->possibly_undefined = $existing_objectlike_entries || $candidate_property_type->possibly_undefined;
                } else {
                ...
                if (is_string($candidate_property_name) && isset($type->class_strings[$candidate_property_name])) {
            ...
            if ($type instanceof Type\Atomic\TTemplateParamClass) {
                ...
                if ($combination->strings !== null && count($combination->strings) < $literal_limit) {
                    ...
                    if (isset($combination->value_types['string']) && $combination->value_types['string'] instanceof TNumericString && \is_numeric($type->value)) {
                        ...
                        if (isset($combination->value_types['string']) && $combination->value_types['string'] instanceof Type\Atomic\TLowercaseString && \strtolower($type->value) === $type->value) {
                            // do nothing
                        } elseif (isset($combination->value_types['string']) && $combination->value_types['string'] instanceof Type\Atomic\TNonFalsyString && $type->value) {
                            // do nothing
                        } elseif (isset($combination->value_types['string']) && $combination->value_types['string'] instanceof Type\Atomic\TNonEmptyString && $type->value !== '') {
                            // do nothing
                        } else {
                ...
                if (!isset($combination->value_types['string'])) {
                    ...
                    if (get_class($type) === TString::class) {
                        $combination->value_types['string'] = $type;
                    } elseif ($combination->value_types['string'] instanceof TTraitString && $type instanceof TClassString) {
        ...
        if ($type instanceof TInt) {
            ...
            if ($type instanceof TLiteralInt) {
                ...
                if ($combination->ints !== null && count($combination->ints) < $literal_limit) {
                    ...
                    $combination->ints[$type_key] = $type;
                    $all_nonnegative = !array_filter($combination->ints, function ($int) : bool {
                        return $int->value < 0;
                    });
                    $combination->ints = null;
                    if (!isset($combination->value_types['int'])) {
                        $combination->value_types['int'] = $all_nonnegative ? new TPositiveInt() : new TInt();
                    } elseif ($combination->value_types['int'] instanceof TPositiveInt && !$all_nonnegative) {
            ...
            if ($had_zero && isset($combination->value_types['int']) && $combination->value_types['int'] instanceof TPositiveInt) {
     ...
     * @return list<Atomic>
     */
    private static function handleKeyedArrayEntries(TypeCombination $combination, bool $overwrite_empty_array) : array
    {
        $new_types = [];
        ...
        if (!$combination->array_type_params || $combination->array_type_params[1]->isEmpty()) {
            if (!$overwrite_empty_array && ($combination->array_type_params && ($combination->array_type_params[1]->isEmpty() || $combination->array_type_params[1]->isMixed()))) {
                foreach ($combination->objectlike_entries as $objectlike_entry) {
                    $objectlike_entry->possibly_undefined = true;
                }
            }
            if ($combination->objectlike_value_type && $combination->objectlike_value_type->isMixed()) {
                $combination->objectlike_entries = array_filter($combination->objectlike_entries, function (Type\Union $type) : bool {
     ...
     */
    private static function getArrayTypeFromGenericParams(?Codebase $codebase, TypeCombination $combination, bool $overwrite_empty_array, bool $allow_mixed_union, Atomic $type, array $generic_type_params) : Atomic
    {
        if ($combination->objectlike_entries) {
            ...
            foreach ($combination->objectlike_entries as $property_name => $property_type) {
                if ($objectlike_generic_type) {
                    $objectlike_generic_type = Type::combineUnionTypes($property_type, $objectlike_generic_type, $codebase, $overwrite_empty_array);
                } else {
            ...
            if ($combination->objectlike_value_type) {
                $objectlike_generic_type = Type::combineUnionTypes($combination->objectlike_value_type, $objectlike_generic_type, $codebase, $overwrite_empty_array);
            }
        ...
        if ($combination->all_arrays_callable) {
        ...
        } elseif ($combination->array_always_filled || $combination->array_sometimes_filled && $overwrite_empty_array || $combination->objectlike_entries && $combination->objectlike_sealed && $overwrite_empty_array) {
            if ($combination->all_arrays_lists) {
                if ($combination->objectlike_entries && $combination->objectlike_sealed) {
            ...
            if ($combination->all_arrays_class_string_maps && count($combination->class_string_map_as_types) === 1 && count($combination->class_string_map_names) === 1) {
                $array_type = new Type\Atomic\TClassStringMap(array_keys($combination->class_string_map_names)[0], array_values($combination->class_string_map_as_types)[0], $generic_type_params[1]);
