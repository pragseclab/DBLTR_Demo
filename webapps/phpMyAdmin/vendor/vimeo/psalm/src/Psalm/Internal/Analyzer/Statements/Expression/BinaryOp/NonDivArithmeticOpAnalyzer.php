...
{
    public static function analyze(?StatementsSource $statements_source, \Psalm\Internal\Provider\NodeDataProvider $nodes, PhpParser\Node\Expr $left, PhpParser\Node\Expr $right, PhpParser\Node $parent, ?Type\Union &$result_type = null, ?Context $context = null) : void
    {
        ...
        if ($left_type && $right_type) {
            if ($left_type->isNull()) {
                if ($statements_source && IssueBuffer::accepts(new NullOperand('Left operand cannot be null', new CodeLocation($statements_source, $left)), $statements_source->getSuppressedIssues())) {
                    // fall through
                }
            ...
            if ($left_type->isNullable() && !$left_type->ignore_nullable_issues) {
                if ($statements_source && IssueBuffer::accepts(new PossiblyNullOperand('Left operand cannot be nullable, got ' . $left_type, new CodeLocation($statements_source, $left)), $statements_source->getSuppressedIssues())) {
                    // fall through
                }
            }
            if ($right_type->isNull()) {
                if ($statements_source && IssueBuffer::accepts(new NullOperand('Right operand cannot be null', new CodeLocation($statements_source, $right)), $statements_source->getSuppressedIssues())) {
                    // fall through
                }
                $result_type = Type::getMixed();
                return;
            ...
            if ($right_type->isNullable() && !$right_type->ignore_nullable_issues) {
                if ($statements_source && IssueBuffer::accepts(new PossiblyNullOperand('Right operand cannot be nullable, got ' . $right_type, new CodeLocation($statements_source, $right)), $statements_source->getSuppressedIssues())) {
                    // fall through
                }
            }
            if ($left_type->isFalse()) {
                if ($statements_source && IssueBuffer::accepts(new FalseOperand('Left operand cannot be false', new CodeLocation($statements_source, $left)), $statements_source->getSuppressedIssues())) {
                    // fall through
                }
                return;
            }
            if ($left_type->isFalsable() && !$left_type->ignore_falsable_issues) {
                if ($statements_source && IssueBuffer::accepts(new PossiblyFalseOperand('Left operand cannot be falsable, got ' . $left_type, new CodeLocation($statements_source, $left)), $statements_source->getSuppressedIssues())) {
                    // fall through
                }
            }
            if ($right_type->isFalse()) {
                if ($statements_source && IssueBuffer::accepts(new FalseOperand('Right operand cannot be false', new CodeLocation($statements_source, $right)), $statements_source->getSuppressedIssues())) {
                    // fall through
                }
                return;
            }
            if ($right_type->isFalsable() && !$right_type->ignore_falsable_issues) {
                if ($statements_source && IssueBuffer::accepts(new PossiblyFalseOperand('Right operand cannot be falsable, got ' . $right_type, new CodeLocation($statements_source, $right)), $statements_source->getSuppressedIssues())) {
                    // fall through
                }
            }
            ...
            foreach ($left_type->getAtomicTypes() as $left_type_part) {
                foreach ($right_type->getAtomicTypes() as $right_type_part) {
                    $candidate_result_type = self::analyzeNonDivOperands($statements_source, $codebase, $config, $context, $left, $right, $parent, $left_type_part, $right_type_part, $invalid_left_messages, $invalid_right_messages, $has_valid_left_operand, $has_valid_right_operand, $has_string_increment, $result_type);
            ...
            if ($invalid_left_messages && $statements_source) {
                ...
                if ($has_valid_left_operand) {
                    if (IssueBuffer::accepts(new PossiblyInvalidOperand($first_left_message, new CodeLocation($statements_source, $left)), $statements_source->getSuppressedIssues())) {
                        // fall through
                    }
                } else {
                    if (IssueBuffer::accepts(new InvalidOperand($first_left_message, new CodeLocation($statements_source, $left)), $statements_source->getSuppressedIssues())) {
                        // fall through
                    }
                }
            }
            if ($invalid_right_messages && $statements_source) {
                ...
                if ($has_valid_right_operand) {
                    if (IssueBuffer::accepts(new PossiblyInvalidOperand($first_right_message, new CodeLocation($statements_source, $right)), $statements_source->getSuppressedIssues())) {
                        // fall through
                    }
                } else {
                    if (IssueBuffer::accepts(new InvalidOperand($first_right_message, new CodeLocation($statements_source, $right)), $statements_source->getSuppressedIssues())) {
                        // fall through
                    }
                }
            }
            if ($has_string_increment && $statements_source) {
                if (IssueBuffer::accepts(new StringIncrement('Possibly unintended string increment', new CodeLocation($statements_source, $left)), $statements_source->getSuppressedIssues())) {
                    // fall through
                }
            }
        }
     ...
     */
    private static function analyzeNonDivOperands(?StatementsSource $statements_source, ?\Psalm\Codebase $codebase, Config $config, ?Context $context, PhpParser\Node\Expr $left, PhpParser\Node\Expr $right, PhpParser\Node $parent, Type\Atomic $left_type_part, Type\Atomic $right_type_part, array &$invalid_left_messages, array &$invalid_right_messages, bool &$has_valid_left_operand, bool &$has_valid_right_operand, bool &$has_string_increment, Type\Union &$result_type = null) : ?Type\Union
    {
        if ($left_type_part instanceof TLiteralInt && $right_type_part instanceof TLiteralInt && ($left instanceof PhpParser\Node\Scalar || $left instanceof PhpParser\Node\Expr\ConstFetch || $left instanceof PhpParser\Node\Expr\ClassConstFetch || $left instanceof PhpParser\Node\Expr\BinaryOp) && ($right instanceof PhpParser\Node\Scalar || $right instanceof PhpParser\Node\Expr\ConstFetch || $right instanceof PhpParser\Node\Expr\ClassConstFetch || $right instanceof PhpParser\Node\Expr\BinaryOp)) {
        ...
        if ($left_type_part instanceof Type\Atomic\TString && $right_type_part instanceof TInt && $parent instanceof PhpParser\Node\Expr\PostInc) {
        ...
        if ($left_type_part instanceof TMixed || $right_type_part instanceof TMixed || $left_type_part instanceof TTemplateParam || $right_type_part instanceof TTemplateParam) {
            if ($statements_source && $codebase && $context) {
                if (!$context->collect_initializations && !$context->collect_mutations && $statements_source->getFilePath() === $statements_source->getRootFilePath() && (!($source = $statements_source->getSource()) instanceof \Psalm\Internal\Analyzer\FunctionLikeAnalyzer || !$source->getSource() instanceof \Psalm\Internal\Analyzer\TraitAnalyzer)) {
            ...
            if ($left_type_part instanceof TMixed || $left_type_part instanceof TTemplateParam) {
                if ($statements_source && IssueBuffer::accepts(new MixedOperand('Left operand cannot be mixed', new CodeLocation($statements_source, $left)), $statements_source->getSuppressedIssues())) {
                    // fall through
                }
            } else {
                if ($statements_source && IssueBuffer::accepts(new MixedOperand('Right operand cannot be mixed', new CodeLocation($statements_source, $right)), $statements_source->getSuppressedIssues())) {
                    // fall through
                }
            }
        ...
        if ($statements_source && $codebase && $context) {
            if (!$context->collect_initializations && !$context->collect_mutations && $statements_source->getFilePath() === $statements_source->getRootFilePath() && (!($parent_source = $statements_source->getSource()) instanceof \Psalm\Internal\Analyzer\FunctionLikeAnalyzer || !$parent_source->getSource() instanceof \Psalm\Internal\Analyzer\TraitAnalyzer)) {
        ...
        if ($left_type_part instanceof TArray || $right_type_part instanceof TArray || $left_type_part instanceof TKeyedArray || $right_type_part instanceof TKeyedArray || $left_type_part instanceof TList || $right_type_part instanceof TList) {
            if (!$right_type_part instanceof TArray && !$right_type_part instanceof TKeyedArray && !$right_type_part instanceof TList || !$left_type_part instanceof TArray && !$left_type_part instanceof TKeyedArray && !$left_type_part instanceof TList) {
                if (!$left_type_part instanceof TArray && !$left_type_part instanceof TKeyedArray && !$left_type_part instanceof TList) {
                ...
                if ($left_type_part instanceof TArray || $left_type_part instanceof TKeyedArray || $left_type_part instanceof TList) {
                    $has_valid_left_operand = true;
                } elseif ($right_type_part instanceof TArray || $right_type_part instanceof TKeyedArray || $right_type_part instanceof TList) {
                    $has_valid_right_operand = true;
                }
            ...
            if ($left_type_part instanceof TKeyedArray && $right_type_part instanceof TKeyedArray) {
                ...
                foreach ($right_type_part->properties as $key => $type) {
                    if (!isset($properties[$key])) {
                        ...
                        $properties[$key] = Type::combineUnionTypes($properties[$key], $type, $codebase);
                ...
                $result_type_member = TypeCombiner::combine([$left_type_part, $right_type_part], $codebase, true);
            }
            if (!$result_type) {
                $result_type = $result_type_member;
            } else {
        ...
        if ($left_type_part instanceof TNamedObject && strtolower($left_type_part->value) === 'gmp' || $right_type_part instanceof TNamedObject && strtolower($right_type_part->value) === 'gmp') {
            if ($left_type_part instanceof TNamedObject && strtolower($left_type_part->value) === 'gmp' && ($right_type_part instanceof TNamedObject && strtolower($right_type_part->value) === 'gmp' || ($right_type_part->isNumericType() || $right_type_part instanceof TMixed)) || $right_type_part instanceof TNamedObject && strtolower($right_type_part->value) === 'gmp' && ($left_type_part instanceof TNamedObject && strtolower($left_type_part->value) === 'gmp' || ($left_type_part->isNumericType() || $left_type_part instanceof TMixed))) {
                if (!$result_type) {
                    ...
                    $result_type = Type::combineUnionTypes(new Type\Union([new TNamedObject('GMP')]), $result_type);
                }
            } else {
                if ($statements_source && IssueBuffer::accepts(new InvalidOperand('Cannot add GMP to non-numeric type', new CodeLocation($statements_source, $parent)), $statements_source->getSuppressedIssues())) {
                    // fall through
                }
            }
            return null;
        ...
        if ($left_type_part->isNumericType() || $right_type_part->isNumericType()) {
            ...
            if ($left_type_part instanceof TInt && $right_type_part instanceof TInt) {
                if ($parent instanceof PhpParser\Node\Expr\BinaryOp\Div) {
                    ...
                    $left_is_positive = $left_type_part instanceof TPositiveInt || $left_type_part instanceof TLiteralInt && $left_type_part->value > 0;
                    $right_is_positive = $right_type_part instanceof TPositiveInt || $right_type_part instanceof TLiteralInt && $right_type_part->value > 0;
                    ...
                    if ($parent instanceof PhpParser\Node\Expr\BinaryOp\Mod) {
                        if ($always_positive) {
                            if ($right_type_part instanceof TLiteralInt && $right_type_part->value === 1) {
                                ...
                                $result_type = new Type\Union([new Type\Atomic\TPositiveInt(), new TLiteralInt(0)]);
                            }
                        } else {
                            $result_type = Type::getInt();
                        }
                        ...
                        $result_type = Type::combineUnionTypes($always_positive ? Type::getPositiveInt(true) : Type::getInt(true), $result_type);
                    }
                }
                $has_valid_right_operand = true;
                $has_valid_left_operand = true;
            ...
            if ($left_type_part instanceof TFloat && $right_type_part instanceof TInt || $left_type_part instanceof TInt && $right_type_part instanceof TFloat) {
                if ($config->strict_binary_operands) {
                    if ($statements_source && IssueBuffer::accepts(new InvalidOperand('Cannot add ints to floats', new CodeLocation($statements_source, $parent)), $statements_source->getSuppressedIssues())) {
                        // fall through
                    }
                }
                if ($parent instanceof PhpParser\Node\Expr\BinaryOp\Mod) {
            ...
            if ($left_type_part->isNumericType() && $right_type_part->isNumericType()) {
                if ($config->strict_binary_operands) {
                    if ($statements_source && IssueBuffer::accepts(new InvalidOperand('Cannot add numeric types together, please cast explicitly', new CodeLocation($statements_source, $parent)), $statements_source->getSuppressedIssues())) {
                        // fall through
                    }
                }
                if ($parent instanceof PhpParser\Node\Expr\BinaryOp\Mod) {
