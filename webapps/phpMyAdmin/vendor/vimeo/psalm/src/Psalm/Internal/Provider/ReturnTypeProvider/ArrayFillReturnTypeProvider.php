    ...
    public static function getFunctionReturnType(FunctionReturnTypeProviderEvent $event) : Type\Union
    {
        ...
        if ($third_arg_type) {
            if ($first_arg_type && $first_arg_type->isSingleIntLiteral() && $first_arg_type->getSingleIntLiteral()->value === 0) {
                return new Type\Union([new Type\Atomic\TNonEmptyList(clone $third_arg_type)]);
            }
            return new Type\Union([new Type\Atomic\TArray([Type::getInt(), clone $third_arg_type])]);
        }
        return new Type\Union([new Type\Atomic\TArray([Type::getInt(), Type::getMixed()])]);
    }
}