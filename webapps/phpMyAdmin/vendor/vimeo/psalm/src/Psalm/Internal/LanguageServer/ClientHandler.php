    ...
    public function request(string $method, $params) : Promise
    {
        ...
        return call(
            ...
            function () use($id, $method, $params) : \Generator {
                (yield $this->protocolWriter->write(new Message(new AdvancedJsonRpc\Request($id, $method, (object) $params))));
                ...
                $listener = function (Message $msg) use($id, $deferred, &$listener) : void {
                     ...
                     */
                    if ($msg->body && AdvancedJsonRpc\Response::isResponse($msg->body) && $msg->body->id === $id) {
                        // Received a response
