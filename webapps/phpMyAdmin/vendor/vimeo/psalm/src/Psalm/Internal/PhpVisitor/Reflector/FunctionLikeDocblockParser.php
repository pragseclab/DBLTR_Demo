...
class FunctionLikeDocblockParser
{
    ...
    public static function parse(PhpParser\Comment\Doc $comment) : FunctionDocblockComment
    {
        ...
        if (isset($parsed_docblock->combined_tags['return'])) {
            self::extractReturnType($comment, $parsed_docblock->combined_tags['return'], $info);
        }
            ...
            foreach ($parsed_docblock->combined_tags['param'] as $offset => $param) {
                ...
                if (count($line_parts) > 1) {
                    if (preg_match('/^&?(\\.\\.\\.)?&?\\$[A-Za-z0-9_]+,?$/', $line_parts[1]) && $line_parts[0][0] !== '{') {
                        ...
                        $info->params[] = ['name' => trim($line_parts[1]), 'type' => $line_parts[0], 'line_number' => $comment->getStartLine() + substr_count($comment_text, "\n", 0, $offset), 'start' => $start, 'end' => $end];
                    }
                } else {
        ...
        if (isset($parsed_docblock->tags['throws'])) {
            foreach ($parsed_docblock->tags['throws'] as $offset => $throws_entry) {
                ...
                $info->throws[] = [$throws_class, $offset + $comment->getStartFilePos(), $comment->getStartLine() + substr_count($comment->getText(), "\n", 0, $offset)];
            }
        }
        if (strpos(strtolower($parsed_docblock->description), '@inheritdoc') !== false || isset($parsed_docblock->tags['inheritdoc']) || isset($parsed_docblock->tags['inheritDoc'])) {
        ...
        if (isset($parsed_docblock->combined_tags['template'])) {
            foreach ($parsed_docblock->combined_tags['template'] as $template_line) {
                ...
                if (count($template_type) > 1 && in_array(strtolower($template_type[0]), ['as', 'super', 'of'], true)) {
                    ...
                    $info->templates[] = [$template_name, $template_modifier, implode(' ', $template_type), false];
                } else {
     ...
     */
    private static function extractReturnType(PhpParser\Comment\Doc $comment, array $return_specials, FunctionDocblockComment $info) : void
    {
