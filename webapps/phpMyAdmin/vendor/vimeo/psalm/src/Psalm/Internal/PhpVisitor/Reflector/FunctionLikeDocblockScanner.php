...
class FunctionLikeDocblockScanner
{
     ...
     */
    public static function addDocblockInfo(Codebase $codebase, FileScanner $file_scanner, FileStorage $file_storage, Aliases $aliases, array $type_aliases, ?ClassLikeStorage $classlike_storage, array $existing_function_template_types, FunctionLikeStorage $storage, PhpParser\Node\FunctionLike $stmt, \Psalm\Internal\Scanner\FunctionDocblockComment $docblock_info, bool $is_functionlike_override, bool $fake_method, string $cased_function_id) : void
    {
        ...
        if ($docblock_info->internal && !$docblock_info->psalm_internal && $aliases->namespace) {
            $storage->internal = explode('\\', $aliases->namespace)[0];
        } elseif (!$classlike_storage || $docblock_info->psalm_internal && strlen($docblock_info->psalm_internal) > strlen($classlike_storage->internal)) {
        ...
        foreach ($docblock_info->throws as [$throw, $offset, $line]) {
            $throw_location = new CodeLocation\DocblockTypeLocation($file_scanner, $offset, $offset + \strlen($throw), $line);
        ...
        foreach ($docblock_info->globals as $global) {
            try {
                ...
                $storage->docblock_issues[] = new InvalidDocblock($e->getMessage() . ' in docblock for ' . $cased_function_id, new CodeLocation($file_scanner, $stmt, null, true));
                continue;
            }
        }
        ...
        if ($storage instanceof MethodStorage) {
            $storage->has_docblock_param_types = (bool) array_filter($storage->params, function (FunctionLikeParameter $p) : bool {
        ...
        if ($docblock_info->self_out && $storage instanceof MethodStorage) {
            $out_type = TypeParser::parseTokens(TypeTokenizer::getFullyQualifiedTokens($docblock_info->self_out['type'], $aliases, $function_template_types + $class_template_types, $type_aliases), null, $function_template_types + $class_template_types, $type_aliases);
        ...
        foreach ($docblock_info->taint_source_types as $taint_source_type) {
            if ($taint_source_type === 'input') {
                $storage->taint_source_types = array_merge($storage->taint_source_types, \Psalm\Type\TaintKindGroup::ALL_INPUT);
            } else {
                ...
                foreach ($storage->params as $j => $param_storage) {
                    if ('$' . $param_storage->name === $token_body) {
                        if (!isset($param_type_mapping[$token_body])) {
                            ...
                            $template_as_type = $param_storage->type ? clone $param_storage->type : Type::getMixed();
                            ...
                            $param_storage->type = new Type\Union([new Type\Atomic\TTemplateParam($template_name, $template_as_type, $template_function_id)]);
                        }
    ...
    private static function getAssertionParts(Codebase $codebase, FileScanner $file_scanner, FileStorage $file_storage, Aliases $aliases, PhpParser\Node\FunctionLike $stmt, FunctionLikeStorage $storage, string $assertion_type, array $class_template_types, array $function_template_types, array $type_aliases, ?string $self_fqcln) : ?array
    {
        ...
        try {
            $namespaced_type = TypeParser::parseTokens(TypeTokenizer::getFullyQualifiedTokens($assertion_type, $aliases, $function_template_types + $class_template_types, $type_aliases, $self_fqcln, null, true));
        } catch (TypeParseTreeException $e) {
            $storage->docblock_issues[] = new InvalidDocblock('Invalid @psalm-assert union type ' . $e, new CodeLocation($file_scanner, $stmt, null, true));
            return null;
        }
        if ($prefix && count($namespaced_type->getAtomicTypes()) > 1) {
            $storage->docblock_issues[] = new InvalidDocblock('Docblock assertions cannot contain | characters together with ' . $prefix, new CodeLocation($file_scanner, $stmt, null, true));
            return null;
        }
        ...
        foreach ($namespaced_type->getAtomicTypes() as $namespaced_type_part) {
            if ($namespaced_type_part instanceof Type\Atomic\TAssertionFalsy || $namespaced_type_part instanceof Type\Atomic\TScalarClassConstant || $namespaced_type_part instanceof Type\Atomic\TList && $namespaced_type_part->type_param->isMixed() || $namespaced_type_part instanceof Type\Atomic\TArray && $namespaced_type_part->type_params[0]->isArrayKey() && $namespaced_type_part->type_params[1]->isMixed() || $namespaced_type_part instanceof Type\Atomic\TIterable && $namespaced_type_part->type_params[0]->isMixed() && $namespaced_type_part->type_params[1]->isMixed()) {
    ...
    private static function improveParamsFromDocblock(Codebase $codebase, FileScanner $file_scanner, FileStorage $file_storage, Aliases $aliases, array $type_aliases, ?ClassLikeStorage $classlike_storage, FunctionLikeStorage $storage, array &$function_template_types, array $class_template_types, array $docblock_params, PhpParser\Node\FunctionLike $function, bool $fake_method, ?string $fq_classlike_name) : void
    {
        ...
        foreach ($docblock_params as $docblock_param) {
            ...
            if (!$fake_method) {
                $docblock_type_location = new CodeLocation\DocblockTypeLocation($file_scanner, $docblock_param['start'], $docblock_param['end'], $docblock_param['line_number']);
            } else {
            ...
            if ($storage_param === null) {
                $param_location = new CodeLocation($file_scanner, $function, null, true, CodeLocation::FUNCTION_PARAM_VAR);
                ...
                $storage_param = new FunctionLikeParameter($param_name, false, null, null, null, false, false, true, null);
                $storage->params[] = $storage_param;
            }
            try {
                $new_param_type = TypeParser::parseTokens(TypeTokenizer::getFullyQualifiedTokens($docblock_param['type'], $aliases, $function_template_types + $class_template_types, $type_aliases, $fq_classlike_name), null, $function_template_types + $class_template_types, $type_aliases);
            } catch (TypeParseTreeException $e) {
            ...
            if (!$storage_param->type || $storage_param->type->hasMixed() || $storage->template_types) {
                ...
                $config = Config::getInstance();
                if ($config->add_param_default_to_docblock_type && $storage_param->default_type && !$storage_param->default_type->hasMixed() && (!$storage_param->type || !$storage_param->type->hasMixed())) {
            ...
            foreach ($new_param_type->getAtomicTypes() as $key => $type) {
                if (isset($storage_param_atomic_types[$key])) {
                    ...
                    if ($storage_param_atomic_types[$key] instanceof Type\Atomic\TArray && $type instanceof Type\Atomic\TArray && $type->type_params[0]->hasArrayKey()) {
        ...
        $params_without_docblock_type = array_filter($storage->params, function (FunctionLikeParameter $p) : bool {
     ...
     */
    private static function handleReturn(Codebase $codebase, \Psalm\Internal\Scanner\FunctionDocblockComment $docblock_info, string $docblock_return_type, bool $fake_method, FileScanner $file_scanner, FunctionLikeStorage $storage, PhpParser\Node\FunctionLike $stmt, Aliases $aliases, array $function_template_types, array $class_template_types, array $type_aliases, ?ClassLikeStorage $classlike_storage, string $cased_function_id, FileStorage $file_storage) : void
    {
        if (!$fake_method && $docblock_info->return_type_line_number && $docblock_info->return_type_start && $docblock_info->return_type_end) {
            $storage->return_type_location = new CodeLocation\DocblockTypeLocation($file_scanner, $docblock_info->return_type_start, $docblock_info->return_type_end, $docblock_info->return_type_line_number);
        } else {
            $storage->return_type_location = new CodeLocation($file_scanner, $stmt, null, false, !$fake_method ? CodeLocation::FUNCTION_PHPDOC_RETURN_TYPE : CodeLocation::FUNCTION_PHPDOC_METHOD, $docblock_info->return_type);
        }
        try {
            ...
            $storage->return_type = TypeParser::parseTokens(\array_values($fixed_type_tokens), null, $function_template_types + $class_template_types, $type_aliases);
    ...
    private static function handleTaintFlow(\Psalm\Internal\Scanner\FunctionDocblockComment $docblock_info, FunctionLikeStorage $storage) : void
    {
     ...
     * @param array<string, non-empty-array<string, Type\Union>> $class_template_types
     */
    private static function handleAssertions(\Psalm\Internal\Scanner\FunctionDocblockComment $docblock_info, FunctionLikeStorage $storage, Codebase $codebase, FileScanner $file_scanner, FileStorage $file_storage, Aliases $aliases, PhpParser\Node\FunctionLike $stmt, array $class_template_types, array $function_template_types, array $type_aliases, ?ClassLikeStorage $classlike_storage) : void
    {
            ...
            foreach ($docblock_info->assertions as $assertion) {
                ...
                foreach ($storage->params as $i => $param) {
                    if ($param->name === $assertion['param_name']) {
                        $storage->assertions[] = new \Psalm\Storage\Assertion($i, [$assertion_type_parts]);
                        continue 2;
                    }
                }
                $storage->assertions[] = new \Psalm\Storage\Assertion((strpos($assertion['param_name'], '$') === false ? '$' : '') . $assertion['param_name'], [$assertion_type_parts]);
            }
        }
        if ($docblock_info->if_true_assertions) {
            ...
            foreach ($docblock_info->if_true_assertions as $assertion) {
                ...
                foreach ($storage->params as $i => $param) {
                    if ($param->name === $assertion['param_name']) {
                        $storage->if_true_assertions[] = new \Psalm\Storage\Assertion($i, [$assertion_type_parts]);
                        continue 2;
                    }
                }
                $storage->if_true_assertions[] = new \Psalm\Storage\Assertion((strpos($assertion['param_name'], '$') === false ? '$' : '') . $assertion['param_name'], [$assertion_type_parts]);
            }
        }
        if ($docblock_info->if_false_assertions) {
            ...
            foreach ($docblock_info->if_false_assertions as $assertion) {
                ...
                foreach ($storage->params as $i => $param) {
                    if ($param->name === $assertion['param_name']) {
                        $storage->if_false_assertions[] = new \Psalm\Storage\Assertion($i, [$assertion_type_parts]);
                        continue 2;
                    }
                }
                $storage->if_false_assertions[] = new \Psalm\Storage\Assertion((strpos($assertion['param_name'], '$') === false ? '$' : '') . $assertion['param_name'], [$assertion_type_parts]);
            }
        }
    }
    /**
     ...
     */
    private static function handleTemplates(FunctionLikeStorage $storage, \Psalm\Internal\Scanner\FunctionDocblockComment $docblock_info, Aliases $aliases, ?array $template_types, array $type_aliases, FileScanner $file_scanner, PhpParser\Node\FunctionLike $stmt, string $cased_function_id) : array
    {
        ...
        foreach ($docblock_info->templates as $i => $template_map) {
            ...
            if ($template_map[1] !== null && $template_map[2] !== null) {
                if (trim($template_map[2])) {
                    try {
                        ...
                        $storage->docblock_issues[] = new InvalidDocblock('Template ' . $template_name . ' has invalid as type - ' . $e->getMessage(), new CodeLocation($file_scanner, $stmt, null, true));
                        $template_type = Type::getMixed();
                    }
                } else {
                    $storage->docblock_issues[] = new InvalidDocblock('Template ' . $template_name . ' missing as type', new CodeLocation($file_scanner, $stmt, null, true));
                    $template_type = Type::getMixed();
                }
            } else {
                $template_type = Type::getMixed();
            ...
            if (isset($template_types[$template_name])) {
                $storage->docblock_issues[] = new InvalidDocblock('Duplicate template param ' . $template_name . ' in docblock for ' . $cased_function_id, new CodeLocation($file_scanner, $stmt, null, true));
            } else {
