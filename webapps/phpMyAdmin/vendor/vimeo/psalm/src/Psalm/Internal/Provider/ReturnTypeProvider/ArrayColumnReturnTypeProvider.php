...
class ArrayColumnReturnTypeProvider implements \Psalm\Plugin\EventHandler\FunctionReturnTypeProviderInterface
{
    ...
    public static function getFunctionReturnType(FunctionReturnTypeProviderEvent $event) : Type\Union
    {
        ...
        if (!$statements_source instanceof \Psalm\Internal\Analyzer\StatementsAnalyzer || \count($call_args) < 2) {
        ...
        if (($first_arg_type = $statements_source->node_data->getType($call_args[0]->value)) && $first_arg_type->isSingle() && $first_arg_type->hasArray()) {
            ...
            $input_array_not_empty = $input_array instanceof Type\Atomic\TNonEmptyList || $input_array instanceof Type\Atomic\TNonEmptyArray || $input_array instanceof Type\Atomic\TKeyedArray;
        }
        ...
        if (isset($call_args[2]) && (string) $third_arg_type !== 'null') {
            $type = $have_at_least_one_res ? new Type\Atomic\TNonEmptyArray([$result_key_type, $result_element_type ?? Type::getMixed()]) : new Type\Atomic\TArray([$result_key_type, $result_element_type ?? Type::getMixed()]);
        } else {
            $type = $have_at_least_one_res ? new Type\Atomic\TNonEmptyList($result_element_type ?? Type::getMixed()) : new Type\Atomic\TList($result_element_type ?? Type::getMixed());
        }
