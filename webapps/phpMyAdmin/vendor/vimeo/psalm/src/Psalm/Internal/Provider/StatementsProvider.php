...
class StatementsProvider
{
    ...
    public function __construct(FileProvider $file_provider, ?ParserCacheProvider $parser_cache_provider = null, ?FileStorageCacheProvider $file_storage_cache_provider = null)
    {
    ...
    public function getStatementsForFile(string $file_path, string $php_version, ?Progress $progress = null) : array
    {
        ...
        if ($stmts === null) {
            ...
            if ($existing_file_contents === $file_contents && $existing_statements) {
                ...
                $this->parser_cache_provider->saveStatementsToCache($file_path, $file_content_hash, $existing_statements, true);
            ...
            $has_errors = false;
            $stmts = self::parseStatements($file_contents, $php_version, $has_errors, $file_path, $existing_file_contents, $existing_statements_copy, $file_changes);
            if ($existing_file_contents && $existing_statements && (!$has_errors || $stmts)) {
                [$unchanged_members, $unchanged_signature_members, $changed_members, $diff_map] = \Psalm\Internal\Diff\FileStatementsDiffer::diff($existing_statements, $stmts, $existing_file_contents, $file_contents);
                ...
                $changed_members = array_map(function (string $key) use($file_path_hash) : string {
     ...
     * @return list<\PhpParser\Node\Stmt>
     */
    public static function parseStatements(string $file_contents, string $php_version, bool &$has_errors, ?string $file_path = null, ?string $existing_file_contents = null, ?array $existing_statements = null, ?array $file_changes = null) : array
    {
        $attributes = ['comments', 'startLine', 'startFilePos', 'endFilePos'];
        if (!self::$lexer) {
            self::$lexer = new PhpParser\Lexer\Emulative(['usedAttributes' => $attributes, 'phpVersion' => $php_version]);
        }
        if (!self::$parser) {
        ...
        if ($existing_statements && $file_changes && $existing_file_contents) {
            ...
            $offset_analyzer = new \Psalm\Internal\PhpVisitor\PartialParserVisitor(self::$parser, $error_handler, $file_changes, $existing_file_contents, $file_contents);
        ...
        if ($error_handler->hasErrors() && $file_path) {
            ...
            foreach ($error_handler->getErrors() as $error) {
                if ($error->hasColumnInfo()) {
                    \Psalm\IssueBuffer::add(new \Psalm\Issue\ParseError($error->getMessage(), new \Psalm\CodeLocation\ParseErrorLocation($error, $file_contents, $file_path, $config->shortenFileName($file_path))));
                }
            }
        }
        $error_handler->clearErrors();
        $resolving_traverser = new PhpParser\NodeTraverser();
        $name_resolver = new \Psalm\Internal\PhpVisitor\SimpleNameResolver($error_handler, $used_cached_statements ? $file_changes : []);
