...
class ClosureAnalyzer extends FunctionLikeAnalyzer
{
     ...
     */
    public static function analyzeExpression(StatementsAnalyzer $statements_analyzer, PhpParser\Node\FunctionLike $stmt, Context $context) : bool
    {
        ...
        if ($context->self) {
            ...
            ClassAnalyzer::addContextProperties($statements_analyzer, $self_class_storage, $use_context, $context->self, $statements_analyzer->getParentFQCLN());
        }
        ...
        if ($stmt instanceof PhpParser\Node\Expr\Closure) {
            foreach ($stmt->uses as $use) {
                ...
                $use_context->vars_in_scope[$use_var_id] = $context->hasVariable($use_var_id) && !$use->byRef ? clone $context->vars_in_scope[$use_var_id] : Type::getMixed();
                if ($use->byRef) {
     ...
     * @return  false|null
     */
    public static function analyzeClosureUses(StatementsAnalyzer $statements_analyzer, PhpParser\Node\Expr\Closure $stmt, Context $context) : ?bool
    {
        $param_names = array_map(function (PhpParser\Node\Param $p) : string {
            if (!$p->var instanceof PhpParser\Node\Expr\Variable || !is_string($p->var->name)) {
                return '';
            }
            return $p->var->name;
        }, $stmt->params);
