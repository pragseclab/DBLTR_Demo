...
class Populator
{
     ...
     */
    private $invalid_class_storages = array();
    /**
     * @var Progress
     */
    private $progress;
    ...
    public function __construct(Config $config, ClassLikeStorageProvider $classlike_storage_provider, FileStorageProvider $file_storage_provider, ClassLikes $classlikes, FileReferenceProvider $file_reference_provider, Progress $progress)
    {
    ...
    private function populateClassLikeStorage(ClassLikeStorage $storage, array $dependent_classlikes = array()) : void
    {
        ...
        if (isset($dependent_classlikes[$fq_classlike_name_lc])) {
            if ($storage->location && IssueBuffer::accepts(new CircularReference('Circular reference discovered when loading ' . $storage->name, $storage->location))) {
                // fall through
            }
            return;
        }
    ...
    private function populateOverriddenMethods(ClassLikeStorage $storage) : void
    {
    ...
    private function populateDataFromTraits(ClassLikeStorage $storage, ClassLikeStorageProvider $storage_provider, array $dependent_classlikes) : void
    {
        foreach ($storage->used_traits as $used_trait_lc => $_) {
            try {
                ...
                if (isset($storage->template_extended_offsets[$trait_storage->name])) {
                    ...
                    foreach ($trait_storage->template_types as $template_name => $template_type_map) {
                        foreach ($template_type_map as $template_type) {
                            ...
                            $storage->template_extended_params[$trait_storage->name][$template_name] = $default_param;
                        }
                    }
                }
            } elseif ($trait_storage->template_extended_params) {
                $storage->template_extended_params = array_merge($storage->template_extended_params ?: [], $trait_storage->template_extended_params);
            }
    ...
    private static function extendType(Type\Union $type, ClassLikeStorage $storage) : Type\Union
    {
        $extended_types = [];
    ...
    private function populateDataFromParentClass(ClassLikeStorage $storage, ClassLikeStorageProvider $storage_provider, array $dependent_classlikes) : void
    {
        ...
        $storage->constants = array_merge(\array_filter($parent_storage->constants, function ($constant) {
            return $constant->visibility === ClassLikeAnalyzer::VISIBILITY_PUBLIC || $constant->visibility === ClassLikeAnalyzer::VISIBILITY_PROTECTED;
        }), $storage->constants);
    ...
    private function populateInterfaceDataFromParentInterfaces(ClassLikeStorage $storage, ClassLikeStorageProvider $storage_provider, array $dependent_classlikes) : void
    {
        $parent_interfaces = [];
        foreach ($storage->direct_interface_parents as $parent_interface_lc => $_) {
            ...
            $storage->constants = array_merge(\array_filter($parent_interface_storage->constants, function ($constant) {
                return $constant->visibility === ClassLikeAnalyzer::VISIBILITY_PUBLIC;
            }), $storage->constants);
            $storage->invalid_dependencies = array_merge($storage->invalid_dependencies, $parent_interface_storage->invalid_dependencies);
                ...
                if (isset($storage->template_extended_offsets[$parent_interface_storage->name])) {
                    ...
                    foreach ($parent_interface_storage->template_types as $template_name => $template_type_map) {
                        foreach ($template_type_map as $template_type) {
                            $default_param = clone $template_type;
                            $default_param->from_docblock = false;
                            $storage->template_extended_params[$parent_interface_storage->name][$template_name] = $default_param;
                        }
                    }
                }
            } elseif ($parent_interface_storage->template_extended_params) {
                $storage->template_extended_params = array_merge($storage->template_extended_params ?: [], $parent_interface_storage->template_extended_params);
            }
    ...
    private function populateDataFromImplementedInterfaces(ClassLikeStorage $storage, ClassLikeStorageProvider $storage_provider, array $dependent_classlikes) : void
    {
        $extra_interfaces = [];
        foreach ($storage->direct_class_interfaces as $implemented_interface_lc => $_) {
            ...
            // copy over any constants
            $storage->constants = array_merge(\array_filter($implemented_interface_storage->constants, function ($constant) {
                return $constant->visibility === ClassLikeAnalyzer::VISIBILITY_PUBLIC;
            }), $storage->constants);
            $storage->invalid_dependencies = array_merge($storage->invalid_dependencies, $implemented_interface_storage->invalid_dependencies);
                ...
                if (isset($storage->template_extended_offsets[$implemented_interface_storage->name])) {
                    ...
                    if ($implemented_interface_storage->template_extended_params) {
                        foreach ($implemented_interface_storage->template_extended_params as $e_i => $type_map) {
                            foreach ($type_map as $i => $type) {
                                $storage->template_extended_params[$e_i][$i] = self::extendType($type, $storage);
                            }
                        }
                    }
                } else {
                    foreach ($implemented_interface_storage->template_types as $template_name => $template_type_map) {
                        foreach ($template_type_map as $template_type) {
                            $default_param = clone $template_type;
                            $default_param->from_docblock = false;
                            $storage->template_extended_params[$implemented_interface_storage->name][$template_name] = $default_param;
                        }
                    }
                }
            } elseif ($implemented_interface_storage->template_extended_params) {
                $storage->template_extended_params = array_merge($storage->template_extended_params ?: [], $implemented_interface_storage->template_extended_params);
            }
        ...
        foreach ($storage->class_implements as $implemented_interface_lc => $_) {
            try {
                $implemented_interface = strtolower($this->classlikes->getUnAliasedName($implemented_interface_lc));
            ...
            foreach ($implemented_interface_storage->methods as $method_name => $method) {
                if ($method->visibility === ClassLikeAnalyzer::VISIBILITY_PUBLIC) {
                    $interface_method_implementers[$method_name][] = new \Psalm\Internal\MethodIdentifier($implemented_interface_storage->name, $method_name);
                }
            }
        }
     ...
     */
    private function populateFileStorage(FileStorage $storage, array $dependent_file_paths = array()) : void
    {
        if ($storage->populated) {
            return;
        }
        ...
        if (isset($atomic_types['array']) && count($atomic_types) > 1 && !isset($atomic_types['null'])) {
            ...
            try {
                foreach ($atomic_types as $type_key => $type) {
                    if ($type instanceof Type\Atomic\TIterable || $type instanceof Type\Atomic\TNamedObject && (!$type->from_docblock || $is_property) && (strtolower($type->value) === 'traversable' || $this->classlikes->interfaceExtends($type->value, 'Traversable') || $this->classlikes->classImplements($type->value, 'Traversable'))) {
    ...
    protected function inheritMethodsFromParent(ClassLikeStorage $storage, ClassLikeStorage $parent_storage) : void
    {
        ...
        foreach ($parent_storage->appearing_method_ids as $method_name_lc => $appearing_method_id) {
            ...
            if ($parent_storage->is_trait && $storage->trait_alias_map) {
                $aliased_method_names = array_merge($aliased_method_names, array_keys($storage->trait_alias_map, $method_name_lc, true));
            }
            foreach ($aliased_method_names as $aliased_method_name) {
                if (isset($storage->appearing_method_ids[$aliased_method_name])) {
                    continue;
                }
                $implemented_method_id = new \Psalm\Internal\MethodIdentifier($fq_class_name, $aliased_method_name);
