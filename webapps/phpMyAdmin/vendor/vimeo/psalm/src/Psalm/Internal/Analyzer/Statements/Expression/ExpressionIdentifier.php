...
class ExpressionIdentifier
{
    ...
    public static function getArrayVarId(PhpParser\Node\Expr $stmt, ?string $this_class_name, ?FileSource $source = null) : ?string
    {
        ...
        if ($stmt instanceof PhpParser\Node\Expr\PropertyFetch) {
            ...
            } elseif ($source instanceof StatementsAnalyzer && ($stmt_name_type = $source->node_data->getType($stmt->name)) && $stmt_name_type->isSingleStringLiteral()) {
        ...
        if ($stmt instanceof PhpParser\Node\Expr\MethodCall && $stmt->name instanceof PhpParser\Node\Identifier && !$stmt->args) {
            $config = \Psalm\Config::getInstance();
            if ($config->memoize_method_calls || isset($stmt->memoizable)) {
                $lhs_var_name = self::getArrayVarId($stmt->var, $this_class_name, $source);
                if (!$lhs_var_name) {
                    return null;
                }
