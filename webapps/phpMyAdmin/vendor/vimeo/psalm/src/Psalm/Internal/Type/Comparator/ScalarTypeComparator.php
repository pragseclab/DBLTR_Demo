...
{
    public static function isContainedBy(Codebase $codebase, Scalar $input_type_part, Scalar $container_type_part, bool $allow_interface_equality = false, bool $allow_float_int_equality = true, ?TypeComparisonResult $atomic_comparison_result = null) : bool
    {
        ...
        if (($input_type_part instanceof Type\Atomic\TLowercaseString || $input_type_part instanceof Type\Atomic\TNonEmptyLowercaseString) && get_class($container_type_part) === TString::class) {
            return true;
        }
        if (($container_type_part instanceof Type\Atomic\TLowercaseString || $container_type_part instanceof Type\Atomic\TNonEmptyLowercaseString) && $input_type_part instanceof TString) {
            if ($input_type_part instanceof Type\Atomic\TLowercaseString && $container_type_part instanceof Type\Atomic\TLowercaseString || $input_type_part instanceof Type\Atomic\TNonEmptyLowercaseString && $container_type_part instanceof Type\Atomic\TNonEmptyLowercaseString) {
                return true;
            }
            if ($input_type_part instanceof Type\Atomic\TNonEmptyLowercaseString && $container_type_part instanceof Type\Atomic\TLowercaseString) {
                return true;
            }
            if ($input_type_part instanceof Type\Atomic\TLowercaseString && $container_type_part instanceof Type\Atomic\TNonEmptyLowercaseString) {
                if ($atomic_comparison_result) {
                    $atomic_comparison_result->type_coerced = true;
                }
                return false;
        ...
        if ($container_type_part instanceof TArrayKey && ($input_type_part instanceof TInt || $input_type_part instanceof TString || $input_type_part instanceof Type\Atomic\TTemplateKeyOf)) {
            return true;
        }
        ...
        if ((get_class($container_type_part) === TInt::class || get_class($container_type_part) === TDependentListKey::class) && ($input_type_part instanceof TLiteralInt || $input_type_part instanceof TPositiveInt)) {
            return true;
        }
        if (get_class($container_type_part) === TFloat::class && $input_type_part instanceof TLiteralFloat) {
            return true;
        }
        if ($container_type_part instanceof TNonEmptyString && $input_type_part instanceof TNonFalsyString) {
            return true;
        }
        if ($container_type_part instanceof TNonFalsyString && get_class($input_type_part) === TNonEmptyString::class) {
            if ($atomic_comparison_result) {
                $atomic_comparison_result->type_coerced = true;
            }
            return false;
        ...
        if ((get_class($container_type_part) === TString::class || get_class($container_type_part) === TNonEmptyString::class || get_class($container_type_part) === TNonFalsyString::class || get_class($container_type_part) === TSingleLetter::class) && $input_type_part instanceof TLiteralString) {
            return true;
        }
        if ((get_class($container_type_part) === TInt::class || get_class($container_type_part) === TDependentListKey::class) && $input_type_part instanceof TInt) {
            return true;
        }
        if (get_class($input_type_part) === TInt::class && $container_type_part instanceof TLiteralInt || get_class($input_type_part) === TPositiveInt::class && $container_type_part instanceof TLiteralInt && $container_type_part->value > 0) {
            if ($atomic_comparison_result) {
                $atomic_comparison_result->type_coerced = true;
        ...
        if ((get_class($input_type_part) === TString::class || get_class($input_type_part) === TSingleLetter::class || get_class($input_type_part) === TNonEmptyString::class || get_class($input_type_part) === TNonFalsyString::class) && $container_type_part instanceof TLiteralString) {
            if ($atomic_comparison_result) {
                $atomic_comparison_result->type_coerced = true;
                $atomic_comparison_result->type_coerced_from_scalar = true;
            }
        ...
        if (($input_type_part instanceof Type\Atomic\TLowercaseString || $input_type_part instanceof Type\Atomic\TNonEmptyLowercaseString) && $container_type_part instanceof TLiteralString && strtolower($container_type_part->value) === $container_type_part->value) {
            if ($atomic_comparison_result && ($container_type_part->value || $input_type_part instanceof Type\Atomic\TLowercaseString)) {
                $atomic_comparison_result->type_coerced = true;
                $atomic_comparison_result->type_coerced_from_scalar = true;
            }
            return false;
        ...
        if ($container_type_part instanceof TTraitString && (get_class($input_type_part) === TString::class || get_class($input_type_part) === TNonEmptyString::class || get_class($input_type_part) === TNonFalsyString::class)) {
            if ($atomic_comparison_result) {
                $atomic_comparison_result->type_coerced = true;
            }
            return false;
        }
        if (($input_type_part instanceof TClassString || $input_type_part instanceof TLiteralClassString) && (get_class($container_type_part) === TString::class || get_class($container_type_part) === TSingleLetter::class || get_class($container_type_part) === TNonEmptyString::class || get_class($container_type_part) === TNonFalsyString::class)) {
            return true;
        }
        if ($input_type_part instanceof TCallableString && (get_class($container_type_part) === TString::class || get_class($container_type_part) === TSingleLetter::class || get_class($container_type_part) === TNonEmptyString::class || get_class($container_type_part) === TNonFalsyString::class)) {
            return true;
        }
        if ($input_type_part instanceof TNumericString && get_class($container_type_part) === TNonEmptyString::class) {
            return true;
        }
        if ($container_type_part instanceof TString && ($input_type_part instanceof TNumericString || $input_type_part instanceof THtmlEscapedString)) {
            if ($container_type_part instanceof TLiteralString) {
        ...
        if ($input_type_part instanceof TString && ($container_type_part instanceof TNumericString || $container_type_part instanceof THtmlEscapedString)) {
            if ($input_type_part instanceof TLiteralString) {
        ...
        if ($container_type_part instanceof TCallableString && $input_type_part instanceof TLiteralString) {
            ...
            if ($input_callable && $container_callable) {
                if (CallableTypeComparator::isContainedBy($codebase, $input_callable, $container_callable, $atomic_comparison_result ?: new TypeComparisonResult()) === false) {
                    return false;
                }
            }
            return true;
