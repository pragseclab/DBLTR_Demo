        ...
        if (!($stmt_expr_type = $statements_analyzer->node_data->getType($stmt->expr))) {
            ...
            foreach ($stmt_expr_type->getAtomicTypes() as $type_string => $type_part) {
                if ($type_part instanceof TInt || $type_part instanceof TString) {
                    ...
                    $type_part = $type_part instanceof Type\Atomic\TLiteralFloat ? new Type\Atomic\TLiteralInt(~$type_part->value) : new TInt();
            ...
            if ($unacceptable_type || !$acceptable_types) {
                ...
                if ($has_valid_operand) {
                    if (IssueBuffer::accepts(new PossiblyInvalidOperand($message, new CodeLocation($statements_analyzer, $stmt)), $statements_analyzer->getSuppressedIssues())) {
                        // fall through
                    }
                } else {
                    if (IssueBuffer::accepts(new InvalidOperand($message, new CodeLocation($statements_analyzer, $stmt)), $statements_analyzer->getSuppressedIssues())) {
                        // fall through
                    }
                }
                $statements_analyzer->node_data->setType($stmt, Type::getMixed());
