...
class CloneAnalyzer
{
    public static function analyze(StatementsAnalyzer $statements_analyzer, PhpParser\Node\Expr\Clone_ $stmt, Context $context) : bool
    {
        ...
        if ($stmt_expr_type) {
            ...
            while ($atomic_types) {
                ...
                if ($clone_type_part instanceof TMixed) {
                    ...
                    if (!$codebase->classlikes->classOrInterfaceExists($clone_type_part->value)) {
                        ...
                        $clone_method_id = new \Psalm\Internal\MethodIdentifier($clone_type_part->value, '__clone');
                        $does_method_exist = $codebase_methods->methodExists($clone_method_id, $context->calling_method_id, new CodeLocation($statements_analyzer->getSource(), $stmt));
                    ...
                    if ($clone_type_part instanceof Type\Atomic\TFalse && $clone_type->ignore_falsable_issues) {
                        continue;
                    }
                    if ($clone_type_part instanceof Type\Atomic\TNull && $clone_type->ignore_nullable_issues) {
                        continue;
                    }
                    $invalid_clones[] = $clone_type_part->getId();
                }
            ...
            if ($mixed_clone) {
                if (IssueBuffer::accepts(new MixedClone('Cannot clone mixed', new CodeLocation($statements_analyzer->getSource(), $stmt)), $statements_analyzer->getSuppressedIssues())) {
                    // fall through
                }
            }
            if ($invalid_clones) {
                if ($possibly_valid) {
                    if (IssueBuffer::accepts(new PossiblyInvalidClone('Cannot clone ' . $invalid_clones[0], new CodeLocation($statements_analyzer->getSource(), $stmt)), $statements_analyzer->getSuppressedIssues())) {
                        // fall through
                    }
                } else {
                    if (IssueBuffer::accepts(new InvalidClone('Cannot clone ' . $invalid_clones[0], new CodeLocation($statements_analyzer->getSource(), $stmt)), $statements_analyzer->getSuppressedIssues())) {
                        // fall through
                    }
                }
                return true;
