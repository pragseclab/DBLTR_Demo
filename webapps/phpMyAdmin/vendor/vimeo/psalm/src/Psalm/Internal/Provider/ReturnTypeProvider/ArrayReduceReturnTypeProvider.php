...
class ArrayReduceReturnTypeProvider implements \Psalm\Plugin\EventHandler\FunctionReturnTypeProviderInterface
{
    ...
    public static function getFunctionReturnType(FunctionReturnTypeProviderEvent $event) : Type\Union
    {
        ...
        if (isset($array_arg_types['array']) && ($array_arg_types['array'] instanceof Type\Atomic\TArray || $array_arg_types['array'] instanceof Type\Atomic\TKeyedArray || $array_arg_types['array'] instanceof Type\Atomic\TList)) {
            ...
            if ($array_arg_atomic_type instanceof Type\Atomic\TKeyedArray) {
                ...
                $array_arg_atomic_type = new Type\Atomic\TArray([Type::getInt(), clone $array_arg_atomic_type->type_param]);
            }
        }
        ...
        if ($closure_types = $function_call_arg_type->getClosureTypes()) {
            ...
            if ($closure_atomic_type->params !== null) {
                if (count($closure_atomic_type->params) < 1) {
                    if (IssueBuffer::accepts(new InvalidArgument('The closure passed to array_reduce at least one parameter', new CodeLocation($statements_source, $function_call_arg)), $statements_source->getSuppressedIssues())) {
                        // fall through
                    }
                    return Type::getMixed();
                }
                ...
                if ($carry_param->type && (!UnionTypeComparator::isContainedBy($codebase, $initial_type, $carry_param->type) || !$reduce_return_type->hasMixed() && !UnionTypeComparator::isContainedBy($codebase, $reduce_return_type, $carry_param->type))) {
                    if (IssueBuffer::accepts(new InvalidArgument('The first param of the closure passed to array_reduce must take ' . $reduce_return_type . ' but only accepts ' . $carry_param->type, $carry_param->type_location ?: new CodeLocation($statements_source, $function_call_arg)), $statements_source->getSuppressedIssues())) {
                        // fall through
                    }
                    return Type::getMixed();
                }
                if ($item_param && $item_param->type && $array_arg_atomic_type && !$array_arg_atomic_type->type_params[1]->hasMixed() && !UnionTypeComparator::isContainedBy($codebase, $array_arg_atomic_type->type_params[1], $item_param->type)) {
                    if (IssueBuffer::accepts(new InvalidArgument('The second param of the closure passed to array_reduce must take ' . $array_arg_atomic_type->type_params[1] . ' but only accepts ' . $item_param->type, $item_param->type_location ?: new CodeLocation($statements_source, $function_call_arg)), $statements_source->getSuppressedIssues())) {
                        // fall through
                    }
                    return Type::getMixed();
                }
                ...
                foreach ($mapping_function_id_parts as $mapping_function_id_part) {
                    if (isset($call_map[$mapping_function_id_part][0])) {
                        ...
                        if (strpos($mapping_function_id_part, '::') !== false) {
                            ...
                            $method_id = new \Psalm\Internal\MethodIdentifier($callable_fq_class_name, strtolower($method_name));
