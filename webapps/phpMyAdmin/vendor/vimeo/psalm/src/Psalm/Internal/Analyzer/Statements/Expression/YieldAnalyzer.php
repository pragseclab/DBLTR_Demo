    ...
    public static function analyze(StatementsAnalyzer $statements_analyzer, PhpParser\Node\Expr\Yield_ $stmt, Context $context) : bool
    {
        ...
        if ($doc_comment) {
            ...
            foreach ($var_comments as $var_comment) {
                ...
                $comment_type = \Psalm\Internal\Type\TypeExpander::expandUnion($codebase, $var_comment->type, $context->self, $context->self ? new Type\Atomic\TNamedObject($context->self) : null, $statements_analyzer->getParentFQCLN());
                $type_location = null;
                ...
                if ($codebase->find_unused_variables && $type_location && isset($context->vars_in_scope[$var_comment->var_id]) && $context->vars_in_scope[$var_comment->var_id]->getId() === $comment_type->getId()) {
                    ...
                    } elseif (IssueBuffer::accepts(new UnnecessaryVarAnnotation('The @var annotation for ' . $var_comment->var_id . ' is unnecessary', $type_location), [], true)) {
                        // fall through
                    }
                }
            ...
            if ($expression_atomic_type instanceof Type\Atomic\TNamedObject) {
                ...
                if ($classlike_storage->yield) {
                    if ($expression_atomic_type instanceof Type\Atomic\TGenericObject) {
                        $yield_candidate_type = AtomicPropertyFetchAnalyzer::localizePropertyType($codebase, clone $classlike_storage->yield, $expression_atomic_type, $classlike_storage, $classlike_storage);
                        if ($yield_type) {
        ...
        if ($source instanceof FunctionLikeAnalyzer && !$source->getSource() instanceof TraitAnalyzer) {
            ...
            if ($storage->return_type && !$yield_type) {
                foreach ($storage->return_type->getAtomicTypes() as $atomic_return_type) {
                    if ($atomic_return_type instanceof Type\Atomic\TNamedObject && $atomic_return_type->value === 'Generator') {
