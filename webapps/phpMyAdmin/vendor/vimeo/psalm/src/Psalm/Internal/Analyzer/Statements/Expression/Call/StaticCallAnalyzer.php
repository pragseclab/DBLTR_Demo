...
{
    public static function analyze(StatementsAnalyzer $statements_analyzer, PhpParser\Node\Expr\StaticCall $stmt, Context $context) : bool
    {
        $method_id = null;
        $lhs_type = null;
            ...
            if (count($stmt->class->parts) === 1 && in_array(strtolower($stmt->class->parts[0]), ['self', 'static', 'parent'], true)) {
                if ($stmt->class->parts[0] === 'parent') {
                    ...
                    if (!$class_storage || !$class_storage->parent_class) {
                        if (IssueBuffer::accepts(new ParentNotFound('Cannot call method on parent as this class does not extend another', new CodeLocation($statements_analyzer->getSource(), $stmt)), $statements_analyzer->getSuppressedIssues())) {
                            return false;
                        }
                        return true;
                    }
                    ...
                    if (IssueBuffer::accepts(new NonStaticSelfCall('Cannot use ' . $stmt->class->parts[0] . ' outside class context', new CodeLocation($statements_analyzer->getSource(), $stmt)), $statements_analyzer->getSuppressedIssues())) {
                        return false;
                    }
                    return true;
                }
                ...
                if (!isset($context->phantom_classes[strtolower($fq_class_name)]) && !$does_class_exist) {
                    $does_class_exist = ClassLikeAnalyzer::checkFullyQualifiedClassLikeName($statements_analyzer, $fq_class_name, new CodeLocation($source, $stmt->class), !$context->collect_initializations && !$context->collect_mutations ? $context->self : null, !$context->collect_initializations && !$context->collect_mutations ? $context->calling_method_id : null, $statements_analyzer->getSuppressedIssues(), false, false, false);
                }
                if (!$does_class_exist) {
        ...
        foreach ($lhs_type->getAtomicTypes() as $lhs_type_part) {
            StaticMethod\AtomicStaticCallAnalyzer::analyze($statements_analyzer, $stmt, $context, $lhs_type_part, $lhs_type->ignore_nullable_issues, $moved_call, $has_mock, $has_existing_method);
        }
        if (!$has_existing_method) {
            return self::checkMethodArgs($method_id, $stmt->args, null, $context, new CodeLocation($statements_analyzer->getSource(), $stmt), $statements_analyzer);
        }
    ...
    public static function taintReturnType(StatementsAnalyzer $statements_analyzer, PhpParser\Node\Expr\StaticCall $stmt, MethodIdentifier $method_id, string $cased_method_id, Type\Union $return_type_candidate, ?\Psalm\Storage\MethodStorage $method_storage, ?\Psalm\Internal\Type\TemplateResult $template_result) : void
    {
        ...
        $method_location = $method_storage ? $method_storage->signature_return_type_location ?: $method_storage->location : null;
        ...
        if ($method_storage && $template_result) {
            foreach ($method_storage->conditionally_removed_taints as $conditionally_removed_taint) {
                ...
                $expanded_type = \Psalm\Internal\Type\TypeExpander::expandUnion($statements_analyzer->getCodebase(), $conditionally_removed_taint, null, null, null, true, true);
        ...
        if ($conditionally_removed_taints && $method_location) {
            ...
            $statements_analyzer->data_flow_graph->addPath($method_source, $assignment_node, 'conditionally-escaped', [], $conditionally_removed_taints);
        ...
        if ($method_storage) {
            FunctionCallReturnTypeFetcher::taintUsingFlows($statements_analyzer, $method_storage, $statements_analyzer->data_flow_graph, (string) $method_id, $stmt->args, $node_location, $method_source, $method_storage->removed_taints);
        }
    }
}