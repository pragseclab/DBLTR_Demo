...
class CallableTypeComparator
{
     ...
     */
    public static function isContainedBy(Codebase $codebase, Type\Atomic $input_type_part, Type\Atomic $container_type_part, ?TypeComparisonResult $atomic_comparison_result) : bool
    {
        ...
        if ($input_type_part->params !== null && $container_type_part->params !== null) {
            foreach ($input_type_part->params as $i => $input_param) {
                ...
                if ($container_param->type && !$container_param->type->hasMixed() && !UnionTypeComparator::isContainedBy($codebase, $container_param->type, $input_param->type ?: Type::getMixed(), false, false, $atomic_comparison_result)) {
                    return false;
                }
            }
        }
        if (isset($container_type_part->return_type)) {
            ...
            if (!$container_type_part->return_type->isVoid() && !UnionTypeComparator::isContainedBy($codebase, $input_return, $container_type_part->return_type, false, false, $atomic_comparison_result)) {
                return false;
            }
        }
        return true;
    }
    public static function isNotExplicitlyCallableTypeCallable(Codebase $codebase, Type\Atomic $input_type_part, TCallable $container_type_part, ?TypeComparisonResult $atomic_comparison_result) : bool
    {
        if ($input_type_part instanceof TList) {
            if ($input_type_part->type_param->isMixed() || $input_type_part->type_param->hasScalar()) {
                if ($atomic_comparison_result) {
                    $atomic_comparison_result->type_coerced_from_mixed = true;
                    $atomic_comparison_result->type_coerced = true;
                }
     ...
     */
    public static function getCallableFromAtomic(Codebase $codebase, Type\Atomic $input_type_part, ?TCallable $container_type_part = null, ?StatementsAnalyzer $statements_analyzer = null) : ?Atomic
    {
        ...
        if ($input_type_part instanceof TLiteralString && $input_type_part->value) {
            try {
                ...
                return new TCallable('callable', $function_storage->params, $function_storage->return_type, $function_storage->pure);
                ...
                if (InternalCallMapHandler::inCallMap($input_type_part->value)) {
                    ...
                    if ($container_type_part && $container_type_part->params) {
                        foreach ($container_type_part->params as $i => $param) {
                            $arg = new \PhpParser\Node\Arg(new \PhpParser\Node\Expr\Variable('_' . $i));
                            if ($param->type) {
                    ...
                    $matching_callable = \Psalm\Internal\Codebase\InternalCallMapHandler::getCallableFromCallMapById($codebase, $input_type_part->value, $args, $nodes);
                    $must_use = false;
                    $matching_callable->is_pure = $codebase->functions->isCallMapFunctionPure($codebase, $statements_analyzer ? $statements_analyzer->node_data : null, $input_type_part->value, null, $must_use);
            ...
            if ($method_id && $method_id !== 'not-callable') {
                try {
                    ...
                    if ($method_storage->return_type) {
                        $converted_return_type = \Psalm\Internal\Type\TypeExpander::expandUnion($codebase, $method_storage->return_type, $method_fqcln, $method_fqcln, null);
                    }
            ...
            $invoke_id = new \Psalm\Internal\MethodIdentifier($input_type_part->value, '__invoke');
    ...
    public static function getCallableMethodIdFromTKeyedArray(TKeyedArray $input_type_part, ?Codebase $codebase = null, ?string $calling_method_id = null, ?string $file_name = null)
    {
        ...
        if ($lhs->isSingleStringLiteral()) {
            ...
            foreach ($lhs->getAtomicTypes() as $lhs_atomic_type) {
                ...
                } elseif ($lhs_atomic_type instanceof Type\Atomic\TClassString && $lhs_atomic_type->as) {
        ...
        return new \Psalm\Internal\MethodIdentifier($class_name, strtolower($method_name));
    }
}