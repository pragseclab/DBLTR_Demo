...
class CastAnalyzer
{
    public static function analyze(StatementsAnalyzer $statements_analyzer, PhpParser\Node\Expr\Cast $stmt, Context $context) : bool
    {
        if ($stmt instanceof PhpParser\Node\Expr\Cast\Int_) {
            ...
            if ($maybe_type) {
                ...
                if (count($maybe) === 1 && current($maybe) instanceof Type\Atomic\TBool) {
                    $as_int = false;
                    $statements_analyzer->node_data->setType($stmt, new Type\Union([new Type\Atomic\TLiteralInt(0), new Type\Atomic\TLiteralInt(1)]));
                }
            }
            if ($as_int) {
                $type = Type::getInt();
                if ($statements_analyzer->data_flow_graph && $statements_analyzer->data_flow_graph instanceof \Psalm\Internal\Codebase\VariableUseGraph) {
            ...
            if ($stmt_expr_type) {
                if ($stmt_expr_type->isString()) {
                    if ($stmt_expr_type->from_docblock) {
                        $issue = new RedundantCastGivenDocblockType('Redundant cast to ' . $stmt_expr_type->getKey() . ' given docblock-provided type', new CodeLocation($statements_analyzer->getSource(), $stmt));
                    } else {
                        $issue = new RedundantCast('Redundant cast to ' . $stmt_expr_type->getKey(), new CodeLocation($statements_analyzer->getSource(), $stmt));
                    }
                    if (IssueBuffer::accepts($issue, $statements_analyzer->getSuppressedIssues())) {
                        // fall through
                    }
                }
                $stmt_type = self::castStringAttempt($statements_analyzer, $context, $stmt_expr_type, $stmt->expr, true);
            } else {
        ...
        if ($stmt instanceof PhpParser\Node\Expr\Cast\Array_) {
            ...
            if ($stmt_expr_type = $statements_analyzer->node_data->getType($stmt->expr)) {
                ...
                foreach ($stmt_expr_type->getAtomicTypes() as $type) {
                    ...
                    } elseif ($type instanceof TArray || $type instanceof TList || $type instanceof TKeyedArray) {
        ...
        if ($stmt instanceof PhpParser\Node\Expr\Cast\Unset_ && $statements_analyzer->getCodebase()->php_major_version < 8) {
            if (ExpressionAnalyzer::analyze($statements_analyzer, $stmt->expr, $context) === false) {
                return false;
            }
        ...
        if (IssueBuffer::accepts(new UnrecognizedExpression('Psalm does not understand the cast ' . get_class($stmt), new CodeLocation($statements_analyzer->getSource(), $stmt)), $statements_analyzer->getSuppressedIssues())) {
            // fall through
        }
        return false;
    }
    public static function castStringAttempt(StatementsAnalyzer $statements_analyzer, Context $context, Type\Union $stmt_type, PhpParser\Node\Expr $stmt, bool $explicit_cast = false) : Type\Union
    {
        ...
        while ($atomic_types) {
            ...
            if ($atomic_type instanceof TFloat || $atomic_type instanceof TInt || $atomic_type instanceof Type\Atomic\TNumeric) {
            ...
            if ($atomic_type instanceof TNull || $atomic_type instanceof Type\Atomic\TFalse) {
                $valid_strings[] = new Type\Atomic\TLiteralString('');
                continue;
            }
            if ($atomic_type instanceof TMixed || $atomic_type instanceof Type\Atomic\TResource || $atomic_type instanceof Type\Atomic\Scalar) {
                $castable_types[] = new TString();
                continue;
            }
            if ($atomic_type instanceof TNamedObject || $atomic_type instanceof Type\Atomic\TObjectWithProperties) {
                ...
                foreach ($intersection_types as $intersection_type) {
                    if ($intersection_type instanceof TNamedObject) {
                        $intersection_method_id = new \Psalm\Internal\MethodIdentifier($intersection_type->value, '__tostring');
                    ...
                    if ($intersection_type instanceof Type\Atomic\TObjectWithProperties && isset($intersection_type->methods['__toString'])) {
                        $castable_types[] = new TString();
                        continue 2;
                    }
                }
        ...
        if ($invalid_casts) {
            if ($valid_strings || $castable_types) {
                if (IssueBuffer::accepts(new PossiblyInvalidCast($invalid_casts[0] . ' cannot be cast to string', new CodeLocation($statements_analyzer->getSource(), $stmt)), $statements_analyzer->getSuppressedIssues())) {
                    // fall through
                }
            } else {
                if (IssueBuffer::accepts(new InvalidCast($invalid_casts[0] . ' cannot be cast to string', new CodeLocation($statements_analyzer->getSource(), $stmt)), $statements_analyzer->getSuppressedIssues())) {
                    // fall through
                }
            }
        ...
        if (!$valid_types) {
            ...
            $str_type = \Psalm\Internal\Type\TypeCombiner::combine($valid_types, $codebase);
        }
        if ($statements_analyzer->data_flow_graph) {
