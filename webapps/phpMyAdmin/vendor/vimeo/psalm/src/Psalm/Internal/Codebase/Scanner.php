...
class Scanner
{
    ...
    public function __construct(Codebase $codebase, Config $config, FileStorageProvider $file_storage_provider, FileProvider $file_provider, Reflection $reflection, FileReferenceProvider $file_reference_provider, Progress $progress)
    {
        ...
        if (!isset($this->classlike_files[$fq_classlike_name_lc]) || $analyze_too && !isset($this->deep_scanned_classlike_files[$fq_classlike_name_lc])) {
            ...
            if (PropertyMap::inPropertyMap($fq_classlike_name_lc)) {
                ...
                foreach ($public_mapped_properties as $public_mapped_property) {
                    ...
                    $property_type->queueClassLikesForScanning($this->codebase, null, $phantom_classes + [$fq_classlike_name_lc => true]);
                }
            }
        }
    }
    ...
    private function scanFilePaths(int $pool_size) : bool
    {
        ...
        $files_to_scan = array_filter($this->files_to_scan, function (string $file_path) : bool {
        ...
        $scanner_worker = function (int $_, string $file_path) use($filetype_scanners, $files_to_deep_scan) : void {
            $this->scanFile($file_path, $filetype_scanners, isset($files_to_deep_scan[$file_path]));
        };
        ...
        if ($pool_size > 1) {
            ...
            $pool = new \Psalm\Internal\Fork\Pool(
                ...
                function () {
                    ...
                    $codebase = $project_analyzer->getCodebase();
                    $statements_provider = $codebase->statements_provider;
                    return ['classlikes_data' => $codebase->classlikes->getThreadData(), 'scanner_data' => $codebase->scanner->getThreadData(), 'issues' => \Psalm\IssueBuffer::getIssuesData(), 'changed_members' => $statements_provider->getChangedMembers(), 'unchanged_signature_members' => $statements_provider->getUnchangedSignatureMembers(), 'diff_map' => $statements_provider->getDiffMap(), 'errors' => $statements_provider->getErrors(), 'classlike_storage' => $codebase->classlike_storage_provider->getAll(), 'file_storage' => $codebase->file_storage_provider->getAll(), 'new_file_content_hashes' => $statements_provider->parser_cache_provider ? $statements_provider->parser_cache_provider->getNewFileContentHashes() : [], 'taint_data' => $codebase->taint_flow_graph];
                }
            );
