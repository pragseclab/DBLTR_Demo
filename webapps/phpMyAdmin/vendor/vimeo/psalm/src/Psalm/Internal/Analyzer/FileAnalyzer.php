...
class FileAnalyzer extends SourceAnalyzer
{
        ...
        if ($codebase->config->check_for_throws_in_global_scope) {
            ...
            foreach ($uncaught_throws as $possibly_thrown_exception => $codelocations) {
                foreach ($codelocations as $codelocation) {
                    ...
                    if (IssueBuffer::accepts(new UncaughtThrowInGlobalScope($possibly_thrown_exception . ' is thrown but not caught in global scope', $codelocation))) {
                        // fall through
                    }
                }
            }
        ...
        if ($file_storage->type_aliases) {
            foreach ($file_storage->type_aliases as $alias) {
                if ($alias instanceof LinkableTypeAlias) {
                    ...
                    if (ClassLikeAnalyzer::checkFullyQualifiedClassLikeName($this->getSource(), $fq_source_classlike, $location, null, null, $this->suppressed_issues, true, false, true, true) === false) {
                        continue;
                    }
                    ...
                    if (!isset($referenced_class_storage->type_aliases[$alias->alias_name])) {
                        IssueBuffer::accepts(new InvalidTypeImport('Type alias ' . $alias->alias_name . ' imported from ' . $fq_source_classlike . ' is not defined on the source class', $location));
                    }
                }
            }
        }
    ...
    public function getMethodMutations(\Psalm\Internal\MethodIdentifier $method_id, Context $this_context, bool $from_project_analyzer = false) : void
    {
