...
class ClassTemplateParamCollector
{
     ...
     */
    public static function collect(Codebase $codebase, ClassLikeStorage $class_storage, ClassLikeStorage $static_class_storage, ?string $method_name = null, ?Type\Atomic $lhs_type_part = null, bool $self_call = false) : ?array
    {
        ...
        if ($lhs_type_part instanceof TGenericObject) {
            ...
            foreach ($template_types as $type_name => $_) {
                    ...
                    foreach ($input_type_extends->getAtomicTypes() as $type_extends_atomic) {
                        if ($type_extends_atomic instanceof Type\Atomic\TTemplateParam) {
                            if (isset($static_class_storage->template_types[$type_extends_atomic->param_name])) {
                                ...
                                if ($mapped_offset !== false && isset($lhs_type_part->type_params[$mapped_offset])) {
                                    ...
                                    if (!$output_type_extends) {
                                        ...
                                        $output_type_extends = Type::combineUnionTypes($candidate_type, $output_type_extends);
                                    }
                                }
                                ...
                                $mapped_offset = array_search($type_extends_atomic->param_name, array_keys($static_class_storage->template_extended_params[$type_extends_atomic->defining_class]), true);
                                if ($mapped_offset !== false && isset($lhs_type_part->type_params[$mapped_offset])) {
                                    $candidate_type = $lhs_type_part->type_params[$mapped_offset];
                                    if (!$output_type_extends) {
                                        $output_type_extends = $candidate_type;
                            ...
                            if (!$output_type_extends) {
                                ...
                                $output_type_extends = Type::combineUnionTypes(new Type\Union([$type_extends_atomic]), $output_type_extends);
                            }
                        }
                    }
                ...
                if ((!$self_call || $static_fq_class_name !== $class_storage->name) && !isset($class_template_params[$type_name])) {
            ...
            foreach ($type_map as $type) {
                foreach ($candidate_class_storages as $candidate_class_storage) {
                    if ($candidate_class_storage !== $static_class_storage && isset($e[$candidate_class_storage->name][$type_name]) && !isset($class_template_params[$type_name][$candidate_class_storage->name])) {
                        $class_template_params[$type_name][$candidate_class_storage->name] = new Type\Union(self::expandType($codebase, $e[$candidate_class_storage->name][$type_name], $e, $static_class_storage->name, $static_class_storage->template_types));
                    }
                }
                if (!$self_call) {
                    if (!isset($class_template_params[$type_name])) {
     ...
     */
    private static function expandType(Codebase $codebase, Type\Union $input_type_extends, array $e, string $static_fq_class_name, ?array $static_template_types) : array
    {
        $output_type_extends = [];
        foreach ($input_type_extends->getAtomicTypes() as $type_extends_atomic) {
            if ($type_extends_atomic instanceof Type\Atomic\TTemplateParam && ($static_fq_class_name !== $type_extends_atomic->defining_class || !isset($static_template_types[$type_extends_atomic->param_name])) && isset($e[$type_extends_atomic->defining_class][$type_extends_atomic->param_name])) {
                $output_type_extends = array_merge($output_type_extends, self::expandType($codebase, $e[$type_extends_atomic->defining_class][$type_extends_atomic->param_name], $e, $static_fq_class_name, $static_template_types));
            } elseif ($type_extends_atomic instanceof TScalarClassConstant) {
                $expanded = TypeExpander::expandAtomic($codebase, $type_extends_atomic, $type_extends_atomic->fq_classlike_name, $type_extends_atomic->fq_classlike_name, null, true, true);
