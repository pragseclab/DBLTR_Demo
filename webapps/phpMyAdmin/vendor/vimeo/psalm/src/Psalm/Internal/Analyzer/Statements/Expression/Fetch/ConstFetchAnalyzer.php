...
class ConstFetchAnalyzer
{
        ...
        switch (strtolower($const_name)) {
            ...
            default:
                ...
                if ($const_type) {
                    ...
                    if (IssueBuffer::accepts(new UndefinedConstant('Const ' . $const_name . ' is not defined', new CodeLocation($statements_analyzer->getSource(), $stmt)), $statements_analyzer->getSuppressedIssues())) {
                        // fall through
                    }
                }
        }
    }
    public static function getGlobalConstType(Codebase $codebase, ?string $fq_const_name, string $const_name) : ?Type\Union
    {
    ...
    public static function getConstType(StatementsAnalyzer $statements_analyzer, string $const_name, bool $is_fully_qualified, ?Context $context) : ?Type\Union
    {
        $aliased_constants = $statements_analyzer->getAliases()->constants;
        if (isset($aliased_constants[$const_name])) {
            $fq_const_name = $aliased_constants[$const_name];
        ...
        return ConstFetchAnalyzer::getGlobalConstType($codebase, $fq_const_name, $const_name) ?? ConstFetchAnalyzer::getGlobalConstType($codebase, $const_name, $const_name);
    }
    public static function setConstType(StatementsAnalyzer $statements_analyzer, string $const_name, Type\Union $const_type, Context $context) : void
    {
    ...
    public static function getConstName(PhpParser\Node\Expr $first_arg_value, \Psalm\Internal\Provider\NodeDataProvider $type_provider, Codebase $codebase, Aliases $aliases) : ?string
    {
        $const_name = null;
