...
class EncapsulatedStringAnalyzer
{
    public static function analyze(StatementsAnalyzer $statements_analyzer, PhpParser\Node\Scalar\Encapsed $stmt, Context $context) : bool
    {
        ...
        foreach ($stmt->parts as $part) {
            if ($part instanceof PhpParser\Node\Scalar\EncapsedStringPart && $part->value) {
                $non_empty = true;
            }
            ...
            if ($part_type) {
                $casted_part_type = CastAnalyzer::castStringAttempt($statements_analyzer, $context, $part_type, $part);
