...
class GlobalAnalyzer
{
    public static function analyze(StatementsAnalyzer $statements_analyzer, PhpParser\Node\Stmt\Global_ $stmt, Context $context, ?Context $global_context) : void
    {
        if (!$context->collect_initializations && !$global_context) {
            if (IssueBuffer::accepts(new InvalidGlobal('Cannot use global scope here', new CodeLocation($statements_analyzer, $stmt)), $statements_analyzer->getSource()->getSuppressedIssues())) {
                // fall through
            }
        }
            ...
            if ($var instanceof PhpParser\Node\Expr\Variable) {
                if (is_string($var->name)) {
                    ...
                    if ($var->name === 'argv' || $var->name === 'argc') {
                        ...
                        $context->vars_in_scope[$var_id] = $global_context && $global_context->hasVariable($var_id) ? clone $global_context->vars_in_scope[$var_id] : VariableFetchAnalyzer::getGlobalType($var_id);
                        $context->vars_possibly_in_scope[$var_id] = true;
                    ...
                    $context->vars_in_scope[$var_id]->parent_nodes = [$assignment_node->id => $assignment_node];
