...
class ParseTreeCreator
{
    ...
    public function create() : ParseTree
    {
        ...
        if ($this->current_leaf !== $this->parse_tree && ($this->parse_tree instanceof ParseTree\GenericTree || $this->parse_tree instanceof ParseTree\CallableTree || $this->parse_tree instanceof ParseTree\KeyedArrayTree)) {
    ...
    private function createMethodParam(array $current_token, ParseTree $current_parent) : void
    {
        ...
        $new_parent_leaf = new ParseTree\MethodParamTree($current_token[0], $byref, $variadic, $current_parent);
    ...
    private function handleOpenRoundBracket() : void
    {
        ...
        $new_leaf = new ParseTree\EncapsulationTree($new_parent);
        if ($this->current_leaf instanceof ParseTree\Root) {
    ...
    private function handleClosedRoundBracket() : void
    {
        ...
        if ($prev_token !== null && $prev_token[0] === '(' && $this->current_leaf instanceof ParseTree\CallableTree) {
            return;
        }
        do {
            if ($this->current_leaf->parent === null) {
            ...
            $this->current_leaf = $this->current_leaf->parent;
        } while (!$this->current_leaf instanceof ParseTree\EncapsulationTree && !$this->current_leaf instanceof ParseTree\CallableTree && !$this->current_leaf instanceof ParseTree\MethodTree);
        if ($this->current_leaf instanceof ParseTree\EncapsulationTree || $this->current_leaf instanceof ParseTree\CallableTree) {
            $this->current_leaf->terminated = true;
        }
    }
    private function handleComma() : void
    {
        ...
        if ($context_node instanceof ParseTree\GenericTree || $context_node instanceof ParseTree\KeyedArrayTree || $context_node instanceof ParseTree\CallableTree || $context_node instanceof ParseTree\MethodTree) {
            $context_node = $context_node->parent;
        }
        while ($context_node && !$context_node instanceof ParseTree\GenericTree && !$context_node instanceof ParseTree\KeyedArrayTree && !$context_node instanceof ParseTree\CallableTree && !$context_node instanceof ParseTree\MethodTree) {
            $context_node = $context_node->parent;
        }
        if (!$context_node) {
    ...
    private function handleEllipsisOrEquals(array $type_token) : void
    {
        ...
        while ($current_parent && !$current_parent instanceof ParseTree\CallableTree && !$current_parent instanceof ParseTree\CallableParamTree) {
            $this->current_leaf = $current_parent;
            $current_parent = $current_parent->parent;
        }
        if (!$current_parent) {
            if ($this->current_leaf instanceof ParseTree\CallableTree && $type_token[0] === '...') {
    ...
    private function handleColon() : void
    {
        ...
        while (($current_parent instanceof ParseTree\UnionTree || $current_parent instanceof ParseTree\CallableWithReturnTypeTree) && $this->current_leaf->parent) {
            $this->current_leaf = $this->current_leaf->parent;
            $current_parent = $this->current_leaf->parent;
        }
    ...
    private function handleQuestionMark() : void
    {
        ...
        if ($next_token === null || $next_token[0] !== ':') {
            while (($this->current_leaf instanceof ParseTree\Value || $this->current_leaf instanceof ParseTree\UnionTree || $this->current_leaf instanceof ParseTree\KeyedArrayTree && $this->current_leaf->terminated || $this->current_leaf instanceof ParseTree\GenericTree && $this->current_leaf->terminated || $this->current_leaf instanceof ParseTree\EncapsulationTree && $this->current_leaf->terminated || $this->current_leaf instanceof ParseTree\CallableTree && $this->current_leaf->terminated || $this->current_leaf instanceof ParseTree\IntersectionTree) && $this->current_leaf->parent) {
                $this->current_leaf = $this->current_leaf->parent;
            }
            if ($this->current_leaf instanceof ParseTree\TemplateIsTree && $this->current_leaf->parent) {
                $current_parent = $this->current_leaf->parent;
                $new_leaf = new ParseTree\ConditionalTree($this->current_leaf, $this->current_leaf->parent);
                $this->current_leaf->parent = $new_leaf;
                array_pop($current_parent->children);
                $current_parent->children[] = $new_leaf;
                $this->current_leaf = $new_leaf;
                ...
                $new_leaf = new ParseTree\NullableTree($new_parent);
                if ($this->current_leaf instanceof ParseTree\Root) {
                    $this->current_leaf = $this->parse_tree = $new_leaf;
                    return;
                }
    ...
    private function handleIsOrAs(array $type_token) : void
    {
        if ($this->t === 0) {
            ...
            if ($type_token[0] === 'as') {
                $next_token = $this->t + 1 < $this->type_token_count ? $this->type_tokens[$this->t + 1] : null;
                if (!$this->current_leaf instanceof ParseTree\Value || !$current_parent instanceof ParseTree\GenericTree || !$next_token) {
                    throw new TypeParseTreeException('Unexpected token ' . $type_token[0]);
                }
                $this->current_leaf = new ParseTree\TemplateAsTree($this->current_leaf->value, $next_token[0], $current_parent);
                $current_parent->children[] = $this->current_leaf;
                ++$this->t;
            } elseif ($this->current_leaf instanceof ParseTree\Value) {
                $this->current_leaf = new ParseTree\TemplateIsTree($this->current_leaf->value, $current_parent);
                if ($current_parent) {
                    $current_parent->children[] = $this->current_leaf;
                }
            }
    ...
    private function handleValue(array $type_token) : void
    {
        ...
        switch ($next_token[0] ?? null) {
            case '<':
                $new_leaf = new ParseTree\GenericTree($type_token[0], $new_parent);
                ++$this->t;
                break;
            case '{':
                $new_leaf = new ParseTree\KeyedArrayTree($type_token[0], $new_parent);
                ++$this->t;
                break;
            case '(':
                if (in_array($type_token[0], ['callable', 'pure-callable', 'Closure', '\\Closure', 'pure-Closure'], true)) {
                    $new_leaf = new ParseTree\CallableTree($type_token[0], $new_parent);
                } elseif ($type_token[0] !== 'array' && $type_token[0][0] !== '\\' && $this->current_leaf instanceof ParseTree\Root) {
                    $new_leaf = new ParseTree\MethodTree($type_token[0], $new_parent);
                } else {
                    throw new TypeParseTreeException('Paranthesis must be preceded by “Closure”, “callable”, "pure-callable" or a valid @method name');
                }
                ++$this->t;
                break;
            case '::':
                ...
                if ($this->current_leaf instanceof ParseTree\KeyedArrayTree) {
                    throw new TypeParseTreeException('Unexpected :: in array key');
                }
                ...
                $new_leaf = new ParseTree\Value($type_token[0] . '::' . $nexter_token[0], $type_token[1], $type_token[1] + 2 + strlen($nexter_token[0]), $type_token[2] ?? null, $new_parent);
                $this->t += 2;
                break;
            default:
                ...
                $new_leaf = new ParseTree\Value($type_token[0], $type_token[1], $type_token[1] + strlen($type_token[0]), $type_token[2] ?? null, $new_parent);
                break;
        }
        if ($this->current_leaf instanceof ParseTree\Root) {
            $this->current_leaf = $this->parse_tree = $new_leaf;
