...
{
    public static function getAll(\Psalm\Codebase $codebase, \Psalm\Internal\Provider\ClassLikeStorageProvider $class_provider, \Psalm\Internal\Provider\FileStorageProvider $file_provider) : string
    {
        $namespaced_nodes = [];
        ...
        foreach ($class_provider->getAll() as $storage) {
            ...
            $namespaced_nodes[$namespace_name][$classlike_name] = ClassLikeStubGenerator::getClassLikeNode($codebase, $storage, $classlike_name);
        }
        $all_function_names = [];
    ...
    private static function getFunctionNode(\Psalm\Storage\FunctionLikeStorage $function_storage, string $function_name, string $namespace_name) : PhpParser\Node\Stmt\Function_
    {
    ...
    public static function getFunctionParamNodes(\Psalm\Storage\FunctionLikeStorage $method_storage) : array
    {
        ...
        foreach ($method_storage->params as $param) {
            $param_nodes[] = new VirtualParam(new VirtualVariable($param->name), $param->default_type ? self::getExpressionFromType($param->default_type) : null, $param->signature_type ? self::getParserTypeFromPsalmType($param->signature_type) : null, $param->by_ref, $param->is_variadic);
        }
        return $param_nodes;
    }
    /**
    ...
    public static function getParserTypeFromPsalmType(Type\Union $type) : ?PhpParser\NodeAbstract
    {
        ...
        foreach ($type->getAtomicTypes() as $atomic_type) {
            ...
            if ($atomic_type instanceof Type\Atomic\Scalar || $atomic_type instanceof Type\Atomic\TObject || $atomic_type instanceof Type\Atomic\TArray || $atomic_type instanceof Type\Atomic\TIterable) {
