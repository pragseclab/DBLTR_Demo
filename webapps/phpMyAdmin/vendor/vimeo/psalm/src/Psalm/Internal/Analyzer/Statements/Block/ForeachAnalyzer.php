...
class ForeachAnalyzer
{
    ...
    public static function analyze(StatementsAnalyzer $statements_analyzer, PhpParser\Node\Stmt\Foreach_ $stmt, Context $context) : ?bool
    {
        ...
        if ($doc_comment) {
            try {
                ...
                if (IssueBuffer::accepts(new InvalidDocblock($e->getMessage(), new CodeLocation($statements_analyzer, $stmt)))) {
                    // fall through
                }
            }
        }
        ...
        foreach ($var_comments as $var_comment) {
            ...
            $comment_type = \Psalm\Internal\Type\TypeExpander::expandUnion($codebase, $var_comment->type, $context->self, $context->self, $statements_analyzer->getParentFQCLN());
            $type_location = null;
            ...
            if (isset($context->vars_in_scope[$var_comment->var_id]) || VariableFetchAnalyzer::isSuperGlobal($var_comment->var_id)) {
                if ($codebase->find_unused_variables && $doc_comment && $type_location && isset($context->vars_in_scope[$var_comment->var_id]) && $context->vars_in_scope[$var_comment->var_id]->getId() === $comment_type->getId() && !$comment_type->isMixed()) {
                    ...
                    } elseif (IssueBuffer::accepts(new UnnecessaryVarAnnotation('The @var ' . $comment_type . ' annotation for ' . $var_comment->var_id . ' is unnecessary', $type_location), [], true)) {
                        // fall through
                    }
                }
        ...
        if ($stmt->keyVar && $stmt->keyVar instanceof PhpParser\Node\Expr\Variable && is_string($stmt->keyVar->name)) {
            ...
            AssignmentAnalyzer::analyze($statements_analyzer, $stmt->keyVar, $stmt->expr, $key_type, $foreach_context, $doc_comment, ['$' . $stmt->keyVar->name => true]);
        }
        $value_type = $value_type ?: Type::getMixed();
        if ($stmt->byRef) {
            $value_type->by_ref = true;
        }
        AssignmentAnalyzer::analyze($statements_analyzer, $stmt->valueVar, $stmt->expr, $value_type, $foreach_context, $doc_comment, $stmt->valueVar instanceof PhpParser\Node\Expr\Variable && is_string($stmt->valueVar->name) ? ['$' . $stmt->valueVar->name => true] : []);
        foreach ($var_comments as $var_comment) {
            if (!$var_comment->var_id || !$var_comment->type) {
                continue;
            }
        ...
        LoopAnalyzer::analyze($statements_analyzer, $stmt->stmts, [], [], $loop_scope, $inner_loop_context, false, $always_non_empty_array);
        ...
        $context->referenced_var_ids = array_intersect_key($foreach_context->referenced_var_ids, $context->referenced_var_ids);
     ...
     * @return false|null
     */
    public static function checkIteratorType(StatementsAnalyzer $statements_analyzer, PhpParser\NodeAbstract $stmt, PhpParser\Node\Expr $expr, Type\Union $iterator_type, Codebase $codebase, Context $context, ?Type\Union &$key_type, ?Type\Union &$value_type, bool &$always_non_empty_array) : ?bool
    {
        if ($iterator_type->isNull()) {
            if (IssueBuffer::accepts(new NullIterator('Cannot iterate over null', new CodeLocation($statements_analyzer->getSource(), $expr)), $statements_analyzer->getSuppressedIssues())) {
                return false;
            }
        } elseif ($iterator_type->isNullable() && !$iterator_type->ignore_nullable_issues) {
            if (IssueBuffer::accepts(new PossiblyNullIterator('Cannot iterate over nullable var ' . $iterator_type, new CodeLocation($statements_analyzer->getSource(), $expr)), $statements_analyzer->getSuppressedIssues())) {
                return false;
            }
        } elseif ($iterator_type->isFalsable() && !$iterator_type->ignore_falsable_issues) {
            if (IssueBuffer::accepts(new PossiblyFalseIterator('Cannot iterate over falsable var ' . $iterator_type, new CodeLocation($statements_analyzer->getSource(), $expr)), $statements_analyzer->getSuppressedIssues())) {
                return false;
            }
        }
        ...
        foreach ($iterator_type->getAtomicTypes() as $iterator_atomic_type) {
            ...
            if ($iterator_atomic_type instanceof Type\Atomic\TArray && $iterator_atomic_type->type_params[1]->isEmpty()) {
                $always_non_empty_array = false;
                $has_valid_iterator = true;
                continue;
            }
            if ($iterator_atomic_type instanceof Type\Atomic\TNull || $iterator_atomic_type instanceof Type\Atomic\TFalse) {
                $always_non_empty_array = false;
                continue;
            }
            if ($iterator_atomic_type instanceof Type\Atomic\TArray || $iterator_atomic_type instanceof Type\Atomic\TKeyedArray || $iterator_atomic_type instanceof Type\Atomic\TList) {
                if ($iterator_atomic_type instanceof Type\Atomic\TKeyedArray) {
                    ...
                    $list_var_id = ExpressionIdentifier::getArrayVarId($expr, $statements_analyzer->getFQCLN(), $statements_analyzer);
                    if (!$iterator_atomic_type instanceof Type\Atomic\TNonEmptyList) {
                        $always_non_empty_array = false;
                    }
                    $iterator_atomic_type = new Type\Atomic\TArray([$list_var_id ? new Type\Union([new Type\Atomic\TDependentListKey($list_var_id)]) : Type::getInt(), $iterator_atomic_type->type_param]);
                ...
                ArrayFetchAnalyzer::taintArrayFetch($statements_analyzer, $expr, null, $value_type, Type::getMixed());
            ...
            $always_non_empty_array = false;
            if ($iterator_atomic_type instanceof Type\Atomic\Scalar || $iterator_atomic_type instanceof Type\Atomic\TVoid) {
                $invalid_iterator_types[] = $iterator_atomic_type->getKey();
                $value_type = Type::getMixed();
            } elseif ($iterator_atomic_type instanceof Type\Atomic\TObject || $iterator_atomic_type instanceof Type\Atomic\TMixed || $iterator_atomic_type instanceof Type\Atomic\TEmpty) {
                $has_valid_iterator = true;
                $value_type = Type::getMixed();
                ArrayFetchAnalyzer::taintArrayFetch($statements_analyzer, $expr, null, $value_type, Type::getMixed());
                if (!$context->pure) {
                    if ($statements_analyzer->getSource() instanceof \Psalm\Internal\Analyzer\FunctionLikeAnalyzer && $statements_analyzer->getSource()->track_mutations) {
                        $statements_analyzer->getSource()->inferred_has_mutation = true;
                        $statements_analyzer->getSource()->inferred_impure = true;
                    }
                } else {
                    if (IssueBuffer::accepts(new ImpureMethodCall('Cannot call a possibly-mutating iterator from a pure context', new CodeLocation($statements_analyzer, $stmt)), $statements_analyzer->getSuppressedIssues())) {
                        // fall through
                    }
                }
                ...
                if (!$context->pure) {
                    ...
                    if (IssueBuffer::accepts(new ImpureMethodCall('Cannot call a possibly-mutating Traversable::getIterator from a pure context', new CodeLocation($statements_analyzer, $stmt)), $statements_analyzer->getSuppressedIssues())) {
                        // fall through
                    }
                }
                ...
                if ($iterator_atomic_type->value !== 'Traversable' && $iterator_atomic_type->value !== $statements_analyzer->getClassName()) {
                    if (ClassLikeAnalyzer::checkFullyQualifiedClassLikeName($statements_analyzer, $iterator_atomic_type->value, new CodeLocation($statements_analyzer->getSource(), $expr), $context->self, $context->calling_method_id, $statements_analyzer->getSuppressedIssues()) === false) {
                        return false;
                    }
                }
                if (AtomicTypeComparator::isContainedBy($codebase, $iterator_atomic_type, new Type\Atomic\TIterable([Type::getMixed(), Type::getMixed()]))) {
        ...
        if ($raw_object_types) {
            if ($has_valid_iterator) {
                if (IssueBuffer::accepts(new PossibleRawObjectIteration('Possibly undesired iteration over regular object ' . \reset($raw_object_types), new CodeLocation($statements_analyzer->getSource(), $expr)), $statements_analyzer->getSuppressedIssues())) {
                    // fall through
                }
            } else {
                if (IssueBuffer::accepts(new RawObjectIteration('Possibly undesired iteration over regular object ' . \reset($raw_object_types), new CodeLocation($statements_analyzer->getSource(), $expr)), $statements_analyzer->getSuppressedIssues())) {
                    // fall through
                }
            }
        }
        if ($invalid_iterator_types) {
            if ($has_valid_iterator) {
                if (IssueBuffer::accepts(new PossiblyInvalidIterator('Cannot iterate over ' . $invalid_iterator_types[0], new CodeLocation($statements_analyzer->getSource(), $expr)), $statements_analyzer->getSuppressedIssues())) {
                    // fall through
                }
            } else {
                if (IssueBuffer::accepts(new InvalidIterator('Cannot iterate over ' . $invalid_iterator_types[0], new CodeLocation($statements_analyzer->getSource(), $expr)), $statements_analyzer->getSuppressedIssues())) {
                    // fall through
                }
            }
        }
    ...
    public static function handleIterable(StatementsAnalyzer $statements_analyzer, Type\Atomic\TNamedObject $iterator_atomic_type, PhpParser\Node\Expr $foreach_expr, Codebase $codebase, Context $context, ?Type\Union &$key_type, ?Type\Union &$value_type, bool &$has_valid_iterator) : void
    {
        if ($iterator_atomic_type->extra_types) {
            $iterator_atomic_type_copy = clone $iterator_atomic_type;
            $iterator_atomic_type_copy->extra_types = [];
        ...
        foreach ($iterator_atomic_types as $iterator_atomic_type) {
            if ($iterator_atomic_type instanceof Type\Atomic\TTemplateParam || $iterator_atomic_type instanceof Type\Atomic\TObjectWithProperties) {
                throw new \UnexpectedValueException('Shouldn’t get a generic param here');
            }
            $has_valid_iterator = true;
            if ($iterator_atomic_type instanceof Type\Atomic\TNamedObject && strtolower($iterator_atomic_type->value) === 'simplexmlelement') {
                if ($value_type) {
                    $value_type = Type::combineUnionTypes($value_type, new Type\Union([clone $iterator_atomic_type]));
                } else {
            ...
            if ($iterator_atomic_type instanceof Type\Atomic\TIterable || (strtolower($iterator_atomic_type->value) === 'traversable' || $codebase->classImplements($iterator_atomic_type->value, 'Traversable') || $codebase->interfaceExists($iterator_atomic_type->value) && $codebase->interfaceExtends($iterator_atomic_type->value, 'Traversable'))) {
                    ...
                    if ($iterator_class_type) {
                        foreach ($iterator_class_type->getAtomicTypes() as $array_atomic_type) {
                            ...
                            if ($array_atomic_type instanceof Type\Atomic\TArray || $array_atomic_type instanceof Type\Atomic\TKeyedArray) {
                                ...
                                if ($array_atomic_type instanceof Type\Atomic\TNamedObject && $codebase->classExists($array_atomic_type->value) && $codebase->classImplements($array_atomic_type->value, 'Traversable')) {
                                ...
                                if ($array_atomic_type instanceof Type\Atomic\TIterable || $array_atomic_type instanceof Type\Atomic\TNamedObject && ($array_atomic_type->value === 'Traversable' || $codebase->classOrInterfaceExists($array_atomic_type->value) && $codebase->classImplements($array_atomic_type->value, 'Traversable'))) {
    ...
    public static function getKeyValueParamsForTraversableObject(Type\Atomic $iterator_atomic_type, Codebase $codebase, ?Type\Union &$key_type, ?Type\Union &$value_type) : void
    {
        if ($iterator_atomic_type instanceof Type\Atomic\TIterable || $iterator_atomic_type instanceof Type\Atomic\TGenericObject && strtolower($iterator_atomic_type->value) === 'traversable') {
        ...
        if ($iterator_atomic_type instanceof Type\Atomic\TNamedObject && ($codebase->classImplements($iterator_atomic_type->value, 'Traversable') || $codebase->interfaceExtends($iterator_atomic_type->value, 'Traversable'))) {
            ...
            if ($generic_storage->template_types || $iterator_atomic_type instanceof Type\Atomic\TGenericObject) {
                // if we're just being passed the non-generic class itself, assume
                // that it's inside the calling class
                $passed_type_params = $iterator_atomic_type instanceof Type\Atomic\TGenericObject ? $iterator_atomic_type->type_params : array_values(array_map(
    ...
    private static function getFakeMethodCallType(StatementsAnalyzer $statements_analyzer, PhpParser\Node\Expr $foreach_expr, Context $context, string $method_name) : ?Type\Union
    {
        $old_data_provider = $statements_analyzer->node_data;
        $statements_analyzer->node_data = clone $statements_analyzer->node_data;
     ...
     */
    private static function getExtendedType(string $template_name, string $template_class, string $calling_class, array $template_extended_params, ?array $class_template_types = null, ?array $calling_type_params = null) : ?Type\Union
    {
