...
class TemplateStandinTypeReplacer
{
     ...
     */
    public static function replace(Union $union_type, TemplateResult $template_result, ?Codebase $codebase, ?StatementsAnalyzer $statements_analyzer, ?Union $input_type, ?int $input_arg_offset = null, ?string $calling_class = null, ?string $calling_function = null, bool $replace = true, bool $add_upper_bound = false, int $depth = 0) : Union
    {
        $atomic_types = [];
        ...
        foreach ($original_atomic_types as $key => $atomic_type) {
            $atomic_types = array_merge($atomic_types, self::handleAtomicStandin($atomic_type, $key, $template_result, $codebase, $statements_analyzer, $input_type, $input_arg_offset, $calling_class, $calling_function, $replace, $add_upper_bound, $depth, count($original_atomic_types) === 1, $had_template));
        }
        if ($replace) {
     ...
     * @return list<Atomic>
     */
    private static function handleAtomicStandin(Atomic $atomic_type, string $key, TemplateResult $template_result, ?Codebase $codebase, ?StatementsAnalyzer $statements_analyzer, ?Union $input_type, ?int $input_arg_offset, ?string $calling_class, ?string $calling_function, bool $replace, bool $add_upper_bound, int $depth, bool $was_single, bool &$had_template) : array
    {
        if ($bracket_pos = strpos($key, '<')) {
            $key = substr($key, 0, $bracket_pos);
        }
        if ($atomic_type instanceof Atomic\TTemplateParam && isset($template_result->template_types[$atomic_type->param_name][$atomic_type->defining_class])) {
            return self::handleTemplateParamStandin($atomic_type, $key, $input_type, $input_arg_offset, $calling_class, $calling_function, $template_result, $codebase, $statements_analyzer, $replace, $add_upper_bound, $depth, $had_template);
        }
        if ($atomic_type instanceof Atomic\TTemplateParamClass && isset($template_result->template_types[$atomic_type->param_name][$atomic_type->defining_class])) {
            if ($replace) {
                return self::handleTemplateParamClassStandin($atomic_type, $input_type, $input_arg_offset, $template_result, $depth, $was_single);
            }
        }
        if ($atomic_type instanceof Atomic\TTemplateIndexedAccess) {
            if ($replace) {
                $atomic_types = [];
                $include_first = true;
                if (isset($template_result->template_types[$atomic_type->array_param_name][$atomic_type->defining_class]) && !empty($template_result->upper_bounds[$atomic_type->offset_param_name])) {
                    $array_template_type = $template_result->template_types[$atomic_type->array_param_name][$atomic_type->defining_class];
                    $offset_template_type = array_values($template_result->upper_bounds[$atomic_type->offset_param_name])[0]->type;
                    if ($array_template_type->isSingle() && $offset_template_type->isSingle() && !$array_template_type->isMixed() && !$offset_template_type->isMixed()) {
                        ...
                        if ($array_template_type instanceof Atomic\TKeyedArray && ($offset_template_type instanceof Atomic\TLiteralString || $offset_template_type instanceof Atomic\TLiteralInt) && isset($array_template_type->properties[$offset_template_type->value])) {
                            $include_first = false;
                            $replacement_type = clone $array_template_type->properties[$offset_template_type->value];
            ...
            if ($replace) {
                ...
                if (isset($template_result->template_types[$atomic_type->param_name][$atomic_type->defining_class])) {
                    ...
                    if ($template_type->isSingle()) {
                        ...
                        if ($template_type instanceof Atomic\TKeyedArray || $template_type instanceof Atomic\TArray || $template_type instanceof Atomic\TList) {
        ...
        if ($input_type && $codebase && !$input_type->hasMixed()) {
            $matching_atomic_types = self::findMatchingAtomicTypesForTemplate($input_type, $atomic_type, $key, $codebase, $statements_analyzer);
        }
        if (!$matching_atomic_types) {
            $atomic_type = $atomic_type->replaceTemplateTypesWithStandins($template_result, $codebase, $statements_analyzer, null, $input_arg_offset, $calling_class, $calling_function, $replace, $add_upper_bound, $depth + 1);
            return [$atomic_type];
        }
        $atomic_types = [];
        foreach ($matching_atomic_types as $matching_atomic_type) {
            $atomic_types[] = $atomic_type->replaceTemplateTypesWithStandins($template_result, $codebase, $statements_analyzer, $matching_atomic_type, $input_arg_offset, $calling_class, $calling_function, $replace, $add_upper_bound, $depth + 1);
        }
        return $atomic_types;
    }
    /**
     * @return list<Atomic>
     */
    private static function findMatchingAtomicTypesForTemplate(Union $input_type, Atomic $base_type, string $key, Codebase $codebase, ?StatementsAnalyzer $statements_analyzer) : array
    {
        $matching_atomic_types = [];
        foreach ($input_type->getAtomicTypes() as $input_key => $atomic_input_type) {
            ...
            if ($atomic_input_type instanceof Atomic\TCallable && $base_type instanceof Atomic\TCallable) {
                $matching_atomic_types[$atomic_input_type->getId()] = $atomic_input_type;
                continue;
            }
            ...
            if (($atomic_input_type instanceof Atomic\TArray || $atomic_input_type instanceof Atomic\TKeyedArray || $atomic_input_type instanceof Atomic\TList) && $key === 'iterable') {
                $matching_atomic_types[$atomic_input_type->getId()] = $atomic_input_type;
                continue;
            }
            ...
            if ($atomic_input_type instanceof Atomic\TLiteralClassString && $base_type instanceof Atomic\TClassString && $base_type->as_type) {
                try {
                    ...
                    if (!empty($classlike_storage->template_extended_params[$base_type->as_type->value])) {
                        $atomic_input_type = new Atomic\TClassString($base_type->as_type->value, new Atomic\TGenericObject($base_type->as_type->value, array_values($classlike_storage->template_extended_params[$base_type->as_type->value])));
                        $matching_atomic_types[$atomic_input_type->getId()] = $atomic_input_type;
                        continue;
                    }
            ...
            if ($atomic_input_type instanceof Atomic\TNamedObject && ($base_type instanceof Atomic\TNamedObject || $base_type instanceof Atomic\TIterable)) {
                if ($base_type instanceof Atomic\TIterable) {
                    if ($atomic_input_type->value === 'Traversable') {
                        $matching_atomic_types[$atomic_input_type->getId()] = $atomic_input_type;
                        continue;
                    }
                    $base_type = new Atomic\TGenericObject('Traversable', $base_type->type_params);
                }
                try {
                    $classlike_storage = $codebase->classlike_storage_provider->get($atomic_input_type->value);
                    if ($atomic_input_type instanceof Atomic\TGenericObject && isset($classlike_storage->template_extended_params[$base_type->value])) {
                        $matching_atomic_types[$atomic_input_type->getId()] = $atomic_input_type;
                        continue;
                    }
                    if (!empty($classlike_storage->template_extended_params[$base_type->value])) {
                        $atomic_input_type = new Atomic\TGenericObject($atomic_input_type->value, array_values($classlike_storage->template_extended_params[$base_type->value]));
                        $matching_atomic_types[$atomic_input_type->getId()] = $atomic_input_type;
                        continue;
                    }
                } catch (\InvalidArgumentException $e) {
    ...
    private static function handleTemplateParamStandin(Atomic\TTemplateParam $atomic_type, string $key, ?Union $input_type, ?int $input_arg_offset, ?string $calling_class, ?string $calling_function, TemplateResult $template_result, ?Codebase $codebase, ?StatementsAnalyzer $statements_analyzer, bool $replace, bool $add_upper_bound, int $depth, bool &$had_template) : array
    {
        ...
        if ($atomic_type->extra_types) {
            foreach ($atomic_type->extra_types as $extra_type) {
                $extra_type = self::replace(new \Psalm\Type\Union([$extra_type]), $template_result, $codebase, $statements_analyzer, $input_type, $input_arg_offset, $calling_class, $calling_function, $replace, $add_upper_bound, $depth + 1);
                if ($extra_type->isSingle()) {
                    $extra_type = array_values($extra_type->getAtomicTypes())[0];
                    if ($extra_type instanceof Atomic\TNamedObject || $extra_type instanceof Atomic\TTemplateParam || $extra_type instanceof Atomic\TIterable || $extra_type instanceof Atomic\TObjectWithProperties) {
        ...
        if ($replace) {
            ...
            if ($replacement_type->hasMixed() && !$atomic_type->as->hasMixed()) {
                ...
                if ($codebase) {
                    $replacement_type = TypeExpander::expandUnion($codebase, $replacement_type, $calling_class, $calling_class, null);
                }
                if ($depth < 10) {
                    $replacement_type = self::replace($replacement_type, $template_result, $codebase, $statements_analyzer, $input_type, $input_arg_offset, $calling_class, $calling_function, $replace, $add_upper_bound, $depth + 1);
                }
                foreach ($replacement_type->getAtomicTypes() as $replacement_atomic_type) {
                    $replacements_found = false;
                    // @codingStandardsIgnoreStart
                    if ($replacement_atomic_type instanceof Atomic\TTemplateKeyOf && isset($template_result->template_types[$replacement_atomic_type->param_name][$replacement_atomic_type->defining_class])) {
                        ...
                        if ($keyed_template instanceof Atomic\TKeyedArray || $keyed_template instanceof Atomic\TArray || $keyed_template instanceof Atomic\TList) {
                            ...
                            $template_result->upper_bounds[$atomic_type->param_name][$atomic_type->defining_class]->type = clone $key_type;
                        }
                    }
                    if ($replacement_atomic_type instanceof Atomic\TTemplateParam && $replacement_atomic_type->defining_class !== $calling_class && $replacement_atomic_type->defining_class !== 'fn-' . $calling_function) {
            ...
            $matching_input_keys = [];
            if ($codebase) {
                $atomic_type->as = TypeExpander::expandUnion($codebase, $atomic_type->as, $calling_class, $calling_class, null);
            }
            $atomic_type->as = self::replace($atomic_type->as, $template_result, $codebase, $statements_analyzer, $input_type, $input_arg_offset, $calling_class, $calling_function, $replace, $add_upper_bound, $depth + 1);
            if ($input_type && !$template_result->readonly && ($atomic_type->as->isMixed() || !$codebase || UnionTypeComparator::canBeContainedBy($codebase, $input_type, $atomic_type->as, false, false, $matching_input_keys))) {
                ...
                if (isset($template_result->upper_bounds[$param_name_key][$atomic_type->defining_class])) {
                    $existing_generic_param = $template_result->upper_bounds[$param_name_key][$atomic_type->defining_class];
                    ...
                    if ($existing_depth === $depth || $input_arg_offset !== $existing_arg_offset) {
                        $generic_param = \Psalm\Type::combineUnionTypes($template_result->upper_bounds[$param_name_key][$atomic_type->defining_class]->type, $generic_param, $codebase);
                    }
                }
                $template_result->upper_bounds[$param_name_key][$atomic_type->defining_class] = new TemplateBound($generic_param, $depth, $input_arg_offset);
            }
            foreach ($atomic_types as &$atomic_type) {
                if ($atomic_type instanceof Atomic\TNamedObject || $atomic_type instanceof Atomic\TTemplateParam || $atomic_type instanceof Atomic\TIterable || $atomic_type instanceof Atomic\TObjectWithProperties) {
        ...
        if ($add_upper_bound && $input_type && !$template_result->readonly) {
            $matching_input_keys = [];
            if ($codebase && UnionTypeComparator::canBeContainedBy($codebase, $input_type, $replacement_type, false, false, $matching_input_keys)) {
                $generic_param = clone $input_type;
                if ($matching_input_keys) {
                    $generic_param_keys = \array_keys($generic_param->getAtomicTypes());
                    foreach ($generic_param_keys as $atomic_key) {
                ...
                if (isset($template_result->lower_bounds[$param_name_key][$atomic_type->defining_class])) {
                    if (!UnionTypeComparator::isContainedBy($codebase, $template_result->lower_bounds[$param_name_key][$atomic_type->defining_class]->type, $generic_param) || !UnionTypeComparator::isContainedBy($codebase, $generic_param, $template_result->lower_bounds[$param_name_key][$atomic_type->defining_class]->type)) {
                        $intersection_type = \Psalm\Type::intersectUnionTypes($template_result->lower_bounds[$param_name_key][$atomic_type->defining_class]->type, $generic_param, $codebase);
                    } else {
                        $intersection_type = $generic_param;
                    }
                    if ($intersection_type) {
                        $template_result->lower_bounds[$param_name_key][$atomic_type->defining_class]->type = $intersection_type;
                    } else {
                        $template_result->lower_bounds_unintersectable_types[] = $template_result->lower_bounds[$param_name_key][$atomic_type->defining_class]->type;
                        $template_result->lower_bounds_unintersectable_types[] = $generic_param;
                        $template_result->lower_bounds[$param_name_key][$atomic_type->defining_class]->type = \Psalm\Type::getMixed();
                    }
                } else {
                    $template_result->lower_bounds[$param_name_key][$atomic_type->defining_class] = new TemplateBound($generic_param);
                }
            }
        }
        return [$atomic_type];
     ...
     */
    public static function handleTemplateParamClassStandin(Atomic\TTemplateParamClass $atomic_type, ?Union $input_type, ?int $input_arg_offset, TemplateResult $template_result, int $depth, bool $was_single) : array
    {
        ...
        if ($input_type && !$template_result->readonly) {
            ...
            foreach ($input_type->getAtomicTypes() as $input_atomic_type) {
                if ($input_atomic_type instanceof Atomic\TLiteralClassString) {
                    $valid_input_atomic_types[] = new Atomic\TNamedObject($input_atomic_type->value);
                } elseif ($input_atomic_type instanceof Atomic\TTemplateParamClass) {
                    $valid_input_atomic_types[] = new Atomic\TTemplateParam($input_atomic_type->param_name, $input_atomic_type->as_type ? new Union([$input_atomic_type->as_type]) : ($input_atomic_type->as === 'object' ? \Psalm\Type::getObject() : \Psalm\Type::getMixed()), $input_atomic_type->defining_class);
                    ...
                    if ($input_atomic_type->as_type) {
                        ...
                        $valid_input_atomic_types[] = new Atomic\TNamedObject($input_atomic_type->as);
                    } else {
            ...
            if ($generic_param) {
                if (isset($template_result->upper_bounds[$atomic_type->param_name][$atomic_type->defining_class])) {
                    $template_result->upper_bounds[$atomic_type->param_name][$atomic_type->defining_class] = new TemplateBound(\Psalm\Type::combineUnionTypes($generic_param, $template_result->upper_bounds[$atomic_type->param_name][$atomic_type->defining_class]->type), $depth);
                } else {
                    $template_result->upper_bounds[$atomic_type->param_name][$atomic_type->defining_class] = new TemplateBound($generic_param, $depth, $input_arg_offset);
                }
            }
        } else {
            $template_type = $template_result->template_types[$atomic_type->param_name][$atomic_type->defining_class];
            foreach ($template_type->getAtomicTypes() as $atomic_type) {
                if ($atomic_type instanceof Atomic\TNamedObject) {
                    $atomic_types[] = new Atomic\TClassString($atomic_type->value, $atomic_type);
     ...
     */
    public static function getRootTemplateType(array $template_types, string $param_name, string $defining_class, array $visited_classes = []) : ?Union
    {
        ...
        if (isset($template_types[$param_name][$defining_class])) {
            ...
            if (count($mapped_type_atomic_types) > 1 || !$mapped_type_atomic_types[0] instanceof Atomic\TTemplateParam) {
                return $template_types[$param_name][$defining_class]->type;
            }
            $first_template = $mapped_type_atomic_types[0];
            return self::getRootTemplateType($template_types, $first_template->param_name, $first_template->defining_class, $visited_classes + [$defining_class => true]) ?? $template_types[$param_name][$defining_class]->type;
        }
        return null;
    }
    /**
    ...
    public static function getMappedGenericTypeParams(Codebase $codebase, Atomic $input_type_part, Atomic $container_type_part, ?array &$container_type_params_covariant = null) : array
    {
        ...
        if ($input_type_part->value !== $container_type_part->value && $input_class_storage) {
            ...
            if ($input_template_types && (!$input_type_part instanceof Atomic\TGenericObject || !$input_type_part->remapped_params) && (!$container_type_part instanceof Atomic\TGenericObject || !$container_type_part->remapped_params)) {
                ...
                foreach ($params as $extended_input_param_type) {
                    ...
                    foreach ($extended_input_param_type->getAtomicTypes() as $et) {
                        if ($et instanceof Atomic\TTemplateParam) {
                            $ets = \Psalm\Internal\Codebase\Methods::getExtendedTemplatedTypes($et, $template_extends);
                        } else {
                            $ets = [];
                        }
                        if ($ets && $ets[0] instanceof Atomic\TTemplateParam && isset($input_class_storage->template_types[$ets[0]->param_name][$ets[0]->defining_class])) {
                        ...
                        if (!$new_input_param) {
                            ...
                            $new_input_param = \Psalm\Type::combineUnionTypes($new_input_param, $candidate_param_type);
                        }
                    }
                    $new_input_param = clone $new_input_param;
                    TemplateInferredTypeReplacer::replace($new_input_param, new TemplateResult([], $replacement_templates), $codebase);
