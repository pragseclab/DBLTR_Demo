...
class Reflection
{
    ...
    public function registerClass(\ReflectionClass $reflected_class) : void
    {
        ...
        $public_mapped_properties = PropertyMap::inPropertyMap($class_name) ? PropertyMap::getPropertyMap()[strtolower($class_name)] : [];
        ...
        foreach ($class_constants as $name => $value) {
            $storage->constants[$name] = new \Psalm\Storage\ClassConstantStorage(ClassLikeAnalyzer::getTypeFromValue($value), ClassLikeAnalyzer::VISIBILITY_PUBLIC, null);
        }
    ...
    public function extractReflectionMethodInfo(\ReflectionMethod $method) : void
    {
        ...
        $class_storage->declaring_method_ids[$method_name_lc] = new \Psalm\Internal\MethodIdentifier($declaring_class->name, $method_name_lc);
    ...
    private function getReflectionParamData(\ReflectionParameter $param) : FunctionLikeParameter
    {
        ...
        $parameter = new FunctionLikeParameter($param_name, $param->isPassedByReference(), $param_type, null, null, $is_optional, $param_type->isNullable(), $param->isVariadic());
