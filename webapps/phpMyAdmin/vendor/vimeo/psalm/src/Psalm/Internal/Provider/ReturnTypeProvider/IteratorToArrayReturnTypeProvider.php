...
class IteratorToArrayReturnTypeProvider implements \Psalm\Plugin\EventHandler\FunctionReturnTypeProviderInterface
{
    ...
    public static function getFunctionReturnType(FunctionReturnTypeProviderEvent $event) : Type\Union
    {
        ...
        if (($first_arg_type = $statements_source->node_data->getType($call_args[0]->value)) && $first_arg_type->hasObjectType()) {
            $key_type = null;
            $value_type = null;
            $codebase = $statements_source->getCodebase();
            foreach ($first_arg_type->getAtomicTypes() as $call_arg_atomic_type) {
                if ($call_arg_atomic_type instanceof Type\Atomic\TNamedObject && AtomicTypeComparator::isContainedBy($codebase, $call_arg_atomic_type, new Type\Atomic\TIterable([Type::getMixed(), Type::getMixed()]))) {
            ...
            if ($value_type) {
                ...
                if ($second_arg_type && (string) $second_arg_type === 'false') {
                    return new Type\Union([new Type\Atomic\TList($value_type)]);
                }
                ...
                return new Type\Union([new Type\Atomic\TArray([$key_type, $value_type])]);
            }
        }
