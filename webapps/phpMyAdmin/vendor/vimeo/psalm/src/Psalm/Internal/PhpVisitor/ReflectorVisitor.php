    ...
    public function enterNode(PhpParser\Node $node) : ?int
    {
        foreach ($node->getComments() as $comment) {
            if ($comment instanceof PhpParser\Comment\Doc && !$node instanceof PhpParser\Node\Stmt\ClassLike) {
                $self_fqcln = $node instanceof PhpParser\Node\Stmt\ClassLike && $node->name !== null ? ($this->aliases->namespace ? $this->aliases->namespace . '\\' : '') . $node->name->name : null;
                try {
                    $type_aliases = Reflector\ClassLikeNodeScanner::getTypeAliasesFromComment($comment, $this->aliases, $this->type_aliases, $self_fqcln);
                    ...
                    $this->file_storage->docblock_issues[] = new InvalidDocblock($e->getMessage(), new CodeLocation($this->file_scanner, $node, null, true));
        ...
        if ($node instanceof PhpParser\Node\Stmt\Namespace_) {
            ...
            $classlike_node_scanner = new Reflector\ClassLikeNodeScanner($this->codebase, $this->file_storage, $this->file_scanner, $this->aliases, $this->namespace_name);
            ...
            $functionlike_node_scanner = new Reflector\FunctionLikeNodeScanner($this->codebase, $this->file_scanner, $this->file_storage, $this->aliases, $this->type_aliases, $classlike_storage, $functionlike_types);
            $functionlike_node_scanner->start($node);
            $this->functionlike_node_scanners[] = $functionlike_node_scanner;
            if ($classlike_storage && $this->codebase->php_major_version >= 8 && $node instanceof PhpParser\Node\Stmt\ClassMethod && strtolower($node->name->name) === '__tostring') {
            ...
            foreach ($node->consts as $const) {
                $const_type = SimpleTypeInferer::infer($this->codebase, new \Psalm\Internal\Provider\NodeDataProvider(), $const->value, $this->aliases) ?: Type::getMixed();
            ...
            if (!$this->fq_classlike_names && !$this->functionlike_node_scanners) {
                ...
                if (Reflector\ExpressionResolver::enterConditional($this->codebase, $this->file_path, $this->exists_cond_expr) === false) {
            ...
            if ($this->skip_if_descendants === $node->getLine()) {
                ...
                if ($this->exists_cond_expr && Reflector\ExpressionResolver::enterConditional($this->codebase, $this->file_path, $this->exists_cond_expr) === true) {
            ...
            Reflector\ExpressionScanner::scan($this->codebase, $this->file_scanner, $this->file_storage, $this->aliases, $node, $functionlike_storage, $this->skip_if_descendants);
        }
        ...
        if ($node instanceof PhpParser\Node\Stmt\Namespace_) {
            ...
            if ($this->codebase->register_stub_files && $node->name && $node->name->parts === ['PHPSTORM_META']) {
                foreach ($node->stmts as $meta_stmt) {
                    if ($meta_stmt instanceof PhpParser\Node\Stmt\Expression && $meta_stmt->expr instanceof PhpParser\Node\Expr\FuncCall && $meta_stmt->expr->name instanceof PhpParser\Node\Name && $meta_stmt->expr->name->parts === ['override'] && count($meta_stmt->expr->args) > 1) {
            ...
            if ($functionlike_node_scanner->storage) {
                foreach ($functionlike_node_scanner->storage->docblock_issues as $docblock_issue) {
                    if (strpos($docblock_issue->code_location->file_path, 'CoreGenericFunctions.phpstub') || strpos($docblock_issue->code_location->file_path, 'CoreGenericClasses.phpstub') || strpos($this->file_path, 'CoreGenericIterators.phpstub')) {
                        ...
                        $message = $e instanceof \Psalm\Issue\TaintedInput ? $e->getJourneyMessage() : $e->message;
                        throw new \Psalm\Exception\CodeException('Error with core stub file docblocks: ' . $issue_type . ' - ' . $e->getShortLocationWithPrevious() . ':' . $e->code_location->getColumn() . ' - ' . $message);
                    }
                }
