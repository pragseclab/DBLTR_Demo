...
class ArrayMapReturnTypeProvider implements \Psalm\Plugin\EventHandler\FunctionReturnTypeProviderInterface
{
    ...
    public static function getFunctionReturnType(FunctionReturnTypeProviderEvent $event) : Type\Union
    {
            ...
            if ($closure_types = $function_call_type->getClosureTypes()) {
                ...
                if ($function_call_arg->value instanceof PhpParser\Node\Expr\Array_ && isset($function_call_arg->value->items[0]) && isset($function_call_arg->value->items[1]) && $function_call_arg->value->items[1]->value instanceof PhpParser\Node\Scalar\String_ && $function_call_arg->value->items[0]->value instanceof PhpParser\Node\Expr\Variable && ($variable_type = $statements_source->node_data->getType($function_call_arg->value->items[0]->value))) {
                    ...
                    foreach ($variable_type->getAtomicTypes() as $variable_atomic_type) {
                        if ($variable_atomic_type instanceof Type\Atomic\TTemplateParam || $variable_atomic_type instanceof Type\Atomic\TTemplateParamClass) {
        ...
        if ($mapping_return_type && $generic_key_type) {
            if ($array_arg_atomic_type instanceof Type\Atomic\TKeyedArray && count($call_args) === 2) {
                $atomic_type = new Type\Atomic\TKeyedArray(array_map(
                    /**
                     * @return Type\Union
                     */
            ...
            if ($array_arg_atomic_type instanceof Type\Atomic\TList || count($call_args) !== 2) {
                if ($array_arg_atomic_type instanceof Type\Atomic\TNonEmptyList) {
                    return new Type\Union([new Type\Atomic\TNonEmptyList($mapping_return_type)]);
                }
                return new Type\Union([new Type\Atomic\TList($mapping_return_type)]);
            }
            if ($array_arg_atomic_type instanceof Type\Atomic\TNonEmptyArray) {
                return new Type\Union([new Type\Atomic\TNonEmptyArray([$generic_key_type, $mapping_return_type])]);
            }
            return new Type\Union([new Type\Atomic\TArray([$generic_key_type, $mapping_return_type])]);
        }
        return count($call_args) === 2 && !($array_arg_type->is_list ?? false) ? new Type\Union([new Type\Atomic\TArray([$array_arg_type->key ?? Type::getArrayKey(), Type::getMixed()])]) : Type::getList();
    }
    /**
     * @param-out array<string, array<array<int, string>>>|null $assertions
     */
    private static function executeFakeCall(\Psalm\Internal\Analyzer\StatementsAnalyzer $statements_analyzer, PhpParser\Node\Expr $fake_call, Context $context, ?array &$assertions = null) : ?Type\Union
    {
        ...
        if ($fake_call instanceof PhpParser\Node\Expr\StaticCall) {
            \Psalm\Internal\Analyzer\Statements\Expression\Call\StaticCallAnalyzer::analyze($statements_analyzer, $fake_call, $context);
        } elseif ($fake_call instanceof PhpParser\Node\Expr\MethodCall) {
            \Psalm\Internal\Analyzer\Statements\Expression\Call\MethodCallAnalyzer::analyze($statements_analyzer, $fake_call, $context);
        } elseif ($fake_call instanceof PhpParser\Node\Expr\FuncCall) {
            \Psalm\Internal\Analyzer\Statements\Expression\Call\FunctionCallAnalyzer::analyze($statements_analyzer, $fake_call, $context);
        } else {
     ...
     * @param-out array<string, array<array<int, string>>>|null $assertions
     */
    public static function getReturnTypeFromMappingIds(\Psalm\Internal\Analyzer\StatementsAnalyzer $statements_source, array $mapping_function_ids, Context $context, PhpParser\Node\Arg $function_call_arg, array $array_args, ?array &$assertions = null) : Type\Union
    {
        $mapping_return_type = null;
        ...
        foreach ($mapping_function_ids as $mapping_function_id) {
            ...
            foreach ($mapping_function_id_parts as $mapping_function_id_part) {
                ...
                foreach ($array_args as $array_arg) {
                    $fake_args[] = new VirtualArg(new VirtualArrayDimFetch($array_arg->value, new VirtualVariable('__fake_offset_var__', $array_arg->value->getAttributes()), $array_arg->value->getAttributes()), false, false, $array_arg->getAttributes());
                }
                if (strpos($mapping_function_id_part, '::') !== false) {
                    ...
                    if ($is_instance) {
                        $fake_method_call = new VirtualMethodCall(new VirtualVariable('__fake_method_call_var__', $function_call_arg->getAttributes()), new VirtualIdentifier($callable_method_name, $function_call_arg->getAttributes()), $fake_args, $function_call_arg->getAttributes());
                        ...
                        if ($callable_type) {
                            foreach ($callable_type->getAtomicTypes() as $atomic_type) {
                                if ($atomic_type instanceof Type\Atomic\TKeyedArray && \count($atomic_type->properties) === 2 && isset($atomic_type->properties[0])) {
                                    $lhs_instance_type = clone $atomic_type->properties[0];
                                }
                            }
                        }
                        $context->vars_in_scope['$__fake_offset_var__'] = Type::getMixed();
                        $context->vars_in_scope['$__fake_method_call_var__'] = $lhs_instance_type ?: new Type\Union([new Type\Atomic\TNamedObject($callable_fq_class_name)]);
