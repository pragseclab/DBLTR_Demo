...
class IncludeAnalyzer
{
    public static function analyze(StatementsAnalyzer $statements_analyzer, PhpParser\Node\Expr\Include_ $stmt, Context $context, ?Context $global_context = null) : bool
    {
        ...
        if ($stmt_expr_type && $statements_analyzer->data_flow_graph instanceof TaintFlowGraph && $stmt_expr_type->parent_nodes && !\in_array('TaintedInput', $statements_analyzer->getSuppressedIssues())) {
            ...
            $include_param_sink = TaintSink::getForMethodArgument('include', 'include', 0, $arg_location);
        ...
        if ($path_to_file) {
            ...
            if ($current_file_analyzer->project_analyzer->fileExists($path_to_file)) {
                ...
                $include_file_analyzer = new \Psalm\Internal\Analyzer\FileAnalyzer($current_file_analyzer->project_analyzer, $path_to_file, $file_name);
            ...
            if (IssueBuffer::accepts(new MissingFile('Cannot find file ' . $path_to_file . ' to include', new CodeLocation($source, $stmt)), $source->getSuppressedIssues())) {
                // fall through
            }
        } else {
            ...
            if (!$var_id || !isset($context->phantom_files[$var_id])) {
                $source = $statements_analyzer->getSource();
                if (IssueBuffer::accepts(new UnresolvableInclude('Cannot resolve the given expression to a file path', new CodeLocation($source, $stmt)), $source->getSuppressedIssues())) {
                    // fall through
                }
            }
        }
     ...
     */
    public static function getPathTo(PhpParser\Node\Expr $stmt, ?\Psalm\Internal\Provider\NodeDataProvider $type_provider, ?StatementsAnalyzer $statements_analyzer, string $file_name, Config $config) : ?string
    {
        if (DIRECTORY_SEPARATOR === '/') {
        ...
        if ($stmt instanceof PhpParser\Node\Expr\ArrayDimFetch) {
            ...
            if ($stmt->args) {
                ...
                $evaled_path = self::getPathTo($stmt->args[0]->value, $type_provider, $statements_analyzer, $file_name, $config);
                if (!$evaled_path) {
                    return null;
                }
