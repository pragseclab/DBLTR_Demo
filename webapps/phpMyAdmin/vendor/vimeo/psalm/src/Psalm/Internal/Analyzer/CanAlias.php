...
trait CanAlias
{
     ...
     */
    private $aliased_classes = array();
    /**
     * @var array<lowercase-string, CodeLocation>
     */
    private $aliased_class_locations = array();
    /**
     * @var array<lowercase-string, string>
     */
    private $aliased_classes_flipped = array();
    /**
     * @var array<lowercase-string, string>
     */
    private $aliased_classes_flipped_replaceable = array();
    /**
     * @var array<lowercase-string, non-empty-string>
     */
    private $aliased_functions = array();
    /**
     * @var array<string, string>
     */
    private $aliased_constants = array();
            ...
            switch ($use->type !== PhpParser\Node\Stmt\Use_::TYPE_UNKNOWN ? $use->type : $stmt->type) {
                ...
                case PhpParser\Node\Stmt\Use_::TYPE_NORMAL:
                    ...
                    if ($codebase->collect_locations) {
                        // register the path
                        $codebase->use_referencing_locations[strtolower($use_path)][] = new \Psalm\CodeLocation($this, $use);
                    ...
                    if ($codebase->alter_code) {
                        if (isset($codebase->class_transforms[strtolower($use_path)])) {
                            ...
                            $file_manipulations = [];
                            $file_manipulations[] = new \Psalm\FileManipulation((int) $use->getAttribute('startFilePos'), (int) $use->getAttribute('endFilePos') + 1, $new_fq_class_name . ($use->alias ? ' as ' . $use_alias : ''));
