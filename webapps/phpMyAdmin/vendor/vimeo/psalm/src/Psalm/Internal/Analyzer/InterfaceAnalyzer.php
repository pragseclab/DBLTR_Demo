...
{
    public function __construct(PhpParser\Node\Stmt\Interface_ $interface, SourceAnalyzer $source, string $fq_interface_name)
    {
    ...
    public function analyze() : void
    {
        ...
        if ($this->class->extends) {
            foreach ($this->class->extends as $extended_interface) {
                ...
                if (!$extended_interface_storage->is_interface) {
                    ...
                    if (\Psalm\IssueBuffer::accepts(new UndefinedInterface($extended_interface_name . ' is not an interface', $code_location, $extended_interface_name), $this->getSuppressedIssues())) {
                        // fall through
                    }
                }
        ...
        foreach ($class_storage->attributes as $attribute) {
            AttributeAnalyzer::analyze($this, $attribute, $class_storage->suppressed_issues + $this->getSuppressedIssues(), 1, $class_storage);
        }
        foreach ($this->class->stmts as $stmt) {
            if ($stmt instanceof PhpParser\Node\Stmt\ClassMethod) {
                ...
                if ($stmt->name->name !== '__construct' && $config->reportIssueInFile('InvalidReturnType', $this->getFilePath())) {
                    ClassAnalyzer::analyzeClassMethodReturnType($stmt, $method_analyzer, $this, $type_provider, $codebase, $class_storage, $fq_interface_name, $actual_method_id, $actual_method_id, false);
                }
            } elseif ($stmt instanceof PhpParser\Node\Stmt\Property) {
                \Psalm\IssueBuffer::add(new \Psalm\Issue\ParseError('Interfaces cannot have properties', new CodeLocation($this, $stmt)));
                return;
            }
        }
    }
