...
class TemplateTypeCollector extends NodeVisitor
{
    ...
    protected function enterNode(TypeNode $type) : ?int
    {
        if ($type instanceof TTemplateParam) {
            ...
            $this->template_types[] = new TTemplateParam($type->param_name, $extends ? new Union([$extends]) : \Psalm\Type::getMixed(), $type->defining_class);
        } elseif ($type instanceof TConditional) {
            $this->template_types[] = new TTemplateParam($type->param_name, \Psalm\Type::getMixed(), $type->defining_class);
        }
        return null;
    }
    /**
