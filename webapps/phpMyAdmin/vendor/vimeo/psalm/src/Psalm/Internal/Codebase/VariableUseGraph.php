...
class VariableUseGraph extends DataFlowGraph
{
    ...
    public function isVariableUsed(DataFlowNode $assignment_node) : bool
    {
        ...
        for ($i = 0; count($sources) && $i < 200; $i++) {
            ...
            foreach ($sources as $source) {
                ...
                $new_sources = array_merge($new_sources, $child_nodes);
            }
            $sources = $new_sources;
        }
        return false;
    ...
    private function getChildNodes(DataFlowNode $generated_source, array $visited_source_ids) : ?array
    {
        ...
        foreach ($this->forward_edges[$generated_source->id] as $to_id => $path) {
            $path_type = $path->type;
            if ($path->type === 'variable-use' || $path->type === 'closure-use' || $path->type === 'global-use' || $path->type === 'use-inside-instance-property' || $path->type === 'use-inside-static-property' || $path->type === 'use-inside-call' || $path->type === 'use-inside-conditional' || $path->type === 'use-inside-isset' || $path->type === 'arg') {
                return null;
            }
