...
class Functions
{
     ...
     */
    public function isCallMapFunctionPure(Codebase $codebase, ?\Psalm\NodeTypeProvider $type_provider, string $function_id, ?array $args, bool &$must_use = true) : bool
    {
        $impure_functions = [
            // file io
            'chdir',
        ...
        $function_callable = InternalCallMapHandler::getCallableFromCallMapById($codebase, $function_id, $args ?: [], null);
        if (!$function_callable->params || $args !== null && \count($args) === 0 || $function_callable->return_type && $function_callable->return_type->isVoid()) {
            return false;
        }
        $must_use = $function_id !== 'array_map' || isset($args[0]) && !$args[0]->value instanceof \PhpParser\Node\Expr\Closure;
