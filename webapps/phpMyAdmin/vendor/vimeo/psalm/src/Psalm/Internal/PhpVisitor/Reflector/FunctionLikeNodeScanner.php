...
class FunctionLikeNodeScanner
{
    ...
    public function start(PhpParser\Node\FunctionLike $stmt, bool $fake_method = false)
    {
        ...
        foreach ($stmt->getParams() as $param) {
            if ($param->var instanceof PhpParser\Node\Expr\Error) {
                $storage->docblock_issues[] = new InvalidDocblock('Param' . ($i + 1) . ' of ' . $cased_function_id . ' has invalid syntax', new CodeLocation($this->file_scanner, $param, null, true));
                ++$i;
                continue;
            }
            ...
            foreach ($param->attrGroups as $attr_group) {
                foreach ($attr_group->attrs as $attr) {
                    $param_storage->attributes[] = AttributeResolver::resolve($this->codebase, $this->file_scanner, $this->file_storage, $this->aliases, $attr, $this->classlike_storage->name ?? null);
                }
            }
            if ($param_storage->name === 'haystack' && (strpos($this->file_path, 'CoreGenericFunctions.phpstub') || strpos($this->file_path, 'CoreGenericClasses.phpstub') || strpos($this->file_path, 'CoreGenericIterators.phpstub'))) {
                $param_storage->expect_variable = true;
            }
            if (isset($existing_params['$' . $param_storage->name])) {
                $storage->docblock_issues[] = new DuplicateParam('Duplicate param $' . $param_storage->name . ' in docblock for ' . $cased_function_id, new CodeLocation($this->file_scanner, $param, null, true));
                ++$i;
                continue;
            }
            ...
            if ($stmt instanceof PhpParser\Node\Stmt\ClassMethod && $storage instanceof MethodStorage && $classlike_storage && !$classlike_storage->mutation_free && $stmt->stmts && count($stmt->stmts) === 1 && !count($stmt->params) && $stmt->stmts[0] instanceof PhpParser\Node\Stmt\Return_ && $stmt->stmts[0]->expr instanceof PhpParser\Node\Expr\PropertyFetch && $stmt->stmts[0]->expr->var instanceof PhpParser\Node\Expr\Variable && $stmt->stmts[0]->expr->var->name === 'this' && $stmt->stmts[0]->expr->name instanceof PhpParser\Node\Identifier) {
                ...
                foreach ($stmt->stmts as $function_stmt) {
                    if ($function_stmt instanceof PhpParser\Node\Stmt\If_) {
                        $final_actions = \Psalm\Internal\Analyzer\ScopeAnalyzer::getControlActions($function_stmt->stmts, null, $this->config->exit_functions, [], false);
                        if ($final_actions !== [\Psalm\Internal\Analyzer\ScopeAnalyzer::ACTION_END]) {
                            $var_assertions = [];
                            break;
                        }
                        $cond_id = \spl_object_id($function_stmt->cond);
                        $if_clauses = FormulaGenerator::getFormula($cond_id, $cond_id, $function_stmt->cond, $this->classlike_storage ? $this->classlike_storage->name : null, $this->file_scanner, null);
                        try {
                        ...
                        foreach ($rules as $var_id => $rule) {
                            ...
                            if (isset($existing_params[$var_id])) {
                                ...
                                $var_assertions[] = new \Psalm\Storage\Assertion($param_offset, $rule);
                            } elseif (strpos($var_id, '$this->') === 0) {
                                $var_assertions[] = new \Psalm\Storage\Assertion($var_id, $rule);
                            }
                        }
                    } else {
                        $var_assertions = [];
        ...
        if ($parser_return_type) {
            ...
            $storage->return_type = TypeHintResolver::resolve($parser_return_type, $this->codebase->scanner, $this->file_storage, $this->classlike_storage, $this->aliases, $this->codebase->php_major_version, $this->codebase->php_minor_version);
        ...
        if ($classlike_storage && !$classlike_storage->is_trait && strlen($classlike_storage->internal) > strlen($storage->internal)) {
        ...
        if ($doc_comment) {
            try {
                ...
                $storage->docblock_issues[] = new MissingDocblockType($e->getMessage() . ' in docblock for ' . $cased_function_id, new CodeLocation($this->file_scanner, $stmt, null, true));
                $docblock_info = null;
            } catch (DocblockParseException $e) {
                $storage->docblock_issues[] = new InvalidDocblock($e->getMessage() . ' in docblock for ' . $cased_function_id, new CodeLocation($this->file_scanner, $stmt, null, true));
                $docblock_info = null;
            }
            if ($docblock_info) {
                if ($docblock_info->since_php_major_version && !$this->aliases->namespace) {
                    ...
                    if ($docblock_info->since_php_major_version === $this->codebase->php_major_version && $docblock_info->since_php_minor_version > $this->codebase->php_minor_version) {
                        return false;
                    }
                }
                FunctionLikeDocblockScanner::addDocblockInfo($this->codebase, $this->file_scanner, $this->file_storage, $this->aliases, $this->type_aliases, $this->classlike_storage, $this->existing_function_template_types, $storage, $stmt, $docblock_info, $is_functionlike_override, $fake_method, $cased_function_id);
            }
        }
        ...
        if ($classlike_storage && $method_name_lc === '__construct') {
            foreach ($stmt->getParams() as $param) {
                ...
                if (isset($classlike_storage->properties[$param_storage->name]) && $param_storage->location) {
                    IssueBuffer::add(new \Psalm\Issue\ParseError('Promoted propertty ' . $param_storage->name . ' clashes with an existing property', $param_storage->location));
        ...
        if ($stmt instanceof PhpParser\Node\Stmt\ClassMethod && $stmt->name->name === '__construct' && $classlike_storage && $storage instanceof MethodStorage && $storage->params && $this->config->infer_property_types_from_constructor) {
            $this->inferPropertyTypeFromConstructor($stmt, $storage, $classlike_storage);
        }
        foreach ($stmt->getAttrGroups() as $attr_group) {
            foreach ($attr_group->attrs as $attr) {
                $attribute = AttributeResolver::resolve($this->codebase, $this->file_scanner, $this->file_storage, $this->aliases, $attr, $this->classlike_storage->name ?? null);
    ...
    private function inferPropertyTypeFromConstructor(PhpParser\Node\Stmt\ClassMethod $stmt, MethodStorage $storage, ClassLikeStorage $classlike_storage) : void
    {
        ...
        foreach ($stmt->stmts as $function_stmt) {
            if ($function_stmt instanceof PhpParser\Node\Stmt\Expression && $function_stmt->expr instanceof PhpParser\Node\Expr\Assign && $function_stmt->expr->var instanceof PhpParser\Node\Expr\PropertyFetch && $function_stmt->expr->var->var instanceof PhpParser\Node\Expr\Variable && $function_stmt->expr->var->var->name === 'this' && $function_stmt->expr->var->name instanceof PhpParser\Node\Identifier && ($property_name = $function_stmt->expr->var->name->name) && isset($classlike_storage->properties[$property_name]) && $function_stmt->expr->expr instanceof PhpParser\Node\Expr\Variable && is_string($function_stmt->expr->expr->name) && ($param_name = $function_stmt->expr->expr->name) && isset($storage->param_lookup[$param_name])) {
                ...
                $assigned_properties[$property_name] = $storage->params[$param_index]->is_variadic ? new Type\Union([new Type\Atomic\TArray([Type::getInt(), $param_type])]) : $param_type;
            } else {
                $assigned_properties = [];
                break;
            }
    ...
    {
        $param_type = null;
        $is_nullable = $param->default !== null && $param->default instanceof PhpParser\Node\Expr\ConstFetch && strtolower($param->default->name->parts[0]) === 'null';
        $param_typehint = $param->type;
        if ($param_typehint) {
            $param_type = TypeHintResolver::resolve($param_typehint, $this->codebase->scanner, $this->file_storage, $this->classlike_storage, $this->aliases, $this->codebase->php_major_version, $this->codebase->php_minor_version);
            if ($is_nullable) {
        ...
        return new FunctionLikeParameter($param->var->name, $param->byRef, $param_type, new CodeLocation($this->file_scanner, $fake_method ? $stmt : $param->var, null, false, !$fake_method ? CodeLocation::FUNCTION_PARAM_VAR : CodeLocation::FUNCTION_PHPDOC_METHOD), $param_typehint ? new CodeLocation($this->file_scanner, $fake_method ? $stmt : $param, null, false, CodeLocation::FUNCTION_PARAM_TYPE) : null, $is_optional, $is_nullable, $param->variadic, $param->default ? SimpleTypeInferer::infer($this->codebase, new \Psalm\Internal\Provider\NodeDataProvider(), $param->default, $this->aliases, null, null, $fq_classlike_name) : null);
    }
    /**
     * @return array{
     *     string,
        ...
        if ($fake_method && $stmt instanceof PhpParser\Node\Stmt\ClassMethod) {
            ...
            if ($this->codebase->register_stub_files || $this->codebase->register_autoload_files) {
                ...
                if (isset($this->file_storage->functions[$function_id])) {
                    ...
                    if (IssueBuffer::accepts(new DuplicateFunction('Method ' . $function_id . ' has already been defined' . ($duplicate_function_storage->location ? ' in ' . $duplicate_function_storage->location->file_path : ''), new CodeLocation($this->file_scanner, $stmt, null, true)))) {
                        // fall through
                    }
                    $this->file_storage->has_visitor_issues = true;
                ...
                if (isset($this->config->getPredefinedFunctions()[$function_id])) {
                    ...
                    if ($reflection_function->getFileName() !== $this->file_path) {
                        if (IssueBuffer::accepts(new DuplicateFunction('Method ' . $function_id . ' has already been defined as a core function', new CodeLocation($this->file_scanner, $stmt, null, true)))) {
                            // fall through
                        }
                    }
                }
            ...
            if (isset($classlike_storage->methods[$method_name_lc])) {
                if (!$this->codebase->register_stub_files) {
                    ...
                    if (IssueBuffer::accepts(new DuplicateMethod('Method ' . $function_id . ' has already been defined' . ($duplicate_method_storage->location ? ' in ' . $duplicate_method_storage->location->file_path : ''), new CodeLocation($this->file_scanner, $stmt, null, true)))) {
                        // fall through
                    }
                    $this->file_storage->has_visitor_issues = true;
            ...
            if ($method_name_lc === strtolower($class_name) && !isset($classlike_storage->methods['__construct']) && strpos($fq_classlike_name, '\\') === false && $this->codebase->php_major_version < 8) {
            ...
            $method_id = new \Psalm\Internal\MethodIdentifier($fq_classlike_name, $method_name_lc);
            $storage->is_static = $stmt->isStatic();
