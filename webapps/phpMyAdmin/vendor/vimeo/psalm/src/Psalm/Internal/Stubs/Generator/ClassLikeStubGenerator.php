...
class ClassLikeStubGenerator
{
     ...
     */
    public static function getClassLikeNode(\Psalm\Codebase $codebase, ClassLikeStorage $storage, string $classlike_name) : PhpParser\Node\Stmt\ClassLike
    {
        $subnodes = ['stmts' => array_merge(self::getConstantNodes($codebase, $storage), self::getPropertyNodes($storage), self::getMethodNodes($storage))];
    ...
    private static function getConstantNodes(\Psalm\Codebase $codebase, ClassLikeStorage $storage) : array
    {
        ...
        foreach ($storage->constants as $constant_name => $constant_storage) {
            if ($constant_storage->unresolved_node) {
                $type = new Type\Union([\Psalm\Internal\Codebase\ConstantTypeResolver::resolve($codebase->classlikes, $constant_storage->unresolved_node)]);
            } elseif ($constant_storage->type) {
                $type = $constant_storage->type;
            } else {
                throw new \UnexpectedValueException('bad');
            }
            $constant_nodes[] = new VirtualClassConst([new VirtualConst($constant_name, StubsGenerator::getExpressionFromType($type))], $constant_storage->visibility === ClassLikeAnalyzer::VISIBILITY_PUBLIC ? PhpParser\Node\Stmt\Class_::MODIFIER_PUBLIC : ($constant_storage->visibility === ClassLikeAnalyzer::VISIBILITY_PROTECTED ? PhpParser\Node\Stmt\Class_::MODIFIER_PROTECTED : PhpParser\Node\Stmt\Class_::MODIFIER_PRIVATE));
        }
    ...
    private static function getMethodNodes(ClassLikeStorage $storage) : array
    {
        ...
        foreach ($storage->methods as $method_storage) {
            ...
            $method_nodes[] = new VirtualClassMethod($method_storage->cased_name, ['flags' => $flag | ($method_storage->is_static ? PhpParser\Node\Stmt\Class_::MODIFIER_STATIC : 0) | ($method_storage->abstract ? PhpParser\Node\Stmt\Class_::MODIFIER_ABSTRACT : 0), 'params' => StubsGenerator::getFunctionParamNodes($method_storage), 'returnType' => $method_storage->signature_return_type ? StubsGenerator::getParserTypeFromPsalmType($method_storage->signature_return_type) : null, 'stmts' => $storage->is_interface || $method_storage->abstract ? null : []], ['comments' => $docblock->tags ? [new PhpParser\Comment\Doc(\rtrim($docblock->render('        ')))] : []]);
        }
        return $method_nodes;
    }
}