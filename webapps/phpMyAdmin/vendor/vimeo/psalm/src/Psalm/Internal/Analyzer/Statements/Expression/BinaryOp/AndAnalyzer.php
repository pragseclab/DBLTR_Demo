...
class AndAnalyzer
{
    public static function analyze(StatementsAnalyzer $statements_analyzer, PhpParser\Node\Expr\BinaryOp $stmt, Context $context, bool $from_stmt = false) : bool
    {
        ...
        if ($left_context->reconciled_expression_clauses) {
            ...
            $context_clauses = array_values(array_filter($context_clauses, function ($c) use($reconciled_expression_clauses) : bool {
        ...
        if ($left_type_assertions) {
            ...
            $right_vars_in_scope = Reconciler::reconcileKeyedTypes($left_type_assertions, $active_left_assertions, $context->vars_in_scope, $changed_var_ids, $left_referenced_var_ids, $statements_analyzer, [], $context->inside_loop, new CodeLocation($statements_analyzer->getSource(), $stmt->left), $context->inside_negation);
        ...
        $context->referenced_var_ids = array_merge($right_context->referenced_var_ids, $left_context->referenced_var_ids);
        ...
        if ($context->if_context && !$context->inside_negation) {
            ...
            $if_context->reconciled_expression_clauses = array_merge($if_context->reconciled_expression_clauses, array_map(function ($c) {
                return $c->hash;
            }, $partitioned_clauses[1]));
            $if_context->vars_possibly_in_scope = array_merge($context->vars_possibly_in_scope, $if_context->vars_possibly_in_scope);
