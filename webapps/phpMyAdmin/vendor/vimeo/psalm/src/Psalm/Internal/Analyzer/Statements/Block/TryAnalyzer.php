...
class TryAnalyzer
{
    ...
    public static function analyze(StatementsAnalyzer $statements_analyzer, PhpParser\Node\Stmt\TryCatch $stmt, Context $context) : ?bool
    {
        ...
        foreach ($stmt->catches as $i => $catch) {
            $catch_actions[$i] = ScopeAnalyzer::getControlActions($catch->stmts, $statements_analyzer->node_data, $codebase->config->exit_functions);
        ...
        $try_block_control_actions = ScopeAnalyzer::getControlActions($stmt->stmts, $statements_analyzer->node_data, $codebase->config->exit_functions, $context->break_types);
        ...
        $possibly_referenced_var_ids = array_merge($context->referenced_var_ids, $old_referenced_var_ids);
        if ($try_context !== $context) {
            ...
            $context->referenced_var_ids = array_intersect_key($try_context->referenced_var_ids, $context->referenced_var_ids);
        }
        $try_leaves_loop = $context->loop_scope && $context->loop_scope->final_actions && !in_array(ScopeAnalyzer::ACTION_NONE, $context->loop_scope->final_actions, true);
        ...
        foreach ($stmt->catches as $i => $catch) {
            ...
            foreach ($catch->types as $catch_type) {
                ...
                if ($original_context->check_classes) {
                    if (ClassLikeAnalyzer::checkFullyQualifiedClassLikeName($statements_analyzer, $fq_catch_class, new CodeLocation($statements_analyzer->getSource(), $catch_type, $context->include_location), $context->self, $context->calling_method_id, $statements_analyzer->getSuppressedIssues(), false) === false) {
                        return false;
                    }
                }
                if ($codebase->classExists($fq_catch_class) && strtolower($fq_catch_class) !== 'exception' && !($codebase->classExtends($fq_catch_class, 'Exception') || $codebase->classImplements($fq_catch_class, 'Throwable')) || $codebase->interfaceExists($fq_catch_class) && strtolower($fq_catch_class) !== 'throwable' && !$codebase->interfaceExtends($fq_catch_class, 'Throwable')) {
                    if (IssueBuffer::accepts(new InvalidCatch('Class/interface ' . $fq_catch_class . ' cannot be caught', new CodeLocation($statements_analyzer->getSource(), $stmt), $fq_catch_class), $statements_analyzer->getSuppressedIssues())) {
                        return false;
                    }
                }
            ...
            if ($catch->var && is_string($catch->var->name)) {
                ...
                $catch_context->vars_in_scope[$catch_var_id] = new Union(array_map(
                    ...
                    function ($fq_catch_class) use($codebase) : \Psalm\Type\Atomic\TNamedObject {
                        ...
                        if (version_compare(PHP_VERSION, '7.0.0dev', '>=') && strtolower($fq_catch_class) !== 'throwable' && $codebase->interfaceExists($fq_catch_class) && !$codebase->interfaceExtends($fq_catch_class, 'Throwable')) {
                ...
                $catch_context->removeDescendents($catch_var_id, null, $catch_context->vars_in_scope[$catch_var_id], $statements_analyzer);
                ...
                if ($statements_analyzer->data_flow_graph) {
                    ...
                    $catch_context->vars_in_scope[$catch_var_id]->parent_nodes = [$catch_var_node->id => $catch_var_node];
            ...
            $catch_actions[$i] = ScopeAnalyzer::getControlActions($catch->stmts, $statements_analyzer->node_data, $codebase->config->exit_functions, $context->break_types);
            foreach ($issues_to_suppress as $issue_to_suppress) {
                if (!in_array($issue_to_suppress, $suppressed_issues, true)) {
            ...
            $context->referenced_var_ids = array_intersect_key($catch_context->referenced_var_ids, $context->referenced_var_ids);
            $possibly_referenced_var_ids = array_merge($catch_context->referenced_var_ids, $possibly_referenced_var_ids);
            if ($catch_context->collect_exceptions) {
        ...
        if ($context->loop_scope && !$try_leaves_loop && !in_array(ScopeAnalyzer::ACTION_NONE, $context->loop_scope->final_actions, true)) {
            ...
            if ($try_context->finally_scope) {
                ...
                foreach ($finally_context->assigned_var_ids as $var_id => $_) {
                    if (isset($context->vars_in_scope[$var_id]) && isset($finally_context->vars_in_scope[$var_id])) {
                        if ($context->vars_in_scope[$var_id]->possibly_undefined && $context->vars_in_scope[$var_id]->possibly_undefined_from_try) {
