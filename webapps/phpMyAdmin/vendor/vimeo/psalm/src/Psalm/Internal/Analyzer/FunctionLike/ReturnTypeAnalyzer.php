...
class ReturnTypeAnalyzer
{
     ...
     * @return  false|null
     */
    public static function verifyReturnType(FunctionLike $function, array $function_stmts, SourceAnalyzer $source, \Psalm\Internal\Provider\NodeDataProvider $type_provider, FunctionLikeAnalyzer $function_like_analyzer, ?Type\Union $return_type = null, ?string $fq_class_name = null, ?CodeLocation $return_type_location = null, array $compatible_method_ids = [], bool $did_explicitly_return = false, bool $closure_inside_call = false) : ?bool
    {
        ...
        } elseif ($source instanceof \Psalm\Internal\Analyzer\ClassAnalyzer || $source instanceof \Psalm\Internal\Analyzer\TraitAnalyzer) {
        ...
        if (!$function->getStmts() && ($function instanceof ClassMethod && ($source instanceof InterfaceAnalyzer || $function->isAbstract()))) {
            if (!$return_type) {
                if (IssueBuffer::accepts(new MissingReturnType('Method ' . $cased_method_id . ' does not have a return type', new CodeLocation($function_like_analyzer, $function->name, null, true)), $suppressed_issues)) {
                    // fall through
                }
            }
            return null;
        ...
        if ($function instanceof ClassMethod && substr($function->name->name, 0, 2) === '__' && !$is_to_string && !$return_type) {
        ...
        $inferred_return_type_parts = ReturnTypeCollector::getReturnTypes($codebase, $type_provider, $function_stmts, $inferred_yield_types, true);
        if (!$inferred_return_type_parts) {
            $did_explicitly_return = true;
        }
        if ((!$return_type || $return_type->from_docblock) && ScopeAnalyzer::getControlActions($function_stmts, $type_provider, $codebase->config->exit_functions) !== [ScopeAnalyzer::ACTION_END] && !$inferred_yield_types && count($inferred_return_type_parts) && !$did_explicitly_return) {
        ...
        if ($return_type && $return_type->isNever() && !$inferred_yield_types && ScopeAnalyzer::getControlActions($function_stmts, $type_provider, $codebase->config->exit_functions, [], false) !== [ScopeAnalyzer::ACTION_END]) {
        ...
        $inferred_return_type = $inferred_return_type_parts ? \Psalm\Type::combineUnionTypeArray($inferred_return_type_parts, $codebase) : Type::getVoid();
        $inferred_yield_type = $inferred_yield_types ? \Psalm\Type::combineUnionTypeArray($inferred_yield_types, $codebase) : null;
        ...
        $inferred_return_type = \Psalm\Internal\Type\TypeExpander::expandUnion($codebase, $inferred_return_type, $source->getFQCLN(), $source->getFQCLN(), $source->getParentFQCLN());
        ...
        if ($is_to_string) {
            if (!$inferred_return_type->hasMixed() && !UnionTypeComparator::isContainedBy($codebase, $inferred_return_type, Type::getString(), $inferred_return_type->ignore_nullable_issues, $inferred_return_type->ignore_falsable_issues)) {
        ...
        if (!$return_type) {
            if ($function instanceof Closure || $function instanceof ArrowFunction) {
                if (!$closure_inside_call || $inferred_return_type->isMixed()) {
                    ...
                    if (IssueBuffer::accepts(new MissingClosureReturnType('Closure does not have a return type, expecting ' . $inferred_return_type->getId(), new CodeLocation($function_like_analyzer, $function, null, true)), $suppressed_issues, !$inferred_return_type->hasMixed() && !$inferred_return_type->isNull())) {
                        // fall through
                    }
                }
                return null;
        ...
        $declared_return_type = \Psalm\Internal\Type\TypeExpander::expandUnion($codebase, $return_type, $self_fq_class_name, $self_fq_class_name, $parent_class, true, true, $function_like_storage instanceof MethodStorage && $function_like_storage->final || $classlike_storage && $classlike_storage->final);
        if (!$inferred_return_type_parts && !$inferred_yield_types && (!$function_like_storage || !$function_like_storage->has_yield)) {
            ...
            if (!$declared_return_type->from_docblock || !$declared_return_type->isNullable()) {
                if (IssueBuffer::accepts(new InvalidReturnType('No return statements were found for method ' . $cased_method_id . ' but return type \'' . $declared_return_type . '\' was expected', $return_type_location), $suppressed_issues, true)) {
                    return false;
                }
            }
            return null;
        ...
        if (!$declared_return_type->hasMixed()) {
            ...
            if (!UnionTypeComparator::isContainedBy($codebase, $inferred_return_type, $declared_return_type, true, true, $union_comparison_results)) {
                ...
                if ($union_comparison_results->type_coerced) {
                    ...
                    if (IssueBuffer::accepts(new InvalidReturnType('The declared return type \'' . $declared_return_type->getId() . '\' for ' . $cased_method_id . ' is incorrect, got \'' . $inferred_return_type->getId() . '\'', $return_type_location), $suppressed_issues, true)) {
                        return false;
                    }
                }
            } elseif (!$inferred_return_type->hasMixed() && !UnionTypeComparator::isContainedBy($codebase, $declared_return_type, $inferred_return_type, false, false)) {
                if ($codebase->alter_code) {
                    ...
                    if ($check_for_less_specific_type && (\Psalm\Config::getInstance()->restrict_return_types || !$inferred_return_type->isNullable() && $declared_return_type->isNullable() || !$inferred_return_type->isFalsable() && $declared_return_type->isFalsable())) {
            ...
            if ($union_comparison_results->to_string_cast) {
                if (IssueBuffer::accepts(new ImplicitToStringCast('The declared return type for ' . $cased_method_id . ' expects \'' . $declared_return_type . '\', ' . '\'' . $inferred_return_type . '\' provided with a __toString method', $return_type_location), $suppressed_issues)) {
                    // fall through
                }
            }
            ...
            if (!$inferred_return_type->ignore_falsable_issues && $inferred_return_type->isFalsable() && !$declared_return_type->isFalsable() && !$declared_return_type->hasBool() && !$declared_return_type->hasScalar()) {
                ...
                if (IssueBuffer::accepts(new InvalidFalsableReturnType('The declared return type \'' . $declared_return_type . '\' for ' . $cased_method_id . ' does not allow false, but \'' . $inferred_return_type . '\' contains false', $return_type_location), $suppressed_issues, true)) {
                    return false;
                }
            }
        }
    ...
    public static function checkReturnType(FunctionLike $function, ProjectAnalyzer $project_analyzer, FunctionLikeAnalyzer $function_like_analyzer, FunctionLikeStorage $storage, Context $context) : ?bool
    {
        ...
        if (!$storage->signature_return_type || $storage->signature_return_type === $storage->return_type) {
            foreach ($storage->return_type->getAtomicTypes() as $type) {
                if ($type instanceof Type\Atomic\TNamedObject && 'parent' === $type->value && null === $parent_class) {
            ...
            $fleshed_out_return_type = \Psalm\Internal\Type\TypeExpander::expandUnion($codebase, $storage->return_type, $classlike_storage ? $classlike_storage->name : null, $classlike_storage ? $classlike_storage->name : null, $parent_class);
        ...
        $fleshed_out_signature_type = \Psalm\Internal\Type\TypeExpander::expandUnion($codebase, $storage->signature_return_type, $classlike_storage ? $classlike_storage->name : null, $classlike_storage ? $classlike_storage->name : null, $parent_class);
        ...
        $fleshed_out_return_type = \Psalm\Internal\Type\TypeExpander::expandUnion($codebase, $storage->return_type, $classlike_storage ? $classlike_storage->name : null, $classlike_storage ? $classlike_storage->name : null, $parent_class, true, true);
        ...
        if ($classlike_storage && $context->self) {
            $class_template_params = ClassTemplateParamCollector::collect($codebase, $classlike_storage, $codebase->classlike_storage_provider->get($context->self), strtolower($function->name->name), new Type\Atomic\TNamedObject($context->self), true);
            ...
            if ($class_template_params) {
                ...
                $fleshed_out_return_type = \Psalm\Internal\Type\TemplateStandinTypeReplacer::replace($fleshed_out_return_type, $template_result, $codebase, null, null, null);
            }
        }
        ...
        if (!UnionTypeComparator::isContainedBy($codebase, $fleshed_out_return_type, $fleshed_out_signature_type, false, false, $union_comparison_result) && !$union_comparison_result->type_coerced_from_mixed) {
            ...
            if (IssueBuffer::accepts(new MismatchingDocblockReturnType('Docblock has incorrect return type \'' . $storage->return_type->getId() . '\', should be \'' . $storage->signature_return_type->getId() . '\'', $storage->return_type_location), $storage->suppressed_issues, true)) {
                return false;
            }
        }
        return null;
     ...
     *
     */
    private static function addOrUpdateReturnType(FunctionLike $function, ProjectAnalyzer $project_analyzer, Type\Union $inferred_return_type, StatementsSource $source, bool $docblock_only = false, ?FunctionLikeStorage $function_like_storage = null) : void
    {
