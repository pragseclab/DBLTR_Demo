...
class TypeChecker extends NodeVisitor
{
     ...
     */
    public function __construct(StatementsSource $source, CodeLocation $code_location, array $suppressed_issues, array $phantom_classes = [], bool $inferred = true, bool $inherited = false, bool $prevent_template_covariance = false, ?string $calling_method_id = null)
    {
        $this->source = $source;
    ...
    protected function enterNode(TypeNode $type) : ?int
    {
        ...
        if ($type instanceof TNamedObject) {
            ...
            if (\count($type->type_params) > 2) {
                if (IssueBuffer::accepts(new TooManyTemplateParams($type->getId() . ' has too many template params, expecting 2', $this->code_location), $this->suppressed_issues)) {
                    // fall through
                }
            }
        }
    ...
    private function checkNamedObject(TNamedObject $atomic) : void
    {
        ...
        if ($this->code_location instanceof CodeLocation\DocblockTypeLocation && $codebase->store_node_types && $atomic->offset_start !== null && $atomic->offset_end !== null) {
        ...
        if ($this->calling_method_id && $atomic->text !== null) {
            $codebase->file_reference_provider->addMethodReferenceToClassMember($this->calling_method_id, 'use:' . $atomic->text . ':' . \md5($this->source->getFilePath()));
        }
        if (!isset($this->phantom_classes[\strtolower($atomic->value)]) && ClassLikeAnalyzer::checkFullyQualifiedClassLikeName($this->source, $atomic->value, $this->code_location, $this->source->getFQCLN(), $this->calling_method_id, $this->suppressed_issues, $this->inferred, false, true, $atomic->from_docblock) === false) {
        ...
        if (!$this->inherited && $codebase->classlike_storage_provider->has($fq_class_name_lc) && $this->source->getFQCLN() !== $atomic->value) {
            ...
            if ($class_storage->deprecated) {
                if (IssueBuffer::accepts(new DeprecatedClass('Class ' . $atomic->value . ' is marked as deprecated', $this->code_location, $atomic->value), $this->source->getSuppressedIssues() + $this->suppressed_issues)) {
                    // fall through
                }
            }
        }
    ...
    private function checkGenericParams(TGenericObject $atomic) : void
    {
        ...
        if ($template_type_count > $template_param_count) {
            if (IssueBuffer::accepts(new MissingTemplateParam($atomic->value . ' has missing template params, expecting ' . $template_type_count, $this->code_location), $this->suppressed_issues)) {
                // fall through
            }
        } elseif ($template_type_count < $template_param_count) {
            if (IssueBuffer::accepts(new TooManyTemplateParams($atomic->getId() . ' has too many template params, expecting ' . $template_type_count, $this->code_location), $this->suppressed_issues)) {
                // fall through
            }
        }
        $expected_type_param_keys = \array_keys($expected_type_params);
        foreach ($atomic->type_params as $i => $type_param) {
            $this->prevent_template_covariance = $this->source instanceof \Psalm\Internal\Analyzer\MethodAnalyzer && $this->source->getMethodName() !== '__construct' && empty($expected_param_covariants[$i]);
            if (isset($expected_type_param_keys[$i])) {
                ...
                foreach ($expected_type_params[$expected_template_name] as $defining_class => $expected_type_param) {
                    $expected_type_param = \Psalm\Internal\Type\TypeExpander::expandUnion($codebase, $expected_type_param, $defining_class, null, null);
                    $type_param = \Psalm\Internal\Type\TypeExpander::expandUnion($codebase, $type_param, $defining_class, null, null);
                    if (!UnionTypeComparator::isContainedBy($codebase, $type_param, $expected_type_param)) {
                        if (IssueBuffer::accepts(new InvalidTemplateParam('Extended template param ' . $expected_template_name . ' of ' . $atomic->getId() . ' expects type ' . $expected_type_param->getId() . ', type ' . $type_param->getId() . ' given', $this->code_location), $this->suppressed_issues)) {
                            // fall through
                        }
                    }
                }
    ...
    public function checkScalarClassConstant(TScalarClassConstant $atomic) : void
    {
        ...
        if (ClassLikeAnalyzer::checkFullyQualifiedClassLikeName($this->source, $fq_classlike_name, $this->code_location, null, null, $this->suppressed_issues, $this->inferred, false, true, $atomic->from_docblock) === false) {
            $this->has_errors = true;
            return;
        }
        $const_name = $atomic->const_name;
        if (\strpos($const_name, '*') !== false) {
            $expanded = TypeExpander::expandAtomic($this->source->getCodebase(), $atomic, $fq_classlike_name, $fq_classlike_name, null, true, true);
        ...
        if (!$is_defined) {
            if (\Psalm\IssueBuffer::accepts(new UndefinedConstant('Constant ' . $fq_classlike_name . '::' . $const_name . ' is not defined', $this->code_location), $this->source->getSuppressedIssues())) {
                // fall through
            }
        }
    }
    ...
    {
        if ($this->prevent_template_covariance && \substr($atomic->defining_class, 0, 3) !== 'fn-') {
            $codebase = $this->source->getCodebase();
            ...
            if ($template_offset !== false && isset($class_storage->template_covariants[$template_offset]) && $class_storage->template_covariants[$template_offset]) {
                $method_storage = $this->source instanceof \Psalm\Internal\Analyzer\MethodAnalyzer ? $this->source->getFunctionLikeStorage() : null;
                if ($method_storage instanceof MethodStorage && $method_storage->mutation_free && !$method_storage->mutation_free_inferred) {
                    // do nothing
                } else {
                    if (\Psalm\IssueBuffer::accepts(new \Psalm\Issue\InvalidTemplateParam('Template param ' . $atomic->param_name . ' of ' . $atomic->defining_class . ' is marked covariant and cannot be used here', $this->code_location), $this->source->getSuppressedIssues())) {
                        // fall through
                    }
                }
            }
    ...
    public function checkResource(TResource $atomic) : void
    {
        if (!$atomic->from_docblock) {
            if (\Psalm\IssueBuffer::accepts(new \Psalm\Issue\ReservedWord('\'resource\' is a reserved word', $this->code_location, 'resource'), $this->source->getSuppressedIssues())) {
                // fall through
            }
        }
    }
