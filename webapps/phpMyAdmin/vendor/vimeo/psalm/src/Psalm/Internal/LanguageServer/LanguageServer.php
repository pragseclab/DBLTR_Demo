...
class LanguageServer extends AdvancedJsonRpc\Dispatcher
{
    ...
    public function __construct(ProtocolReader $reader, ProtocolWriter $writer, ProjectAnalyzer $project_analyzer)
    {
        ...
        return call(
            ...
            function () {
                ...
                if ($this->textDocument === null) {
                    $this->textDocument = new TextDocument($this, $codebase, $this->project_analyzer->onchange_line_limit);
                }
    ...
    public function emitIssues(array $uris) : void
    {
        ...
        foreach ($uris as $file_path => $uri) {
            $diagnostics = array_map(function (IssueData $issue_data) : Diagnostic {
    ...
    private function verboseLog(string $message, int $type = 4) : Promise
    {
        if ($this->project_analyzer->language_server_verbose) {
            try {
                return $this->client->logMessage('[Psalm ' . PSALM_VERSION . ' - PHP Language Server] ' . $message, $type);
            } catch (\Throwable $err) {
                // do nothing
            }
        }
    ...
    private function clientStatus(string $status, ?string $additional_info = null) : Promise
    {
        try {
            ...
            return $this->client->logMessage($status . (!empty($additional_info) ? ': ' . $additional_info : ''), 3, 'telemetry/event');
        } catch (\Throwable $err) {
            return new Success(null);
        }
    }
