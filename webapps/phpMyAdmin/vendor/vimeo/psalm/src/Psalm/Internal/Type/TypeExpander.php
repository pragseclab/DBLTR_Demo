...
class TypeExpander
{
     ...
     *
     */
    public static function expandUnion(Codebase $codebase, Type\Union $return_type, ?string $self_class, $static_class_type, ?string $parent_class, bool $evaluate_class_constants = true, bool $evaluate_conditional_types = false, bool $final = false, bool $expand_generic = false) : Type\Union
    {
        $return_type = clone $return_type;
        $new_return_type_parts = [];
        $has_array_output = false;
        foreach ($return_type->getAtomicTypes() as $return_type_part) {
            $parts = self::expandAtomic($codebase, $return_type_part, $self_class, $static_class_type, $parent_class, $evaluate_class_constants, $evaluate_conditional_types, $final, $expand_generic);
            if (is_array($parts)) {
        ...
        if ($has_array_output) {
            $fleshed_out_type = TypeCombiner::combine($new_return_type_parts, $codebase);
        } else {
     ...
     */
    public static function expandAtomic(Codebase $codebase, Type\Atomic &$return_type, ?string $self_class, $static_class_type, ?string $parent_class, bool $evaluate_class_constants = true, bool $evaluate_conditional_types = false, bool $final = false, bool $expand_generic = false)
    {
        if ($return_type instanceof TNamedObject || $return_type instanceof TTemplateParam) {
            if ($return_type->extra_types) {
                ...
                foreach ($return_type->extra_types as &$extra_type) {
                    self::expandAtomic($codebase, $extra_type, $self_class, $static_class_type, $parent_class, $evaluate_class_constants, $evaluate_conditional_types, $expand_generic);
        ...
        if ($return_type instanceof Type\Atomic\TClassString && $return_type->as_type) {
            $new_as_type = clone $return_type->as_type;
            self::expandAtomic($codebase, $new_as_type, $self_class, $static_class_type, $parent_class, $evaluate_class_constants, $evaluate_conditional_types, $final, $expand_generic);
            ...
            $new_as_type = self::expandUnion($codebase, clone $return_type->as, $self_class, $static_class_type, $parent_class, $evaluate_class_constants, $evaluate_conditional_types, $final, $expand_generic);
        ...
        if ($return_type instanceof Type\Atomic\TScalarClassConstant) {
            ...
            if ($evaluate_class_constants && $codebase->classOrInterfaceExists($return_type->fq_classlike_name)) {
                ...
                if (strpos($return_type->const_name, '*') !== false) {
                    ...
                    if ($const_name_part) {
                        $matching_constants = \array_filter($matching_constants, function ($constant_name) use($const_name_part) : bool {
                            return $constant_name !== $const_name_part && \strpos($constant_name, $const_name_part) === 0;
                        });
                    }
                } else {
                ...
                if (isset($class_storage->type_aliases[$type_alias_name])) {
                    ...
                    if ($resolved_type_alias->replacement_atomic_types) {
                        ...
                        foreach ($replacement_atomic_types as $replacement_atomic_type) {
                            $recursively_fleshed_out_type = self::expandAtomic($codebase, $replacement_atomic_type, $self_class, $static_class_type, $parent_class, $evaluate_class_constants, $evaluate_conditional_types, $final, $expand_generic);
                            if (is_array($recursively_fleshed_out_type)) {
                                $recursively_fleshed_out_types = array_merge($recursively_fleshed_out_type, $recursively_fleshed_out_types);
                            } else {
        ...
        if ($return_type instanceof Type\Atomic\TKeyOfClassConstant || $return_type instanceof Type\Atomic\TValueOfClassConstant) {
            if ($return_type->fq_classlike_name === 'self' && $self_class) {
                $return_type->fq_classlike_name = $self_class;
            }
            if ($evaluate_class_constants && $codebase->classOrInterfaceExists($return_type->fq_classlike_name)) {
                try {
                    $class_constant_type = $codebase->classlikes->getClassConstantType($return_type->fq_classlike_name, $return_type->const_name, \ReflectionProperty::IS_PRIVATE);
                } catch (\Psalm\Exception\CircularReferenceException $e) {
        ...
        if ($return_type instanceof Type\Atomic\TIntMask) {
            ...
            foreach ($return_type->values as $value_type) {
                $new_value_type = self::expandAtomic($codebase, $value_type, $self_class, $static_class_type, $parent_class, $evaluate_class_constants, $evaluate_conditional_types, $final, $expand_generic);
        ...
        if ($return_type instanceof Type\Atomic\TIntMaskOf) {
            ...
            $new_value_types = self::expandAtomic($codebase, $value_type, $self_class, $static_class_type, $parent_class, $evaluate_class_constants, $evaluate_conditional_types, $final, $expand_generic);
        ...
        if ($return_type instanceof Type\Atomic\TArray || $return_type instanceof Type\Atomic\TGenericObject || $return_type instanceof Type\Atomic\TIterable) {
            foreach ($return_type->type_params as $k => $type_param) {
                /** @psalm-suppress PropertyTypeCoercion */
                $return_type->type_params[$k] = self::expandUnion($codebase, $type_param, $self_class, $static_class_type, $parent_class, $evaluate_class_constants, $evaluate_conditional_types, $final, $expand_generic);
            }
        } elseif ($return_type instanceof Type\Atomic\TKeyedArray) {
            foreach ($return_type->properties as &$property_type) {
                $property_type = self::expandUnion($codebase, $property_type, $self_class, $static_class_type, $parent_class, $evaluate_class_constants, $evaluate_conditional_types, $final, $expand_generic);
            }
        } elseif ($return_type instanceof Type\Atomic\TList) {
            $return_type->type_param = self::expandUnion($codebase, $return_type->type_param, $self_class, $static_class_type, $parent_class, $evaluate_class_constants, $evaluate_conditional_types, $final, $expand_generic);
        }
        ...
        if ($return_type instanceof Type\Atomic\TCallable || $return_type instanceof Type\Atomic\TClosure) {
            if ($return_type->params) {
                foreach ($return_type->params as $param) {
                    if ($param->type) {
                        $param->type = self::expandUnion($codebase, $param->type, $self_class, $static_class_type, $parent_class, $evaluate_class_constants, $evaluate_conditional_types, $final, $expand_generic);
                    }
                }
            }
            if ($return_type->return_type) {
                $return_type->return_type = self::expandUnion($codebase, $return_type->return_type, $self_class, $static_class_type, $parent_class, $evaluate_class_constants, $evaluate_conditional_types, $final, $expand_generic);
            }
        }
        if ($return_type instanceof Type\Atomic\TConditional) {
            return self::expandConditional($codebase, $return_type, $self_class, $static_class_type, $parent_class, $evaluate_class_constants, $evaluate_conditional_types, $final, $expand_generic);
        }
        return $return_type;
    }
    /**
     ...
     */
    private static function expandNamedObject(Codebase $codebase, Type\Atomic\TNamedObject $return_type, ?string $self_class, $static_class_type, ?string $parent_class, bool $final = false, bool &$expand_generic = false)
    {
        if ($expand_generic && \get_class($return_type) === TNamedObject::class && !$return_type->extra_types && $codebase->classOrInterfaceExists($return_type->value)) {
            ...
            if ($container_class_storage->template_types && \array_filter($container_class_storage->template_types, function ($type_map) {
                return !reset($type_map)->hasMixed();
            })) {
                $return_type = new Type\Atomic\TGenericObject($return_type->value, \array_values(\array_map(function ($type_map) {
        ...
        if ($static_class_type && ($return_type_lc === 'static' || $return_type_lc === '$this')) {
            if (is_string($static_class_type)) {
                ...
                if ($return_type instanceof Type\Atomic\TGenericObject && $static_class_type instanceof Type\Atomic\TGenericObject) {
        ...
        } elseif ($return_type->was_static && ($static_class_type instanceof Type\Atomic\TNamedObject || $static_class_type instanceof Type\Atomic\TTemplateParam)) {
            $return_type = clone $return_type;
     ...
     * @return Type\Atomic|non-empty-list<Type\Atomic>
     */
    private static function expandConditional(Codebase $codebase, Type\Atomic\TConditional $return_type, ?string $self_class, $static_class_type, ?string $parent_class, bool $evaluate_class_constants = true, bool $evaluate_conditional_types = false, bool $final = false, bool &$expand_generic = false)
    {
        if ($evaluate_conditional_types) {
            ...
            if ($return_type->conditional_type->isSingle()) {
                foreach ($return_type->conditional_type->getAtomicTypes() as $condition_atomic_type) {
                    $candidate = self::expandAtomic($codebase, $condition_atomic_type, $self_class, $static_class_type, $parent_class, $evaluate_class_constants, $evaluate_conditional_types, $final, $expand_generic);
            ...
            foreach ($return_type->if_type->getAtomicTypes() as $if_atomic_type) {
                $candidate = self::expandAtomic($codebase, $if_atomic_type, $self_class, $static_class_type, $parent_class, $evaluate_class_constants, $evaluate_conditional_types, $final, $expand_generic);
                $candidate_types = is_array($candidate) ? $candidate : [$candidate];
                $if_conditional_return_types = array_merge($if_conditional_return_types, $candidate_types);
            }
            $else_conditional_return_types = [];
            foreach ($return_type->else_type->getAtomicTypes() as $else_atomic_type) {
                $candidate = self::expandAtomic($codebase, $else_atomic_type, $self_class, $static_class_type, $parent_class, $evaluate_class_constants, $evaluate_conditional_types, $final, $expand_generic);
                $candidate_types = is_array($candidate) ? $candidate : [$candidate];
                $else_conditional_return_types = array_merge($else_conditional_return_types, $candidate_types);
            }
            if ($assertion && $return_type->param_name === (string) $return_type->if_type) {
                $if_conditional_return_type = TypeCombiner::combine($if_conditional_return_types, $codebase);
                $if_conditional_return_type = \Psalm\Internal\Type\SimpleAssertionReconciler::reconcile($assertion, $codebase, $if_conditional_return_type);
            ...
            if ($assertion && $return_type->param_name === (string) $return_type->else_type) {
                $else_conditional_return_type = TypeCombiner::combine($else_conditional_return_types, $codebase);
                $else_conditional_return_type = \Psalm\Internal\Type\SimpleNegatedAssertionReconciler::reconcile($assertion, $else_conditional_return_type);
                if ($else_conditional_return_type) {
                    $else_conditional_return_types = array_values($else_conditional_return_type->getAtomicTypes());
                }
            }
            $all_conditional_return_types = array_merge($if_conditional_return_types, $else_conditional_return_types);
            foreach ($all_conditional_return_types as $i => $conditional_return_type) {
                if ($conditional_return_type instanceof Type\Atomic\TVoid && count($all_conditional_return_types) > 1) {
                    $all_conditional_return_types[$i] = new Type\Atomic\TNull();
                    $all_conditional_return_types[$i]->from_docblock = true;
                }
            }
            $combined = TypeCombiner::combine(array_values($all_conditional_return_types), $codebase);
            return array_values($combined->getAtomicTypes());
        }
        $return_type->conditional_type = self::expandUnion($codebase, $return_type->conditional_type, $self_class, $static_class_type, $parent_class, $evaluate_class_constants, $evaluate_conditional_types, $final, $expand_generic);
        $return_type->if_type = self::expandUnion($codebase, $return_type->if_type, $self_class, $static_class_type, $parent_class, $evaluate_class_constants, $evaluate_conditional_types, $final, $expand_generic);
        $return_type->else_type = self::expandUnion($codebase, $return_type->else_type, $self_class, $static_class_type, $parent_class, $evaluate_class_constants, $evaluate_conditional_types, $final, $expand_generic);
        return $return_type;
    }
}