...
class PhpStormMetaScanner
{
    ...
    public static function handleOverride(array $args, Codebase $codebase) : void
    {
        ...
        $map = [];
        if ($args[1]->value->name->parts === ['map'] && $args[1]->value->args && $args[1]->value->args[0]->value instanceof PhpParser\Node\Expr\Array_) {
            foreach ($args[1]->value->args[0]->value->items as $array_item) {
                if ($array_item && $array_item->key instanceof PhpParser\Node\Scalar\String_) {
                    if ($array_item->value instanceof PhpParser\Node\Expr\ClassConstFetch && $array_item->value->class instanceof PhpParser\Node\Name\FullyQualified && $array_item->value->name instanceof PhpParser\Node\Identifier && strtolower($array_item->value->name->name)) {
                        $map[$array_item->key->value] = new Type\Union([new Type\Atomic\TNamedObject(implode('\\', $array_item->value->class->parts))]);
        ...
        $type_offset = null;
        if ($args[1]->value->name->parts === ['type'] && $args[1]->value->args && $args[1]->value->args[0]->value instanceof PhpParser\Node\Scalar\LNumber) {
            $type_offset = $args[1]->value->args[0]->value->value;
        }
        $element_type_offset = null;
        if ($args[1]->value->name->parts === ['elementType'] && $args[1]->value->args && $args[1]->value->args[0]->value instanceof PhpParser\Node\Scalar\LNumber) {
            $element_type_offset = $args[1]->value->args[0]->value->value;
        }
        if ($identifier instanceof PhpParser\Node\Expr\StaticCall && $identifier->class instanceof PhpParser\Node\Name\FullyQualified && $identifier->name instanceof PhpParser\Node\Identifier && $identifier->args && $identifier->args[0]->value instanceof PhpParser\Node\Scalar\LNumber) {
            ...
            if ($map) {
                ...
                $codebase->methods->return_type_provider->registerClosure(
                     ...
                     */
                    function (MethodReturnTypeProviderEvent $event) use($map, $offset, $meta_fq_classlike_name, $meta_method_name) : ?Type\Union {
                        ...
                        if (isset($call_args[$offset]->value) && ($call_arg_type = $statements_analyzer->node_data->getType($call_args[$offset]->value)) && $call_arg_type->isSingleStringLiteral()) {
                            ...
                            if (($mapped_type = $map[''] ?? null) && is_string($mapped_type)) {
                                if (strpos($mapped_type, '@') !== false) {
                                    ...
                                    if (strpos($mapped_type, '.') === false) {
                                        return new Type\Union([new Type\Atomic\TNamedObject($mapped_type)]);
                                    }
                                }
                            }
                        }
                ...
                $codebase->methods->return_type_provider->registerClosure(
                     ...
                     * @param list<PhpParser\Node\Arg> $call_args
                     */
                    function (MethodReturnTypeProviderEvent $event) use($type_offset, $meta_fq_classlike_name, $meta_method_name) : ?Type\Union {
                        $statements_analyzer = $event->getSource();
                        $call_args = $event->getCallArgs();
                        $method_name = $event->getMethodNameLowercase();
                        $fq_classlike_name = $event->getFqClasslikeName();
                ...
                $codebase->methods->return_type_provider->registerClosure(
                     ...
                     * @param list<PhpParser\Node\Arg> $call_args
                     */
                    function (MethodReturnTypeProviderEvent $event) use($element_type_offset, $meta_fq_classlike_name, $meta_method_name) : ?Type\Union {
                        $statements_analyzer = $event->getSource();
                        $call_args = $event->getCallArgs();
                        $method_name = $event->getMethodNameLowercase();
                        $fq_classlike_name = $event->getFqClasslikeName();
        ...
        if ($identifier instanceof PhpParser\Node\Expr\FuncCall && $identifier->name instanceof PhpParser\Node\Name\FullyQualified && $identifier->args && $identifier->args[0]->value instanceof PhpParser\Node\Scalar\LNumber) {
            ...
            if ($map) {
                ...
                $codebase->functions->return_type_provider->registerClosure(
                     ...
                     * @param list<PhpParser\Node\Arg> $call_args
                     */
                    function (FunctionReturnTypeProviderEvent $event) use($map, $offset) : Type\Union {
                ...
                $codebase->functions->return_type_provider->registerClosure(
                     ...
                     * @param list<PhpParser\Node\Arg> $call_args
                     */
                    function (FunctionReturnTypeProviderEvent $event) use($type_offset) : Type\Union {
                        $statements_analyzer = $event->getStatementsSource();
                        $call_args = $event->getCallArgs();
                        $function_id = $event->getFunctionId();
                        if (!$statements_analyzer instanceof \Psalm\Internal\Analyzer\StatementsAnalyzer) {
                ...
                $codebase->functions->return_type_provider->registerClosure(
                     ...
                     * @param list<PhpParser\Node\Arg> $call_args
                     */
                    function (FunctionReturnTypeProviderEvent $event) use($element_type_offset) : Type\Union {
                        $statements_analyzer = $event->getStatementsSource();
                        $call_args = $event->getCallArgs();
                        $function_id = $event->getFunctionId();
                        if (!$statements_analyzer instanceof \Psalm\Internal\Analyzer\StatementsAnalyzer) {
