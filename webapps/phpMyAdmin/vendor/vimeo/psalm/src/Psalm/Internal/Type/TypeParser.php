...
class TypeParser
{
     ...
     *
     */
    public static function parseTokens(array $type_tokens, ?array $php_version = null, array $template_type_map = [], array $type_aliases = []) : Union
    {
        ...
        $parsed_type = self::getTypeFromTree($parse_tree, $codebase, $php_version, $template_type_map, $type_aliases);
     ...
     * @return  Atomic|Union
     */
    public static function getTypeFromTree(ParseTree $parse_tree, Codebase $codebase, ?array $php_version = null, array $template_type_map = [], array $type_aliases = []) : TypeNode
    {
        if ($parse_tree instanceof ParseTree\GenericTree) {
            return self::getTypeFromGenericTree($parse_tree, $codebase, $template_type_map, $type_aliases);
        }
        ...
        if ($parse_tree instanceof ParseTree\CallableWithReturnTypeTree) {
            $callable_type = self::getTypeFromTree($parse_tree->children[0], $codebase, null, $template_type_map, $type_aliases);
            ...
            $return_type = self::getTypeFromTree($parse_tree->children[1], $codebase, null, $template_type_map, $type_aliases);
        ...
        if ($parse_tree instanceof ParseTree\EncapsulationTree) {
            return self::getTypeFromTree($parse_tree->children[0], $codebase, null, $template_type_map, $type_aliases);
        }
        if ($parse_tree instanceof ParseTree\NullableTree) {
            ...
            $non_nullable_type = self::getTypeFromTree($parse_tree->children[0], $codebase, null, $template_type_map, $type_aliases);
            if ($non_nullable_type instanceof Union) {
                $non_nullable_type->addType(new TNull());
                return $non_nullable_type;
            }
            return TypeCombiner::combine([new TNull(), $non_nullable_type]);
        }
        if ($parse_tree instanceof ParseTree\MethodTree || $parse_tree instanceof ParseTree\MethodWithReturnTypeTree) {
        ...
        if ($parse_tree instanceof ParseTree\TemplateAsTree) {
            return new Atomic\TTemplateParam($parse_tree->param_name, new Union([new TNamedObject($parse_tree->as)]), 'class-string-map');
        }
        if ($parse_tree instanceof ParseTree\ConditionalTree) {
            ...
            $conditional_type = self::getTypeFromTree($parse_tree->condition->children[0], $codebase, null, $template_type_map, $type_aliases);
            $if_type = self::getTypeFromTree($parse_tree->children[0], $codebase, null, $template_type_map, $type_aliases);
            $else_type = self::getTypeFromTree($parse_tree->children[1], $codebase, null, $template_type_map, $type_aliases);
            ...
            return new Atomic\TConditional($template_param_name, $first_class, $template_type_map[$template_param_name][$first_class], $conditional_type, $if_type, $else_type);
        }
        ...
        if (strpos($parse_tree->value, '::')) {
            ...
            if (isset($template_type_map[$fq_classlike_name]) && $const_name === 'class') {
                ...
                return self::getGenericParamClass($fq_classlike_name, $template_type_map[$fq_classlike_name][$first_class], $first_class);
            }
            if ($const_name === 'class') {
    ...
    private static function getGenericParamClass(string $param_name, Union $as, string $defining_class) : Atomic\TTemplateParamClass
    {
        if ($as->hasMixed()) {
            return new Atomic\TTemplateParamClass($param_name, 'object', null, $defining_class);
        }
        if (!$as->isSingle()) {
        ...
        foreach ($as->getAtomicTypes() as $t) {
            ...
            if ($t instanceof TIterable) {
                $traversable = new TGenericObject('Traversable', $t->type_params);
                $as->substitute(new Union([$t]), new Union([$traversable]));
                return new Atomic\TTemplateParamClass($param_name, $traversable->value, $traversable, $defining_class);
            }
            if ($t instanceof Atomic\TTemplateParam) {
                ...
                return new Atomic\TTemplateParamClass($t->param_name, $t_atomic_type ? $t_atomic_type->value : 'object', $t_atomic_type, $t->defining_class);
            }
            if (!$t instanceof TNamedObject) {
                throw new TypeParseTreeException('Invalid templated classname \'' . $t->getId() . '\'');
            }
            return new Atomic\TTemplateParamClass($param_name, $t->value, $t, $defining_class);
        }
     ...
     */
    private static function getTypeFromGenericTree(ParseTree\GenericTree $parse_tree, Codebase $codebase, array $template_type_map, array $type_aliases)
    {
        $generic_type = $parse_tree->value;
        $generic_params = [];
        foreach ($parse_tree->children as $i => $child_tree) {
            $tree_type = self::getTypeFromTree($child_tree, $codebase, null, $template_type_map, $type_aliases);
        ...
        if ($generic_type_value === 'class-string') {
            ...
            if (isset($template_type_map[$class_name])) {
                ...
                return self::getGenericParamClass($class_name, $template_type_map[$class_name][$first_class], $first_class);
            }
        ...
        if ($generic_type_value === 'class-string-map') {
            ...
            return new TClassStringMap($template_param_name, $template_as_type, $generic_params[1]);
        }
        if ($generic_type_value === 'key-of') {
            ...
            if (isset($template_type_map[$param_name])) {
                ...
                return new Atomic\TTemplateKeyOf($param_name, $defining_class, $template_type_map[$param_name][$defining_class]);
            }
            $param_union_types = array_values($generic_params[0]->getAtomicTypes());
            if (count($param_union_types) > 1) {
            ...
            if (!$param_union_types[0] instanceof Atomic\TScalarClassConstant) {
                throw new TypeParseTreeException('Untemplated key-of param ' . $param_name . ' should be a class constant');
            }
            return new Atomic\TKeyOfClassConstant($param_union_types[0]->fq_classlike_name, $param_union_types[0]->const_name);
        }
        if ($generic_type_value === 'value-of') {
            ...
            if (!$param_union_types[0] instanceof Atomic\TScalarClassConstant) {
                throw new TypeParseTreeException('Untemplated value-of param ' . $param_name . ' should be a class constant');
            }
            return new Atomic\TValueOfClassConstant($param_union_types[0]->fq_classlike_name, $param_union_types[0]->const_name);
        }
        if ($generic_type_value === 'int-mask') {
            ...
            foreach ($generic_params as $generic_param) {
                ...
                if (!$atomic_type instanceof TLiteralInt && !($atomic_type instanceof Atomic\TScalarClassConstant && strpos($atomic_type->const_name, '*') === false)) {
        ...
        if ($generic_type_value === 'int-mask-of') {
            ...
            if (!$param_type instanceof Atomic\TScalarClassConstant && !$param_type instanceof Atomic\TValueOfClassConstant && !$param_type instanceof Atomic\TKeyOfClassConstant) {
                throw new TypeParseTreeException('Invalid reference passed to int-mask-of');
            } elseif ($param_type instanceof Atomic\TScalarClassConstant && strpos($param_type->const_name, '*') === false) {
        ...
        if (isset(TypeTokenizer::PSALM_RESERVED_WORDS[$generic_type_value]) && $generic_type_value !== 'self' && $generic_type_value !== 'static') {
     ...
     * @throws TypeParseTreeException
     */
    private static function getTypeFromUnionTree(ParseTree\UnionTree $parse_tree, Codebase $codebase, array $template_type_map, array $type_aliases) : Union
    {
        $has_null = false;
        $atomic_types = [];
        foreach ($parse_tree->children as $child_tree) {
            if ($child_tree instanceof ParseTree\NullableTree) {
                ...
                $atomic_type = self::getTypeFromTree($child_tree->children[0], $codebase, null, $template_type_map, $type_aliases);
                $has_null = true;
            } else {
                $atomic_type = self::getTypeFromTree($child_tree, $codebase, null, $template_type_map, $type_aliases);
            }
     ...
     * @throws TypeParseTreeException
     */
    private static function getTypeFromIntersectionTree(ParseTree\IntersectionTree $parse_tree, Codebase $codebase, array $template_type_map, array $type_aliases)
    {
        $intersection_types = array_map(function (ParseTree $child_tree) use($codebase, $template_type_map, $type_aliases) {
            $atomic_type = self::getTypeFromTree($child_tree, $codebase, null, $template_type_map, $type_aliases);
        ...
        if ($onlyTKeyedArray) {
            ...
            foreach ($intersection_types as $intersection_type) {
                foreach ($intersection_type->properties as $property => $property_type) {
                    ...
                    $intersection_type = \Psalm\Type::intersectUnionTypes($properties[$property], $property_type, $codebase);
        ...
        if ($intersection_types[0] instanceof TTypeAlias) {
            ...
            foreach ($intersection_types as $intersection_type) {
                if (!$intersection_type instanceof TIterable && !$intersection_type instanceof TNamedObject && !$intersection_type instanceof TTemplateParam && !$intersection_type instanceof TObjectWithProperties) {
                    throw new TypeParseTreeException('Intersection types must be all objects or all object-like arrays, ' . get_class($intersection_type) . ' provided');
                }
     ...
     * @throws TypeParseTreeException
     */
    private static function getTypeFromCallableTree(ParseTree\CallableTree $parse_tree, Codebase $codebase, array $template_type_map, array $type_aliases)
    {
        $params = array_map(
            /**
            ...
            function (ParseTree $child_tree) use($codebase, $template_type_map, $type_aliases) : FunctionLikeParameter {
                ...
                if ($child_tree instanceof ParseTree\CallableParamTree) {
                    if (isset($child_tree->children[0])) {
                        $tree_type = self::getTypeFromTree($child_tree->children[0], $codebase, null, $template_type_map, $type_aliases);
                    } else {
                ...
                $param = new FunctionLikeParameter('', false, $tree_type, null, null, $is_optional, false, $is_variadic);
     ...
     * @throws TypeParseTreeException
     */
    private static function getTypeFromIndexAccessTree(ParseTree\IndexedAccessTree $parse_tree, array $template_type_map) : Atomic\TTemplateIndexedAccess
    {
        ...
        if ($offset_defining_class !== $array_defining_class && substr($offset_defining_class, 0, 3) !== 'fn-') {
            throw new TypeParseTreeException('Template params are defined in different locations');
        }
        return new Atomic\TTemplateIndexedAccess($array_param_name, $offset_param_name, $array_defining_class);
    }
    /**
     * @param  array<string, array<string, Union>> $template_type_map
     * @param  array<string, TypeAlias> $type_aliases
     ...
     * @throws TypeParseTreeException
     */
    private static function getTypeFromKeyedArrayTree(ParseTree\KeyedArrayTree $parse_tree, Codebase $codebase, array $template_type_map, array $type_aliases)
    {
        $properties = [];
        ...
        foreach ($parse_tree->children as $i => $property_branch) {
            if (!$property_branch instanceof ParseTree\KeyedArrayPropertyTree) {
                $property_type = self::getTypeFromTree($property_branch, $codebase, null, $template_type_map, $type_aliases);
                $property_maybe_undefined = false;
                $property_key = (string) $i;
            } elseif (count($property_branch->children) === 1) {
                $property_type = self::getTypeFromTree($property_branch->children[0], $codebase, null, $template_type_map, $type_aliases);
