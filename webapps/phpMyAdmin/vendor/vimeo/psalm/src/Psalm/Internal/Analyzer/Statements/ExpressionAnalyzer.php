...
{
    public static function analyze(StatementsAnalyzer $statements_analyzer, PhpParser\Node\Expr $stmt, Context $context, bool $array_assignment = false, ?Context $global_context = null, bool $from_stmt = false) : bool
    {
        $codebase = $statements_analyzer->getCodebase();
        if (self::handleExpression($statements_analyzer, $stmt, $context, $array_assignment, $global_context, $from_stmt) === false) {
            return false;
        }
        if (!$context->inside_conditional && ($stmt instanceof PhpParser\Node\Expr\BinaryOp || $stmt instanceof PhpParser\Node\Expr\Instanceof_ || $stmt instanceof PhpParser\Node\Expr\Assign || $stmt instanceof PhpParser\Node\Expr\BooleanNot || $stmt instanceof PhpParser\Node\Expr\Empty_ || $stmt instanceof PhpParser\Node\Expr\Isset_ || $stmt instanceof PhpParser\Node\Expr\FuncCall)) {
            ...
            if ($assertions === null) {
                ...
                Expression\AssertionFinder::scrapeAssertions($stmt, $context->self, $statements_analyzer, $codebase, $negate, true, false);
            }
        }
    ...
    private static function handleExpression(StatementsAnalyzer $statements_analyzer, PhpParser\Node\Expr $stmt, Context $context, bool $array_assignment, ?Context $global_context, bool $from_stmt) : bool
    {
        if ($stmt instanceof PhpParser\Node\Expr\Variable) {
            return Expression\Fetch\VariableFetchAnalyzer::analyze($statements_analyzer, $stmt, $context, false, null, $array_assignment);
        }
        if ($stmt instanceof PhpParser\Node\Expr\Assign) {
            $assignment_type = Expression\AssignmentAnalyzer::analyze($statements_analyzer, $stmt->var, $stmt->expr, null, $context, $stmt->getDocComment());
        ...
        if ($stmt instanceof PhpParser\Node\Expr\PropertyFetch) {
            return Expression\Fetch\InstancePropertyFetchAnalyzer::analyze($statements_analyzer, $stmt, $context, $array_assignment);
        }
        if ($stmt instanceof PhpParser\Node\Expr\StaticPropertyFetch) {
            return Expression\Fetch\StaticPropertyFetchAnalyzer::analyze($statements_analyzer, $stmt, $context);
        }
        ...
        if ($stmt instanceof PhpParser\Node\Expr\BinaryOp) {
            return Expression\BinaryOpAnalyzer::analyze($statements_analyzer, $stmt, $context, 0, $from_stmt);
        }
        ...
        if ($stmt instanceof PhpParser\Node\Expr\FuncCall) {
            return FunctionCallAnalyzer::analyze($statements_analyzer, $stmt, $context);
        }
        ...
        if ($stmt instanceof PhpParser\Node\Expr\ArrayDimFetch) {
            return Expression\Fetch\ArrayFetchAnalyzer::analyze($statements_analyzer, $stmt, $context);
        }
        ...
        if ($stmt instanceof PhpParser\Node\Expr\ShellExec) {
            if (IssueBuffer::accepts(new ForbiddenCode('Use of shell_exec', new CodeLocation($statements_analyzer->getSource(), $stmt)), $statements_analyzer->getSuppressedIssues())) {
                // continue
            }
            return true;
        }
        ...
        if ($stmt instanceof PhpParser\Node\Expr\Match_ && $statements_analyzer->getCodebase()->php_major_version >= 8) {
            return Expression\MatchAnalyzer::analyze($statements_analyzer, $stmt, $context);
        }
        if ($stmt instanceof PhpParser\Node\Expr\Throw_ && $statements_analyzer->getCodebase()->php_major_version >= 8) {
            return ThrowAnalyzer::analyze($statements_analyzer, $stmt, $context);
        }
        if (($stmt instanceof PhpParser\Node\Expr\NullsafePropertyFetch || $stmt instanceof PhpParser\Node\Expr\NullsafeMethodCall) && $statements_analyzer->getCodebase()->php_major_version >= 8) {
        ...
        if (IssueBuffer::accepts(new UnrecognizedExpression('Psalm does not understand ' . get_class($stmt), new CodeLocation($statements_analyzer->getSource(), $stmt)), $statements_analyzer->getSuppressedIssues())) {
            // fall through
        }
        return false;
    }
