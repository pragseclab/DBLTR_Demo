...
class Context
{
    ...
    {
        unset($this->referenced_var_ids[$remove_var_id], $this->vars_possibly_in_scope[$remove_var_id]);
    ...
    public static function filterClauses(string $remove_var_id, array $clauses, ?Union $new_type = null, ?StatementsAnalyzer $statements_analyzer = null) : array
    {
        ...
        foreach ($clauses as $clause) {
            ...
            if (!isset($clause->possibilities[$remove_var_id]) || $clause->possibilities[$remove_var_id] === [$new_type_string]) {
                ...
                foreach ($clause->possibilities[$remove_var_id] as $type) {
                    ...
                    if (($type === '!falsy' || $type === 'falsy') && ($new_type->hasArray() || $new_type->hasPossiblyNumericType())) {
                        $type_changed = true;
                        break;
                    }
                    $result_type = AssertionReconciler::reconcile($type, clone $new_type, null, $statements_analyzer, false, [], null, [], $failed_reconciliation);
     ...
     */
    public function removeDescendents(string $remove_var_id, ?Union $existing_type = null, ?Union $new_type = null, ?StatementsAnalyzer $statements_analyzer = null) : void
    {
    ...
    public function removeMutableObjectVars() : void
    {
        $vars_to_remove = [];
        foreach ($this->vars_in_scope as $var_id => $type) {
            if ($type->has_mutations && (strpos($var_id, '->') !== false || strpos($var_id, '::') !== false)) {
    ...
    public function hasVariable(?string $var_name) : bool
    {
        ...
        if ($stripped_var !== '$this' || $var_name !== $stripped_var) {
            ...
            if (!isset($this->vars_possibly_in_scope[$var_name]) && !isset($this->vars_in_scope[$var_name])) {
                return false;
            }
        }
    ...
    {
        $globals = ['$argv' => new Type\Union([new Type\Atomic\TArray([Type::getInt(), Type::getString()])]), '$argc' => Type::getInt()];
    ...
    public function isSuppressingExceptions(StatementsAnalyzer $statements_analyzer) : bool
    {
        ...
        if ($suppressed_issue_position !== false) {
            if (is_int($suppressed_issue_position)) {
                ...
                IssueBuffer::addUsedSuppressions([$file => [$suppressed_issue_position => true]]);
            }
            return true;
        }
        return false;
    }
    public function mergeFunctionExceptions(FunctionLikeStorage $function_storage, CodeLocation $codelocation) : void
    {
