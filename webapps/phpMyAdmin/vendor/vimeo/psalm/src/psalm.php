...
$valid_short_options = ['f:', 'm', 'h', 'v', 'c:', 'i', 'r:'];
$valid_long_options = [
    'clear-cache',
    'clear-global-cache',
    'config:',
...
$first_autoloader = $include_collector->runAndCollect(function () use($current_dir, $options, $vendor_dir) : ?\Composer\Autoload\ClassLoader {
    return requireAutoloaders($current_dir, isset($options['r']), $vendor_dir);
});
$run_taint_analysis = isset($options['track-tainted-input']) || isset($options['security-analysis']) || isset($options['taint-analysis']);
...
$output_format = isset($options['output-format']) && is_string($options['output-format']) ? $options['output-format'] : \Psalm\Report::TYPE_CONSOLE;
$init_level = null;
...
if (isset($options['i'])) {
    ...
    $args = array_values(array_filter($args, function (string $arg) : bool {
        return $arg !== '--ansi' && $arg !== '--no-ansi' && $arg !== '-i' && $arg !== '--init' && $arg !== '--debug' && $arg !== '--debug-by-line' && $arg !== '--debug-emitted-issues' && strpos($arg, '--disable-extension=') !== 0 && strpos($arg, '--root=') !== 0 && strpos($arg, '--r=') !== 0;
    }));
    if (count($args)) {
        if (count($args) > 2) {
    ...
    if ($init_level === null) {
        ...
        try {
            $template_contents = \Psalm\Config\Creator::getContents($current_dir, $init_source_dir, $init_level, $vendor_dir);
    ...
    if (isset($options['error-level']) && is_numeric($options['error-level'])) {
        ...
        if (!in_array($config_level, [1, 2, 3, 4, 5, 6, 7, 8], true)) {
            throw new \Psalm\Exception\ConfigException('Invalid error level ' . $config_level);
        }
        $config->level = $config_level;
    }
}
$config->setIncludeCollector($include_collector);
$in_ci = isset($_SERVER['TRAVIS']) || isset($_SERVER['CIRCLECI']) || isset($_SERVER['APPVEYOR']) || isset($_SERVER['JENKINS_URL']) || isset($_SERVER['SCRUTINIZER']) || isset($_SERVER['GITLAB_CI']) || isset($_SERVER['GITHUB_WORKFLOW']);
...
if (!isset($options['threads']) && !isset($options['debug']) && $threads === 1 && ini_get('pcre.jit') === '1' && PHP_OS === 'Darwin' && version_compare(PHP_VERSION, '7.3.0') >= 0 && version_compare(PHP_VERSION, '7.4.0') < 0) {
    echo 'If you want to run Psalm as a language server, or run Psalm with' . PHP_EOL . 'multiple processes (--threads=4), beware:' . PHP_EOL . \Psalm\Internal\Fork\Pool::MAC_PCRE_MESSAGE . PHP_EOL . PHP_EOL;
}
...
$is_diff = !isset($options['no-diff']) && !isset($options['set-baseline']) && !isset($options['update-baseline']);
...
if (isset($options['no-cache']) || isset($options['i'])) {
    $providers = new Provider\Providers(new Provider\FileProvider());
} else {
    $no_reflection_cache = isset($options['no-reflection-cache']);
    $no_file_cache = isset($options['no-file-cache']);
    $file_storage_cache_provider = $no_reflection_cache ? null : new Provider\FileStorageCacheProvider($config);
    $classlike_storage_cache_provider = $no_reflection_cache ? null : new Provider\ClassLikeStorageCacheProvider($config);
    $providers = new Provider\Providers(new Provider\FileProvider(), new Provider\ParserCacheProvider($config, !$no_file_cache), $file_storage_cache_provider, $classlike_storage_cache_provider, new Provider\FileReferenceCacheProvider($config), new Provider\ProjectCacheProvider(Composer::getLockFilePath($current_dir)));
}
...
$project_analyzer = new ProjectAnalyzer($config, $providers, $stdout_report_options, ProjectAnalyzer::getFileReportOptions($report_file_paths, isset($options['report-show-info']) ? $options['report-show-info'] !== 'false' && $options['report-show-info'] !== '0' : true), $threads, $progress);
...
if (isset($options['set-baseline']) && is_string($options['set-baseline'])) {
    ...
    ErrorBaseline::create(new \Psalm\Internal\Provider\FileProvider(), $options['set-baseline'], IssueBuffer::getIssuesData(), $config->include_php_versions_in_error_baseline || isset($options['include-php-versions']));
    fwrite(STDERR, "Baseline saved to {$options['set-baseline']}.");
    update_config_file($config, $path_to_config ?? $current_dir, $options['set-baseline']);
...
if (isset($options['update-baseline'])) {
    ...
    try {
        $issue_current_baseline = ErrorBaseline::read(new \Psalm\Internal\Provider\FileProvider(), $baselineFile);
        $total_issues_current_baseline = ErrorBaseline::countTotalIssues($issue_current_baseline);
        $issue_baseline = ErrorBaseline::update(new \Psalm\Internal\Provider\FileProvider(), $baselineFile, IssueBuffer::getIssuesData(), $config->include_php_versions_in_error_baseline || isset($options['include-php-versions']));
...
if ($baseline_file_path && !isset($options['ignore-baseline'])) {
    try {
        $issue_baseline = ErrorBaseline::read(new \Psalm\Internal\Provider\FileProvider(), $baseline_file_path);
    } catch (\Psalm\Exception\ConfigException $exception) {
...
if ($type_map_location) {
    ...
    $reference_dictionary = \Psalm\Internal\Codebase\ReferenceMapGenerator::getReferenceMap($providers->classlike_storage_provider, $expected_references);
...
if ($stubs_location) {
    $providers->file_provider->setContents($stubs_location, \Psalm\Internal\Stubs\Generator\StubsGenerator::getAll($project_analyzer->getCodebase(), $providers->classlike_storage_provider, $providers->file_storage_provider));
}
if (!isset($options['i'])) {
    IssueBuffer::finish($project_analyzer, !$paths_to_check, $start_time, isset($options['stats']), $issue_baseline);
} else {
    ...
    if (!$issues_by_file) {
        ...
        $init_level = \Psalm\Config\Creator::getLevel(array_merge(...array_values($issues_by_file)), array_sum($mixed_counts));
    }
