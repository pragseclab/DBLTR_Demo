    ...
    public static function parse(Parser $parser, TokensList $list, array $options = array())
    {
        ...
        if ($state !== 3) {
            ...
            for (; $list->idx < $list->count; ++$list->idx) {
                ...
                if ($asFound && $token->type === Token::TYPE_KEYWORD && ($token->flags & Token::FLAG_KEYWORD_RESERVED || $token->flags & Token::FLAG_KEYWORD_FUNCTION)) {
                    $parser->error('An alias expected after AS but got ' . $token->value, $token);
                    $asFound = false;
                    break;
                }
                if ($asFound || $token->type === Token::TYPE_STRING || $token->type === Token::TYPE_SYMBOL && !$token->flags & Token::FLAG_SYMBOL_VARIABLE || $token->type === Token::TYPE_NONE) {
