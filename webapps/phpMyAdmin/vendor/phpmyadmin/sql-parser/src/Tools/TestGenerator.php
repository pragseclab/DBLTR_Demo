    ...
    public static function generate($query, $type = 'parser')
    {
        ...
        if (!empty($lexer->errors)) {
            foreach ($lexer->errors as $err) {
                $lexerErrors[] = [$err->getMessage(), $err->ch, $err->pos, $err->getCode()];
            }
            $lexer->errors = [];
        }
        ...
        if (!empty($parser->errors)) {
            foreach ($parser->errors as $err) {
                $parserErrors[] = [$err->getMessage(), $err->token, $err->getCode()];
            }
            $parser->errors = [];
        }
        ...
        foreach ($files as $file) {
            ...
            if (is_dir($inputFile)) {
                ...
                if (!file_exists($outputFile)) {
                    ...
                    static::build(strpos($inputFile, 'lex') !== false ? 'lexer' : 'parser', $inputFile, $outputFile, $debugFile, strpos($inputFile, 'ansi') !== false);
                } else {
