...
class Config
{
    ...
    public function __set($name, $value)
    {
        ...
        switch ($name) {
            case 'reportWidth':
                ...
                if ($value === 'auto' && function_exists('shell_exec') === true && preg_match('|\\d+ (\\d+)|', shell_exec('stty size 2>&1'), $matches) === 1) {
                    $value = (int) $matches[1];
                } else {
                    $value = (int) $value;
                }
    ...
    public function __construct(array $cliArgs = array(), $dieOnUnknownArg = true)
    {
        ...
        if ($this->stdin === true || Util\Common::isStdinATTY() === false && feof($handle) === false) {
            $readStreams = [$handle];
            $writeSteams = null;
            $fileContents = '';
