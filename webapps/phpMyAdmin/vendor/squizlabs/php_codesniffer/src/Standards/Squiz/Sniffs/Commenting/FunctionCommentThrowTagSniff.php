            ...
            */
            ...
            if ($tokens[$nextToken]['code'] === T_NEW || $tokens[$nextToken]['code'] === T_NS_SEPARATOR || $tokens[$nextToken]['code'] === T_STRING) {
                if ($tokens[$nextToken]['code'] === T_NEW) {
                    $currException = $phpcsFile->findNext([T_NS_SEPARATOR, T_STRING], $currPos, $stackPtrEnd, false, null, true);
                } else {
                    $currException = $nextToken;
                }
                if ($currException !== false) {
                    $endException = $phpcsFile->findNext([T_NS_SEPARATOR, T_STRING], $currException + 1, $stackPtrEnd, true, null, true);
