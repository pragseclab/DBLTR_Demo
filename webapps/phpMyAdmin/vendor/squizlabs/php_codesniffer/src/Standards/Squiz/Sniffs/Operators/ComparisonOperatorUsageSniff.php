...
class ComparisonOperatorUsageSniff implements Sniff
{
     ...
     * @var array
     */
    private static $validOps = array(T_IS_IDENTICAL => true, T_IS_NOT_IDENTICAL => true, T_LESS_THAN => true, T_GREATER_THAN => true, T_IS_GREATER_OR_EQUAL => true, T_IS_SMALLER_OR_EQUAL => true, T_INSTANCEOF => true);
    /**
     * A list of invalid operators with their alternatives.
     *
     * @var array<int, string>
     */
    private static $invalidOps = array('PHP' => array(T_IS_EQUAL => '===', T_IS_NOT_EQUAL => '!==', T_BOOLEAN_NOT => '=== FALSE'), 'JS' => array(T_IS_EQUAL => '===', T_IS_NOT_EQUAL => '!=='));
    /**
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        for ($i = $start; $i <= $end; $i++) {
            ...
            if (isset(self::$invalidOps[$tokenizer][$type]) === true) {
                ...
                if (isset(self::$validOps[$type]) === true) {
                    $foundOps++;
                }
            }
