...
class File
{
     ...
     * @return boolean
     */
    public function addError($error, $stackPtr, $code, $data = array(), $severity = 0, $fixable = false)
    {
        if ($stackPtr === null) {
            $line = 1;
            $column = 1;
    ...
    public function isReference($stackPtr)
    {
        ...
        $tokenBefore = $this->findPrevious(Util\Tokens::$emptyTokens, $stackPtr - 1, null, true);
        ...
        $tokenAfter = $this->findNext(Util\Tokens::$emptyTokens, $stackPtr + 1, null, true);
    ...
    public function findStartOfStatement($start, $ignore = null)
    {
        ...
        for ($i = $start; $i >= 0; $i--) {
            ...
            if (isset($this->tokens[$i]['scope_opener']) === true && $i === $this->tokens[$i]['scope_closer'] && $this->tokens[$i]['code'] !== T_CLOSE_PARENTHESIS && $this->tokens[$i]['code'] !== T_END_NOWDOC && $this->tokens[$i]['code'] !== T_END_HEREDOC) {
    ...
    {
        $endTokens = [T_COLON => true, T_COMMA => true, T_DOUBLE_ARROW => true, T_SEMICOLON => true, T_CLOSE_PARENTHESIS => true, T_CLOSE_SQUARE_BRACKET => true, T_CLOSE_CURLY_BRACKET => true, T_CLOSE_SHORT_ARRAY => true, T_OPEN_TAG => true, T_CLOSE_TAG => true];
        if ($ignore !== null) {
            $ignore = (array) $ignore;
            foreach ($ignore as $code) {
                unset($endTokens[$code]);
