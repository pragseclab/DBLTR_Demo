...
class Generator
{
    ...
    public function getObjectForTrait($traitName, array $arguments = [], $traitClassName = '', $callOriginalConstructor = true, $callOriginalClone = true, $callAutoload = true)
    {
        ...
        return $this->getObject($classTemplate->render(), $className['className'], '', $callOriginalConstructor, $callAutoload, $arguments);
    }
    /**
    ...
    private function generateMock($type, $explicitMethods, $mockClassName, $callOriginalClone, $callAutoload, $cloneArguments, $callOriginalMethods)
    {
            ...
            foreach ($type as $_type) {
                ...
                foreach ($this->getClassMethods($_type) as $method) {
                    ...
                    if ($this->canMockMethod($methodReflection)) {
                        $mockMethods->addMethods(MockMethod::fromReflection($methodReflection, $callOriginalMethods, $cloneArguments));
        ...
        if (!$isClass && !$isInterface) {
            ...
            if ($isInterface && $class->implementsInterface(\Throwable::class)) {
                ...
                $mockClassName = $this->generateClassName($actualClassName, $mockClassName['className'], 'Mock_');
            }
        ...
        if ($explicitMethods === [] && ($isClass || $isInterface)) {
            $mockMethods->addMethods(...$this->mockClassMethods($mockClassName['fullClassName'], $callOriginalMethods, $cloneArguments));
        }
        if (\is_array($explicitMethods)) {
            foreach ($explicitMethods as $methodName) {
                if ($class !== null && $class->hasMethod($methodName)) {
                    ...
                    $mockMethods->addMethods(MockMethod::fromName($mockClassName['fullClassName'], $methodName, $cloneArguments));
                }
            }
        }
        $mockedMethods = '';
