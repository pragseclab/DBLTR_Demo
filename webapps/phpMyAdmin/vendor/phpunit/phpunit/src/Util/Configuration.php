...
final class Configuration
{
    ...
    public function getExtensionConfiguration() : array
    {
        ...
        foreach ($this->xpath->query('extensions/extension') as $extension) {
            ...
            if ($extension->getAttribute('file')) {
                $file = $this->toAbsolutePath((string) $extension->getAttribute('file'), true);
            }
    ...
    public function getPHPUnitConfiguration() : array
    {
        ...
        if ($root->hasAttribute('stopOnDefect')) {
            $result['stopOnDefect'] = $this->getBoolean((string) $root->getAttribute('stopOnDefect'), false);
        }
        ...
        if ($root->hasAttribute('defaultTimeLimit')) {
            $result['defaultTimeLimit'] = $this->getInteger((string) $root->getAttribute('defaultTimeLimit'), 1);
        }
        ...
        if ($root->hasAttribute('cacheResult')) {
            $result['cacheResult'] = $this->getBoolean((string) $root->getAttribute('cacheResult'), false);
        }
        if ($root->hasAttribute('cacheResultFile')) {
            $result['cacheResultFile'] = $this->toAbsolutePath((string) $root->getAttribute('cacheResultFile'));
        }
