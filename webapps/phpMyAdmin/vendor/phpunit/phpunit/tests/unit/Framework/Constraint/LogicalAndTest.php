...
final class LogicalAndTest extends ConstraintTestCase
{
    ...
    {
        $constraints = [new \TruthyConstraint(), new \FalsyConstraint(), new \stdClass()];
    ...
    {
        $names = ['is healthy', 'is rich in amino acids', 'is rich in unsaturated fats'];
...
EOF;
    ...
    {
        $values = ['single' => [new \FalsyConstraint()], 'multiple' => [new \TruthyConstraint(), new \FalsyConstraint(), new \TruthyConstraint()]];
        foreach ($values as $key => $constraints) {
            (yield $key => [$constraints]);
        }
    }
    public function providerSucceedingConstraints() : \Generator
    {
        $values = ['single' => [new \TruthyConstraint()], 'multiple' => [new \TruthyConstraint(), new \TruthyConstraint(), new \TruthyConstraint()]];
        foreach ($values as $key => $constraints) {
            (yield $key => [$constraints]);
        }
    }
