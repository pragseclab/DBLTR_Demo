...
class GeneratorTest extends TestCase
{
    ...
    {
        $mock = $this->generator->getMockForAbstractClass(AbstractMockTestClass::class, [], '', true, true, true, ['nonexistentMethod']);
    ...
    {
        $mock = $this->generator->getMockForTrait(AbstractTrait::class, [], '', true, true, true, ['nonexistentMethod']);
        $this->assertTrue(\method_exists($mock, 'nonexistentMethod'));
        $this->assertTrue(\method_exists($mock, 'doSomething'));
    ...
    public function testVariadicArgumentsArePassedToOriginalMethod()
    {
        ...
        $mock = $this->generator->getMock(ClassWithVariadicArgumentMethod::class, [], [], '', true, false, true, false, true);
    ...
    public function testVariadicArgumentsArePassedToMockedMethod()
    {
        ...
        $arguments = [1, 'foo', false];
        $mock->expects($this->once())->method('foo')->with(...$arguments);
        $mock->foo(...$arguments);
    }
}