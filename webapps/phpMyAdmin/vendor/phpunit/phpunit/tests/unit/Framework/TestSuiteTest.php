...
class TestSuiteTest extends TestCase
{
    ...
    {
        $reflection = $this->getMockBuilder(\ReflectionClass::class)->setConstructorArgs([$this])->getMock();
        $reflection->expects($this->once())->method('getConstructor')->willReturn(null);
        $reflection->expects($this->once())->method('isInstantiable')->willReturn(true);
        $reflection->expects($this->once())->method('getName')->willReturn(__CLASS__);
    ...
    public function testTearDownAfterClassInTestSuite() : void
    {
        ...
        $this->assertSame('Exception in ExceptionInTearDownAfterClassTest::tearDownAfterClass' . \PHP_EOL . 'throw Exception in tearDownAfterClass()', $failure->thrownException()->getMessage());
    }
}