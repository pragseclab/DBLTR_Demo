...
abstract class PHP_TokenWithScope extends PHP_Token
{
    ...
    public function getEndTokenId()
    {
        ...
        while ($this->endTokenId === null && isset($tokens[$i])) {
            if ($tokens[$i] instanceof PHP_Token_OPEN_CURLY || $tokens[$i] instanceof PHP_Token_DOLLAR_OPEN_CURLY_BRACES || $tokens[$i] instanceof PHP_Token_CURLY_OPEN) {
                $block++;
...
abstract class PHP_TokenWithScopeAndVisibility extends PHP_TokenWithScope
{
    ...
    public function getVisibility()
    {
        ...
        for ($i = $this->id - 2; $i > $this->id - 7; $i -= 2) {
            if (isset($tokens[$i]) && ($tokens[$i] instanceof PHP_Token_PRIVATE || $tokens[$i] instanceof PHP_Token_PROTECTED || $tokens[$i] instanceof PHP_Token_PUBLIC)) {
                return strtolower(str_replace('PHP_Token_', '', PHP_Token_Util::getClass($tokens[$i])));
            }
    ...
    public function getKeywords()
    {
        ...
        for ($i = $this->id - 2; $i > $this->id - 7; $i -= 2) {
            ...
            if (isset($tokens[$i]) && ($tokens[$i] instanceof PHP_Token_STATIC || $tokens[$i] instanceof PHP_Token_FINAL || $tokens[$i] instanceof PHP_Token_ABSTRACT)) {
                $keywords[] = strtolower(str_replace('PHP_Token_', '', PHP_Token_Util::getClass($tokens[$i])));
            }
        }
...
abstract class PHP_Token_Includes extends PHP_Token
{
    ...
    private function process()
    {
        ...
        if ($tokens[$this->id + 2] instanceof PHP_Token_CONSTANT_ENCAPSED_STRING) {
            ...
            $this->type = strtolower(str_replace('PHP_Token_', '', PHP_Token_Util::getClass($tokens[$this->id])));
        }
    }
}
