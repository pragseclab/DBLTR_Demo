...
class ExporterTest extends TestCase
{
    ...
    public function exportProvider()
    {
        ...
        return [
            ...
            'export multidimentional array' => [[[1, 2, 3], [3, 4, 5]], <<<EOF
Array &0 (
    0 => Array &1 (
        0 => 1
        1 => 2
...
EOF
            ...
            'export non empty stdclass' => [$obj, <<<EOF
stdClass Object &%x (
    'null' => null
    'boolean' => true
    'integer' => 1
...
EOF
            ...
            'export splObjectStorage' => [$storage, <<<EOF
...
EOF
            ...
            'export stdClass with numeric properties' => [$obj3, <<<EOF
stdClass Object &%x (
    0 => 1
    1 => 2
    2 => 'Test\\r\\n
...
EOF
            ...
            [\chr(0) . \chr(1) . \chr(2) . \chr(3) . \chr(4) . \chr(5), 'Binary String: 0x000102030405'],
            [\implode('', \array_map('chr', \range(0xe, 0x1f))), 'Binary String: 0x0e0f101112131415161718191a1b1c1d1e1f'],
            [\chr(0x0) . \chr(0x9), 'Binary String: 0x0009'],
            ['', "''"],
            'export Exception without trace' => [new \Exception('The exception message', 42), <<<EOF
Exception Object &%x (
...
EOF
            ...
            'export Error without trace' => [new \Error('The exception message', 42), <<<EOF
Error Object &%x (
    'message' => 'The exception message'
    'string' => ''
    'code' => 42
...
EOF
    ...
    public function testExport2()
    {
        ...
        $obj = new \stdClass();
        $obj->foo = 'bar';
        $array = [0 => 0, 'null' => null, 'boolean' => true, 'integer' => 1, 'double' => 1.2, 'string' => '1', 'text' => "this\nis\na\nvery\nvery\nvery\nvery\nvery\nvery\rlong\n\rtext", 'object' => $obj, 'objectagain' => $obj, 'array' => ['foo' => 'bar']];
        $array['self'] =& $array;
        $expected = <<<EOF
Array &%d (
    0 => 0
...
EOF;
    ...
    {
        return [[\implode('', \array_map('chr', \range(0x9, 0xd))), 9], [\implode('', \array_map('chr', \range(0x20, 0x7f))), 96], [\implode('', \array_map('chr', \range(0x80, 0xff))), 128]];
    }
    /**
    ...
    {
        return ['export null' => [[null], 'null'], 'export boolean true' => [[true], 'true'], 'export boolean false' => [[false], 'false'], 'export int 1' => [[1], '1'], 'export float 1.0' => [[1.0], '1.0'], 'export float 1.2' => [[1.2], '1.2'], 'export numeric string' => [['1'], "'1'"], 'export with numeric array key' => [[2 => 1], '1'], 'export with assoc array key' => [['foo' => 'bar'], '\'bar\''], 'export multidimentional array' => [[[1, 2, 3], [3, 4, 5]], 'array(1, 2, 3), array(3, 4, 5)'], 'export object' => [[new \stdClass()], 'stdClass Object ()']];
    }
