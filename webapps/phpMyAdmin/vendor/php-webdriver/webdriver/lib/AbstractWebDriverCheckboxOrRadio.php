...
abstract class AbstractWebDriverCheckboxOrRadio implements WebDriverSelectInterface
{
    ...
    protected function byValue($value, $select = true)
    {
        ...
        if (!$matched) {
            throw new NoSuchElementException(sprintf('Cannot locate %s with value: %s', $this->type, $value));
        }
    }
    /**
    ...
    protected function getRelatedElements($value = null)
    {
        ...
        return $this->element->findElements(WebDriverBy::xpath(sprintf('//form[@id = %1$s]//input[@name = %2$s%3$s' . ' and ((boolean(@form) = true() and @form = %1$s) or boolean(@form) = false())]' . ' | //input[@form = %1$s and @name = %2$s%3$s]', XPathEscaper::escapeQuotes($formId), XPathEscaper::escapeQuotes($this->name), $valueSelector)));
    }
    /**
