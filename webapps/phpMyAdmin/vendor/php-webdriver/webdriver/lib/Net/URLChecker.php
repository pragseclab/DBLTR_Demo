...
class URLChecker
{
    ...
    public function waitUntilAvailable($timeout_in_ms, $url)
    {
        ...
        throw new TimeoutException(sprintf('Timed out waiting for %s to become available after %d ms.', $url, $timeout_in_ms));
    }
    public function waitUntilUnavailable($timeout_in_ms, $url)
    {
        ...
        throw new TimeoutException(sprintf('Timed out waiting for %s to become unavailable after %d ms.', $url, $timeout_in_ms));
    }
    private function getHTTPResponseCode($url)
    {
        ...
        if (!defined('CURLOPT_CONNECTTIMEOUT_MS')) {
            ...
            // default value for CURLOPT_CONNECTTIMEOUT_MS
        }
