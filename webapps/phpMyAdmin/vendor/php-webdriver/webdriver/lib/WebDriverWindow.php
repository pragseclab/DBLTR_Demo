...
class WebDriverWindow
{
    ...
    {
        $position = $this->executor->execute(DriverCommand::GET_WINDOW_POSITION, [':windowHandle' => 'current']);
        return new WebDriverPoint($position['x'], $position['y']);
    }
    /**
    ...
    {
        $size = $this->executor->execute(DriverCommand::GET_WINDOW_SIZE, [':windowHandle' => 'current']);
    ...
    public function maximize()
    {
        if ($this->isW3cCompliant) {
            ...
            $this->executor->execute(DriverCommand::MAXIMIZE_WINDOW, [':windowHandle' => 'current']);
        }
        return $this;
    }
    /**
    ...
    {
        $params = ['width' => $size->getWidth(), 'height' => $size->getHeight(), ':windowHandle' => 'current'];
    ...
    {
        $params = ['x' => $position->getX(), 'y' => $position->getY(), ':windowHandle' => 'current'];
    ...
    public function setScreenOrientation($orientation)
    {
        ...
        if (!in_array($orientation, ['PORTRAIT', 'LANDSCAPE'])) {
            throw new IndexOutOfBoundsException('Orientation must be either PORTRAIT, or LANDSCAPE');
        }
        $this->executor->execute(DriverCommand::SET_SCREEN_ORIENTATION, ['orientation' => $orientation]);
        return $this;
    }
}