...
class RemoteWebElement implements WebDriverElement, WebDriverLocatable
{
    ...
    {
        $this->executor->execute(DriverCommand::CLEAR_ELEMENT, [':id' => $this->id]);
        return $this;
    }
    /**
     * Click this element.
    ...
    {
        $this->executor->execute(DriverCommand::CLICK_ELEMENT, [':id' => $this->id]);
        return $this;
    }
    /**
    ...
    {
        $params = [':name' => $attribute_name, ':id' => $this->id];
    ...
    {
        $params = [':propertyName' => $css_property_name, ':id' => $this->id];
        return $this->executor->execute(DriverCommand::GET_ELEMENT_VALUE_OF_CSS_PROPERTY, $params);
    }
    /**
    ...
    {
        $location = $this->executor->execute(DriverCommand::GET_ELEMENT_LOCATION, [':id' => $this->id]);
    ...
    {
        $location = $this->executor->execute(DriverCommand::GET_ELEMENT_LOCATION_ONCE_SCROLLED_INTO_VIEW, [':id' => $this->id]);
        return new WebDriverPoint($location['x'], $location['y']);
    }
    /**
    ...
    public function getCoordinates()
    {
        ...
        return new WebDriverCoordinates($on_screen, $in_view_port, $on_page, $auxiliary);
    }
    /**
    ...
    {
        $size = $this->executor->execute(DriverCommand::GET_ELEMENT_SIZE, [':id' => $this->id]);
    ...
    public function getTagName()
    {
        ...
        return mb_strtolower($this->executor->execute(DriverCommand::GET_ELEMENT_TAG_NAME, [':id' => $this->id]));
    }
    /**
    ...
    {
        return $this->executor->execute(DriverCommand::GET_ELEMENT_TEXT, [':id' => $this->id]);
    }
    /**
    ...
    {
        return $this->executor->execute(DriverCommand::IS_ELEMENT_DISPLAYED, [':id' => $this->id]);
    }
    /**
    ...
    {
        return $this->executor->execute(DriverCommand::IS_ELEMENT_ENABLED, [':id' => $this->id]);
    }
    /**
    ...
    {
        return $this->executor->execute(DriverCommand::IS_ELEMENT_SELECTED, [':id' => $this->id]);
    }
    /**
        ...
        if ($local_file === null) {
            if ($this->isW3cCompliant) {
                ...
                foreach ($encodedValues as $encodedValue) {
                    $params[] = ['text' => $encodedValue, ':id' => $this->id];
                }
            } else {
                $params[] = ['value' => WebDriverKeys::encode($value), ':id' => $this->id];
            }
        } else {
            if ($this->isW3cCompliant) {
                try {
                ...
                $params[] = ['text' => $fileName, ':id' => $this->id];
            } else {
                $params[] = ['value' => WebDriverKeys::encode($this->upload($local_file)), ':id' => $this->id];
            }
        }
    ...
    public function submit()
    {
        if ($this->isW3cCompliant) {
            $this->executor->execute(DriverCommand::EXECUTE_SCRIPT, [
                ...
                'script' => sprintf('return Object.getPrototypeOf(%1$s).submit.call(%1$s);', $this->getTagName() === 'form' ? 'arguments[0]' : 'arguments[0].form'),
                'args' => [[JsonWireCompat::WEB_DRIVER_ELEMENT_IDENTIFIER => $this->id]],
            ]);
            return $this;
        }
        $this->executor->execute(DriverCommand::SUBMIT_ELEMENT, [':id' => $this->id]);
        return $this;
    }
    /**
    ...
    {
        $screenshot = base64_decode($this->executor->execute(DriverCommand::TAKE_ELEMENT_SCREENSHOT, [':id' => $this->id]));
        if ($save_as !== null) {
    ...
    public function equals(WebDriverElement $other)
    {
        ...
        return $this->executor->execute(DriverCommand::ELEMENT_EQUALS, [':id' => $this->id, ':other' => $other->getID()]);
    }
    /**
    ...
    protected function upload($local_file)
    {
        ...
        $remote_path = $this->executor->execute(DriverCommand::UPLOAD_FILE, ['file' => base64_encode(file_get_contents($temp_zip_path))]);
