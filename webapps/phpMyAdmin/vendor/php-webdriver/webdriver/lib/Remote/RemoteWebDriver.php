...
class RemoteWebDriver implements WebDriver, JavaScriptExecutor, WebDriverHasInputDevices
{
     ...
     */
    protected function __construct(HttpCommandExecutor $commandExecutor, $sessionId, WebDriverCapabilities $capabilities = null, $isW3cCompliant = false)
    {
    ...
    public static function create($selenium_server_url = 'http://localhost:4444/wd/hub', $desired_capabilities = null, $connection_timeout_in_ms = null, $request_timeout_in_ms = null, $http_proxy = null, $http_proxy_port = null, DesiredCapabilities $required_capabilities = null)
    {
        ...
        // W3C
        $parameters = ['capabilities' => ['firstMatch' => [(object) $desired_capabilities->toW3cCompatibleArray()]]];
        ...
        $command = new WebDriverCommand(null, DriverCommand::NEW_SESSION, $parameters);
    ...
    {
        $raw_element = $this->execute(DriverCommand::FIND_ELEMENT, JsonWireCompat::getUsing($by, $this->isW3cCompliant));
    ...
    {
        $raw_elements = $this->execute(DriverCommand::FIND_ELEMENTS, JsonWireCompat::getUsing($by, $this->isW3cCompliant));
        $elements = [];
    ...
    {
        return $this->execute(DriverCommand::GET_CURRENT_WINDOW_HANDLE, []);
    }
    /**
    ...
    {
        $params = ['script' => $script, 'args' => $this->prepareScriptArguments($arguments)];
    ...
    {
        $params = ['script' => $script, 'args' => $this->prepareScriptArguments($arguments)];
        return $this->execute(DriverCommand::EXECUTE_ASYNC_SCRIPT, $params);
    }
    /**
    ...
    {
        $screenshot = base64_decode($this->execute(DriverCommand::SCREENSHOT));
        if ($save_as !== null) {
    ...
    public static function getAllSessions($selenium_server_url = 'http://localhost:4444/wd/hub', $timeout_in_ms = 30000)
    {
        ...
        $command = new WebDriverCommand(null, DriverCommand::GET_ALL_SESSIONS, []);
     ...
     * @return mixed|null
     */
    public function executeCustomCommand($endpointUrl, $method = 'GET', $params = array())
    {
        $command = new CustomWebDriverCommand($this->sessionID, $endpointUrl, $method, $params);
        if ($this->executor) {
            $response = $this->executor->execute($command);
            return $response->getValue();
        }
    ...
    protected function prepareScriptArguments(array $arguments)
    {
        ...
        foreach ($arguments as $key => $value) {
            if ($value instanceof WebDriverElement) {
                $args[$key] = [$this->isW3cCompliant ? JsonWireCompat::WEB_DRIVER_ELEMENT_IDENTIFIER : 'ELEMENT' => $value->getID()];
            } else {
                if (is_array($value)) {
