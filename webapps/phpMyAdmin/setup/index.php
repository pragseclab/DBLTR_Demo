...
if ($page === 'form') {
    ...
    echo $controller->index(['formset' => $_GET['formset'] ?? null]);
    return;
}
if ($page === 'config') {
    $controller = new ConfigController($GLOBALS['ConfigFile'], new Template());
    echo $controller->index(['formset' => $_GET['formset'] ?? null, 'eol' => $_GET['eol'] ?? null]);
    return;
}
if ($page === 'servers') {
    ...
    if (isset($_GET['mode']) && $_GET['mode'] === 'remove' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        $controller->destroy(['id' => $_GET['id'] ?? null]);
        header('Location: index.php' . Url::getCommonRaw());
        return;
    }
    echo $controller->index(['formset' => $_GET['formset'] ?? null, 'mode' => $_GET['mode'] ?? null, 'id' => $_GET['id'] ?? null]);
    return;
}
$controller = new HomeController($GLOBALS['ConfigFile'], new Template());
echo $controller->index(['formset' => $_GET['formset'] ?? null, 'action_done' => $_GET['action_done'] ?? null, 'version_check' => $_GET['version_check'] ?? null]);