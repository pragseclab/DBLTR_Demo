...
if (!empty($languages) || !empty($translations)) {
    ...
    wp_dropdown_languages(array('name' => 'WPLANG', 'id' => 'WPLANG', 'selected' => $locale, 'languages' => $languages, 'translations' => $translations, 'show_available_translations' => current_user_can('install_languages') && wp_can_install_language_pack()));
...
<span>
    ...
    if (in_array($tzstring, timezone_identifiers_list(), true)) {
        ...
        if (!empty($transitions[1])) {
            echo ' ';
            $message = $transitions[1]['isdst'] ? __('Daylight saving time begins on: %s.') : __('Standard time begins on: %s.');
