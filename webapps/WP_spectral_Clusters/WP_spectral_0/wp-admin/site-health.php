...
$wrapper_classes = array('health-check-tabs-wrapper', 'hide-if-no-js', 'tab-count-' . count($tabs));
$title = sprintf(
...
if (isset($_GET['https_updated'])) {
    if ($_GET['https_updated']) {
        ?>
			<div id="message" class="notice notice-success is-dismissible"><p><?php 
        _e('Site URLs switched to HTTPS.');
        ?></p></div>
			<?php 
    } else {
        ?>
			<div id="message" class="notice notice-error is-dismissible"><p><?php 
        _e('Site URLs could not be switched to HTTPS.');
        ?></p></div>
			<?php 
    }
}
...
foreach ($tabs_slice as $slug => $label) {
    printf('<a href="%s" class="health-check-tab %s">%s</a>', esc_url(add_query_arg(array('tab' => $slug), admin_url('site-health.php'))), isset($_GET['tab']) && $_GET['tab'] === $slug ? 'active' : '', esc_html($label));
}
?>
