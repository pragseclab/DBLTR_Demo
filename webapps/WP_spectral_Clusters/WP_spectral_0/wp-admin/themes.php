...
get_current_screen()->set_help_sidebar('<p><strong>' . __('For more information:') . '</strong></p>' . '<p>' . __('<a href="https://wordpress.org/support/article/using-themes/">Documentation on Using Themes</a>') . '</p>' . $help_sidebar_autoupdates . '<p>' . __('<a href="https://wordpress.org/support/">Support</a>') . '</p>');
if (current_user_can('switch_themes')) {
...
echo !empty($_GET['search']) ? __('&hellip;') : count($themes);
?></span>
	</h1>
...
if (!validate_current_theme() || isset($_GET['broken'])) {
    ...
    _e('Theme will be auto-updated.');
    ?></p></div>
	<?php 
} elseif (isset($_GET['disabled-auto-update'])) {
    ?>
	<div id="message8" class="updated notice is-dismissible"><p><?php 
    _e('Theme will no longer be auto-updated.');
    ?></p></div>
	<?php 
}
...
<table>
    ...
    foreach ($broken_themes as $broken_theme) {
		...
		<tr>
			<td><?php 
        echo $broken_theme->get('Name') ? $broken_theme->display('Name') : esc_html($broken_theme->get_stylesheet());
        ?></td>
			<td><?php 
        ...
        if ($can_resume) {
            if ('theme_paused' === $broken_theme->errors()->get_error_code()) {
                ...
                ?>
					<td><a href="<?php 
                echo esc_url($resume_url);
                ?>" class="button resume-theme"><?php 
                _e('Resume');
                ?></a></td>
					<?php 
            } else {
        ...
        if ($can_install && 'theme_no_parent' === $broken_theme->errors()->get_error_code()) {
            ...
            if (!is_wp_error($parent_theme)) {
                ...
                ?>" class="button install-theme"><?php 
