...
switch ($wp_list_table->current_action()) {
    ...
    case 'promote':
        ...
        $editable_roles['none'] = array('name' => __('&mdash; No role for this site &mdash;'));
    ...
    case 'resetpassword':
        ...
        $redirect = add_query_arg(array('reset_count' => $reset_count, 'update' => 'resetpassword'), $redirect);
        wp_redirect($redirect);
        exit;
    case 'delete':
        if (is_multisite()) {
...
<ul>
    ...
    default:
        ...
        if ($wp_list_table->current_action() && !empty($_REQUEST['users'])) {
            ...
            $sendback = apply_filters("handle_bulk_actions-{$screen}", $sendback, $wp_list_table->current_action(), $userids);
            ...
            switch ($_GET['update']) {
                ...
                case 'add':
                    ...
                    if ($user_id && current_user_can('edit_user', $user_id)) {
                        $message .= sprintf(' <a href="%s">%s</a>', esc_url(add_query_arg('wp_http_referer', urlencode(wp_unslash($_SERVER['REQUEST_URI'])), self_admin_url('user-edit.php?user_id=' . $user_id))), __('Edit user'));
                    }
