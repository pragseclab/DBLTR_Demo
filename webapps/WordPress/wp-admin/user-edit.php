...
<div id="message" class="updated notice is-dismissible">
	...
	<?php 
            ...
            if ($wp_http_referer && false === strpos($wp_http_referer, 'user-new.php') && !IS_PROFILE_PAGE) {
                ?>
	<p><a href="<?php 
                echo esc_url(wp_validate_redirect(esc_url_raw($wp_http_referer), self_admin_url('users.php')));
                ?>"><?php 
                _e('&larr; Go to Users');
                ?></a></p>
	<?php 
...
<table class="form-table" role="presentation">
	...
	<tr class="user-rich-editing-wrap">
        ...
        ?>
		<?php 
        $show_syntax_highlighting_preference = user_can($profileuser, 'edit_theme_options') || user_can($profileuser, 'edit_plugins') || user_can($profileuser, 'edit_themes');
        ?>
...
<tr id="password" class="user-pass1-wrap">
	...
	<td>
		...
		<button type="button" class="button wp-generate-pw hide-if-no-js" aria-expanded="false"><?php 
            _e('Set New Password');
            ?></button>
...
<tr class="user-pass2-wrap hide-if-js">
	...
	<input name="pass2" type="password" id="pass2" class="regular-text" value="" autocomplete="off" aria-describedby="pass2-desc" />
            ...
            if (IS_PROFILE_PAGE) {
                ?>
				<p class="description" id="pass2-desc"><?php 
			...
			<?php 
                ...
                ?>
				<p class="description" id="pass2-desc"><?php 
                _e('Type the new password again.');
                ?></p>
			<?php 
            }
            ?>
...
<?php 
	...
	<div class="application-passwords hide-if-no-js" id="application-passwords-section">
		<h2><?php 
            _e('Application Passwords');
            ?></h2>
		<p><?php 
            _e('Application passwords allow authentication via non-interactive systems, such as XML-RPC or the REST API, without providing your actual password. Application passwords can be easily revoked. They cannot be used for traditional logins to your website.');
            ?></p>
			<?php 
            if (is_multisite()) {
                ...
                if ($blogs_count > 1) {
                    ...
                    printf(
                        ...
                        _n('Application passwords grant access to <a href="%1$s">the %2$s blog in this installation that you have permissions on</a>.', 'Application passwords grant access to <a href="%1$s">all %2$s blogs in this installation that you have permissions on</a>.', $blogs_count),
			...
			<div class="create-application-password form-wrap">
				<div class="form-field">
					<label for="new_application_password_name"><?php 
                _e('New Application Password Name');
                ?></label>
					<input type="text" size="30" id="new_application_password_name" name="new_application_password_name" placeholder="<?php 
                esc_attr_e('WordPress App on My Phone');
                ?>" class="input" aria-required="true" aria-describedby="new_application_password_name_desc" />
					<p class="description" id="new_application_password_name_desc"><?php 
                _e('Required to create an Application Password, but not to update the user.');
                ?></p>
				</div>
				...
				<?php 
                submit_button(__('Add New Application Password'), 'secondary', 'do_new_application_password');
                ?>
			</div>
		<?php 
            } else {
