...
class WP_MS_Themes_List_Table extends WP_List_Table
{
    ...
    public function prepare_items()
    {
        ...
        foreach ((array) $themes['all'] as $key => $theme) {
            ...
            $theme_data = array('update_supported' => isset($theme->update_supported) ? $theme->update_supported : true);
             ...
             */
            $filter_payload = array('theme' => $key, 'new_version' => '', 'url' => '', 'package' => '', 'requires' => '', 'requires_php' => '');
        ...
        wp_localize_script('updates', '_wpUpdatesItemCounts', array('themes' => $js_themes, 'totals' => wp_get_update_data()));
        if ($orderby) {
    ...
    {
        $columns = array('cb' => '<input type="checkbox" />', 'name' => __('Theme'), 'description' => __('Description'));
        if ($this->show_autoupdates) {
    ...
    protected function get_views()
    {
        ...
        foreach ($totals as $type => $count) {
            ...
            if ('search' !== $type) {
                $status_links[$type] = sprintf("<a href='%s'%s>%s</a>", esc_url(add_query_arg('theme_status', $type, $url)), $type === $status ? ' class="current" aria-current="page"' : '', sprintf($text, number_format_i18n($count)));
            }
        }
        return $status_links;
    }
    ...
    public function column_autoupdates($theme)
    {
        ...
        $query_args = array('action' => "{$action}-auto-update", 'theme' => $stylesheet, 'paged' => $page, 'theme_status' => $status);
