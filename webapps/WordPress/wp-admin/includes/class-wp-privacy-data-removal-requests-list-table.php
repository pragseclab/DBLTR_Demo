...
class WP_Privacy_Data_Removal_Requests_List_Table extends WP_Privacy_Requests_Table
{
    ...
    public function column_email($item)
    {
        ...
        if ('request-completed' !== $status) {
            ...
            $complete_request_markup .= sprintf('<a href="%s" class="complete-request" aria-label="%s">%s</a>', esc_url(wp_nonce_url(add_query_arg(array('action' => 'complete', 'request_id' => array($request_id)), admin_url('erase-personal-data.php')), 'bulk-privacy_requests')), esc_attr(sprintf(
    ...
    public function column_next_steps($item)
    {
        ...
        switch ($status) {
            ...
            case 'request-confirmed':
                ...
                ?> <button type="button" class="button-link remove-personal-data-handle"><?php 
                _e('Retry');
                ?></button></span>
				<?php 
                echo '</div>';
            ...
            case 'request-completed':
                echo '<a href="' . esc_url(wp_nonce_url(add_query_arg(array('action' => 'delete', 'request_id' => array($item->ID)), admin_url('erase-personal-data.php')), 'bulk-privacy_requests')) . '">' . esc_html__('Remove request') . '</a>';
                break;
        }
    }
}