...
class WP_REST_Block_Types_Controller extends WP_REST_Controller
{
    ...
    public function prepare_item_for_response($block_type, $request)
    {
        ...
        $extra_fields = array('api_version', 'name', 'title', 'description', 'icon', 'category', 'keywords', 'parent', 'provides_context', 'uses_context', 'supports', 'styles', 'textdomain', 'example', 'editor_script', 'script', 'editor_style', 'style', 'variations');
    ...
    protected function prepare_links($block_type)
    {
        ...
        $links = array('collection' => array('href' => rest_url(sprintf('%s/%s', $this->namespace, $this->rest_base))), 'self' => array('href' => rest_url(sprintf('%s/%s/%s', $this->namespace, $this->rest_base, $block_type->name))), 'up' => array('href' => rest_url(sprintf('%s/%s/%s', $this->namespace, $this->rest_base, $namespace))));
        if ($block_type->is_dynamic()) {
            $links['https://api.w.org/render-block'] = array('href' => add_query_arg('context', 'edit', rest_url(sprintf('%s/%s/%s', 'wp/v2', 'block-renderer', $block_type->name))));
        }
        return $links;
    }
    /**
