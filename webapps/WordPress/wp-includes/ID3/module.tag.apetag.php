...
class getid3_apetag extends getid3_handler
{
            ...
            switch (strtolower($item_key)) {
                ...
                case 'replaygain_track_gain':
                    if (preg_match('#^([\\-\\+][0-9\\.,]{8})( dB)?$#', $thisfile_ape_items_current['data'][0], $matches)) {
                        $thisfile_replaygain['track']['adjustment'] = (float) str_replace(',', '.', $matches[1]);
                ...
                case 'replaygain_track_peak':
                    if (preg_match('#^([0-9\\.,]{8})$#', $thisfile_ape_items_current['data'][0], $matches)) {
                        $thisfile_replaygain['track']['peak'] = (float) str_replace(',', '.', $matches[1]);
                        // float casting will see "0,95" as zero!
                        $thisfile_replaygain['track']['originator'] = 'unspecified';
                ...
                case 'replaygain_album_gain':
                    if (preg_match('#^([\\-\\+][0-9\\.,]{8})( dB)?$#', $thisfile_ape_items_current['data'][0], $matches)) {
                        $thisfile_replaygain['album']['adjustment'] = (float) str_replace(',', '.', $matches[1]);
                        // float casting will see "0,95" as zero!
                ...
                case 'replaygain_album_peak':
                    if (preg_match('#^([0-9\\.,]{8})$#', $thisfile_ape_items_current['data'][0], $matches)) {
                        $thisfile_replaygain['album']['peak'] = (float) str_replace(',', '.', $matches[1]);
                        // float casting will see "0,95" as zero!
                        $thisfile_replaygain['album']['originator'] = 'unspecified';
    ...
    public function parseAPEtagFlags($rawflagint)
    {
        ...
        $flags['item_contents_raw'] = ($rawflagint & 0x6) >> 1;
        $flags['read_only'] = (bool) ($rawflagint & 0x1);
