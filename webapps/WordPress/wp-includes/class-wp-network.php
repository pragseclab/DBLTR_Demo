...
class WP_Network
{
    ...
    private function get_main_site_id()
    {
        ...
        if (defined('DOMAIN_CURRENT_SITE') && defined('PATH_CURRENT_SITE') && DOMAIN_CURRENT_SITE === $this->domain && PATH_CURRENT_SITE === $this->path || defined('SITE_ID_CURRENT_SITE') && SITE_ID_CURRENT_SITE == $this->id) {
        ...
        if ($site->domain === $this->domain && $site->path === $this->path) {
            ...
            if (false === $main_site_id) {
                $_sites = get_sites(array('fields' => 'ids', 'number' => 1, 'domain' => $this->domain, 'path' => $this->path, 'network_id' => $this->id));
