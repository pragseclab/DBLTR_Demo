    ...
    while (have_posts()) {
        ...
        if (wp_attachment_is_image()) {
            ...
            printf(
                ...
                sprintf('<a href="%1$s" title="%2$s">%3$s &times; %4$s</a>', esc_url(wp_get_attachment_url()), esc_attr(__('Link to full-size image', 'twentyten')), $metadata['width'], $metadata['height'])
            );
        }
        ?>
							<?php 
		...
		<?php 
            ...
            the_title_attribute();
            ?>" rel="attachment"><?php 
            echo esc_html(wp_basename(get_permalink()));
            ?></a>
	<?php 
        }
        ?>
