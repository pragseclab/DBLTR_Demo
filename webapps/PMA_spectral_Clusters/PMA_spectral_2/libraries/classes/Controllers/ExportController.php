...
final class ExportController extends AbstractController
{
    ...
    public function index() : void
    {
        ...
        $export_plugin = Plugins::getPlugin('export', $what, 'libraries/classes/Plugins/Export/', ['export_type' => $export_type, 'single_table' => isset($single_table)]);
        // Check export type
        ...
        if ($_POST['output_format'] === 'astext') {
            ...
            $asfile = true;
            if (isset($_POST['as_separate_files']) && !empty($_POST['as_separate_files'])) {
                if (isset($_POST['compression']) && !empty($_POST['compression']) && $_POST['compression'] === 'zip') {
            ...
            if ($quick_export && !empty($_POST['quick_export_onserver']) || !$quick_export && !empty($_POST['onserver'])) {
                if ($quick_export) {
        ...
        if ($export_type === 'server') {
            ...
            $err_url = Url::getFromRoute('/table/export', ['db' => $db, 'table' => $table]);
        ...
        $output_charset_conversion = $asfile && Encoding::isSupported() && isset($charset) && $charset !== 'utf-8';
        ...
        if ($asfile) {
            ...
            [$filename, $mime_type] = $this->export->getFilenameAndMimetype($export_type, $remember_template, $export_plugin, $compression, $filename_template);
        } else {
            $mime_type = '';
        }
        ...
        if ($save_on_server) {
            ...
            if ($asfile) {
                ...
                if ($export_type === 'database') {
                    ...
                    if ($num_tables === 0) {
                        $message = Message::error(__('No tables found in database.'));
                ...
                [$html, $back_button, $refreshButton] = $this->export->getHtmlForDisplayedExportHeader($export_type, $db, $table);
                echo $html;
                unset($html);
            }
        }
        try {
            ...
            if ($export_type === 'server') {
                ...
                $this->export->exportServer($db_select, $whatStrucOrData, $export_plugin, $crlf, $err_url, $export_type, $do_relation, $do_comments, $do_mime, $do_dates, $aliases, $separate_files);
                ...
                if (isset($lock_tables)) {
                    ...
                    try {
                        $this->export->exportDatabase($db, $tables, $whatStrucOrData, $table_structure, $table_data, $export_plugin, $crlf, $err_url, $export_type, $do_relation, $do_comments, $do_mime, $do_dates, $aliases, $separate_files);
                    } finally {
                ...
                Export::exportRaw($whatStrucOrData, $export_plugin, $crlf, $err_url, $sql_query, $export_type);
            } else {
                ...
                if (isset($lock_tables)) {
                    try {
                        ...
                        $this->export->exportTable($db, $table, $whatStrucOrData, $export_plugin, $crlf, $err_url, $export_type, $do_relation, $do_comments, $do_mime, $do_dates, $allrows, $limit_to, $limit_from, $sql_query, $aliases);
                    } finally {
                        $this->export->unlockTables();
                    }
                } else {
        ...
        if ($compression) {
            if (!empty($separate_files)) {
                $dump_buffer = $this->export->compress($dump_buffer_objects, $compression, $filename);
            } else {
        ...
        if ($save_on_server) {
            $message = $this->export->closeFile($file_handle, $dump_buffer, $save_filename);
            $this->export->showPage($export_type);
            return;
        }
        echo $dump_buffer;
