...
class Menu
{
    ...
    {
        return substr(md5($this->getMenu() . $this->getBreadcrumbs()), 0, 8);
    }
    /**
    ...
    private function getMenu() : string
    {
        ...
        return $this->template->render('top_menu', ['tabs' => $tabs, 'url_params' => $url_params]);
    }
    /**
    ...
    private function getAllowedTabs($level)
    {
        ...
        if ($cfgRelation['menuswork']) {
            ...
            $sql_query = 'SELECT `tab` FROM ' . $groupTable . " WHERE `allowed` = 'N'" . " AND `tab` LIKE '" . $level . "%'" . ' AND `usergroup` = (SELECT usergroup FROM ' . $userTable . " WHERE `username` = '" . $dbi->escapeString($GLOBALS['cfg']['Server']['user']) . "')";
    ...
    private function getBreadcrumbs() : string
    {
        ...
        $server['name'] = !empty($cfg['Server']['verbose']) ? $cfg['Server']['verbose'] : $cfg['Server']['host'];
        $server['name'] .= empty($cfg['Server']['port']) ? '' : ':' . $cfg['Server']['port'];
    ...
    private function getTableTabs()
    {
        ...
        $tbl_is_view = $dbi->getTable($this->db, $this->table)->isView();
        $updatable_view = false;
        if ($tbl_is_view) {
            $updatable_view = $dbi->getTable($this->db, $this->table)->isUpdatableView();
        }
        ...
        if (($is_superuser || $isCreateOrGrantUser) && !$isSystemSchema) {
        ...
        if (!$isSystemSchema && Util::currentUserHasPrivilege('TRIGGER', $this->db, $this->table) && !$tbl_is_view) {
    ...
    private function getServerTabs()
    {
        ...
        if (SessionCache::has('binary_logs')) {
            ...
            $binary_logs = $dbi->fetchResult('SHOW MASTER LOGS', 'Log_name', null, DatabaseInterface::CONNECT_USER, DatabaseInterface::QUERY_STORE);
        ...
        $tabs['status']['active'] = in_array($route, ['/server/status', '/server/status/advisor', '/server/status/monitor', '/server/status/processes', '/server/status/queries', '/server/status/variables']);
        if ($is_superuser || $isCreateOrGrantUser) {
        ...
        $tabs['settings']['active'] = in_array($route, ['/preferences/export', '/preferences/features', '/preferences/import', '/preferences/main-panel', '/preferences/manage', '/preferences/navigation', '/preferences/sql', '/preferences/two-factor']);
