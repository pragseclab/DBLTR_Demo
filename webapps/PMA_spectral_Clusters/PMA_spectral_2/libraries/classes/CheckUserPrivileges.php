...
class CheckUserPrivileges
{
    ...
    public function getItemsFromShowGrantsRow(string $row) : array
    {
        ...
        $show_grants_dbname = mb_substr($row, $db_name_offset, $tblname_start_offset - $db_name_offset);
        $show_grants_dbname = Util::unQuote($show_grants_dbname, '`');
        $show_grants_str = mb_substr($row, 6, mb_strpos($row, ' ON ') - 6);
        $show_grants_tblname = mb_substr($row, $tblname_start_offset + 1, $tblname_end_offset - $tblname_start_offset - 1);
        $show_grants_tblname = Util::unQuote($show_grants_tblname, '`');
        return [$show_grants_str, $show_grants_dbname, $show_grants_tblname];
    }
    /**
     ...
     */
    public function checkRequiredPrivilegesForAdjust(string $show_grants_str, string $show_grants_dbname, string $show_grants_tblname) : void
    {
        // '... ALL PRIVILEGES ON *.* ...' OR '... ALL PRIVILEGES ON `mysql`.* ..'
        // OR
        // SELECT, INSERT, UPDATE, DELETE .... ON *.* OR `mysql`.*
        if ($show_grants_str !== 'ALL' && $show_grants_str !== 'ALL PRIVILEGES' && mb_strpos($show_grants_str, 'SELECT, INSERT, UPDATE, DELETE') === false) {
            return;
        }
        if ($show_grants_dbname === '*' && $show_grants_tblname === '*') {
            $GLOBALS['col_priv'] = true;
            $GLOBALS['db_priv'] = true;
            $GLOBALS['proc_priv'] = true;
            $GLOBALS['table_priv'] = true;
            if ($show_grants_str === 'ALL PRIVILEGES' || $show_grants_str === 'ALL') {
    ...
    private function analyseShowGrant() : void
    {
        ...
        while ($row = $this->dbi->fetchRow($rs_usr)) {
            [$show_grants_str, $show_grants_dbname, $show_grants_tblname] = $this->getItemsFromShowGrantsRow($row[0]);
            ...
            $this->checkRequiredPrivilegesForAdjust($show_grants_str, $show_grants_dbname, $show_grants_tblname);
            /**
            ...
            // does this db exist?
            if ((!preg_match('/' . $re0 . '%|_/', $show_grants_dbname) || preg_match('/\\\\%|\\\\_/', $show_grants_dbname)) && ($this->dbi->tryQuery('USE ' . preg_replace('/' . $re1 . '(%|_)/', '\\1\\3', $dbname_to_test)) || mb_substr((string) $this->dbi->getError(), 1, 4) == 1044)) {
                continue;
            }
            /**
