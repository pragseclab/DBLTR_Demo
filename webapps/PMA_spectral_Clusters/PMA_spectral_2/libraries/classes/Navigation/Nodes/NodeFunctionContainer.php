...
class NodeFunctionContainer extends NodeDatabaseChildContainer
{
    ...
    public function __construct()
    {
        ...
        $this->links = ['text' => Url::getFromRoute('/database/routines', ['server' => $GLOBALS['server'], 'type' => 'FUNCTION']) . '&amp;db=%1$s', 'icon' => Url::getFromRoute('/database/routines', ['server' => $GLOBALS['server'], 'type' => 'FUNCTION']) . '&amp;db=%1$s'];
        ...
        $new->links = ['text' => Url::getFromRoute('/database/routines', ['server' => $GLOBALS['server'], 'item_type' => 'FUNCTION', 'add_item' => 1]) . '&amp;db=%2$s', 'icon' => Url::getFromRoute('/database/routines', ['server' => $GLOBALS['server'], 'item_type' => 'FUNCTION', 'add_item' => 1]) . '&amp;db=%2$s'];
        $this->addChild($new);
    }
}