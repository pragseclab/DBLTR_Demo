...
abstract class NodeDatabaseChild extends Node
{
    ...
    public function getHtmlForControlButtons() : string
    {
        ...
        if ($cfgRelation['navwork']) {
            ...
            $item = $this->realName;
            $params = ['hideNavItem' => true, 'itemType' => $this->getItemType(), 'itemName' => $item, 'dbName' => $db];
            $ret = '<span class="navItemControls">' . '<a href="' . Url::getFromRoute('/navigation') . '" data-post="' . Url::getCommon($params, '') . '"' . ' class="hideNavItem ajax">' . Generator::getImage('hide', __('Hide')) . '</a></span>';
        }
        return $ret;
    }
}