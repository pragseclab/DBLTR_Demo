...
class Charsets
{
    ...
    private static function loadCharsets(DatabaseInterface $dbi, bool $disableIs) : void
    {
        ...
        if ($disableIs) {
            ...
            $sql = 'SELECT `CHARACTER_SET_NAME` AS `Charset`,' . ' `DEFAULT_COLLATE_NAME` AS `Default collation`,' . ' `DESCRIPTION` AS `Description`,' . ' `MAXLEN` AS `Maxlen`' . ' FROM `information_schema`.`CHARACTER_SETS`';
        }
        $res = $dbi->query($sql);
        self::$charsets = [];
    ...
    private static function loadCollations(DatabaseInterface $dbi, bool $disableIs) : void
    {
        ...
        if ($disableIs) {
            ...
            $sql = 'SELECT `COLLATION_NAME` AS `Collation`,' . ' `CHARACTER_SET_NAME` AS `Charset`,' . ' `ID` AS `Id`,' . ' `IS_DEFAULT` AS `Default`,' . ' `IS_COMPILED` AS `Compiled`,' . ' `SORTLEN` AS `Sortlen`' . ' FROM `information_schema`.`COLLATIONS`';
        }
        $res = $dbi->query($sql);
        self::$collations = [];
        while ($row = $dbi->fetchAssoc($res)) {
