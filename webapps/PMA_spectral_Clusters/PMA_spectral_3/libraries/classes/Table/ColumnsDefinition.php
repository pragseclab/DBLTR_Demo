...
final class ColumnsDefinition
{
     ...
     */
    public static function displayForm(Transformations $transformations, Relation $relation, DatabaseInterface $dbi, string $action, $num_fields = 0, $regenerate = null, $selected = null, $fields_meta = null, $field_fulltext = null) : array
    {
        global $db, $table, $cfg, $col_priv, $is_reload_priv, $mime_map;
        Util::checkParameters(['server', 'db', 'table', 'action', 'num_fields']);
        ...
        if ($action == Url::getFromRoute('/table/create')) {
            ...
            if ($action == Url::getFromRoute('/table/add-field')) {
                $form_params = array_merge($form_params, ['field_where' => Util::getValueByKey($_POST, 'field_where')]);
        ...
        $form_params = array_merge($form_params, ['orig_field_where' => Util::getValueByKey($_POST, 'field_where'), 'orig_after_field' => Util::getValueByKey($_POST, 'after_field')]);
        ...
        foreach ($mime_types as $mime_type) {
            ...
            foreach ($available_mime[$mime_type] as $mimekey => $transform) {
                $available_mime[$mime_type . '_file_quoted'][$mimekey] = preg_quote($available_mime[$mime_type . '_file'][$mimekey], '@');
            }
        }
        ...
        for ($columnNumber = 0; $columnNumber < $num_fields; $columnNumber++) {
            ...
            if (!empty($regenerate)) {
                ...
                $mime_map[$columnMeta['Field']] = array_merge($mime_map[$columnMeta['Field']] ?? [], ['mimetype' => Util::getValueByKey($_POST, "field_mimetype.{$columnNumber}"), 'transformation' => Util::getValueByKey($_POST, "field_transformation.{$columnNumber}"), 'transformation_options' => Util::getValueByKey($_POST, "field_transformation_options.{$columnNumber}")]);
            ...
            if (isset($columnMeta['Field'], $form_params['table']) && $dbi->getVersion() < 50606) {
                $columnMeta['column_status'] = $relation->checkChildForeignReferences($form_params['db'], $form_params['table'], $columnMeta['Field'], $foreigners, $child_references);
            }
            // some types, for example longtext, are reported as
            // "longtext character set latin7" when their charset and / or collation
            // differs from the ones of the corresponding database.
            // rtrim the type, for cases like "float unsigned"
            $type = rtrim(preg_replace('/[\\s]character set[\\s][\\S]+/', '', $type));
            /**
