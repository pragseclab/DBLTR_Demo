...
class Form
{
    ...
    {
        $key = ltrim(mb_substr($option_name, mb_strrpos($option_name, '/')), '/');
        return isset($this->_fieldsTypes[$key]) ? $this->_fieldsTypes[$key] : null;
    }
    /**
    ...
    protected function readFormPaths($form)
    {
        ...
        foreach ($paths as $path) {
            $key = ltrim(mb_substr($path, mb_strrpos($path, '/')), '/');
