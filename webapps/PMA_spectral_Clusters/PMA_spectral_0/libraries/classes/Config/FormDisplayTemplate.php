...
class FormDisplayTemplate
{
     ...
     */
    public function displayFormTop($action = null, $method = 'post', $hiddenFields = null) : string
    {
        ...
        return $this->template->render('config/form_display/form_top', ['method' => $method, 'action' => $action, 'has_check_page_refresh' => $hasCheckPageRefresh, 'hidden_fields' => (array) $hiddenFields]);
    }
    /**
     ...
     */
    public function displayFieldsetTop($title = '', $description = '', $errors = null, array $attributes = array()) : string
    {
        $this->group = 0;
        $attributes = array_merge(['class' => 'optbox'], $attributes);
        return $this->template->render('config/form_display/fieldset_top', ['attributes' => $attributes, 'title' => $title, 'description' => $description, 'errors' => $errors]);
    }
    /**
     ...
     */
    public function displayInput($path, $name, $type, $value, $description = '', $valueIsDefault = true, $opts = null) : string
    {
        static $icons;
        ...
        if ($icons === null) {
            ...
            $iconInit = ['edit' => ['b_edit', ''], 'help' => ['b_help', __('Documentation')], 'reload' => ['s_reload', ''], 'tblops' => ['b_tblops', '']];
            if ($isSetupScript) {
                ...
                foreach ($iconInit as $k => $v) {
                    ...
                    $icons[$k] = sprintf('<img alt="%s" src="%s"%s>', $v[1], '../themes/pmahomme/img/' . $v[0] . '.png', $title);
                }
            } else {
                // In this case we just use getImage() because it's available
                foreach ($iconInit as $k => $v) {
                    $icons[$k] = Generator::getImage($v[0], $v[1]);
                }
            }
        }
        ...
        $nameId = 'name="' . htmlspecialchars($path) . '" id="' . htmlspecialchars($path) . '"';
        $fieldClass = $type === 'checkbox' ? 'checkbox' : '';
        if (!$valueIsDefault) {
            $fieldClass .= ($fieldClass == '' ? '' : ' ') . ($hasErrors ? 'custom field-error' : 'custom');
        }
        ...
        $htmlOutput .= '<th>';
        $htmlOutput .= '<label for="' . htmlspecialchars($path) . '">' . htmlspecialchars_decode($name) . '</label>';
        if (!empty($opts['doc'])) {
        ...
        switch ($type) {
            case 'text':
                $htmlOutput .= '<input type="text" class="w-75" ' . $nameId . $fieldClass . ' value="' . htmlspecialchars($value) . '">';
                break;
            case 'password':
                $htmlOutput .= '<input type="password" class="w-75" ' . $nameId . $fieldClass . ' value="' . htmlspecialchars($value) . '">';
                break;
            case 'short_text':
                ...
                if (is_string($value)) {
                    $htmlOutput .= '<input type="text" size="25" ' . $nameId . $fieldClass . ' value="' . htmlspecialchars($value) . '">';
                }
                break;
            case 'number_text':
                $htmlOutput .= '<input type="number" ' . $nameId . $fieldClass . ' value="' . htmlspecialchars((string) $value) . '">';
                break;
            case 'checkbox':
                $htmlOutput .= '<span' . $fieldClass . '><input type="checkbox" ' . $nameId . ($value ? ' checked="checked"' : '') . '></span>';
                break;
            case 'select':
                ...
                foreach ($opts['values'] as $optValueKey => $optValue) {
                    ...
                    if (is_bool($optValue)) {
                        $optValue = mb_strtolower($optValue ? __('Yes') : __('No'));
                    }
                    // escape if necessary
                    if ($escape) {
            ...
            case 'list':
                ...
                $htmlOutput .= '<textarea cols="35" rows="5" ' . $nameId . $fieldClass . '>' . htmlspecialchars(implode("\n", $val)) . '</textarea>';
                break;
        }
        ...
        if (isset($opts['setvalue']) && $opts['setvalue']) {
            $htmlOutput .= '<a class="set-value hide" href="#' . htmlspecialchars($path . '=' . $opts['setvalue']) . '" title="' . sprintf(__('Set value: %s'), htmlspecialchars($opts['setvalue'])) . '">' . $icons['edit'] . '</a>';
        }
        if (isset($opts['show_restore_default']) && $opts['show_restore_default']) {
            $htmlOutput .= '<a class="restore-default hide" href="#' . $path . '" title="' . __('Restore default value') . '">' . $icons['reload'] . '</a>';
        }
    ...
    {
        return $this->template->render('config/form_display/fieldset_bottom', ['show_buttons' => $showButtons, 'is_setup' => $this->config->get('is_setup')]);
    }
    /**
     * Closes form tabs
     */
    ...
    {
        echo('<html><head>    <meta charset="utf-8">    <meta http-equiv="X-UA-Compatible" content="IE=edge">    <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no">    <title>Error, Target Function Has Been Removed</title>    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css" integrity="sha384-BVYiiSIFeK1dGmJRAkycuHAHRg32OmUcww7on3RYdg4Va+PmSTsz/K68vbdEjh4u" crossorigin="anonymous">    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap-theme.min.css" integrity="sha384-rHyoN1iRsVXV4nD0JutlnGaslCJuC7uwjduW9SVrLvRYooPp2bWYgmgJQIXwl/Sp" crossorigin="anonymous">    <style>        * {            font-family: tahoma;        }        div.container .panel {            position: relative !important;        }        div.container {            width: 50% !important;            height: 50% !important;            overflow: auto !important;            margin: auto !important;            position: absolute !important;            top: 0 !important;            left: 0 !important;            bottom: 0 !important;            right: 0 !important;        }    </style></head><body>    <div class="container">        <div class="panel panel-danger center">            <div class="panel-heading" style="text-align: left;"> Error </div>            <div class="panel-body">                <p class="text-center">                  This function has been removed ("displayErrors") from ("/home/jovyan/work/WebApps/PMA_spectral_Clusters/PMA_spectral_0/libraries/classes/Config/FormDisplayTemplate.php at line 367")                </p>            </div>        </div>    </div></body></html>');
        error_log('Removed function called displayErrors:367@/home/jovyan/work/WebApps/PMA_spectral_Clusters/PMA_spectral_0/libraries/classes/Config/FormDisplayTemplate.php');
        die();
    }
}