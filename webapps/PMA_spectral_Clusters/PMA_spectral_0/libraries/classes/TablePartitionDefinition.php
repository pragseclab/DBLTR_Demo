...
class TablePartitionDefinition
{
    ...
    {
        $partitionParams = ['partition_by' => null, 'partition_expr' => null, 'subpartition_by' => null, 'subpartition_expr' => null];
    ...
    protected static function extractPartitions(array $partitionDetails) : array
    {
        ...
        for ($i = 0; $i < $partitionCount; $i++) {
            if (!isset($partitions[$i])) {
                ...
                $partitions[$i] = ['name' => 'p' . $i, 'value_type' => '', 'value' => '', 'engine' => '', 'comment' => '', 'data_directory' => '', 'index_directory' => '', 'max_rows' => '', 'min_rows' => '', 'tablespace' => '', 'node_group' => ''];
            }
            ...
            for ($j = 0; $j < $subpartitionCount; $j++) {
                if (!isset($subpartitions[$j])) {
                    ...
                    $subpartitions[$j] = ['name' => $partition['name'] . '_s' . $j, 'engine' => '', 'comment' => '', 'data_directory' => '', 'index_directory' => '', 'max_rows' => '', 'min_rows' => '', 'tablespace' => '', 'node_group' => ''];
                }
                $subpartitions[$j]['prefix'] = 'partitions[' . $i . ']' . '[subpartitions][' . $j . ']';
            }
        }
