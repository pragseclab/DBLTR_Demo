...
final class DbTableExists
{
    ...
    private static function checkTable() : void
    {
        ...
        if (strlen($table) > 0) {
            ...
            $_result = $dbi->tryQuery('SHOW TABLES LIKE \'' . $dbi->escapeString($table) . '\';', DatabaseInterface::CONNECT_USER, DatabaseInterface::QUERY_STORE);
        ...
        if (strlen($table) > 0) {
             ...
             */
            $_result = $dbi->tryQuery('SELECT COUNT(*) FROM ' . Util::backquote($table) . ';', DatabaseInterface::CONNECT_USER, DatabaseInterface::QUERY_STORE);
