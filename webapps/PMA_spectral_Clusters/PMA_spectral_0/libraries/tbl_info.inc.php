 ...
 */
global $showtable, $tbl_is_view, $tbl_storage_engine, $show_comment, $tbl_collation, $table_info_num_rows, $auto_increment;
/**
 ...
 * @name $showtable
 */
$GLOBALS['showtable'] = $GLOBALS['dbi']->getTable($GLOBALS['db'], $GLOBALS['table'])->getStatusInfo(null, isset($reread_info) && $reread_info ? true : false);
...
if ($showtable) {
    if ($GLOBALS['dbi']->getTable($GLOBALS['db'], $GLOBALS['table'])->isView()) {
        ...
        $tbl_is_view = false;
        $tbl_storage_engine = isset($showtable['Engine']) ? mb_strtoupper($showtable['Engine']) : '';
        $show_comment = '';
    ...
    if (null === $showtable['Rows']) {
        $showtable['Rows'] = $GLOBALS['dbi']->getTable($GLOBALS['db'], $showtable['Name'])->countRecords(true);
    }
    ...
    $create_options_tmp = isset($showtable['Create_options']) ? explode(' ', $showtable['Create_options']) : array();
    ...
    $create_options['pack_keys'] = !isset($create_options['pack_keys']) || strlen($create_options['pack_keys']) == 0 ? 'DEFAULT' : $create_options['pack_keys'];
