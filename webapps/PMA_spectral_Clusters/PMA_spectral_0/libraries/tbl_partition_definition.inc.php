...
if (!isset($partitionDetails)) {
    ...
    $partitionParams = array('partition_by', 'partition_expr', 'subpartition_by', 'subpartition_expr');
    foreach ($partitionParams as $partitionParam) {
        $partitionDetails[$partitionParam] = isset($_REQUEST[$partitionParam]) ? $_REQUEST[$partitionParam] : '';
    }
    ...
    $partitionDetails['partition_count'] = $partition_count === 0 ? '' : $partition_count;
    ...
    $partitionDetails['subpartition_count'] = $subpartition_count === 0 ? '' : $subpartition_count;
    // Only LIST and RANGE type parameters allow subpartitioning
    $partitionDetails['can_have_subpartitions'] = $partition_count > 1 && isset($_REQUEST['partition_by']) && ($_REQUEST['partition_by'] == 'RANGE' || $_REQUEST['partition_by'] == 'RANGE COLUMNS' || $_REQUEST['partition_by'] == 'LIST' || $_REQUEST['partition_by'] == 'LIST COLUMNS');
    // Values are specified only for LIST and RANGE type partitions
    $partitionDetails['value_enabled'] = isset($_REQUEST['partition_by']) && ($_REQUEST['partition_by'] == 'RANGE' || $_REQUEST['partition_by'] == 'RANGE COLUMNS' || $_REQUEST['partition_by'] == 'LIST' || $_REQUEST['partition_by'] == 'LIST COLUMNS');
    // Has partitions
    if ($partition_count > 1) {
        ...
        for ($i = 0; $i < $partition_count; $i++) {
            if (!isset($partitions[$i])) {
                ...
                $partitions[$i] = array('name' => 'p' . $i, 'value_type' => '', 'value' => '', 'engine' => '', 'comment' => '', 'data_directory' => '', 'index_directory' => '', 'max_rows' => '', 'min_rows' => '', 'tablespace' => '', 'node_group' => '');
            }
