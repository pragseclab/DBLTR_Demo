...
$strPrivDescGrantTbl = __('Allows user to give to other users or remove from other users the privileges ' . 'that user possess yourself.');
...
if (isset($_REQUEST['change_copy']) && $username == $_REQUEST['old_username'] && $hostname == $_REQUEST['old_hostname']) {
    $response->addHTML(PMA\libraries\Message::error(__("Username and hostname didn't change. " . "If you only want to change the password, " . "'Change password' tab should be used."))->getDisplay());
...
if (!empty($_POST['update_privs'])) {
    if (is_array($dbname)) {
        foreach ($dbname as $key => $db_name) {
            list($sql_query[$key], $message) = PMA_updatePrivileges(isset($username) ? $username : '', isset($hostname) ? $hostname : '', isset($tablename) ? $tablename : (isset($routinename) ? $routinename : ''), isset($db_name) ? $db_name : '', $itemType);
        }
        $sql_query = implode("\n", $sql_query);
    } else {
        list($sql_query, $message) = PMA_updatePrivileges(isset($username) ? $username : '', isset($hostname) ? $hostname : '', isset($tablename) ? $tablename : (isset($routinename) ? $routinename : ''), isset($dbname) ? $dbname : '', $itemType);
    }
}
/**
...
if (isset($_REQUEST['revokeall'])) {
    list($message, $sql_query) = PMA_getMessageAndSqlQueryForPrivilegesRevoke(isset($dbname) ? $dbname : '', isset($tablename) ? $tablename : (isset($routinename) ? $routinename : ''), $username, $hostname, $itemType);
}
/**
 ...
 */
if ($response->isAjax() && empty($_REQUEST['ajax_page_request']) && !isset($_REQUEST['export']) && (!isset($_REQUEST['submit_mult']) || $_REQUEST['submit_mult'] != 'export') && (!isset($_REQUEST['initial']) || $_REQUEST['initial'] === null || $_REQUEST['initial'] === '' || isset($_REQUEST['delete']) && $_REQUEST['delete'] === __('Go')) && !isset($_REQUEST['showall']) && !isset($_REQUEST['edit_user_group_dialog']) && !isset($_REQUEST['db_specific'])) {
...
if (isset($_REQUEST['adduser'])) {
    ...
    if (!isset($username)) {
        ...
        if (!empty($routinename)) {
            $response->addHTML(PMA_getHtmlForRoutineSpecificPrivilges($username, $hostname, $dbname, $routinename, isset($url_dbname) ? $url_dbname : ''));
        } else {
