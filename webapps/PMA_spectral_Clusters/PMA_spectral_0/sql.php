...
// end if
if (!isset($err_url)) {
    $err_url = (!empty($back) ? $back : $goto) . '?' . URL::getCommon(array('db' => $GLOBALS['db'])) . (mb_strpos(' ' . $goto, 'db_') != 1 && strlen($table) > 0 ? '&amp;table=' . urlencode($table) : '');
}
// end if
...
if (isset($_REQUEST['get_default_fk_check_value']) && $_REQUEST['get_default_fk_check_value'] == true) {
    $response = Response::getInstance();
    $response->addJSON('default_fk_check_value', Util::isForeignKeyCheck());
    exit;
}
/**
