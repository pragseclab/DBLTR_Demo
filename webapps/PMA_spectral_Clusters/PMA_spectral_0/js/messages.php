...
$js_messages['strDatabaseRenameToSameName'] = __('Cannot rename database to the same name. Change the name and try again');
...
$js_messages['NoExportable'] = __('No routine is exportable. Required privileges may be lacking.');
/* For ENUM/SET editor*/
...
$js_messages['strMoveRepeatingGroup'] = '<ol><b>' . __('The following actions will be performed:') . '</b>' . '<li>' . __('DROP columns %s from the table %s') . '</li>' . '<li>' . __('Create the following table') . '</li>';
...
$js_messages['strMysqlAllowedValuesTipTime'] = __('MySQL accepts additional values not selectable by the slider;' . ' key in those values directly if desired');
/* For Tip to be shown on Date field */
$js_messages['strMysqlAllowedValuesTipDate'] = __('MySQL accepts additional values not selectable by the datepicker;' . ' key in those values directly if desired');
/* For Lock symbol Tooltip */
$js_messages['strLockToolTip'] = __('Indicates that you have made changes to this page;' . ' you will be prompted for confirmation before abandoning changes');
...
$js_messages['strErrorOccurred'] = __("A fatal JavaScript error has occurred. Would you like to send an error report?");
...
$js_messages['strTooManyInputs'] = __("Warning: a form on this page has more than %d fields. On submission, " . "some of the fields might be ignored, due to PHP's " . "max_input_vars configuration.");
$js_messages['phpErrorsFound'] = '<div class="error">' . __('Some errors have been detected on the server!') . '<br/>' . __('Please look at the bottom of this window.') . '<div>' . '<input id="pma_ignore_errors_popup" type="submit" value="' . __('Ignore') . '" class="floatright" style="margin-top: 20px;">' . '<input id="pma_ignore_all_errors_popup" type="submit" value="' . __('Ignore All') . '" class="floatright" style="margin-top: 20px;">' . '</div></div>';
$js_messages['phpErrorsBeingSubmitted'] = '<div class="error">' . __('Some errors have been detected on the server!') . '<br/>' . __('As per your settings, they are being submitted currently, please be ' . 'patient.') . '<br/>' . '<img src="' . $_SESSION['PMA_Theme']->getImgPath('ajax_clock_small.gif') . '" width="16" height="16" alt="ajax clock"/>' . '</div>';
// For console
...
echo 'var maxInputVars = ', false === $maxInputVars || '' == $maxInputVars ? 'false' : (int) $maxInputVars, ';' . "\n";
echo "if (\$.datepicker) {\n";
/* l10n: Display text for calendar close link */
Sanitize::printJsValue("\$.datepicker.regional['']['closeText']", __('Done'));
/* l10n: Display text for previous month link in calendar */
Sanitize::printJsValue("\$.datepicker.regional['']['prevText']", _pgettext('Previous month', 'Prev'));
/* l10n: Display text for next month link in calendar */
Sanitize::printJsValue("\$.datepicker.regional['']['nextText']", _pgettext('Next month', 'Next'));
/* l10n: Display text for current month link in calendar */
Sanitize::printJsValue("\$.datepicker.regional['']['currentText']", __('Today'));
Sanitize::printJsValue("\$.datepicker.regional['']['monthNames']", array(__('January'), __('February'), __('March'), __('April'), __('May'), __('June'), __('July'), __('August'), __('September'), __('October'), __('November'), __('December')));
Sanitize::printJsValue("\$.datepicker.regional['']['monthNamesShort']", array(
...
Sanitize::printJsValue("\$.datepicker.regional['']['dayNames']", array(__('Sunday'), __('Monday'), __('Tuesday'), __('Wednesday'), __('Thursday'), __('Friday'), __('Saturday')));
Sanitize::printJsValue("\$.datepicker.regional['']['dayNamesShort']", array(
...
Sanitize::printJsValue("\$.datepicker.regional['']['dayNamesMin']", array(
...
Sanitize::printJsValue("\$.datepicker.regional['']['yearSuffix']", $year_suffix == 'none' ? '' : $year_suffix);
?>
