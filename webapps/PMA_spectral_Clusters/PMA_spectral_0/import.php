...
if (isset($_REQUEST['console_bookmark_add'])) {
    if (isset($_REQUEST['label']) && isset($_REQUEST['db']) && isset($_REQUEST['bookmark_query']) && isset($_REQUEST['shared'])) {
        ...
        $bookmarkFields = array('bkm_database' => $_REQUEST['db'], 'bkm_user' => $cfgBookmark['user'], 'bkm_sql_query' => $_REQUEST['bookmark_query'], 'bkm_label' => $_REQUEST['label']);
 ...
 */
$post_params = array('charset_of_file', 'format', 'import_type', 'is_js_confirmed', 'MAX_FILE_SIZE', 'message_to_show', 'noplugin', 'skip_queries', 'local_import_file');
...
if (!empty($sql_query)) {
    ...
    if (!empty($_REQUEST['parameterized']) && !empty($_REQUEST['parameters']) && is_array($_REQUEST['parameters'])) {
        ...
        foreach ($parameters as $parameter => $replacement) {
            ...
            $sql_query = preg_replace('/' . $quoted . '([^a-zA-Z0-9_])/', $GLOBALS['dbi']->escapeString($replacement) . '${1}', $sql_query);
            // for parameters the appear at the end of the string
            $sql_query = preg_replace('/' . $quoted . '$/', $GLOBALS['dbi']->escapeString($replacement), $sql_query);
        }
    }
    // run SQL query
...
if ($_POST == array() && $_GET == array()) {
    $message = PMA\libraries\Message::error(__('You probably tried to upload a file that is too large. Please refer ' . 'to %sdocumentation%s for a workaround for this limit.'));
...
if (!empty($_REQUEST['id_bookmark'])) {
    ...
    switch ($_REQUEST['action_bookmark']) {
        case 0:
            ...
            $bookmark = Bookmark::get($db, $id_bookmark, 'id', isset($_REQUEST['action_bookmark_all']));
            if (!empty($_REQUEST['bookmark_variable'])) {
                $import_text = $bookmark->applyVariables($_REQUEST['bookmark_variable']);
            } else {
...
if (!$error) {
    ...
    $import_plugin = PMA_getPlugin("import", $format, 'libraries/plugins/import/', $import_type);
...
if ($timeout_passed) {
    ...
    $message = PMA\libraries\Message::error(__('Script timeout passed, if you want to finish import,' . ' please %sresubmit the same file%s and import will resume.'));
