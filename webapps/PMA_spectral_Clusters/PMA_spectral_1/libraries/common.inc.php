...
if (!@is_readable('./vendor/autoload.php')) {
    die('File <tt>./vendor/autoload.php</tt> missing or not readable. <br />' . 'Most likely you did not run Composer to ' . '<a href="https://docs.phpmyadmin.net/en/latest/setup.html#installing-from-git">install library files</a>.');
}
...
if (isset($_COOKIE)) {
    if (!isset($_COOKIE['pmaCookieVer']) || $_COOKIE['pmaCookieVer'] != $pma_cookie_version) {
        // delete all cookies
...
if (PMA_isValid($_REQUEST['selected_recent_table'])) {
    ...
    $GLOBALS['db'] = array_key_exists('db', $recent_table) && is_string($recent_table['db']) ? $recent_table['db'] : '';
    $GLOBALS['url_params']['db'] = $GLOBALS['db'];
    $GLOBALS['table'] = array_key_exists('table', $recent_table) && is_string($recent_table['table']) ? $recent_table['table'] : '';
...
if (@extension_loaded('mbstring') && @ini_get('mbstring.func_overload') != '0') {
    PMA_fatalError(__('You have enabled mbstring.func_overload in your PHP ' . 'configuration. This option is incompatible with phpMyAdmin ' . 'and might cause some data to be corrupted!'));
}
/******************************************************************************/
...
if (!defined('PMA_MINIMUM_COMMON')) {
    ...
    if (!empty($cfg['Server'])) {
        ...
        if (!file_exists('./libraries/plugins/auth/' . $auth_class . '.php')) {
        ...
        if ($cfg['Server']['controluser'] != '') {
            $controllink = $GLOBALS['dbi']->connect(DatabaseInterface::CONNECT_CONTROL);
        }
        ...
        if ($cfg['Server']['SessionTimeZone'] != '') {
            $sql_query_tz = 'SET ' . Util::backquote('time_zone') . ' = ' . '\'' . $GLOBALS['dbi']->escapeString($cfg['Server']['SessionTimeZone']) . '\'';
