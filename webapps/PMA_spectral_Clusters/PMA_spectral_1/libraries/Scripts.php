...
class Scripts
{
     ...
     * @return void
     */
    public function addFile($filename, $before_statics = false)
    {
        $hash = md5($filename);
        ...
        $this->_files[$hash] = array('has_onload' => $has_onload, 'filename' => $filename, 'before_statics' => $before_statics);
    }
    /**
    ...
    {
        if (strpos($filename, 'jquery') !== false || strpos($filename, 'codemirror') !== false || strpos($filename, 'messages.php') !== false || strpos($filename, 'ajax.js') !== false || strpos($filename, 'get_image.js.php') !== false || strpos($filename, 'cross_framing_protection.js') !== false) {
            return 0;
        }
        return 1;
    }
    ...
    public function getFiles()
    {
        ...
        foreach ($this->_files as $file) {
            ...
            $retval[] = array('name' => $file['filename'], 'fire' => $file['has_onload']);
        }
        return $retval;
    }
    /**
