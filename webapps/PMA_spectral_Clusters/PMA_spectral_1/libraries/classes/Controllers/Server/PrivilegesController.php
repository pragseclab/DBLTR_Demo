...
class PrivilegesController extends AbstractController
{
    ...
    public function index() : void
    {
        ...
        $databaseController = new DatabaseController($this->response, $this->template, $db, $serverPrivileges, $this->dbi);
        $tableController = new TableController($this->response, $this->template, $db, $table, $serverPrivileges, $this->dbi);
        if (isset($_GET['viewing_mode']) && $_GET['viewing_mode'] === 'server' && $GLOBALS['cfgRelation']['menuswork']) {
            $this->response->addHTML('<div class="container-fluid">');
            $this->render('server/privileges/subnav', ['active' => 'privileges', 'is_super_user' => $this->dbi->isSuperUser()]);
        }
        /**
         * Sets globals from $_POST patterns, for privileges and max_* vars
         */
        $post_patterns = ['/_priv$/i', '/^max_/i'];
         ...
         */
        [$username, $hostname, $dbname, $tablename, $routinename, $db_and_table, $dbname_is_wildcard] = $serverPrivileges->getDataForDBInfo();
        /**
        ...
        if (!$this->dbi->isSuperUser() && !$isGrantUser && !$isCreateUser) {
            ...
            $this->response->addHTML(Message::error(__('No Privileges'))->getDisplay());
            return;
        }
        if (!$isGrantUser && !$isCreateUser) {
            $this->response->addHTML(Message::notice(__('You do not have the privileges to administrate the users!'))->getDisplay());
        }
        /**
         * Checks if the user is using "Change Login Information / Copy User" dialog
         * only to update the password
         */
        if (isset($_POST['change_copy']) && $username == $_POST['old_username'] && $hostname == $_POST['old_hostname']) {
            $this->response->addHTML(Message::error(__("Username and hostname didn't change. " . 'If you only want to change the password, ' . "'Change password' tab should be used."))->getDisplay());
         ...
         */
        [$ret_message, $ret_queries, $queries_for_display, $sql_query, $_add_user_error] = $serverPrivileges->addUser($dbname ?? null, $username ?? null, $hostname ?? null, $password ?? null, (bool) $cfgRelation['menuswork']);
        ...
        if (!empty($_POST['update_privs'])) {
            if (is_array($dbname)) {
                foreach ($dbname as $key => $db_name) {
                    [$sql_query[$key], $message] = $serverPrivileges->updatePrivileges($username ?? '', $hostname ?? '', $tablename ?? $routinename ?? '', $db_name ?? '', $itemType);
                }
                $sql_query = implode("\n", $sql_query);
            } else {
                [$sql_query, $message] = $serverPrivileges->updatePrivileges($username ?? '', $hostname ?? '', $tablename ?? $routinename ?? '', $dbname ?? '', $itemType);
            }
        }
        /**
         * Assign users to user groups
         */
        if (!empty($_POST['changeUserGroup']) && $cfgRelation['menuswork'] && $this->dbi->isSuperUser() && $this->dbi->isCreateUser()) {
        ...
        if (isset($_POST['revokeall'])) {
            [$message, $sql_query] = $serverPrivileges->getMessageAndSqlQueryForPrivilegesRevoke($dbname ?? '', $tablename ?? $routinename ?? '', $username, $hostname, $itemType);
        }
        /**
         ...
         */
        if ($this->response->isAjax() && empty($_REQUEST['ajax_page_request']) && !isset($_GET['export']) && (!isset($_POST['submit_mult']) || $_POST['submit_mult'] !== 'export') && (!isset($_GET['initial']) || $_GET['initial'] === null || $_GET['initial'] === '' || isset($_POST['delete']) && $_POST['delete'] === __('Go')) && !isset($_GET['showall']) && !isset($_GET['edit_user_group_dialog'])) {
        ...
        if (isset($_GET['adduser'])) {
            // Add user
            $this->response->addHTML($serverPrivileges->getHtmlForAddUser($dbname ?? ''));
        } elseif (isset($_GET['checkprivsdb'])) {
            if (isset($_GET['checkprivstable'])) {
                $this->response->addHTML($tableController->index(['checkprivsdb' => $_GET['checkprivsdb'], 'checkprivstable' => $_GET['checkprivstable']]));
            } elseif ($this->response->isAjax() === true && empty($_REQUEST['ajax_page_request'])) {
                $message = Message::success(__('User has been added.'));
                $this->response->addJSON('message', $message);
                return;
            } else {
                $this->response->addHTML($databaseController->index(['checkprivsdb' => $_GET['checkprivsdb']]));
            }
        } else {
            ...
            if (!isset($username)) {
                ...
                $this->response->addHTML($serverPrivileges->getHtmlForUserOverview($PMA_Theme->getImgPath(), $text_dir));
            } elseif (!empty($routinename)) {
                $this->response->addHTML($serverPrivileges->getHtmlForRoutineSpecificPrivileges($username, $hostname ?? '', $dbname, $routinename, $url_dbname ?? ''));
            } else {
                ...
                $this->response->addHTML($serverPrivileges->getHtmlForUserProperties($dbname_is_wildcard, $url_dbname ?? '', $username, $hostname ?? '', $dbname ?? '', $tablename ?? ''));
            }
        }
