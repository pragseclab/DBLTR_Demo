...
class ViewCreateController extends AbstractController
{
    ...
    public function index() : void
    {
        ...
        $view_algorithm_options = ['UNDEFINED', 'MERGE', 'TEMPTABLE'];
        $view_with_options = ['CASCADED', 'LOCAL'];
        $view_security_options = ['DEFINER', 'INVOKER'];
        ...
        if (isset($_POST['createview']) || isset($_POST['alterview'])) {
            ...
            if (!empty($_POST['view']['definer'])) {
                if (strpos($_POST['view']['definer'], '@') === false) {
                    $sql_query .= $sep . 'DEFINER=' . Util::backquote($_POST['view']['definer']);
                } else {
            ...
            if (isset($_POST['view']['sql_security']) && in_array($_POST['view']['sql_security'], $view_security_options)) {
                $sql_query .= $sep . ' SQL SECURITY ' . $_POST['view']['sql_security'];
            }
            $sql_query .= $sep . ' VIEW ' . Util::backquote($_POST['view']['name']);
            ...
            if (!$this->dbi->tryQuery($sql_query)) {
                ...
                $this->response->addJSON('message', Message::error('<i>' . htmlspecialchars($sql_query) . '</i><br><br>' . $this->dbi->getError()));
                $this->response->setRequestStatus(false);
                return;
            }
            ...
            if ($pma_transformation_data !== false) {
                ...
                $new_transformations_sql = $systemDb->getNewTransformationDataSql($pma_transformation_data, $column_map, $_POST['view']['name'], $db);
            ...
            if (!isset($_POST['ajax_dialog'])) {
                ...
                $this->response->addJSON('message', Generator::getMessage(Message::success(), $sql_query));
        ...
        $view = ['operation' => 'create', 'or_replace' => '', 'algorithm' => '', 'definer' => '', 'sql_security' => '', 'name' => '', 'column_names' => '', 'as' => $sql_query, 'with' => ''];
        // Used to prefill the fields when editing a view
        if (isset($_GET['db'], $_GET['table'])) {
            $item = $this->dbi->fetchSingleRow(sprintf("SELECT `VIEW_DEFINITION`, `CHECK_OPTION`, `DEFINER`,\n            `SECURITY_TYPE`\n            FROM `INFORMATION_SCHEMA`.`VIEWS`\n            WHERE TABLE_SCHEMA='%s'\n            AND TABLE_NAME='%s';", $this->dbi->escapeString($_GET['db']), $this->dbi->escapeString($_GET['table'])));
            $createView = $this->dbi->getTable($_GET['db'], $_GET['table'])->showCreate();
        ...
        $url_params['db'] = $db;
        $url_params['reload'] = 1;
        echo $this->template->render('view_create', ['ajax_dialog' => isset($_POST['ajax_dialog']), 'text_dir' => $text_dir, 'url_params' => $url_params, 'view' => $view, 'view_algorithm_options' => $view_algorithm_options, 'view_with_options' => $view_with_options, 'view_security_options' => $view_security_options]);
    }
}