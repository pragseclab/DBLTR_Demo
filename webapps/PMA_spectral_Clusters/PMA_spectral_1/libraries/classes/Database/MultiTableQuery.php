...
class MultiTableQuery
{
     ...
     */
    public function __construct(DatabaseInterface $dbi, Template $template, $dbName, $defaultNoOfColumns = 3)
    {
        $this->dbi = $dbi;
        $this->db = $dbName;
    ...
    public function getFormHtml()
    {
        ...
        foreach ($this->tables as $table) {
            ...
            $tables[$table]['columns'] = array_keys($this->dbi->getColumns($this->db, $table));
        }
        return $this->template->render('database/multi_table_query/form', ['db' => $this->db, 'tables' => $tables, 'default_no_of_columns' => $this->defaultNoOfColumns]);
    }
    /**
    ...
    public static function displayResults($sqlQuery, $db, $themeImagePath) : string
    {
        ...
        $sql = new Sql($dbi, $relation, new RelationCleanup($dbi, $relation), new Operations($dbi, $relation), new Transformations(), new Template());
