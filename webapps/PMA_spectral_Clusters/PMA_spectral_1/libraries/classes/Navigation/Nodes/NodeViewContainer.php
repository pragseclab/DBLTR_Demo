...
class NodeViewContainer extends NodeDatabaseChildContainer
{
    ...
    public function __construct()
    {
        ...
        $this->links = ['text' => Url::getFromRoute('/database/structure', ['server' => $GLOBALS['server'], 'tbl_type' => 'view']) . '&amp;db=%1$s', 'icon' => Url::getFromRoute('/database/structure', ['server' => $GLOBALS['server'], 'tbl_type' => 'view']) . '&amp;db=%1$s'];
        ...
        $new->links = ['text' => Url::getFromRoute('/view/create', ['server' => $GLOBALS['server']]) . '&amp;db=%2$s', 'icon' => Url::getFromRoute('/view/create', ['server' => $GLOBALS['server']]) . '&amp;db=%2$s'];
        $this->addChild($new);
    }
}