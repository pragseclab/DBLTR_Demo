...
function PMA_getItemsFromShowGrantsRow($row)
{
    ...
    $show_grants_dbname = mb_substr($row, $db_name_offset, mb_strpos($row, '.', $db_name_offset) - $db_name_offset);
    ...
    $show_grants_tblname = mb_substr($row, $tblname_start_offset, $tblname_end_offset - $tblname_start_offset);
    $show_grants_tblname = PMA\libraries\Util::unQuote($show_grants_tblname, '`');
    return array($show_grants_str, $show_grants_dbname, $show_grants_tblname);
}
/**
 ...
 * @return void
 */
function PMA_checkRequiredPrivilegesForAdjust($show_grants_str, $show_grants_dbname, $show_grants_tblname)
{
