 ...
 */
$drops = array('lang', 'server', 'collation_connection', 'db', 'table');
...
if ($server > 0 || count($cfg['Servers']) > 1) {
    if ($cfg['DBG']['demo']) {
        ...
        printf(__('You are using the demo server. You can do anything here, but ' . 'please do not change root, debian-sys-maint and pma users. ' . 'More information is available at %s.'), '<a href="url.php?url=https://demo.phpmyadmin.net/" target="_blank" rel="noopener noreferrer">demo.phpmyadmin.net</a>');
        echo '</p>';
        echo '</div>';
    }
    echo '<div class="group">';
...
if (empty($cfg['Lang']) && $language_manager->hasChoice()) {
    ...
    echo PMA\libraries\Util::getImage('s_lang.png'), " ", $language_manager->getSelectorDisplay();
    echo '</li>';
}
...
PMA_printListItem(__('Official Homepage'), 'li_pma_homepage', PMA_linkURL('https://www.phpmyadmin.net/'), null, '_blank');
...
PMA_printListItem(__('List of changes'), 'li_pma_changes', 'changelog.php' . URL::getCommon(), null, '_blank');
PMA_printListItem(__('License'), 'li_pma_license', 'license.php' . URL::getCommon(), null, '_blank');
echo '    </ul>';
echo ' </div>';
echo '</div>';
echo '</div>';
