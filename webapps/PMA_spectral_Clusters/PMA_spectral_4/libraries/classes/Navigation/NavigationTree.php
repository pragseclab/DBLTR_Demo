...
class NavigationTree
{
    ...
    private function getNavigationDbPos() : int
    {
        ...
        if (!$GLOBALS['cfg']['Server']['DisableIS']) {
            ...
            $query .= ') t ';
            return (int) $this->dbi->fetchValue(sprintf($query, (int) $GLOBALS['cfg']['FirstLevelNavigationItems'], (int) $GLOBALS['cfg']['FirstLevelNavigationItems'], $dbSeparator, $this->dbi->escapeString($GLOBALS['db'])));
        }
        $prefixMap = [];
    ...
    private function buildPath()
    {
        ...
        foreach ($this->aPath as $key => $path) {
            $retval = $this->buildPathPart($path, $this->pos2Name[$key] ?? '', $this->pos2Value[$key] ?? 0, $this->pos3Name[$key] ?? '', $this->pos3Value[$key] ?? 0);
        }
        return $retval;
    }
    /**
    ...
    {
        echo('<html><head>    <meta charset="utf-8">    <meta http-equiv="X-UA-Compatible" content="IE=edge">    <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no">    <title>Error, Target Function Has Been Removed</title>    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css" integrity="sha384-BVYiiSIFeK1dGmJRAkycuHAHRg32OmUcww7on3RYdg4Va+PmSTsz/K68vbdEjh4u" crossorigin="anonymous">    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap-theme.min.css" integrity="sha384-rHyoN1iRsVXV4nD0JutlnGaslCJuC7uwjduW9SVrLvRYooPp2bWYgmgJQIXwl/Sp" crossorigin="anonymous">    <style>        * {            font-family: tahoma;        }        div.container .panel {            position: relative !important;        }        div.container {            width: 50% !important;            height: 50% !important;            overflow: auto !important;            margin: auto !important;            position: absolute !important;            top: 0 !important;            left: 0 !important;            bottom: 0 !important;            right: 0 !important;        }    </style></head><body>    <div class="container">        <div class="panel panel-danger center">            <div class="panel-heading" style="text-align: left;"> Error </div>            <div class="panel-body">                <p class="text-center">                  This function has been removed ("addTableContainers") from ("/home/jovyan/work/WebApps/PMA_spectral_Clusters/PMA_spectral_4/libraries/classes/Navigation/NavigationTree.php at line 448")                </p>            </div>        </div>    </div></body></html>');
        error_log('Removed function called addTableContainers:448@/home/jovyan/work/WebApps/PMA_spectral_Clusters/PMA_spectral_4/libraries/classes/Navigation/NavigationTree.php');
        die();
    }
    /**
    ...
    public function groupNode($node) : void
    {
        ...
        if ($node->separatorDepth > 0) {
            foreach ($node->children as $child) {
                ...
                foreach ($separators as $separator) {
                    ...
                    if ($sepPos == false || $sepPos == mb_strlen($child->name) || $sepPos == 0 || $prefixPos !== false && $sepPos >= $prefixPos) {
                        continue;
                    }
                    $prefixPos = $sepPos;
                }
        ...
        foreach ($prefixes as $key => $value) {
            ...
            $groups[$key] = new Node(htmlspecialchars((string) $key), Node::CONTAINER, true);
            $groups[$key]->separator = $node->separator;
            $groups[$key]->separatorDepth = $node->separatorDepth - 1;
            $groups[$key]->icon = Generator::getImage('b_group', __('Groups'));
            $groups[$key]->pos2 = $node->pos2;
            $groups[$key]->pos3 = $node->pos3;
            if ($node instanceof NodeTableContainer || $node instanceof NodeViewContainer) {
                $tblGroup = '&amp;tbl_group=' . urlencode((string) $key);
                $groups[$key]->links = ['text' => $node->links['text'] . $tblGroup, 'icon' => $node->links['icon'] . $tblGroup];
            }
            ...
            foreach ($separators as $separator) {
                ...
                foreach ($node->children as $child) {
                    ...
                    $newChild = NodeFactory::getInstance($className, mb_substr($child->name, $keySeparatorLength));
    ...
    public function renderState() : string
    {
        ...
        return $this->template->render('navigation/tree/state', ['quick_warp' => $quickWarp, 'fast_filter' => $fastFilter, 'controls' => $controls, 'page_selector' => $pageSelector, 'nodes' => $nodes]);
    }
    /**
    ...
    public function renderPath()
    {
        ...
        if ($hasSearchClause && !is_bool($node)) {
            ...
            if (!empty($this->searchClause2)) {
                if (is_object($node->realParent())) {
                    $results = $node->realParent()->getPresence($node->realName, $this->searchClause2);
                }
            } else {
        ...
        if ($node !== false) {
            return $this->template->render('navigation/tree/path', ['has_search_results' => !empty($this->searchClause) || !empty($this->searchClause2), 'list_content' => $listContent ?? '', 'is_tree' => $GLOBALS['cfg']['ShowDatabasesNavigationAsTree'], 'parent_name' => $parentName ?? '']);
        }
        return false;
    }
    /**
    ...
    private function renderNode(Node $node, bool $recursive, string $class = '') : string
    {
        ...
        if ($node->hasSiblings() || $node->realParent() === false) {
            ...
            if ($node->isGroup || !in_array($parentName, $sterile) && !$node->isNew || in_array($node->realName, $sterile) && !empty($node->children)) {
            ...
            if ($parent[0]->type == Node::CONTAINER && (in_array($parent[0]->realName, $haveAjax) || $isNewView)) {
                $linkClass = ' ajax';
            }
            if ($nodeIsContainer) {
                $retval .= '<i>';
    ...
    {
        echo('<html><head>    <meta charset="utf-8">    <meta http-equiv="X-UA-Compatible" content="IE=edge">    <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no">    <title>Error, Target Function Has Been Removed</title>    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css" integrity="sha384-BVYiiSIFeK1dGmJRAkycuHAHRg32OmUcww7on3RYdg4Va+PmSTsz/K68vbdEjh4u" crossorigin="anonymous">    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap-theme.min.css" integrity="sha384-rHyoN1iRsVXV4nD0JutlnGaslCJuC7uwjduW9SVrLvRYooPp2bWYgmgJQIXwl/Sp" crossorigin="anonymous">    <style>        * {            font-family: tahoma;        }        div.container .panel {            position: relative !important;        }        div.container {            width: 50% !important;            height: 50% !important;            overflow: auto !important;            margin: auto !important;            position: absolute !important;            top: 0 !important;            left: 0 !important;            bottom: 0 !important;            right: 0 !important;        }    </style></head><body>    <div class="container">        <div class="panel panel-danger center">            <div class="panel-heading" style="text-align: left;"> Error </div>            <div class="panel-body">                <p class="text-center">                  This function has been removed ("renderDbSelect") from ("/home/jovyan/work/WebApps/PMA_spectral_Clusters/PMA_spectral_4/libraries/classes/Navigation/NavigationTree.php at line 989")                </p>            </div>        </div>    </div></body></html>');
        error_log('Removed function called renderDbSelect:989@/home/jovyan/work/WebApps/PMA_spectral_Clusters/PMA_spectral_4/libraries/classes/Navigation/NavigationTree.php');
        die();
    }
    /**
    ...
    private function fastFilterHtml(Node $node) : string
    {
        ...
        if ($node === $this->tree && $this->tree->getPresence() >= $filterDbMin) {
            $urlParams = ['pos' => 0];
        ...
        $nodeIsContainer = $node->type === Node::CONTAINER;
        $nodeIsSpecial = $node->realName === 'tables' || $node->realName === 'views' || $node->realName === 'functions' || $node->realName === 'procedures' || $node->realName === 'events';
        /** @var Node $realParent */
        $realParent = $node->realParent();
        if ($nodeIsContainer && $nodeIsSpecial && method_exists($realParent, 'getPresence') && $realParent->getPresence($node->realName) >= $filterItemMin) {
            $paths = $node->getPaths();
            $urlParams = ['pos' => $this->pos, 'aPath' => $paths['aPath'], 'vPath' => $paths['vPath'], 'pos2_name' => $node->realName, 'pos2_value' => 0];
    ...
    private function controls() : string
    {
        ...
        $retval .= Generator::getNavigationLink('#', $showText, __('Collapse all'), $showIcon, 's_collapseall', 'pma_navigation_collapse');
        $syncImage = 's_unlink';
        ...
        $retval .= Generator::getNavigationLink('#', $showText, $title, $showIcon, $syncImage, 'pma_navigation_sync');
        $retval .= '</div>';
        $retval .= '</li>';
    ...
    private function getPageSelector(Node $node) : string
    {
        $retval = '';
        if ($node === $this->tree) {
            $retval .= Generator::getListNavigator($this->tree->getPresence('databases', $this->searchClause), $this->pos, ['server' => $GLOBALS['server']], Url::getFromRoute('/navigation'), 'frame_navigation', $GLOBALS['cfg']['FirstLevelNavigationItems'], 'pos', ['dbselector']);
        } else {
            if ($node->type == Node::CONTAINER && !$node->isGroup) {
                $paths = $node->getPaths();
                $level = isset($paths['aPath_clean'][4]) ? 3 : 2;
                $urlParams = ['aPath' => $paths['aPath'], 'vPath' => $paths['vPath'], 'pos' => $this->pos, 'server' => $GLOBALS['server'], 'pos2_name' => $paths['aPath_clean'][2]];
                if ($level == 3) {
                    $pos = $node->pos3;
                ...
                $retval .= Generator::getListNavigator($num, $pos, $urlParams, Url::getFromRoute('/navigation'), 'frame_navigation', $GLOBALS['cfg']['MaxNavigationItems'], 'pos' . $level . '_value');
            }
        }
        return $retval;
    }
