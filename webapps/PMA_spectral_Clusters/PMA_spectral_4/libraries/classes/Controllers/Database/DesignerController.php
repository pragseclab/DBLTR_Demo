...
class DesignerController extends AbstractController
{
     ...
     */
    public function __construct($response, Template $template, $db, Designer $databaseDesigner, DesignerCommon $designerCommon)
    {
    ...
    public function index() : void
    {
        ...
        if (isset($_POST['dialog'])) {
            if ($_POST['dialog'] === 'edit') {
                ...
                $html = $this->databaseDesigner->getDatabaseTables($_POST['db'], $script_display_field, [], -1, $tab_column, $tables_all_keys, $tables_pk_or_unique_keys);
            }
            if (!empty($html)) {
            ...
            if ($_POST['operation'] === 'deletePage') {
                ...
                if ($_POST['save_page'] === 'same') {
                    ...
                    $this->response->addJSON(
                        ...
                        __(sprintf('There already exists a page named "%s" please rename it to something else.', htmlspecialchars($_POST['selected_value'])))
                    );
                ...
                [$success, $message] = $this->designerCommon->saveDisplayField($_POST['db'], $_POST['table'], $_POST['field']);
                $this->response->setRequestStatus($success);
                $this->response->addJSON('message', $message);
            } elseif ($_POST['operation'] === 'addNewRelation') {
                [$success, $message] = $this->designerCommon->addNewRelation($_POST['db'], $_POST['T1'], $_POST['F1'], $_POST['T2'], $_POST['F2'], $_POST['on_delete'], $_POST['on_update'], $_POST['DB1'], $_POST['DB2']);
                $this->response->setRequestStatus($success);
                $this->response->addJSON('message', $message);
            } elseif ($_POST['operation'] === 'removeRelation') {
                [$success, $message] = $this->designerCommon->removeRelation($_POST['T1'], $_POST['F1'], $_POST['T2'], $_POST['F2']);
                $this->response->setRequestStatus($success);
                $this->response->addJSON('message', $message);
        ...
        $this->addScriptFiles(['vendor/jquery/jquery.fullscreen.js', 'designer/database.js', 'designer/objects.js', 'designer/page.js', 'designer/history.js', 'designer/move.js', 'designer/init.js']);
        ...
        $this->response->addHTML($this->databaseDesigner->getHtmlForMain($db, $_GET['db'], $script_display_field, $script_tables, $script_contr, $script_display_field, $display_page, $visualBuilderMode, $selected_page, $classes_side_menu, $tab_pos, $tab_column, $tables_all_keys, $tables_pk_or_unique_keys));
