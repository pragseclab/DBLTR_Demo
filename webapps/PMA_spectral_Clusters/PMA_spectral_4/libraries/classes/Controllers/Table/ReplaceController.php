...
final class ReplaceController extends AbstractController
{
     ...
     */
    public function __construct($response, Template $template, $db, $table, InsertEdit $insertEdit, Transformations $transformations, Relation $relation, $dbi)
    {
    ...
    public function index() : void
    {
         ...
         */
        $goto_include = false;
        $this->addScriptFiles(['makegrid.js', 'vendor/stickyfill.min.js', 'sql.js', 'indexes.js', 'gis_data_editor.js']);
         ...
         */
        [$loop_array, $using_key, $is_insert, $is_insertignore] = $this->insertEdit->getParamsForUpdateOrInsert();
        $query = [];
        $value_sets = [];
        $func_no_param = ['CONNECTION_ID', 'CURRENT_USER', 'CURDATE', 'CURTIME', 'CURRENT_DATE', 'CURRENT_TIME', 'DATABASE', 'LAST_INSERT_ID', 'NOW', 'PI', 'RAND', 'SYSDATE', 'UNIX_TIMESTAMP', 'USER', 'UTC_DATE', 'UTC_TIME', 'UTC_TIMESTAMP', 'UUID', 'UUID_SHORT', 'VERSION'];
        $func_optional_param = ['RAND', 'UNIX_TIMESTAMP'];
        $gis_from_text_functions = ['GeomFromText', 'GeomCollFromText', 'LineFromText', 'MLineFromText', 'PointFromText', 'MPointFromText', 'PolyFromText', 'MPolyFromText'];
        $gis_from_wkb_functions = [];
        if ($this->dbi->getVersion() >= 50600) {
            $gis_from_wkb_functions = ['ST_GeomFromText', 'ST_GeomCollFromText', 'ST_LineFromText', 'ST_MLineFromText', 'ST_PointFromText', 'ST_MPointFromText', 'ST_PolyFromText', 'ST_MPolyFromText'];
        }
        ...
        foreach ($loop_array as $rownumber => $where_clause) {
            ...
            foreach ($multi_edit_columns_name as $key => $column_name) {
                ...
                if (!empty($mime_map[$column_name]) && !empty($mime_map[$column_name]['input_transformation'])) {
                    ...
                    if (is_file($filename)) {
                        ...
                        if (class_exists($classname)) {
                            ...
                            $transformation_options = $this->transformations->getOptions($mime_map[$column_name]['input_transformation_options']);
                ...
                $current_value = $this->insertEdit->getCurrentValueForDifferentTypes($possibly_uploaded_val, $key, $multi_edit_columns_type, $current_value, $multi_edit_auto_increment, $rownumber, $multi_edit_columns_name, $multi_edit_columns_null, $multi_edit_columns_null_prev, $is_insert, $using_key, $where_clause, $table, $multi_edit_funcs);
                $current_value_as_an_array = $this->insertEdit->getCurrentValueAsAnArrayForMultipleEdit($multi_edit_funcs, $multi_edit_salt, $gis_from_text_functions, $current_value, $gis_from_wkb_functions, $func_optional_param, $func_no_param, $key);
                if (!isset($multi_edit_virtual, $multi_edit_virtual[$key])) {
                    [$query_values, $query_fields] = $this->insertEdit->getQueryValuesForInsertAndUpdateInMultipleEdit($multi_edit_columns_name, $multi_edit_columns_null, $current_value, $multi_edit_columns_prev, $multi_edit_funcs, $is_insert, $query_values, $query_fields, $current_value_as_an_array, $value_sets, $key, $multi_edit_columns_null_prev);
                }
            ...
            if ($is_insert) {
                ...
                // build update query
                $query[] = 'UPDATE ' . Util::backquote($table) . ' SET ' . implode(', ', $query_values) . ' WHERE ' . $where_clause . ($_POST['clause_is_unique'] ? '' : ' LIMIT 1');
            }
        }
         ...
         * page
         */
        [$url_params, $total_affected_rows, $last_messages, $warning_messages, $error_messages, $return_to_sql_query] = $this->insertEdit->executeSqlQuery($url_params, $query);
        ...
        unset($error_messages, $warning_messages, $total_affected_rows, $last_messages, $row_skipped, $insert_errors);
        /**
        ...
        if ($this->response->isAjax() && !isset($_POST['ajax_page_request'])) {
            ...
            if (isset($_POST['rel_fields_list']) && $_POST['rel_fields_list'] != '') {
                ...
                foreach ($relation_fields as $cell_index => $curr_rel_field) {
                    foreach ($curr_rel_field as $relation_field => $relation_field_value) {
                        ...
                        $dispval = $this->insertEdit->getDisplayValueForForeignTableColumn($where_comparison, $map, $relation_field);
                        $extra_data['relations'][$cell_index] = $this->insertEdit->getLinkForRelationalDisplayField($map, $relation_field, $where_comparison, $dispval, $relation_field_value);
                    }
                }
            }
            if (isset($_POST['do_transformations']) && $_POST['do_transformations'] == true) {
                ...
                $transformation_types = ['input_transformation', 'transformation'];
                foreach ($mime_map as $transformation) {
                    $column_name = $transformation['column_name'];
                    foreach ($transformation_types as $type) {
                        $file = Core::securePath($transformation[$type]);
                        $extra_data = $this->insertEdit->transformEditedValues($db, $table, $transformation, $edited_values, $file, $column_name, $extra_data, $type);
                    }
                }
            }
            ...
            $this->insertEdit->verifyWhetherValueCanBeTruncatedAndAppendExtraData($db, $table, $column_name, $extra_data);
