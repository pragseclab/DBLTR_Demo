...
final class ImportController extends AbstractController
{
    ...
    public function index() : void
    {
        ...
        if (isset($_POST['console_bookmark_add'])) {
            ...
            $bookmarkFields = ['bkm_database' => $_POST['db'], 'bkm_user' => $cfgBookmark['user'], 'bkm_sql_query' => $_POST['bookmark_query'], 'bkm_label' => $_POST['label']];
        ...
        if (!empty($sql_query)) {
            ...
            if (!empty($_POST['parameterized']) && !empty($_POST['parameters']) && is_array($_POST['parameters'])) {
                ...
                foreach ($parameters as $parameter => $replacement) {
                    ...
                    $sql_query = preg_replace('/' . $quoted . '([^a-zA-Z0-9_])/', $this->dbi->escapeString($replacement) . '${1}', $sql_query);
                    // for parameters the appear at the end of the string
                    $sql_query = preg_replace('/' . $quoted . '$/', $this->dbi->escapeString($replacement), $sql_query);
                }
            }
            // run SQL query
        ...
        if ($_POST == [] && $_GET == []) {
            $message = Message::error(__('You probably tried to upload a file that is too large. Please refer ' . 'to %sdocumentation%s for a workaround for this limit.'));
        ...
        $post_patterns = ['/^force_file_/', '/^' . $format . '_/'];
        ...
        if (!empty($_POST['id_bookmark'])) {
            ...
            switch ($_POST['action_bookmark']) {
                case 0:
                    ...
                    $bookmark = Bookmark::get($this->dbi, $cfg['Server']['user'], $db, $id_bookmark, 'id', isset($_POST['action_bookmark_all']));
        ...
        if (!empty($local_import_file) && !empty($cfg['UploadDir'])) {
            ...
            $import_file = Util::userDir($cfg['UploadDir']) . $local_import_file;
            /*
        ...
        if ($import_file !== 'none' && !$error) {
            ...
            $message = Message::error(__('No data was received to import. Either no file name was ' . 'submitted, or the file size exceeded the maximum size permitted ' . 'by your PHP configuration. See [doc@faq1-16]FAQ 1.16[/doc].'));
            $_SESSION['Import_message']['message'] = $message->getDisplay();
            $this->response->setRequestStatus(false);
        ...
        if (!$error) {
             ...
             */
            $import_plugin = Plugins::getPlugin('import', $format, 'libraries/classes/Plugins/Import/', $import_type);
            if ($import_plugin == null) {
                $message = Message::error(__('Could not load import plugins, please check your installation!'));
                $_SESSION['Import_message']['message'] = $message->getDisplay();
                $this->response->setRequestStatus(false);
                $this->response->addJSON('message', $message->getDisplay());
                $this->response->addHTML($message->getDisplay());
        ...
        if (!empty($id_bookmark) && $_POST['action_bookmark'] == 2) {
            ...
            if ($import_type !== 'query') {
                $message = Message::success('<em>' . _ngettext('Import has been successfully finished, %d query executed.', 'Import has been successfully finished, %d queries executed.', $executed_queries) . '</em>');
        ...
        if ($timeout_passed) {
            ...
            $message = Message::error(__('Script timeout passed, if you want to finish import,' . ' please %sresubmit the same file%s and import will resume.'));
            $message->addParamHtml('<a href="' . $importUrl . '">');
            $message->addParamHtml('</a>');
            if ($offset == 0 || isset($original_skip) && $original_skip == $offset) {
                $message->addText(__('However on last run no data has been parsed,' . ' this usually means phpMyAdmin won\'t be able to' . ' finish this import unless you increase php time limits.'));
            }
        }
        ...
        if (isset($my_die)) {
            foreach ($my_die as $key => $die) {
                Generator::mysqlDie($die['error'], $die['sql'], false, $err_url, $error);
            }
        }
        if ($go_sql) {
            ...
            foreach ($sql_queries as $sql_query) {
                ...
                if ($this->sql->hasNoRightsToDropDatabase($analyzed_sql_results, $cfg['AllowUserDropDatabase'], $this->dbi->isSuperUser())) {
                    Generator::mysqlDie(__('"DROP DATABASE" statements are disabled.'), '', false, $_SESSION['Import_message']['go_back_url']);
                    return;
                }
                if ($table != $table_from_sql && !empty($table_from_sql)) {
                    $table = $table_from_sql;
            ...
            if (!empty($_POST['bkm_label']) && !empty($import_text)) {
                ...
                $this->sql->storeTheQueryAsBookmark($db, $cfgBookmark['user'], $_POST['sql_query'], $_POST['bkm_label'], isset($_POST['bkm_replace']));
            }
        ...
        if ($result) {
            ...
            $this->response->addJSON('sql_query', Generator::getMessage($msg, $sql_query, 'success'));
        } elseif ($result === false) {
            $this->response->setRequestStatus(false);
