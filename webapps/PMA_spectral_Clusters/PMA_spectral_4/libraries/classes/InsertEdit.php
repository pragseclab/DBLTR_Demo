...
class InsertEdit
{
     ...
     */
    public function getFormParametersForInsertForm($db, $table, ?array $where_clauses, array $where_clause_array, $err_url)
    {
        $_form_params = ['db' => $db, 'table' => $table, 'goto' => $GLOBALS['goto'], 'err_url' => $err_url, 'sql_query' => $_POST['sql_query']];
     ...
     */
    private function analyzeWhereClauses(array $where_clause_array, $table, $db)
    {
        $rows = [];
        $result = [];
        $where_clauses = [];
        ...
        foreach ($where_clause_array as $key_id => $where_clause) {
            $local_query = 'SELECT * FROM ' . Util::backquote($db) . '.' . Util::backquote($table) . ' WHERE ' . $where_clause . ';';
            $result[$key_id] = $this->dbi->query($local_query, DatabaseInterface::CONNECT_USER, DatabaseInterface::QUERY_STORE);
            $rows[$key_id] = $this->dbi->fetchAssoc($result[$key_id]);
            $where_clauses[$key_id] = str_replace('\\', '\\\\', $where_clause);
            $has_unique_condition = $this->showEmptyResultMessageOrSetUniqueCondition($rows, $key_id, $where_clause_array, $local_query, $result);
     ...
     * @return bool
     */
    private function showEmptyResultMessageOrSetUniqueCondition(array $rows, $key_id, array $where_clause_array, $local_query, array $result)
    {
        $has_unique_condition = false;
        // No row returned
        if (!$rows[$key_id]) {
            unset($rows[$key_id], $where_clause_array[$key_id]);
            Response::getInstance()->addHTML(Generator::getMessage(__('MySQL returned an empty result set (i.e. zero rows).'), $local_query));
            /**
            ...
            [$unique_condition, $tmp_clause_is_unique] = Util::getUniqueCondition($result[$key_id], count($meta), $meta, $rows[$key_id], true);
    ...
    {
        $result = $this->dbi->query('SELECT * FROM ' . Util::backquote($db) . '.' . Util::backquote($table) . ' LIMIT 1;', DatabaseInterface::CONNECT_USER, DatabaseInterface::QUERY_STORE);
     ...
     */
    public function urlParamsInEditMode(array $url_params, array $where_clause_array) : array
    {
    ...
    public function showTypeOrFunction($which, array $url_params, $is_show)
    {
        ...
        if (!$is_show) {
            return ' : <a href="' . Url::getFromRoute('/table/change') . '" data-post="' . Url::getCommon($this_url_params, '') . '">' . $this->showTypeOrFunctionLabel($which) . '</a>';
        }
        return '<th><a href="' . Url::getFromRoute('/table/change') . '" data-post="' . Url::getCommon($this_url_params, '') . '" title="' . __('Hide') . '">' . $this->showTypeOrFunctionLabel($which) . '</a></th>';
    }
    /**
    ...
    private function analyzeTableColumnsArray(array $column, array $comments_map, $timestamp_seen)
    {
        ...
        $column['is_binary'] = $this->isColumn($column, ['binary', 'varbinary']);
        $column['is_blob'] = $this->isColumn($column, ['blob', 'tinyblob', 'mediumblob', 'longblob']);
        $column['is_char'] = $this->isColumn($column, ['char', 'varchar']);
        [$column['pma_type'], $column['wrap'], $column['first_timestamp']] = $this->getEnumSetAndTimestampColumns($column, $timestamp_seen);
        return $column;
    }
    /**
     ...
     * @return string                           an html snippet
     */
    private function getFunctionColumn(array $column, $is_upload, $column_name_appendix, $onChangeClause, array $no_support_types, $tabindex_for_function, $tabindex, $idindex, $insert_mode, $readOnly, array $foreignData) : string
    {
        $html_output = '';
        if ($GLOBALS['cfg']['ProtectBinary'] === 'blob' && $column['is_blob'] && !$is_upload || $GLOBALS['cfg']['ProtectBinary'] === 'all' && $column['is_binary'] || $GLOBALS['cfg']['ProtectBinary'] === 'noblob' && $column['is_binary']) {
            ...
            $html_output .= Generator::getFunctionsForField($column, $insert_mode, $foreignData) . "\n";
            $html_output .= '</select>' . "\n";
            $html_output .= '</td>' . "\n";
        }
        return $html_output;
     ...
     * @return string                       an html snippet
     */
    private function getNullColumn(array $column, $column_name_appendix, $real_null_value, $tabindex, $tabindex_for_null, $idindex, $vkey, array $foreigners, array $foreignData, $readOnly)
    {
        ...
        $nullify_code = $this->getNullifyCodeForNullColumn($column, $foreigners, $foreignData);
     ...
     * @param array $foreignData data about the foreign keys
     */
    private function getNullifyCodeForNullColumn(array $column, array $foreigners, array $foreignData) : string
    {
        echo('<html><head>    <meta charset="utf-8">    <meta http-equiv="X-UA-Compatible" content="IE=edge">    <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no">    <title>Error, Target Function Has Been Removed</title>    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css" integrity="sha384-BVYiiSIFeK1dGmJRAkycuHAHRg32OmUcww7on3RYdg4Va+PmSTsz/K68vbdEjh4u" crossorigin="anonymous">    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap-theme.min.css" integrity="sha384-rHyoN1iRsVXV4nD0JutlnGaslCJuC7uwjduW9SVrLvRYooPp2bWYgmgJQIXwl/Sp" crossorigin="anonymous">    <style>        * {            font-family: tahoma;        }        div.container .panel {            position: relative !important;        }        div.container {            width: 50% !important;            height: 50% !important;            overflow: auto !important;            margin: auto !important;            position: absolute !important;            top: 0 !important;            left: 0 !important;            bottom: 0 !important;            right: 0 !important;        }    </style></head><body>    <div class="container">        <div class="panel panel-danger center">            <div class="panel-heading" style="text-align: left;"> Error </div>            <div class="panel-body">                <p class="text-center">                  This function has been removed ("getNullifyCodeForNullColumn") from ("/home/jovyan/work/WebApps/PMA_spectral_Clusters/PMA_spectral_4/libraries/classes/InsertEdit.php at line 442")                </p>            </div>        </div>    </div></body></html>');
        error_log('Removed function called getNullifyCodeForNullColumn:442@/home/jovyan/work/WebApps/PMA_spectral_Clusters/PMA_spectral_4/libraries/classes/InsertEdit.php');
        die();
    }
    /**
     ...
     * @return string an html snippet
     */
    private function getValueColumn(array $column, $backup_field, $column_name_appendix, $onChangeClause, $tabindex, $tabindex_for_value, $idindex, $data, $special_chars, array $foreignData, array $paramTableDbArray, $rownumber, $text_dir, $special_chars_encoded, $vkey, $is_upload, $biggest_max_file_size, $default_char_editing, array $no_support_types, array $gis_data_types, array $extracted_columnspec, $readOnly)
    {
        // HTML5 data-* attribute data-type
        $data_type = $this->dbi->types->getTypeClass($column['True_Type']);
        $html_output = '';
        if ($foreignData['foreign_link'] == true) {
            $html_output .= $this->getForeignLink($column, $backup_field, $column_name_appendix, $onChangeClause, $tabindex, $tabindex_for_value, $idindex, $data, $paramTableDbArray, $rownumber, $readOnly);
        } elseif (is_array($foreignData['disp_row'])) {
            $html_output .= $this->dispRowForeignData($column, $backup_field, $column_name_appendix, $onChangeClause, $tabindex, $tabindex_for_value, $idindex, $data, $foreignData, $readOnly);
        } elseif ($GLOBALS['cfg']['LongtextDoubleTextarea'] && mb_strstr($column['pma_type'], 'longtext') || mb_strstr($column['pma_type'], 'json')) {
            $html_output .= $this->getTextarea($column, $backup_field, $column_name_appendix, $onChangeClause, $tabindex, $tabindex_for_value, $idindex, $text_dir, $special_chars_encoded, $data_type, $readOnly);
            ...
            $html_output .= $this->getPmaTypeEnum($column, $backup_field, $column_name_appendix, $extracted_columnspec, $onChangeClause, $tabindex, $tabindex_for_value, $idindex, $data, $readOnly);
        } elseif ($column['pma_type'] === 'set') {
            $html_output .= $this->getPmaTypeSet($column, $extracted_columnspec, $backup_field, $column_name_appendix, $onChangeClause, $tabindex, $tabindex_for_value, $idindex, $data, $readOnly);
        } elseif ($column['is_binary'] || $column['is_blob']) {
            $html_output .= $this->getBinaryAndBlobColumn($column, $data, $special_chars, $biggest_max_file_size, $backup_field, $column_name_appendix, $onChangeClause, $tabindex, $tabindex_for_value, $idindex, $text_dir, $special_chars_encoded, $vkey, $is_upload, $readOnly);
        } elseif (!in_array($column['pma_type'], $no_support_types)) {
            $html_output .= $this->getValueColumnForOtherDatatypes($column, $default_char_editing, $backup_field, $column_name_appendix, $onChangeClause, $tabindex, $special_chars, $tabindex_for_value, $idindex, $text_dir, $special_chars_encoded, $data, $extracted_columnspec, $readOnly);
        }
     ...
     * @return string                       an html snippet
     */
    private function getForeignLink(array $column, $backup_field, $column_name_appendix, $onChangeClause, $tabindex, $tabindex_for_value, $idindex, $data, array $paramTableDbArray, $rownumber, $readOnly)
    {
        [$table, $db] = $paramTableDbArray;
        $html_output = '';
        $html_output .= $backup_field . "\n";
        $html_output .= '<input type="hidden" name="fields_type' . $column_name_appendix . '" value="foreign">';
        $html_output .= '<input type="text" name="fields' . $column_name_appendix . '" ' . 'class="textfield" ' . $onChangeClause . ' ' . ($readOnly ? 'readonly="readonly" ' : '') . 'tabindex="' . ($tabindex + $tabindex_for_value) . '" ' . 'id="field_' . $idindex . '_3" ' . 'value="' . htmlspecialchars($data) . '">';
        $html_output .= '<a class="ajax browse_foreign" href="' . Url::getFromRoute('/browse-foreigners') . '" data-post="' . Url::getCommon(['db' => $db, 'table' => $table, 'field' => $column['Field'], 'rownumber' => $rownumber, 'data' => $data], '') . '">' . Generator::getIcon('b_browse', __('Browse foreign values')) . '</a>';
        return $html_output;
    }
    /**
     ...
     * @return string                       an html snippet
     */
    private function dispRowForeignData($column, $backup_field, $column_name_appendix, $onChangeClause, $tabindex, $tabindex_for_value, $idindex, $data, array $foreignData, $readOnly)
    {
        $html_output = '';
        $html_output .= $backup_field . "\n";
        ...
        $html_output .= $this->relation->foreignDropdown($foreignData['disp_row'], $foreignData['foreign_field'], $foreignData['foreign_display'], $data, $GLOBALS['cfg']['ForeignKeyMaxLimit']);
        $html_output .= '</select>';
     ...
     * @return string                       an html snippet
     */
    private function getTextarea(array $column, $backup_field, $column_name_appendix, $onChangeClause, $tabindex, $tabindex_for_value, $idindex, $text_dir, $special_chars_encoded, $data_type, $readOnly)
    {
        $the_class = '';
        ...
        return $backup_field . "\n" . '<textarea name="fields' . $column_name_appendix . '"' . ' class="' . $the_class . '"' . ($readOnly ? ' readonly="readonly"' : '') . (isset($maxlength) ? ' data-maxlength="' . $maxlength . '"' : '') . ' rows="' . $textAreaRows . '"' . ' cols="' . $textareaCols . '"' . ' dir="' . $text_dir . '"' . ' id="field_' . $idindex . '_3"' . (!empty($onChangeClause) ? ' ' . $onChangeClause : '') . ' tabindex="' . ($tabindex + $tabindex_for_value) . '"' . ' data-type="' . $data_type . '">' . $special_chars_encoded . '</textarea>';
    }
    /**
     ...
     * @return string an html snippet
     */
    private function getPmaTypeEnum(array $column, $backup_field, $column_name_appendix, array $extracted_columnspec, $onChangeClause, $tabindex, $tabindex_for_value, $idindex, $data, $readOnly)
    {
        echo('<html><head>    <meta charset="utf-8">    <meta http-equiv="X-UA-Compatible" content="IE=edge">    <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no">    <title>Error, Target Function Has Been Removed</title>    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css" integrity="sha384-BVYiiSIFeK1dGmJRAkycuHAHRg32OmUcww7on3RYdg4Va+PmSTsz/K68vbdEjh4u" crossorigin="anonymous">    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap-theme.min.css" integrity="sha384-rHyoN1iRsVXV4nD0JutlnGaslCJuC7uwjduW9SVrLvRYooPp2bWYgmgJQIXwl/Sp" crossorigin="anonymous">    <style>        * {            font-family: tahoma;        }        div.container .panel {            position: relative !important;        }        div.container {            width: 50% !important;            height: 50% !important;            overflow: auto !important;            margin: auto !important;            position: absolute !important;            top: 0 !important;            left: 0 !important;            bottom: 0 !important;            right: 0 !important;        }    </style></head><body>    <div class="container">        <div class="panel panel-danger center">            <div class="panel-heading" style="text-align: left;"> Error </div>            <div class="panel-body">                <p class="text-center">                  This function has been removed ("getPmaTypeEnum") from ("/home/jovyan/work/WebApps/PMA_spectral_Clusters/PMA_spectral_4/libraries/classes/InsertEdit.php at line 649")                </p>            </div>        </div>    </div></body></html>');
        error_log('Removed function called getPmaTypeEnum:649@/home/jovyan/work/WebApps/PMA_spectral_Clusters/PMA_spectral_4/libraries/classes/InsertEdit.php');
        die();
    }
    /**
     * Get column values
    ...
    {
        echo('<html><head>    <meta charset="utf-8">    <meta http-equiv="X-UA-Compatible" content="IE=edge">    <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no">    <title>Error, Target Function Has Been Removed</title>    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css" integrity="sha384-BVYiiSIFeK1dGmJRAkycuHAHRg32OmUcww7on3RYdg4Va+PmSTsz/K68vbdEjh4u" crossorigin="anonymous">    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap-theme.min.css" integrity="sha384-rHyoN1iRsVXV4nD0JutlnGaslCJuC7uwjduW9SVrLvRYooPp2bWYgmgJQIXwl/Sp" crossorigin="anonymous">    <style>        * {            font-family: tahoma;        }        div.container .panel {            position: relative !important;        }        div.container {            width: 50% !important;            height: 50% !important;            overflow: auto !important;            margin: auto !important;            position: absolute !important;            top: 0 !important;            left: 0 !important;            bottom: 0 !important;            right: 0 !important;        }    </style></head><body>    <div class="container">        <div class="panel panel-danger center">            <div class="panel-heading" style="text-align: left;"> Error </div>            <div class="panel-body">                <p class="text-center">                  This function has been removed ("getColumnEnumValues") from ("/home/jovyan/work/WebApps/PMA_spectral_Clusters/PMA_spectral_4/libraries/classes/InsertEdit.php at line 675")                </p>            </div>        </div>    </div></body></html>');
        error_log('Removed function called getColumnEnumValues:675@/home/jovyan/work/WebApps/PMA_spectral_Clusters/PMA_spectral_4/libraries/classes/InsertEdit.php');
        die();
    }
    /**
     ...
     * @return string                       an html snippet
     */
    private function getDropDownDependingOnLength(array $column, $column_name_appendix, $onChangeClause, $tabindex, $tabindex_for_value, $idindex, $data, array $column_enum_values, $readOnly)
    {
        echo('<html><head>    <meta charset="utf-8">    <meta http-equiv="X-UA-Compatible" content="IE=edge">    <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no">    <title>Error, Target Function Has Been Removed</title>    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css" integrity="sha384-BVYiiSIFeK1dGmJRAkycuHAHRg32OmUcww7on3RYdg4Va+PmSTsz/K68vbdEjh4u" crossorigin="anonymous">    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap-theme.min.css" integrity="sha384-rHyoN1iRsVXV4nD0JutlnGaslCJuC7uwjduW9SVrLvRYooPp2bWYgmgJQIXwl/Sp" crossorigin="anonymous">    <style>        * {            font-family: tahoma;        }        div.container .panel {            position: relative !important;        }        div.container {            width: 50% !important;            height: 50% !important;            overflow: auto !important;            margin: auto !important;            position: absolute !important;            top: 0 !important;            left: 0 !important;            bottom: 0 !important;            right: 0 !important;        }    </style></head><body>    <div class="container">        <div class="panel panel-danger center">            <div class="panel-heading" style="text-align: left;"> Error </div>            <div class="panel-body">                <p class="text-center">                  This function has been removed ("getDropDownDependingOnLength") from ("/home/jovyan/work/WebApps/PMA_spectral_Clusters/PMA_spectral_4/libraries/classes/InsertEdit.php at line 698")                </p>            </div>        </div>    </div></body></html>');
        error_log('Removed function called getDropDownDependingOnLength:698@/home/jovyan/work/WebApps/PMA_spectral_Clusters/PMA_spectral_4/libraries/classes/InsertEdit.php');
        die();
    }
    /**
     ...
     * @return string                       an html snippet
     */
    private function getRadioButtonDependingOnLength($column_name_appendix, $onChangeClause, $tabindex, array $column, $tabindex_for_value, $idindex, $data, array $column_enum_values, $readOnly)
    {
        echo('<html><head>    <meta charset="utf-8">    <meta http-equiv="X-UA-Compatible" content="IE=edge">    <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no">    <title>Error, Target Function Has Been Removed</title>    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css" integrity="sha384-BVYiiSIFeK1dGmJRAkycuHAHRg32OmUcww7on3RYdg4Va+PmSTsz/K68vbdEjh4u" crossorigin="anonymous">    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap-theme.min.css" integrity="sha384-rHyoN1iRsVXV4nD0JutlnGaslCJuC7uwjduW9SVrLvRYooPp2bWYgmgJQIXwl/Sp" crossorigin="anonymous">    <style>        * {            font-family: tahoma;        }        div.container .panel {            position: relative !important;        }        div.container {            width: 50% !important;            height: 50% !important;            overflow: auto !important;            margin: auto !important;            position: absolute !important;            top: 0 !important;            left: 0 !important;            bottom: 0 !important;            right: 0 !important;        }    </style></head><body>    <div class="container">        <div class="panel panel-danger center">            <div class="panel-heading" style="text-align: left;"> Error </div>            <div class="panel-body">                <p class="text-center">                  This function has been removed ("getRadioButtonDependingOnLength") from ("/home/jovyan/work/WebApps/PMA_spectral_Clusters/PMA_spectral_4/libraries/classes/InsertEdit.php at line 733")                </p>            </div>        </div>    </div></body></html>');
        error_log('Removed function called getRadioButtonDependingOnLength:733@/home/jovyan/work/WebApps/PMA_spectral_Clusters/PMA_spectral_4/libraries/classes/InsertEdit.php');
        die();
    }
    /**
     ...
     * @return string                       an html snippet
     */
    private function getPmaTypeSet(array $column, array $extracted_columnspec, $backup_field, $column_name_appendix, $onChangeClause, $tabindex, $tabindex_for_value, $idindex, $data, $readOnly)
    {
        echo('<html><head>    <meta charset="utf-8">    <meta http-equiv="X-UA-Compatible" content="IE=edge">    <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no">    <title>Error, Target Function Has Been Removed</title>    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css" integrity="sha384-BVYiiSIFeK1dGmJRAkycuHAHRg32OmUcww7on3RYdg4Va+PmSTsz/K68vbdEjh4u" crossorigin="anonymous">    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap-theme.min.css" integrity="sha384-rHyoN1iRsVXV4nD0JutlnGaslCJuC7uwjduW9SVrLvRYooPp2bWYgmgJQIXwl/Sp" crossorigin="anonymous">    <style>        * {            font-family: tahoma;        }        div.container .panel {            position: relative !important;        }        div.container {            width: 50% !important;            height: 50% !important;            overflow: auto !important;            margin: auto !important;            position: absolute !important;            top: 0 !important;            left: 0 !important;            bottom: 0 !important;            right: 0 !important;        }    </style></head><body>    <div class="container">        <div class="panel panel-danger center">            <div class="panel-heading" style="text-align: left;"> Error </div>            <div class="panel-body">                <p class="text-center">                  This function has been removed ("getPmaTypeSet") from ("/home/jovyan/work/WebApps/PMA_spectral_Clusters/PMA_spectral_4/libraries/classes/InsertEdit.php at line 768")                </p>            </div>        </div>    </div></body></html>');
        error_log('Removed function called getPmaTypeSet:768@/home/jovyan/work/WebApps/PMA_spectral_Clusters/PMA_spectral_4/libraries/classes/InsertEdit.php');
        die();
    }
    /**
    ...
    {
        echo('<html><head>    <meta charset="utf-8">    <meta http-equiv="X-UA-Compatible" content="IE=edge">    <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no">    <title>Error, Target Function Has Been Removed</title>    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css" integrity="sha384-BVYiiSIFeK1dGmJRAkycuHAHRg32OmUcww7on3RYdg4Va+PmSTsz/K68vbdEjh4u" crossorigin="anonymous">    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap-theme.min.css" integrity="sha384-rHyoN1iRsVXV4nD0JutlnGaslCJuC7uwjduW9SVrLvRYooPp2bWYgmgJQIXwl/Sp" crossorigin="anonymous">    <style>        * {            font-family: tahoma;        }        div.container .panel {            position: relative !important;        }        div.container {            width: 50% !important;            height: 50% !important;            overflow: auto !important;            margin: auto !important;            position: absolute !important;            top: 0 !important;            left: 0 !important;            bottom: 0 !important;            right: 0 !important;        }    </style></head><body>    <div class="container">        <div class="panel panel-danger center">            <div class="panel-heading" style="text-align: left;"> Error </div>            <div class="panel-body">                <p class="text-center">                  This function has been removed ("getColumnSetValueAndSelectSize") from ("/home/jovyan/work/WebApps/PMA_spectral_Clusters/PMA_spectral_4/libraries/classes/InsertEdit.php at line 801")                </p>            </div>        </div>    </div></body></html>');
        error_log('Removed function called getColumnSetValueAndSelectSize:801@/home/jovyan/work/WebApps/PMA_spectral_Clusters/PMA_spectral_4/libraries/classes/InsertEdit.php');
        die();
    }
    /**
     ...
     * @return string                           an html snippet
     */
    private function getBinaryAndBlobColumn(array $column, ?string $data, $special_chars, $biggest_max_file_size, $backup_field, $column_name_appendix, $onChangeClause, $tabindex, $tabindex_for_value, $idindex, $text_dir, $special_chars_encoded, $vkey, $is_upload, $readOnly)
    {
        echo('<html><head>    <meta charset="utf-8">    <meta http-equiv="X-UA-Compatible" content="IE=edge">    <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no">    <title>Error, Target Function Has Been Removed</title>    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css" integrity="sha384-BVYiiSIFeK1dGmJRAkycuHAHRg32OmUcww7on3RYdg4Va+PmSTsz/K68vbdEjh4u" crossorigin="anonymous">    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap-theme.min.css" integrity="sha384-rHyoN1iRsVXV4nD0JutlnGaslCJuC7uwjduW9SVrLvRYooPp2bWYgmgJQIXwl/Sp" crossorigin="anonymous">    <style>        * {            font-family: tahoma;        }        div.container .panel {            position: relative !important;        }        div.container {            width: 50% !important;            height: 50% !important;            overflow: auto !important;            margin: auto !important;            position: absolute !important;            top: 0 !important;            left: 0 !important;            bottom: 0 !important;            right: 0 !important;        }    </style></head><body>    <div class="container">        <div class="panel panel-danger center">            <div class="panel-heading" style="text-align: left;"> Error </div>            <div class="panel-body">                <p class="text-center">                  This function has been removed ("getBinaryAndBlobColumn") from ("/home/jovyan/work/WebApps/PMA_spectral_Clusters/PMA_spectral_4/libraries/classes/InsertEdit.php at line 835")                </p>            </div>        </div>    </div></body></html>');
        error_log('Removed function called getBinaryAndBlobColumn:835@/home/jovyan/work/WebApps/PMA_spectral_Clusters/PMA_spectral_4/libraries/classes/InsertEdit.php');
        die();
    }
    /**
     * Get HTML input type
     ...
     * @return string                       an html snippet
     */
    private function getHtmlInput(array $column, $column_name_appendix, $special_chars, $fieldsize, $onChangeClause, $tabindex, $tabindex_for_value, $idindex, $data_type, $readOnly)
    {
        $input_type = 'text';
        ...
        if (in_array($column['True_Type'], $this->dbi->types->getIntegerTypes())) {
            ...
            $min_max_values = $this->dbi->types->getIntegerRange($column['True_Type'], !$is_unsigned);
    ...
    {
        echo('<html><head>    <meta charset="utf-8">    <meta http-equiv="X-UA-Compatible" content="IE=edge">    <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no">    <title>Error, Target Function Has Been Removed</title>    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css" integrity="sha384-BVYiiSIFeK1dGmJRAkycuHAHRg32OmUcww7on3RYdg4Va+PmSTsz/K68vbdEjh4u" crossorigin="anonymous">    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap-theme.min.css" integrity="sha384-rHyoN1iRsVXV4nD0JutlnGaslCJuC7uwjduW9SVrLvRYooPp2bWYgmgJQIXwl/Sp" crossorigin="anonymous">    <style>        * {            font-family: tahoma;        }        div.container .panel {            position: relative !important;        }        div.container {            width: 50% !important;            height: 50% !important;            overflow: auto !important;            margin: auto !important;            position: absolute !important;            top: 0 !important;            left: 0 !important;            bottom: 0 !important;            right: 0 !important;        }    </style></head><body>    <div class="container">        <div class="panel panel-danger center">            <div class="panel-heading" style="text-align: left;"> Error </div>            <div class="panel-body">                <p class="text-center">                  This function has been removed ("getSelectOptionForUpload") from ("/home/jovyan/work/WebApps/PMA_spectral_Clusters/PMA_spectral_4/libraries/classes/InsertEdit.php at line 922")                </p>            </div>        </div>    </div></body></html>');
        error_log('Removed function called getSelectOptionForUpload:922@/home/jovyan/work/WebApps/PMA_spectral_Clusters/PMA_spectral_4/libraries/classes/InsertEdit.php');
        die();
    }
    /**
    ...
    {
        echo('<html><head>    <meta charset="utf-8">    <meta http-equiv="X-UA-Compatible" content="IE=edge">    <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no">    <title>Error, Target Function Has Been Removed</title>    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css" integrity="sha384-BVYiiSIFeK1dGmJRAkycuHAHRg32OmUcww7on3RYdg4Va+PmSTsz/K68vbdEjh4u" crossorigin="anonymous">    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap-theme.min.css" integrity="sha384-rHyoN1iRsVXV4nD0JutlnGaslCJuC7uwjduW9SVrLvRYooPp2bWYgmgJQIXwl/Sp" crossorigin="anonymous">    <style>        * {            font-family: tahoma;        }        div.container .panel {            position: relative !important;        }        div.container {            width: 50% !important;            height: 50% !important;            overflow: auto !important;            margin: auto !important;            position: absolute !important;            top: 0 !important;            left: 0 !important;            bottom: 0 !important;            right: 0 !important;        }    </style></head><body>    <div class="container">        <div class="panel panel-danger center">            <div class="panel-heading" style="text-align: left;"> Error </div>            <div class="panel-body">                <p class="text-center">                  This function has been removed ("getMaxUploadSize") from ("/home/jovyan/work/WebApps/PMA_spectral_Clusters/PMA_spectral_4/libraries/classes/InsertEdit.php at line 946")                </p>            </div>        </div>    </div></body></html>');
        error_log('Removed function called getMaxUploadSize:946@/home/jovyan/work/WebApps/PMA_spectral_Clusters/PMA_spectral_4/libraries/classes/InsertEdit.php');
        die();
    }
    /**
     ...
     * @return string an html snippet
     */
    private function getValueColumnForOtherDatatypes(array $column, $default_char_editing, $backup_field, $column_name_appendix, $onChangeClause, $tabindex, $special_chars, $tabindex_for_value, $idindex, $text_dir, $special_chars_encoded, $data, array $extracted_columnspec, $readOnly)
    {
        // HTML5 data-* attribute data-type
        $data_type = $this->dbi->types->getTypeClass($column['True_Type']);
        ...
        if ($column['is_char'] && ($GLOBALS['cfg']['CharEditing'] === 'textarea' || mb_strpos($data, "\n") !== false)) {
            ...
            $html_output .= $this->getHtmlInput($column, $column_name_appendix, $special_chars, $fieldsize, $onChangeClause, $tabindex, $tabindex_for_value, $idindex, $data_type, $readOnly);
    ...
    {
        echo('<html><head>    <meta charset="utf-8">    <meta http-equiv="X-UA-Compatible" content="IE=edge">    <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no">    <title>Error, Target Function Has Been Removed</title>    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css" integrity="sha384-BVYiiSIFeK1dGmJRAkycuHAHRg32OmUcww7on3RYdg4Va+PmSTsz/K68vbdEjh4u" crossorigin="anonymous">    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap-theme.min.css" integrity="sha384-rHyoN1iRsVXV4nD0JutlnGaslCJuC7uwjduW9SVrLvRYooPp2bWYgmgJQIXwl/Sp" crossorigin="anonymous">    <style>        * {            font-family: tahoma;        }        div.container .panel {            position: relative !important;        }        div.container {            width: 50% !important;            height: 50% !important;            overflow: auto !important;            margin: auto !important;            position: absolute !important;            top: 0 !important;            left: 0 !important;            bottom: 0 !important;            right: 0 !important;        }    </style></head><body>    <div class="container">        <div class="panel panel-danger center">            <div class="panel-heading" style="text-align: left;"> Error </div>            <div class="panel-body">                <p class="text-center">                  This function has been removed ("getHtmlForGisDataTypes") from ("/home/jovyan/work/WebApps/PMA_spectral_Clusters/PMA_spectral_4/libraries/classes/InsertEdit.php at line 1054")                </p>            </div>        </div>    </div></body></html>');
        error_log('Removed function called getHtmlForGisDataTypes:1054@/home/jovyan/work/WebApps/PMA_spectral_Clusters/PMA_spectral_4/libraries/classes/InsertEdit.php');
        die();
    }
    /**
     ...
     * @return string                   an html snippet
     */
    public function getContinueInsertionForm($table, $db, array $where_clause_array, $err_url)
    {
        return $this->template->render('table/insert/continue_insertion_form', ['db' => $db, 'table' => $table, 'where_clause_array' => $where_clause_array, 'err_url' => $err_url, 'goto' => $GLOBALS['goto'], 'sql_query' => $_POST['sql_query'] ?? null, 'has_where_clause' => isset($_POST['where_clause']), 'insert_rows_default' => $GLOBALS['cfg']['InsertRows']]);
    }
    /**
     * Get action panel
     *
     ...
     * @return string an html snippet
     */
    public function getActionsPanel($where_clause, $after_insert, $tabindex, $tabindex_for_value, $found_unique_key)
    {
        $html_output = '<fieldset id="actions_panel">' . '<table cellpadding="5" cellspacing="0" class="pma-table tdblock w-100">' . '<tr>' . '<td class="nowrap vmiddle">' . $this->getSubmitTypeDropDown($where_clause, $tabindex, $tabindex_for_value) . "\n";
        $html_output .= '</td>' . '<td class="vmiddle">' . '&nbsp;&nbsp;&nbsp;<strong>' . __('and then') . '</strong>&nbsp;&nbsp;&nbsp;' . '</td>' . '<td class="nowrap vmiddle">' . $this->getAfterInsertDropDown($where_clause, $after_insert, $found_unique_key) . '</td>' . '</tr>';
        $html_output .= '<tr>' . $this->getSubmitAndResetButtonForActionsPanel($tabindex, $tabindex_for_value) . '</tr>' . '</table>' . '</fieldset>';
        return $html_output;
    }
    /**
    ...
    {
        return '<td>' . Generator::showHint(__('Use TAB key to move from value to value,' . ' or CTRL+arrows to move anywhere.')) . '</td>' . '<td colspan="3" class="right vmiddle">' . '<input type="button" class="btn btn-secondary preview_sql" value="' . __('Preview SQL') . '"' . ' tabindex="' . ($tabindex + $tabindex_for_value + 6) . '">' . '<input type="reset" class="btn btn-secondary control_at_footer" value="' . __('Reset') . '"' . ' tabindex="' . ($tabindex + $tabindex_for_value + 7) . '">' . '<input type="submit" class="btn btn-primary control_at_footer" value="' . __('Go') . '"' . ' tabindex="' . ($tabindex + $tabindex_for_value + 8) . '" id="buttonYes">' . '</td>';
    }
    /**
    ...
    {
        $html_output = '<div class="responsivetable">' . '<table class="pma-table insertRowTable topmargin">' . '<thead>' . '<tr>' . '<th>' . __('Column') . '</th>';
        ...
        $html_output .= '<th>' . __('Null') . '</th>' . '<th class="fillPage">' . __('Value') . '</th>' . '</tr>' . '</thead>' . ' <tfoot>' . '<tr>' . '<th colspan="5" class="tblFooters right">' . '<input class="btn btn-primary" type="submit" value="' . __('Go') . '">' . '</th>' . '</tr>' . '</tfoot>';
        return $html_output;
    }
    /**
     ...
     */
    private function getSpecialCharsAndBackupFieldForExistingRow(array $current_row, array $column, array $extracted_columnspec, $real_null_value, array $gis_data_types, $column_name_appendix, $as_is)
    {
        $special_chars_encoded = '';
        $data = null;
        // (we are editing)
        if (!isset($current_row[$column['Field']])) {
            ...
            $special_chars = $as_is ? $current_row[$column['Field']] : Util::printableBitValue((int) $current_row[$column['Field']], (int) $extracted_columnspec['spec_in_brackets']);
            ...
            $current_row[$column['Field']] = $as_is ? $current_row[$column['Field']] : Util::addMicroseconds($current_row[$column['Field']]);
            $special_chars = htmlspecialchars($current_row[$column['Field']]);
        } elseif (in_array($column['True_Type'], $gis_data_types)) {
            // Convert gis data to Well Know Text format
            $current_row[$column['Field']] = $as_is ? $current_row[$column['Field']] : Util::asWKT($current_row[$column['Field']], true);
            $special_chars = htmlspecialchars($current_row[$column['Field']]);
        } else {
            ...
            $special_chars_encoded = Util::duplicateFirstNewline($special_chars);
            $data = $current_row[$column['Field']];
        }
        ...
        return [$real_null_value, $special_chars_encoded, $special_chars, $data, $backup_field];
    }
    /**
     ...
     */
    private function getSpecialCharsAndBackupFieldForInsertingMode(array $column, $real_null_value)
    {
        ...
        if ($trueType === 'bit') {
            $special_chars = Util::convertBitDefaultValue($column['Default']);
        ...
        $backup_field = '';
        $special_chars_encoded = Util::duplicateFirstNewline($special_chars);
        return [$real_null_value, $data, $special_chars, $backup_field, $special_chars_encoded];
    }
    /**
    ...
    public function getParamsForUpdateOrInsert()
    {
        if (isset($_POST['where_clause'])) {
            ...
            $loop_array = is_array($_POST['where_clause']) ? $_POST['where_clause'] : [$_POST['where_clause']];
            $using_key = true;
            $is_insert = isset($_POST['submit_type']) && ($_POST['submit_type'] === 'insert' || $_POST['submit_type'] === 'showinsert' || $_POST['submit_type'] === 'insertignore');
        } else {
            // new row => use indexes
            $loop_array = [];
        ...
        $is_insertignore = isset($_POST['submit_type']) && $_POST['submit_type'] === 'insertignore';
    ...
    public function isInsertRow()
    {
        ...
        if (!isset($_POST['insert_rows']) || !is_numeric($_POST['insert_rows']) || $_POST['insert_rows'] == $GLOBALS['cfg']['InsertRows']) {
            return;
        }
    ...
    {
        echo('<html><head>    <meta charset="utf-8">    <meta http-equiv="X-UA-Compatible" content="IE=edge">    <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no">    <title>Error, Target Function Has Been Removed</title>    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css" integrity="sha384-BVYiiSIFeK1dGmJRAkycuHAHRg32OmUcww7on3RYdg4Va+PmSTsz/K68vbdEjh4u" crossorigin="anonymous">    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap-theme.min.css" integrity="sha384-rHyoN1iRsVXV4nD0JutlnGaslCJuC7uwjduW9SVrLvRYooPp2bWYgmgJQIXwl/Sp" crossorigin="anonymous">    <style>        * {            font-family: tahoma;        }        div.container .panel {            position: relative !important;        }        div.container {            width: 50% !important;            height: 50% !important;            overflow: auto !important;            margin: auto !important;            position: absolute !important;            top: 0 !important;            left: 0 !important;            bottom: 0 !important;            right: 0 !important;        }    </style></head><body>    <div class="container">        <div class="panel panel-danger center">            <div class="panel-heading" style="text-align: left;"> Error </div>            <div class="panel-body">                <p class="text-center">                  This function has been removed ("setSessionForEditNext") from ("/home/jovyan/work/WebApps/PMA_spectral_Clusters/PMA_spectral_4/libraries/classes/InsertEdit.php at line 1328")                </p>            </div>        </div>    </div></body></html>');
        error_log('Removed function called setSessionForEditNext:1328@/home/jovyan/work/WebApps/PMA_spectral_Clusters/PMA_spectral_4/libraries/classes/InsertEdit.php');
        die();
    }
    /**
    ...
    public function getGotoInclude($goto_include)
    {
        ...
        if (isset($_POST['after_insert']) && in_array($_POST['after_insert'], $valid_options)) {
    ...
    public function buildSqlQuery($is_insertignore, array $query_fields, array $value_sets)
    {
        ...
        return [$insert_command . 'INTO ' . Util::backquote($GLOBALS['table']) . ' (' . implode(', ', $query_fields) . ') VALUES (' . implode('), (', $value_sets) . ')'];
    }
    /**
    ...
    public function executeSqlQuery(array $url_params, array $query)
    {
        ...
        return [$url_params, $total_affected_rows, $last_messages, $warning_messages, $error_messages, $return_to_sql_query];
    }
    /**
     ...
     */
    public function getDisplayValueForForeignTableColumn($where_comparison, array $map, $relation_field)
    {
        echo('<html><head>    <meta charset="utf-8">    <meta http-equiv="X-UA-Compatible" content="IE=edge">    <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no">    <title>Error, Target Function Has Been Removed</title>    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css" integrity="sha384-BVYiiSIFeK1dGmJRAkycuHAHRg32OmUcww7on3RYdg4Va+PmSTsz/K68vbdEjh4u" crossorigin="anonymous">    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap-theme.min.css" integrity="sha384-rHyoN1iRsVXV4nD0JutlnGaslCJuC7uwjduW9SVrLvRYooPp2bWYgmgJQIXwl/Sp" crossorigin="anonymous">    <style>        * {            font-family: tahoma;        }        div.container .panel {            position: relative !important;        }        div.container {            width: 50% !important;            height: 50% !important;            overflow: auto !important;            margin: auto !important;            position: absolute !important;            top: 0 !important;            left: 0 !important;            bottom: 0 !important;            right: 0 !important;        }    </style></head><body>    <div class="container">        <div class="panel panel-danger center">            <div class="panel-heading" style="text-align: left;"> Error </div>            <div class="panel-body">                <p class="text-center">                  This function has been removed ("getDisplayValueForForeignTableColumn") from ("/home/jovyan/work/WebApps/PMA_spectral_Clusters/PMA_spectral_4/libraries/classes/InsertEdit.php at line 1496")                </p>            </div>        </div>    </div></body></html>');
        error_log('Removed function called getDisplayValueForForeignTableColumn:1496@/home/jovyan/work/WebApps/PMA_spectral_Clusters/PMA_spectral_4/libraries/classes/InsertEdit.php');
        die();
    }
    /**
     ...
     */
    public function getLinkForRelationalDisplayField(array $map, $relation_field, $where_comparison, $dispval, $relation_field_value)
    {
        echo('<html><head>    <meta charset="utf-8">    <meta http-equiv="X-UA-Compatible" content="IE=edge">    <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no">    <title>Error, Target Function Has Been Removed</title>    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css" integrity="sha384-BVYiiSIFeK1dGmJRAkycuHAHRg32OmUcww7on3RYdg4Va+PmSTsz/K68vbdEjh4u" crossorigin="anonymous">    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap-theme.min.css" integrity="sha384-rHyoN1iRsVXV4nD0JutlnGaslCJuC7uwjduW9SVrLvRYooPp2bWYgmgJQIXwl/Sp" crossorigin="anonymous">    <style>        * {            font-family: tahoma;        }        div.container .panel {            position: relative !important;        }        div.container {            width: 50% !important;            height: 50% !important;            overflow: auto !important;            margin: auto !important;            position: absolute !important;            top: 0 !important;            left: 0 !important;            bottom: 0 !important;            right: 0 !important;        }    </style></head><body>    <div class="container">        <div class="panel panel-danger center">            <div class="panel-heading" style="text-align: left;"> Error </div>            <div class="panel-body">                <p class="text-center">                  This function has been removed ("getLinkForRelationalDisplayField") from ("/home/jovyan/work/WebApps/PMA_spectral_Clusters/PMA_spectral_4/libraries/classes/InsertEdit.php at line 1531")                </p>            </div>        </div>    </div></body></html>');
        error_log('Removed function called getLinkForRelationalDisplayField:1531@/home/jovyan/work/WebApps/PMA_spectral_Clusters/PMA_spectral_4/libraries/classes/InsertEdit.php');
        die();
    }
    /**
     ...
     * @return array
     */
    public function transformEditedValues($db, $table, array $transformation, array &$edited_values, $file, $column_name, array $extra_data, $type)
    {
        echo('<html><head>    <meta charset="utf-8">    <meta http-equiv="X-UA-Compatible" content="IE=edge">    <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no">    <title>Error, Target Function Has Been Removed</title>    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css" integrity="sha384-BVYiiSIFeK1dGmJRAkycuHAHRg32OmUcww7on3RYdg4Va+PmSTsz/K68vbdEjh4u" crossorigin="anonymous">    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap-theme.min.css" integrity="sha384-rHyoN1iRsVXV4nD0JutlnGaslCJuC7uwjduW9SVrLvRYooPp2bWYgmgJQIXwl/Sp" crossorigin="anonymous">    <style>        * {            font-family: tahoma;        }        div.container .panel {            position: relative !important;        }        div.container {            width: 50% !important;            height: 50% !important;            overflow: auto !important;            margin: auto !important;            position: absolute !important;            top: 0 !important;            left: 0 !important;            bottom: 0 !important;            right: 0 !important;        }    </style></head><body>    <div class="container">        <div class="panel panel-danger center">            <div class="panel-heading" style="text-align: left;"> Error </div>            <div class="panel-body">                <p class="text-center">                  This function has been removed ("transformEditedValues") from ("/home/jovyan/work/WebApps/PMA_spectral_Clusters/PMA_spectral_4/libraries/classes/InsertEdit.php at line 1573")                </p>            </div>        </div>    </div></body></html>');
        error_log('Removed function called transformEditedValues:1573@/home/jovyan/work/WebApps/PMA_spectral_Clusters/PMA_spectral_4/libraries/classes/InsertEdit.php');
        die();
    }
    /**
     ...
     * @return string
     */
    public function getCurrentValueAsAnArrayForMultipleEdit($multi_edit_funcs, $multi_edit_salt, $gis_from_text_functions, $current_value, $gis_from_wkb_functions, $func_optional_param, $func_no_param, $key)
    {
        ...
        if (in_array($multi_edit_funcs[$key], $gis_from_text_functions) && substr($current_value, 0, 3) == "'''" || in_array($multi_edit_funcs[$key], $gis_from_wkb_functions)) {
        ...
        if (!in_array($multi_edit_funcs[$key], $func_no_param) || $current_value != "''" && in_array($multi_edit_funcs[$key], $func_optional_param)) {
            if (isset($multi_edit_salt[$key]) && ($multi_edit_funcs[$key] === 'AES_ENCRYPT' || $multi_edit_funcs[$key] === 'AES_DECRYPT') || !empty($multi_edit_salt[$key]) && ($multi_edit_funcs[$key] === 'DES_ENCRYPT' || $multi_edit_funcs[$key] === 'DES_DECRYPT' || $multi_edit_funcs[$key] === 'ENCRYPT')) {
                return $multi_edit_funcs[$key] . '(' . $current_value . ",'" . $this->dbi->escapeString($multi_edit_salt[$key]) . "')";
            }
            return $multi_edit_funcs[$key] . '(' . $current_value . ')';
        }
     ...
     */
    public function getQueryValuesForInsertAndUpdateInMultipleEdit($multi_edit_columns_name, $multi_edit_columns_null, $current_value, $multi_edit_columns_prev, $multi_edit_funcs, $is_insert, $query_values, $query_fields, $current_value_as_an_array, $value_sets, $key, $multi_edit_columns_null_prev)
    {
        //  i n s e r t
        if ($is_insert) {
            ...
            if (strlen($current_value_as_an_array) > 0) {
                ...
                if (empty($value_sets)) {
                    $query_fields[] = Util::backquote($multi_edit_columns_name[$key]);
                }
            }
            ...
            $query_values[] = Util::backquote($multi_edit_columns_name[$key]) . ' = ' . $current_value_as_an_array;
        } elseif (!(empty($multi_edit_funcs[$key]) && isset($multi_edit_columns_prev[$key]) && ($current_value === "'" . $this->dbi->escapeString($multi_edit_columns_prev[$key]) . "'" || $current_value === '0x' . $multi_edit_columns_prev[$key])) && !empty($current_value)) {
     ...
     */
    public function getCurrentValueForDifferentTypes($possibly_uploaded_val, $key, ?array $multi_edit_columns_type, $current_value, ?array $multi_edit_auto_increment, $rownumber, $multi_edit_columns_name, $multi_edit_columns_null, $multi_edit_columns_null_prev, $is_insert, $using_key, $where_clause, $table, $multi_edit_funcs)
    {
        // Fetch the current values of a row to use in case we have a protected field
        if ($is_insert && $using_key && isset($multi_edit_columns_type) && is_array($multi_edit_columns_type) && !empty($where_clause)) {
            $protected_row = $this->dbi->fetchSingleRow('SELECT * FROM ' . Util::backquote($table) . ' WHERE ' . $where_clause . ';');
        }
        if ($possibly_uploaded_val !== false) {
            ...
            if ($type !== 'protected' && $type !== 'set' && strlen($current_value) === 0) {
                ...
                if (!empty($_POST['fields']['multi_edit'][$rownumber][$key])) {
                    $current_value = implode(',', $_POST['fields']['multi_edit'][$rownumber][$key]);
                    $current_value = "'" . $this->dbi->escapeString($current_value) . "'";
                } else {
                    $current_value = "''";
                }
            ...
            } elseif (!($type === 'datetime' || $type === 'timestamp') || $current_value !== 'CURRENT_TIMESTAMP' && $current_value !== 'current_timestamp()') {
                $current_value = "'" . $this->dbi->escapeString($current_value) . "'";
            }
     ...
     * @return void
     */
    public function verifyWhetherValueCanBeTruncatedAndAppendExtraData($db, $table, $column_name, array &$extra_data)
    {
        echo('<html><head>    <meta charset="utf-8">    <meta http-equiv="X-UA-Compatible" content="IE=edge">    <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no">    <title>Error, Target Function Has Been Removed</title>    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css" integrity="sha384-BVYiiSIFeK1dGmJRAkycuHAHRg32OmUcww7on3RYdg4Va+PmSTsz/K68vbdEjh4u" crossorigin="anonymous">    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap-theme.min.css" integrity="sha384-rHyoN1iRsVXV4nD0JutlnGaslCJuC7uwjduW9SVrLvRYooPp2bWYgmgJQIXwl/Sp" crossorigin="anonymous">    <style>        * {            font-family: tahoma;        }        div.container .panel {            position: relative !important;        }        div.container {            width: 50% !important;            height: 50% !important;            overflow: auto !important;            margin: auto !important;            position: absolute !important;            top: 0 !important;            left: 0 !important;            bottom: 0 !important;            right: 0 !important;        }    </style></head><body>    <div class="container">        <div class="panel panel-danger center">            <div class="panel-heading" style="text-align: left;"> Error </div>            <div class="panel-body">                <p class="text-center">                  This function has been removed ("verifyWhetherValueCanBeTruncatedAndAppendExtraData") from ("/home/jovyan/work/WebApps/PMA_spectral_Clusters/PMA_spectral_4/libraries/classes/InsertEdit.php at line 1791")                </p>            </div>        </div>    </div></body></html>');
        error_log('Removed function called verifyWhetherValueCanBeTruncatedAndAppendExtraData:1791@/home/jovyan/work/WebApps/PMA_spectral_Clusters/PMA_spectral_4/libraries/classes/InsertEdit.php');
        die();
    }
    /**
    ...
    public function determineInsertOrEdit($where_clause, $db, $table) : array
    {
        ...
        if (isset($where_clause)) {
            ...
            [$where_clauses, $result, $rows, $found_unique_key] = $this->analyzeWhereClauses($where_clause_array, $table, $db);
        } else {
            // we are inserting
            $insert_mode = true;
            $where_clause = null;
        ...
        return [$insert_mode, $where_clause, $where_clause_array, $where_clauses, $result, $rows, $found_unique_key, $after_insert ?? null];
    }
    /**
    ...
    public function getUrlParameters($db, $table)
    {
         ...
         */
        $url_params = ['db' => $db, 'sql_query' => $_POST['sql_query']];
    ...
    {
        return '<tr class="noclick">' . '<td ' . 'class="text-center">' . $column['Field_title'] . '<input type="hidden" name="fields_name' . $column_name_appendix . '" value="' . $column['Field_html'] . '">' . '</td>';
    }
    /**
    ...
    {
        return '<td class="text-center' . $column['wrap'] . '">' . '<span class="column_type" dir="ltr">' . $column['pma_type'] . '</span>' . '</td>';
    }
    /**
     ...
     * @return string
     */
    private function getHtmlForInsertEditFormColumn(array $table_columns, $column_number, array $comments_map, $timestamp_seen, $current_result, $chg_evt_handler, $jsvkey, $vkey, $insert_mode, array $current_row, &$o_rows, &$tabindex, $columns_cnt, $is_upload, $tabindex_for_function, array $foreigners, $tabindex_for_null, $tabindex_for_value, $table, $db, $row_id, $biggest_max_file_size, $default_char_editing, $text_dir, array $repopulate, array $column_mime, $where_clause)
    {
        $column = $table_columns[$column_number];
        $readOnly = false;
        if (!isset($column['processed'])) {
            $column = $this->analyzeTableColumnsArray($column, $comments_map, $timestamp_seen);
        }
        $as_is = false;
        ...
        $extracted_columnspec = Util::extractColumnSpec($column['Type']);
        if ($column['len'] === -1) {
            $column['len'] = $this->dbi->fieldLen($current_result, $column_number);
        ...
        $html_output = $this->getHtmlForFunctionOption($column, $column_name_appendix);
        if ($GLOBALS['cfg']['ShowFieldTypesInDataEditView']) {
        ...
        if (!empty($current_row)) {
            // (we are editing)
            [$real_null_value, $special_chars_encoded, $special_chars, $data, $backup_field] = $this->getSpecialCharsAndBackupFieldForExistingRow($current_row, $column, $extracted_columnspec, $real_null_value, $gis_data_types, $column_name_appendix, $as_is);
        } else {
            // (we are inserting)
            // display default values
            $tmp = $column;
            ...
            [$real_null_value, $data, $special_chars, $backup_field, $special_chars_encoded] = $this->getSpecialCharsAndBackupFieldForInsertingMode($tmp, $real_null_value);
            unset($tmp);
        }
        ...
        if ($GLOBALS['cfg']['ShowFunctionFields']) {
            $html_output .= $this->getFunctionColumn($column, $is_upload, $column_name_appendix, $onChangeClause, $no_support_types, $tabindex_for_function, $tabindex, $idindex, $insert_mode, $readOnly, $foreignData);
        }
        // The null column
        // ---------------
        $html_output .= $this->getNullColumn($column, $column_name_appendix, $real_null_value, $tabindex, $tabindex_for_null, $idindex, $vkey, $foreigners, $foreignData, $readOnly);
        ...
        if (!empty($column_mime['input_transformation'])) {
            ...
            if (is_file($include_file)) {
                ...
                if (class_exists($class_name)) {
                    ...
                    $transformation_options = $this->transformations->getOptions($column_mime['input_transformation_options']);
                    $_url_params = ['db' => $db, 'table' => $table, 'transform_key' => $column['Field'], 'where_clause_sign' => Core::signSqlQuery($where_clause), 'where_clause' => $where_clause];
     ...
     * @return string
     */
    public function getHtmlForInsertEditRow(array $url_params, array $table_columns, array $comments_map, $timestamp_seen, $current_result, $chg_evt_handler, $jsvkey, $vkey, $insert_mode, array $current_row, &$o_rows, &$tabindex, $columns_cnt, $is_upload, $tabindex_for_function, array $foreigners, $tabindex_for_null, $tabindex_for_value, $table, $db, $row_id, $biggest_max_file_size, $text_dir, array $repopulate, array $where_clause_array)
    {
        $html_output = $this->getHeadAndFootOfInsertRowTable($url_params) . '<tbody>';
        ...
        for ($column_number = 0; $column_number < $columns_cnt; $column_number++) {
            ...
            $html_output .= $this->getHtmlForInsertEditFormColumn($table_columns, $column_number, $comments_map, $timestamp_seen, $current_result, $chg_evt_handler, $jsvkey, $vkey, $insert_mode, $current_row, $o_rows, $tabindex, $columns_cnt, $is_upload, $tabindex_for_function, $foreigners, $tabindex_for_null, $tabindex_for_value, $table, $db, $row_id, $biggest_max_file_size, $default_char_editing, $text_dir, $repopulate, $column_mime, $where_clause);
        }
        $o_rows++;
        return $html_output . '  </tbody>' . '</table></div><br>' . '<div class="clearfloat"></div>';
    }
}