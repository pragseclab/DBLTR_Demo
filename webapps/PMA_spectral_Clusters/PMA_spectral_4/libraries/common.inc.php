...
if (PHP_VERSION_ID < 70103) {
    die('<p>PHP 7.1.3+ is required.</p>' . '<p>Currently installed version is: ' . PHP_VERSION . '</p>');
}
...
if (!@is_readable(AUTOLOAD_FILE)) {
    die('<p>File <samp>' . AUTOLOAD_FILE . '</samp> missing or not readable.</p>' . '<p>Most likely you did not run Composer to ' . '<a href="https://docs.phpmyadmin.net/en/latest/setup.html#installing-from-git">' . 'install library files</a>.</p>');
}
...
if (!defined('PMA_MINIMUM_COMMON')) {
    ...
    if (!empty($cfg['Server'])) {
        ...
        if ($dbi->getVersion() < $cfg['MysqlMinVersion']['internal']) {
            Core::fatalError(__('You should upgrade to %s %s or later.'), ['MySQL', $cfg['MysqlMinVersion']['human']]);
        }
