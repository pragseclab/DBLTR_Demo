...
class Linter
{
    ...
    public static function lint($query)
    {
        ...
        if (mb_strlen($query) > 10000) {
            return [['message' => __('Linting is disabled for this query because it exceeds the ' . 'maximum length.'), 'fromLine' => 0, 'fromColumn' => 0, 'toLine' => 0, 'toColumn' => 0, 'severity' => 'warning']];
        }
        /**
        ...
        foreach ($errors as $idx => $error) {
            ...
            [$toLine, $toColumn] = static::findLineNumberAndColumn($lines, $error[3] + mb_strlen((string) $error[2]));
            // Building the response.
            $response[] = ['message' => sprintf(__('%1$s (near <code>%2$s</code>)'), htmlspecialchars((string) $error[0]), htmlspecialchars((string) $error[2])), 'fromLine' => $fromLine, 'fromColumn' => $fromColumn, 'toLine' => $toLine, 'toColumn' => $toColumn, 'severity' => 'error'];
        }
