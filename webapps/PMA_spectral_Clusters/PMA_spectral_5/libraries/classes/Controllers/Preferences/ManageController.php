...
class ManageController extends AbstractController
{
     ...
     */
    public function __construct($response, Template $template, UserPreferences $userPreferences, Relation $relation)
    {
    ...
    public function index() : void
    {
        ...
        if (isset($_POST['submit_import'])) {
            ...
            if (!is_array($config)) {
                ...
                if (!$all_ok) {
                    ...
                    echo $this->template->render('preferences/header', ['route' => $route, 'is_saved' => !empty($_GET['saved']), 'has_config_storage' => $cfgRelation['userconfigwork']]);
                    echo $this->template->render('preferences/manage/error', ['form_errors' => $form_display->displayErrors(), 'json' => $json, 'import_merge' => $_POST['import_merge'] ?? null, 'return_url' => $return_url]);
                    return;
                }
        ...
        echo $this->template->render('preferences/manage/main', ['error' => $error, 'max_upload_size' => $max_upload_size, 'exists_setup_and_not_exists_config' => @file_exists(ROOT_PATH . 'setup/index.php') && !@file_exists(CONFIG_FILE)]);
